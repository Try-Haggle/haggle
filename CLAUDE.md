...
- 사용자에게 하는 모든 답변은 **반드시 한국어**로 작성한다. 보고, 계획, 질문, 확인 요청, 요약 모두 포함한다.
- 코드, 식별자, 코드 주석, 커밋 메시지, PR 본문의 코드 블록은 저장소 관례대로 영어를 유지한다.
- 사용자가 영어로 질문하더라도 명시적으로 다른 언어를 요청하지 않는 한 한국어로 답한다.
...
- feature 브랜치는 하나의 사용자 가치, 버그 수정, 또는 인프라 변경만 다룬다.
- 각 브랜치는 독립적으로 리뷰·테스트·되돌리기가 가능해야 한다.
- 기본 예산은 제품 코드 기준 변경 500줄 이하, 코드 파일 15개 이하로 잡는다. 테스트,
  문서, 생성 파일, additive migration은 이 수치에서 제외하되 리뷰 부담은 별도로 확인한다.
- 예산을 넘길 것으로 보이면 구현 전에 수직 slice나 선행 리팩터링 브랜치로 나눈다.
- 두 개 이상의 독립된 목적이나 여러 도메인의 동시 변경이 생기면 줄 수와 관계없이
  브랜치를 분리한다.
