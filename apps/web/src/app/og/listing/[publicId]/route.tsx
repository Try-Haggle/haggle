...
export async function GET(_req: Request, { params }: { params: Promise<{ publicId: string }> }) {
  ...
  const preset = listing.sellerAgentPreset ? PRESET_LABELS[listing.sellerAgentPreset] : null;
  ...
  const response = new ImageResponse(
    <div
      style={{
        ...
        background: "linear-gradient(180deg, #0b1120 0%, #0b1120 55%, #0f172a 100%)",
        color: "#f8fafc",
        padding: "96px 80px",
    ...
    >
      ...
      >
        {listing.photoUrl ? (
          ...
          // biome-ignore lint/performance/noImgElement: next/image is unsupported inside OG ImageResponse (Satori)
          <img
      ...
      >
        ...
        >
          {listing.title.length > 38 ? listing.title.slice(0, 36) + "…" : listing.title}
        </div>
        <div
          style={{
            fontSize: "96px",
