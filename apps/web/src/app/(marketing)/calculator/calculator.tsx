...
function PlatformRow({
  ...
  p: PlatformResult;
  best: { sellerNet: number; buyerCost: number };
}) {
  ...
  return (
    <tr className={isHaggle ? "bg-action-primary/5" : ""}>
      {/* Platform */}
      <td className="py-3 px-3">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: p.color }} />
          <span
            className={`text-sm font-medium ${isHaggle ? "text-action-primary-hover" : "text-ink"}`}
          >
            {p.platformName}
          </span>
          {p.negotiable && (
      ...
      <td className="py-3 px-3 text-right">
        <span className={`text-sm ${isHaggle ? "text-action-primary" : "text-error"}`}>
          {fmt(p.totalFee)}
        </span>
        <span className="text-[10px] text-ink-muted ml-1">({p.feePercent.toFixed(1)}%)</span>
      </td>
      {/* Shipping */}
      <td className="py-3 px-3 text-right">
        <span className="text-xs text-ink-secondary">
          {fmt(p.sellerShippingCost + p.buyerShippingCost)}
        </span>
        <p className="text-[10px] text-ink-muted leading-tight">
          {p.shippingModel.split("(")[0].trim()}
        </p>
      </td>
      {/* Seller gets */}
      <td className="py-3 px-3 text-right">
        <span
          className={`text-sm font-semibold ${isBestSeller ? "text-success" : "text-ink-secondary"}`}
        >
          {fmtR(p.sellerNet)}
        </span>
        {isBestSeller && !isHaggle && <span className="text-[10px] text-success ml-1">best</span>}
        {isHaggle && <span className="text-[10px] text-action-primary ml-1">best</span>}
      </td>
      {/* Buyer pays */}
      <td className="py-3 px-3 text-right">
        <span
          className={`text-sm font-semibold ${isBestBuyer ? "text-success" : "text-ink-secondary"}`}
        >
          {fmtR(p.buyerTotalCost)}
        </span>
        {p.buyerProtectionFee > 0 && (
          <p className="text-[10px] text-ink-muted">incl. {fmt(p.buyerProtectionFee)} protection</p>
        )}
        {p.buyerShippingCost > 0 && (
          <p className="text-[10px] text-ink-muted">incl. {fmt(p.buyerShippingCost)} shipping</p>
        )}
      </td>
    </tr>
  );
    ...
    <div className="rounded-2xl border border-action-primary/20 bg-surface-raised p-6">
      ...
      <div className="mb-6">
        ...
        <div className="relative h-10 rounded-lg bg-surface-sunken overflow-hidden">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to right, var(--fb-info-fg), var(--action-primary), var(--fb-warning-fg))",
            }}
          />
          {/* Midpoint marker */}
          <div
            className="absolute top-0 bottom-0 w-0.5 bg-surface-raised/80"
            style={{ left: `${midPct}%` }}
          />
          <div
            className="absolute -top-5 text-[10px] text-ink font-medium"
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div className="rounded-lg bg-info-soft border border-info/20 p-3">
          <p className="text-[10px] text-info mb-1">Buyer&apos;s best</p>
          <p className="text-lg font-bold text-ink">{fmtR(zone.buyerIdeal)}</p>
          <p className="text-[10px] text-ink-muted">
            Seller still gets {fmtR(zone.sellerNetAtBuyerIdeal)}
          </p>
          <p className="text-[10px] text-success">vs {fmtR(zone.ebaySellerNet)} on eBay</p>
        </div>
        <div className="rounded-lg bg-action-primary/10 border border-action-primary/20 p-3">
          <p className="text-[10px] text-action-primary mb-1">AI likely price</p>
          <p className="text-lg font-bold text-action-primary-hover">{fmtR(zone.midpoint)}</p>
          <p className="text-[10px] text-ink-muted">Seller keeps {fmtR(zone.sellerNetAtMid)}</p>
          <p className="text-[10px] text-success">
            +{fmtR(zone.sellerNetAtMid - zone.ebaySellerNet)} vs eBay
          </p>
        </div>
        <div className="rounded-lg bg-warning-soft border border-warning/20 p-3">
          <p className="text-[10px] text-warning mb-1">Seller&apos;s ask</p>
          <p className="text-lg font-bold text-ink">{fmtR(zone.sellerAsk)}</p>
          <p className="text-[10px] text-ink-muted">Seller keeps {fmtR(zone.sellerNetAtAsk)}</p>
          <p className="text-[10px] text-success">
            +{fmtR(zone.sellerNetAtAsk - zone.ebaySellerNet)} vs eBay
          </p>
        </div>
      </div>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 pt-10 sm:pt-14 pb-20">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-ink mb-2">
            Marketplace Fee &amp; Shipping Calculator
          </h1>
          <p className="text-ink-secondary max-w-2xl mx-auto">
            Compare fees, shipping, seller payouts, and buyer costs across 6 platforms. See the full
            picture — not just fees.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto mb-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted text-lg">
              $
            </span>
            <input
              type="number"
          ...
          <select
            ...
            className="rounded-xl border border-line bg-surface-overlay px-4 py-3 text-sm text-ink focus:border-focus focus:outline-none focus:ring-1 focus:ring-focus"
          >
            {CATEGORIES.map((c) => (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-line">
                      <th className="text-left text-xs text-ink-muted font-medium py-3 px-3">
                        Platform
                      </th>
                      <th className="text-right text-xs text-ink-muted font-medium py-3 px-3">
                        Fees
                      </th>
                      <th className="text-right text-xs text-ink-muted font-medium py-3 px-3">
                        Shipping
                  ...
                  <tbody className="divide-y divide-line">
                    {results.map((p) => (
                      <PlatformRow
                        key={p.platformName}
                        p={p}
                        best={{ sellerNet: bestSellerNet, buyerCost: bestBuyerCost }}
                      />
                    ))}
                  </tbody>
                </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              ...
              <div className="rounded-2xl border border-line bg-surface-raised p-5">
                <h3 className="text-sm font-semibold text-ink mb-4">
                  Seller Keeps (after fees + shipping)
                </h3>
                <div className="space-y-2.5">
                  {results
                    .sort((a, b) => b.sellerNet - a.sellerNet)
                    .map((p) => {
                      ...
                      return (
                        <div key={p.platformName}>
                          <div className="flex items-center justify-between mb-0.5">
                            <span
                              className={`text-xs ${isHaggle ? "text-action-primary font-medium" : "text-ink-secondary"}`}
                            >
                              {p.platformName}
                            </span>
                            <span
                              className={`text-xs font-mono ${isHaggle ? "text-action-primary-hover" : "text-ink-secondary"}`}
                            >
                              {fmtR(p.sellerNet)}
                            </span>
                          </div>
                          <div className="h-5 rounded bg-surface-sunken overflow-hidden">
                            <div
              ...
              <div className="rounded-2xl border border-line bg-surface-raised p-5">
                <h3 className="text-sm font-semibold text-ink mb-4">
                  Buyer Total Cost (item + shipping + fees)
                </h3>
                <div className="space-y-2.5">
                  {results
                    .sort((a, b) => a.buyerTotalCost - b.buyerTotalCost)
                    .map((p) => {
            ...
            {sellerSavings > 0 && (
              <div className="rounded-2xl border border-action-primary/30 bg-action-primary/5 p-6 text-center mb-8">
                ...
                <button
                  type="button"
                  onClick={handleShare}
                  className="rounded-xl bg-action-primary px-6 py-2.5 text-sm font-medium text-on-accent hover:bg-action-primary-hover transition-colors"
                >
                  Share on Twitter
                </button>
              </div>
        ...
        <div className="mt-16 max-w-3xl mx-auto">
          <h2 className="text-xl font-semibold text-ink mb-6">Frequently Asked Questions</h2>
          <div className="space-y-4">
            {[
              ...
              },
              {
                ...
                a: "Poshmark provides a flat-rate $8.27 USPS Priority Mail label for packages up to 5 lbs. The buyer pays this fee. Sellers ship for free using the prepaid label. Items over 5 lbs incur a surcharge.",
              },
              {
                q: "How does Haggle's shipping work?",
                a: "Haggle uses EasyPost to offer commercial shipping rates (40-60% off USPS retail). The unique part: shipping cost is negotiable between buyer and seller. You can split it any way — 100% buyer, 100% seller, or anything in between.",
              },
              {
                q: "What can you negotiate on Haggle?",
              ...
              },
              {
                ...
                a: "Yes. In addition to the 10% seller fee, Mercari charges buyers a 3.6% 'buyer protection fee' on each purchase. This is separate from shipping costs.",
              },
            ].map((faq) => (
              <details
                key={faq.q}
                className="group rounded-xl border border-line bg-surface-raised"
              >
                <summary className="flex items-center justify-between px-5 py-4 text-sm font-medium text-ink cursor-pointer list-none">
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    strokeWidth="2"
                    className="text-ink-muted group-open:rotate-180 transition-transform shrink-0 ml-2"
                  >
                    <polyline points="6 9 12 15 18 9" />
                  </svg>
                </summary>
                <p className="px-5 pb-4 text-sm text-ink-secondary leading-relaxed">{faq.a}</p>
              </details>
            ))}
          </div>
        </div>
