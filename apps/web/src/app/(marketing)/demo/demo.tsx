...
const STRATEGY_PRESETS: Record<
  string,
  { label: string; desc: string; buyer: Partial<AgentConfig>; seller: Partial<AgentConfig> }
> = {
  balanced: {
    label: "Balanced",
    desc: "Fair negotiation — both sides compromise evenly",
    buyer: {
      beta: 0.8,
      aspiration: 0.7,
      priceWeight: 0.5,
  ...
  aggressive_buyer: {
    ...
    desc: "Buyer pushes hard, seller is flexible",
    buyer: {
      beta: 0.5,
      aspiration: 0.8,
      priceWeight: 0.65,
  ...
  aggressive_seller: {
    label: "Firm Seller",
    desc: "Seller barely budges, buyer needs to come up",
    buyer: {
      beta: 1.2,
      aspiration: 0.55,
      priceWeight: 0.4,
  ...
  rush: {
    label: "Quick Deal",
    desc: "Both want to close fast — fewer rounds",
    buyer: {
      beta: 1.5,
      aspiration: 0.5,
      priceWeight: 0.3,
  ...
  low_trust: {
    label: "Low Trust",
    desc: "Both parties are cautious — risk dominates",
    buyer: {
      trustScore: 0.3,
  ...
  veteran: {
    label: "Veteran Traders",
    desc: "High trust, lots of past deals — smooth negotiation",
    buyer: {
      trustScore: 0.95,
      pastSuccesses: 8,
      vsBase: 0.7,
...
function adjustWeights(
  ...
  const keys: Array<"priceWeight" | "timeWeight" | "riskWeight" | "relWeight"> = [
    "priceWeight",
    "timeWeight",
    "riskWeight",
    "relWeight",
  ...
  if (othersSum > 0) {
    ...
    others.forEach((k) => {
      result[k] = round2(remaining / others.length);
    });
  }
  ...
  return (
    <div className="mb-2.5">
      ...
      <input
        ...
        disabled={disabled}
        className="w-full h-1.5 rounded-full appearance-none bg-surface-sunken accent-action-primary disabled:opacity-40"
      />
...
function SectionHeader({
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className="flex items-center justify-between w-full text-left py-1.5 text-xs font-semibold text-ink-secondary uppercase tracking-wider cursor-pointer hover:text-ink transition-colors"
    >
      {title}
...
function UtilityBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-ink-secondary w-8 text-right">{label}</span>
      ...
      <span className="text-[10px] font-mono text-ink-secondary w-8">
        {(value * 100).toFixed(0)}%
      </span>
    </div>
  );
...
function AgentPanel({
  ...
  const [collapsedSections, setCollapsed] = useState<Record<string, boolean>>({
    price: false,
    time: true,
    risk: true,
    rel: true,
  ...
  const prices = computePriceTargets(item, cfg, side);
  ...
  return (
    <div className={`rounded-xl border ${borderColor} ${bgColor} p-4`}>
      ...
      {/* Faratin Concession */}
      <Slider
        label="Concession Speed (\u03B2)"
        value={cfg.beta}
        onChange={(v) => set({ beta: v })}
        min={0.2}
        max={2.5}
        step={0.1}
        hint="<1 boulware(slow) | >1 conceder(fast)"
        disabled={disabled}
      />

      {/* Decision */}
      <Slider
        label="Aspiration (accept threshold)"
        value={cfg.aspiration}
        onChange={(v) => set({ aspiration: v })}
        min={0.2}
        max={0.95}
        step={0.05}
        hint="Min utility to accept"
        disabled={disabled}
      />
      <Slider
      ...
      {/* Price Section */}
      <div className="border-t border-line-subtle mt-3 pt-2">
        <SectionHeader
          title={`Price (target: $${prices.target}, limit: $${prices.limit})`}
        ...
        {!collapsedSections.price && (
          <>
            <Slider
              label={side === "buyer" ? "Target (% below list)" : "Target (% below list)"}
              value={cfg.targetOffset}
              onChange={(v) => set({ targetOffset: v })}
              min={side === "buyer" ? -40 : -10}
              max={side === "buyer" ? 0 : 5}
              step={1}
              suffix="%"
              disabled={disabled}
            />
            <Slider
              label={side === "buyer" ? "Walk-away (% below list)" : "Walk-away (% below list)"}
              value={cfg.limitOffset}
              onChange={(v) => set({ limitOffset: v })}
              min={side === "buyer" ? -10 : -40}
              max={side === "buyer" ? 10 : 0}
              step={1}
              suffix="%"
              disabled={disabled}
      ...
      <div className="border-t border-line-subtle mt-2 pt-2">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`text-xs font-mono ${Math.abs(cfg.priceWeight + cfg.timeWeight + cfg.riskWeight + cfg.relWeight - 1) < 0.02 ? "text-success" : "text-error"}`}
          >
        ...
        <Slider
          label="Price (w_p)"
          value={cfg.priceWeight}
          onChange={(v) => set(adjustWeights(cfg, "priceWeight", v))}
          min={0.05}
          max={0.85}
          step={0.05}
          disabled={disabled}
        ...
        <Slider
          ...
          onChange={(v) => set(adjustWeights(cfg, "timeWeight", v))}
          min={0.05}
          max={0.85}
          step={0.05}
          disabled={disabled}
        ...
        <Slider
          ...
          onChange={(v) => set(adjustWeights(cfg, "riskWeight", v))}
          min={0.05}
          max={0.85}
          step={0.05}
          disabled={disabled}
        />
        <Slider
          label="Relationship (w_s)"
          value={cfg.relWeight}
          onChange={(v) => set(adjustWeights(cfg, "relWeight", v))}
          min={0.05}
          max={0.85}
          step={0.05}
          disabled={disabled}
        ...
        {!collapsedSections.time && (
          <>
            ...
            <Slider
              ...
              max={5.0}
              step={0.5}
              hint="Higher = steeper pressure near deadline"
              disabled={disabled}
            />
            <Slider
              label="Floor (V_t min)"
              value={cfg.vtFloor}
              onChange={(v) => set({ vtFloor: v })}
              min={0}
              max={0.3}
              step={0.05}
              hint="Minimum time utility even at deadline"
              disabled={disabled}
            />
          </>
        )}
      ...
      <div className="border-t border-line-subtle mt-2 pt-2">
        ...
        {!collapsedSections.risk && (
          <>
            <Slider
              label="Counterparty trust (r_score)"
            ...
            <Slider
              ...
              value={cfg.infoCompleteness}
              onChange={(v) => set({ infoCompleteness: v })}
              min={0.0}
              max={1.0}
              step={0.05}
              hint="How much you know about the item"
              disabled={disabled}
            />
            <Slider
              label="Reputation weight"
              value={cfg.wRep}
              onChange={(v) => set({ wRep: v, wInfo: +(1 - v).toFixed(2) })}
              min={0.0}
              max={1.0}
              step={0.1}
              hint={`Info weight: ${(1 - cfg.wRep).toFixed(1)}`}
              disabled={disabled}
            />
          </>
        )}
      ...
      <div className="border-t border-line-subtle mt-2 pt-2">
        <SectionHeader
          ...
          collapsed={collapsedSections.rel}
          onToggle={() => toggle("rel")}
        />
        {!collapsedSections.rel && (
          <>
            <Slider
              ...
              value={cfg.pastSuccesses}
              onChange={(v) => set({ pastSuccesses: v })}
              min={0}
              max={20}
              step={1}
              disabled={disabled}
            />
            <Slider
              label="Past dispute losses"
              value={cfg.pastDisputeLosses}
              onChange={(v) => set({ pastDisputeLosses: v })}
              min={0}
              max={5}
              step={1}
              disabled={disabled}
            ...
            <Slider
              ...
              value={cfg.successThreshold}
              onChange={(v) => set({ successThreshold: v })}
              min={1}
              max={20}
              step={1}
              hint="n_threshold for V_s calculation"
              disabled={disabled}
            />
            <Slider
              label="Base relationship (V_s base)"
              value={cfg.vsBase}
              onChange={(v) => set({ vsBase: v })}
              min={0.0}
              max={1.0}
              step={0.05}
              hint="Starting relationship utility"
              disabled={disabled}
            />
          </>
        )}
...
export function Demo() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: scroll-to-bottom keyed on rounds changes
  useEffect(() => {
  ...
  const runSimulation = useCallback(async () => {
    ...
    for (let r = 2; r <= maxRounds; r++) {
      ...
      const ctx = {
        ...
        time: {
          t_elapsed: elapsed,
          t_deadline: cfg.deadline,
          alpha: cfg.alpha,
          v_t_floor: cfg.vtFloor,
        },
        risk: {
          r_score: cfg.trustScore,
          i_completeness: cfg.infoCompleteness,
          w_rep: cfg.wRep,
          w_info: cfg.wInfo,
        },
        relationship: {
          n_success: cfg.pastSuccesses,
          n_dispute_losses: cfg.pastDisputeLosses,
          n_threshold: cfg.successThreshold,
          v_s_base: cfg.vsBase,
        },
      };
      ...
      const decision = makeDecision(
        utility,
        { u_threshold: cfg.threshold, u_aspiration: cfg.aspiration },
        { rounds_no_concession: noConcess },
      );
      ...
      if (decision.action === "ACCEPT" || decision.action === "NEAR_DEAL") {
        ...
        newRounds.push({
          ...
          action: "ACCEPT",
          price: acceptPrice,
          note: `Accepts $${acceptPrice}`,
          utility,
          decision: decision.action,
          noConcess,
        });
        setRounds([...newRounds]);
        setFinalPrice(acceptPrice);
        setSimState("done");
        return;
      }
      ...
      if (decision.action === "REJECT" || decision.action === "ESCALATE") {
        newRounds.push({
          ...
          price: currentPrice,
          note: decision.action === "REJECT" ? "Walks away" : "Requests escalation",
          utility,
          decision: decision.action,
          noConcess,
        });
      ...
      const counter = computeCounterOffer({
        p_start: prices.target,
        p_limit: prices.limit,
        t: elapsed,
        T: cfg.deadline,
        beta: cfg.beta,
      });
      ...
      if (isSeller) {
        // Track concession
        if (roundedCounter >= prevSellerPrice) {
          sellerNoConcess++;
        } else {
          sellerNoConcess = 0;
        }
        ...
        newRounds.push({
          ...
          actor: "seller",
          action: "COUNTER",
          price: roundedCounter,
          note: `Counters $${roundedCounter}`,
          utility,
          decision: decision.action,
          noConcess,
        });
      } else {
        if (roundedCounter <= prevBuyerPrice) {
          buyerNoConcess++;
        } else {
          buyerNoConcess = 0;
        }
    ...
    newRounds.push({
      number: newRounds.length,
      actor: "buyer",
      action: "ACCEPT",
      price: mid,
      note: `Both agree at $${mid}`,
    });
    setRounds([...newRounds]);
    setFinalPrice(mid);
    setSimState("done");
  ...
  return (
    <div className="min-h-screen">
      <section className="mx-auto max-w-6xl px-4 sm:px-6 pt-10 sm:pt-14 pb-20">
        ...
        <div className="text-center mb-6">
          <h1 className="text-3xl sm:text-4xl font-bold text-ink mb-2">
            Watch AI Negotiate in Real Time
          </h1>
          <p className="text-ink-secondary">
            Pick your side, tune both AI engines, and watch them negotiate.
          </p>
        </div>

        <div className="mx-auto mb-6 max-w-3xl rounded-2xl border border-action-primary/25 bg-[color-mix(in_srgb,var(--action-primary)_10%,transparent)] p-4 sm:p-5">
        ...
        <div className="flex flex-col items-center gap-4 mb-6">
          <div className="inline-flex rounded-xl border border-line bg-surface-raised p-1">
            <button
              ...
              disabled={isRunning}
              className={`rounded-lg px-5 py-2 text-sm font-medium transition-all cursor-pointer disabled:cursor-not-allowed ${perspective === "buyer" ? "bg-info-soft text-info border border-info/30" : "text-ink-secondary hover:text-ink border border-transparent"}`}
            >
              I&apos;m Buying
            </button>
            <button
              ...
              disabled={isRunning}
              className={`rounded-lg px-5 py-2 text-sm font-medium transition-all cursor-pointer disabled:cursor-not-allowed ${perspective === "seller" ? "bg-badge text-badge-text border border-badge-text/30" : "text-ink-secondary hover:text-ink border border-transparent"}`}
            >
              I&apos;m Selling
            </button>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {ITEMS.map((item) => (
              <button
                ...
                disabled={isRunning}
                className={`flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-medium transition-all cursor-pointer disabled:cursor-not-allowed ${selectedItem.id === item.id ? "border-action-primary/40 bg-[color-mix(in_srgb,var(--action-primary)_10%,transparent)] text-action-primary" : "border-line bg-surface-raised text-ink-secondary hover:border-line-strong"}`}
              >
                <span>{item.emoji}</span>
                <span>{item.name}</span>
                <span className="text-xs text-ink-muted">${item.listPrice}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="text-center mb-4">
          <button
            ...
            className="text-sm text-ink-secondary hover:text-action-primary transition-colors"
          >
        ...
        {showAdvanced && (
          <div className="mb-6">
            <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
              {Object.entries(STRATEGY_PRESETS).map(([id, p]) => (
                <button
                  ...
                  disabled={isRunning}
                  className="rounded-lg border border-line bg-surface-raised px-3 py-1.5 text-xs text-ink-secondary hover:border-action-primary/40 hover:text-action-primary transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                  title={p.desc}
                >
                  {p.label}
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              />
              <AgentPanel
                title="Seller AI Engine"
                color="text-badge-text"
                borderColor="border-badge-text/20"
                bgColor="bg-badge"
                cfg={sellerCfg}
                onChange={(c) => {
                  setSellerCfg(c);
        ...
        {rounds.length > 0 && (
          <div className="max-w-3xl mx-auto mb-8 space-y-3">
            {rounds.map((round) => {
              const isYou =
                (isBuyer && round.actor === "buyer") || (!isBuyer && round.actor === "seller");
              const isAccept = round.action === "ACCEPT";
              const isFail = round.action === "REJECT" || round.action === "ESCALATE";
              return (
                <div
                  key={`${round.number}-${round.actor}-${round.action}`}
                  className={`flex ${isYou ? "justify-start" : "justify-end"}`}
                  style={{ animation: "fadeInUp 0.3s ease-out" }}
                >
                  <div
                    className={`rounded-xl px-5 py-3 ${showAdvanced ? "max-w-md" : "max-w-xs"} ${
                      isAccept
                        ? "bg-success-soft border border-success/30"
                        : isFail
                          ? "bg-error-soft border border-error/30"
                          : isYou
                            ? isBuyer
                              ? "bg-info-soft border border-info/20"
                              : "bg-badge border border-badge-text/20"
                            : "bg-surface-sunken border border-line"
                    }`}
                  >
                    <p className="text-xs text-ink-secondary mb-1">
                      {isYou ? "Your AI" : "Their AI"} (
                      {round.actor === "buyer" ? "Buyer" : "Seller"}){isAccept && " \u2714"}
                      {isFail && " \u2718"}
                    </p>
                    <p
                      className={`text-sm font-medium ${isAccept ? "text-success" : isFail ? "text-error" : "text-ink"}`}
                    >
                      {round.note}
                    </p>
                    {showAdvanced && round.utility && (
                      <div className="mt-2 pt-2 border-t border-line-subtle space-y-1">
                        <UtilityBar
                          label="U"
                          value={round.utility.u_total}
                          color="bg-action-primary"
                        />
                        <UtilityBar label="V_p" value={round.utility.v_p} color="bg-success" />
                        <UtilityBar label="V_t" value={round.utility.v_t} color="bg-warning" />
                        <UtilityBar label="V_r" value={round.utility.v_r} color="bg-info" />
                        <UtilityBar label="V_s" value={round.utility.v_s} color="bg-badge" />
                        <p className="text-[10px] text-ink-muted">
                          {round.decision}
                          {round.noConcess ? ` (stall: ${round.noConcess})` : ""}
                        </p>
                      </div>
                    )}
                  </div>
        ...
        {/* Failed negotiation */}
        {failed && (
          <div className="max-w-lg mx-auto rounded-2xl border border-error/30 bg-error-soft p-6 sm:p-8 mb-8 text-center">
            ...
            <p className="text-sm text-ink-secondary mb-4">
              The AI agents couldn&apos;t agree on a price. Try adjusting the engine parameters.
            </p>
            <button
              type="button"
              onClick={reset}
        ...
        {simState === "done" && finalPrice && (
          <div className="max-w-lg mx-auto rounded-2xl border border-action-primary/30 bg-surface-raised p-6 sm:p-8 mb-8">
            <div className="text-center mb-6">
              <p className={`text-sm mb-2 ${isBuyer ? "text-info" : "text-badge-text"}`}>
                {isBuyer ? "Buyer Result" : "Seller Result"}
              </p>
              {isBuyer ? (
                <>
        ...
        <div className="text-center mt-6 mb-10">
          <Link
            ...
            className="text-sm text-ink-secondary hover:text-action-primary transition-colors"
          >
            Want the original playground? &rarr;
          </Link>
        </div>
        <div className="max-w-md mx-auto">
          <p className="text-center text-ink-secondary mb-4">
            Want AI to negotiate your real deals?
          </p>
      ...
      <style
        jsx
        global
      >{`@keyframes fadeInUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }`}</style>
    </div>
  );
}
