...
function TableWrapper({
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-raised overflow-hidden">
      <div className="px-3 py-2 border-b border-line-subtle bg-surface-sunken flex items-center justify-between">
        <h4 className="text-[11px] font-semibold text-ink-secondary uppercase tracking-wider font-mono">
          {title}
        </h4>
        {rowCount !== undefined && (
          <span className="text-[10px] font-mono text-ink-muted">{rowCount}행</span>
        )}
      </div>
...
function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="px-2.5 py-1.5 text-left text-[10px] font-semibold text-ink-muted uppercase tracking-wider whitespace-nowrap">
      {children}
    </th>
  );
}

function Td({ children, highlight }: { children: React.ReactNode; highlight?: "new" | "updated" }) {
  const bg =
    highlight === "new" ? "bg-success-soft" : highlight === "updated" ? "bg-warning-soft" : "";

  return (
    <td
      className={`px-2.5 py-1.5 text-[11px] font-mono text-ink-secondary whitespace-nowrap ${bg}`}
    >
      {children}
    </td>
  );
  ...
  return (
    <div className="space-y-4" style={{ animation: "fadeInUp 0.3s ease-out" }}>
      <h3 className="text-xs font-semibold text-ink-muted uppercase tracking-wider">
        시뮬레이션 DB 상태
        <span className="ml-2 text-[10px] text-ink-muted normal-case font-normal">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            <tr>
              ...
              <Td highlight={rounds.length > 0 ? "updated" : undefined}>
                {sessionRow.current_round}
              </Td>
              <Td highlight={rounds.length > 0 ? "updated" : undefined}>
                {fmt(sessionRow.last_offer_price_minor)}
              </Td>
          ...
          <tbody>
            {roundRows.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="px-2.5 py-3 text-center text-[11px] text-ink-muted italic"
                >
                  라운드 실행 시 행이 추가됩니다
                </td>
              </tr>
              ...
              roundRows.map((row, i) => (
                <tr key={row.round_no} className="border-b border-line-subtle">
            ...
            {factRows.length === 0 ? (
              <tr>
                <td
                  ...
                  className="px-2.5 py-3 text-center text-[11px] text-ink-muted italic"
                >
                  라운드 실행 시 해시 체인이 생성됩니다
                </td>
              </tr>
            ) : (
              factRows.map((row, i) => (
                <tr key={row.round} className="border-b border-line-subtle">
                  <Td highlight={i === factRows.length - 1 ? "new" : undefined}>{row.round}</Td>
                  <Td highlight={i === factRows.length - 1 ? "new" : undefined}>
                    <span className="text-action-primary">{row.fact_hash}</span>
                  </Td>
                  <Td highlight={i === factRows.length - 1 ? "new" : undefined}>
                    <span className="text-ink-muted">{row.prev_hash}</span>
                    {i > 0 && <span className="text-[9px] text-info ml-1">← chain</span>}
                  </Td>
                  <Td highlight={i === factRows.length - 1 ? "new" : undefined}>
                    {fmt(row.buyer_offer)}
                  </Td>
                  <Td highlight={i === factRows.length - 1 ? "new" : undefined}>
                    {fmt(row.seller_offer)}
                  </Td>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {telemetryRows.map((row) => {
              ...
              return (
                <tr key={row.stage} className="border-b border-line-subtle">
                  <Td highlight={isNew ? "new" : undefined}>{row.stage}</Td>
                  <Td highlight={isNew ? "new" : undefined}>
