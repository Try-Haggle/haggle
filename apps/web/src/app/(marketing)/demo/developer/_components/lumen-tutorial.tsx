...
const tutorialSteps: TutorialStep[] = [
  {
    ...
    role: "초보 딜러의 시선",
    product:
      "첫 화면은 에그가 아니라 리스팅과 검색이다. 유저는 당근마켓처럼 사고팔 수 있고, 피즈는 선택을 방해하지 않는 작은 반응만 남긴다.",
    line: "거래는 먼저 물건을 보는 일이다. 빛은 거래가 끝난 뒤에 남는다.",
    image: "/lumen/kai.png",
    imageAlt: "카이 캐릭터 바이블 이미지",
    scene:
      "상단에는 검색, 필터, 추천 리스팅이 먼저 보인다. 루멘 요소는 도움말 배지나 미세한 반응으로만 존재한다.",
  ...
  },
  {
    ...
    role: "만드는 흐름",
    product:
      "판매자는 사진, 상태, 설명, 카테고리를 입력한다. 세계관 설명보다 정확한 상품 정보와 신뢰 신호가 우선이다.",
    line: "만든 것에는 형태가 필요하다. 설명하지 않으면 아무도 제대로 원할 수 없다.",
    image: "/lumen/fab.png",
    imageAlt: "팹 캐릭터 이미지",
    scene:
      "상품 등록 폼은 일반 마켓 UI로 유지한다. 상태 설명이 좋아질 때 카드의 신뢰도가 조금 선명해진다.",
  ...
  },
  {
    ...
    role: "원하는 흐름",
    product:
      "구매자 의도와 판매자 조건이 만나는 지점을 보여준다. 버디가 거래를 대신하지 않고, 사용자의 오퍼 판단을 돕는다.",
  ...
  },
  {
    ...
    role: "재는 흐름",
    product:
      "시세, 비교 거래, 최대 지불가, floor price를 투명하게 보여준다. 정답 가격이 아니라 공정 범위를 제시해서 협상을 가르친다.",
  ...
  },
  {
    ...
    role: "정산과 회복",
    product:
      "배송 확인, 정산, 리뷰가 먼저 끝난다. 문제가 있으면 분쟁 UX로 가고, 문제가 없으면 거래 완료가 명확하게 보인다.",
    line: "규칙만으로는 아픈 사람이 남고, 이해만으로는 책임이 사라진다.",
    image: "/lumen/mia/default.png",
    imageAlt: "미아 캐릭터 이미지",
    scene:
      "거래 완료 화면은 지급 완료, 배송 확인, 리뷰 상태를 먼저 보여준다. 루멘 보상은 그 아래 선택 카드로만 등장한다.",
    signal:
      "분쟁이 없으면 피즈가 짧게 반응하고, 분쟁이 있으면 증거 타임라인과 조정 UI가 먼저 열린다.",
  ...
  },
  {
    ...
    role: "공명의 시작",
    product:
      "첫 거래 완료 후에만 에그를 소개한다. 관심 없는 유저는 닫고 다음 거래로 가며, 관심 있는 유저만 열어서 버디 레이어에 들어간다.",
    line: "버디는 거래를 시작하게 만드는 이유가 아니라, 좋은 거래가 남긴 흔적이다.",
    image: "/lumen/fizz.png",
    imageAlt: "피즈 버디 이미지",
    scene:
      "상호 평가 뒤 '축하해요, 에그가 떨어졌어요' 카드가 나타난다. 기본 CTA는 다음 거래이고, 보조 CTA가 에그 열기다.",
...
export function LumenTutorial() {
  ...
  return (
    <div className="mb-10 overflow-hidden rounded-2xl border border-action-primary/20 bg-black shadow-2xl shadow-black/20">
      <div className="relative min-h-[520px]">
        {/* biome-ignore lint/performance/noImgElement: decorative background art, optimization not needed in demo */}
        <img
        ...
        <div className="relative grid gap-6 p-5 sm:p-6 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:p-7">
          <div className="space-y-5">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-action-primary/30 bg-action-primary/10 px-3 py-1 text-xs font-medium text-action-primary">
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">
                튜토리얼의 목적은 사용자가 검색, 리스팅, 오퍼, 결제, 정산 흐름을 이해하게 만드는
                것입니다. 버디와 에그는 첫 거래가 끝난 뒤 관심 있는 유저가 더 들어가는 선택형
                레이어로만 등장합니다.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {flowLabels.map((label, index) => (
                <div key={label} className="rounded-lg border border-white/15 bg-white/5 px-3 py-2">
                ...
                <button
                  ...
                  className={`min-h-16 rounded-xl border px-3 py-2 text-left transition-colors ${
                    step.id === activeStep.id
                      ? "border-action-primary/70 bg-action-primary/15 text-white"
            ...
            <div className="rounded-xl border border-white/15 bg-white/5 p-4">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-md bg-action-primary/10 px-2 py-1 text-[11px] font-semibold text-action-primary">
                  {activeStep.role}
                </span>
              ...
              <p className="mt-3 border-l-2 border-action-primary/60 pl-3 text-sm italic leading-6 text-action-primary">
                {activeStep.line}
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-rows-[minmax(0,1fr)_auto]">
            <div className="grid min-h-[360px] overflow-hidden rounded-xl border border-white/15 bg-black/40 sm:grid-cols-[0.72fr_1fr]">
              <div className="relative min-h-[240px] border-b border-white/10 sm:border-b-0 sm:border-r">
                {/* biome-ignore lint/performance/noImgElement: decorative guide art, optimization not needed in demo */}
                <img
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.2em] text-action-primary">
                    Guide
                  </div>
              ...
              <div className="flex flex-col justify-between p-4 sm:p-5">
                ...
                <div className="mt-5 rounded-lg border border-success/20 bg-success-soft p-3">
