...
export function FilterSheet({
  ...
  }, [open, filters.categories, filters.conditions, filters.minPrice, filters.maxPrice, bounds?.min, bounds?.max]);
    ...
    >
      <Drawer.Portal>
        ...
        <Drawer.Content
          ...
          className="fixed inset-x-0 bottom-0 z-50 flex flex-col rounded-t-2xl border-t border-slate-800 bg-bg-primary outline-none"
          style={{
            height:
              typeof snap === "number" ? `${snap * 100}dvh` : "100dvh",
          }}
        >
          <div className="mx-auto mt-2 h-1 w-10 shrink-0 rounded-full bg-slate-700" />
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain">
            ...
            {/* Price */}
            <section className="border-t border-slate-800 px-5 py-5">
              <h3 className="mb-2 text-xs uppercase tracking-wide text-slate-500">
                Price
              </h3>
              {!bounds ? (
                <p className="text-sm text-slate-400">No priced listings to filter.</p>
              ) : (
                <>
                  ...
                  >
                    ...
                    <Slider.Thumb
                      className="block h-5 w-5 cursor-pointer rounded-full border-2 border-cyan-500 bg-slate-950 shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {priceBuckets.map((b) => (
                          <button
                            ...
                            className="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs text-slate-300 transition-colors hover:border-slate-500 hover:text-white"
                          >
                            {formatBucketLabel(b)}
          ...
          <div className="flex shrink-0 items-center gap-3 border-t border-slate-800 px-5 py-3">
            <button
              type="button"
              onClick={resetLocal}
            ...
            <button
              type="button"
              onClick={applyAll}
              className="ml-auto cursor-pointer rounded-lg bg-cyan-500 px-5 py-2 text-sm font-medium text-slate-950 transition-colors hover:bg-cyan-400"
            >
