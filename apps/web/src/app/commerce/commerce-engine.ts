...
function transitionApprovalState(
  ...
  const table: Record<ApprovalState, Partial<Record<ApprovalEvent, ApprovalState>>> = mode ===
  "AUTO_WITHIN_POLICY"
    ? {
        NEGOTIATING: {
...
const PAYMENT_TRANSITIONS: Record<
  PaymentIntentStatus,
  Partial<Record<PaymentEvent, PaymentIntentStatus>>
> = {
...
function transitionPaymentIntent(
...
const SHIPMENT_TRANSITIONS: Record<
  ShipmentStatus,
  Partial<Record<ShipmentEventType, ShipmentStatus>>
> = {
...
function computeOrderPhase(s: {
  ...
  if (
    (s.approval_state === "DECLINED" || s.approval_state === "EXPIRED") &&
    (!s.payment_status || s.payment_status === "NONE")
  )
    return "CANCELED";
  ...
  if (
    s.shipment_status === "DELIVERY_EXCEPTION" ||
    s.shipment_status === "IN_TRANSIT" ||
    s.shipment_status === "OUT_FOR_DELIVERY"
  )
    return "DELIVERY";
  if (
    s.shipment_status === "LABEL_CREATED" ||
    s.shipment_status === "PENDING_PICKUP" ||
    s.shipment_status === "SLA_MISSED"
  )
    return "FULFILLMENT";
  if (s.payment_status === "SETTLED") return "FULFILLMENT";
  if (
    s.payment_status === "INTENT_CREATED" ||
    s.payment_status === "AUTHORIZED" ||
    s.payment_status === "PENDING" ||
    s.payment_status === "CREATED" ||
    s.payment_status === "QUOTED" ||
    s.payment_status === "SETTLEMENT_PENDING"
  )
    return "PAYMENT";
  if (s.approval_state === "APPROVED") return "PAYMENT";
  if (s.approval_state && s.approval_state !== "NEGOTIATING" && s.approval_state !== "NONE")
    return "APPROVAL";
  return "NEGOTIATION";
}
...
function computeSettlementReliability(snap: {
...
export interface TimelineEvent {
  id: string;
  timestamp: string;
  phase: OrderPhase | "APPROVAL";
  title: string;
  detail: string;
  actor: "buyer" | "seller" | "system" | "ai";
  icon: string;
}
...
export function createInitialState(negotiation: Partial<NegotiationResult> = {}): CommerceState {
  ...
  return {
    ...
    trust_scores: {
      buyer_reliability: computeSettlementReliability({
        successful_settlements: 12,
        approval_defaults: 1,
        shipment_sla_misses: 0,
        dispute_wins: 0,
        dispute_losses: 0,
      }),
      seller_reliability: computeSettlementReliability({
        successful_settlements: 18,
        approval_defaults: 0,
...
export function buyerApprove(state: CommerceState): CommerceState {
  ...
  const next = transitionApprovalState(
    state.negotiation.seller_approval_mode,
  ...
  const events = [
    createEvent(
      "APPROVAL",
      "구매자 승인",
      `${state.negotiation.buyer_name}이(가) $${(state.negotiation.agreed_price / 100).toFixed(2)}에 거래를 승인했습니다`,
      "buyer",
      "✅",
    ),
  ];
  if (next === "APPROVED")
    events.push(
      createEvent(
        "APPROVAL",
        "자동 승인 완료",
        "판매자 정책: AUTO_WITHIN_POLICY — 거래가 자동 승인되었습니다",
        "system",
        "⚡",
      ),
    );
  ...
  return {
    ...state,
    approval_state: next,
    phase: derivePhase(state, { approval_state: next, payment_status: ps }),
    payment_status: ps,
  ...
  return {
    ...
    timeline: [
      ...
      createEvent(
        "APPROVAL",
        "판매자 수동 승인",
        `${state.negotiation.seller_name}이(가) 거래를 수동 승인했습니다`,
        "seller",
        "✅",
      ),
    ],
...
export function processPayment(state: CommerceState): CommerceState {
  ...
  let status: PaymentIntentStatus =
    state.payment_status === "IDLE" ? "CREATED" : (state.payment_status as PaymentIntentStatus);
  ...
  const steps: {
  ...
  }[] = [
    {
      event: "quote",
      title: "결제 견적 생성",
      detail: `x402 견적: Base 네트워크에서 ${amount / 100} USDC`,
      icon: "📋",
      actor: "system",
    },
    {
      event: "authorize",
      title: "결제 승인",
      detail: `구매자 지갑 ${state.buyer_wallet.address}에서 ${amount / 100} USDC 승인`,
      icon: "🔐",
      actor: "buyer",
    },
    {
      event: "mark_settlement_pending",
      title: "정산 대기",
      detail: `에스크로: 스마트 컨트랙트에 ${amount / 100} USDC 보관`,
      icon: "⏳",
      actor: "system",
    },
    {
      event: "settle",
      title: "결제 완료",
      detail: `판매자 수령: $${((amount - fee) / 100).toFixed(2)}, 플랫폼 수수료: $${(fee / 100).toFixed(2)} (1.5%)`,
      icon: "💰",
      actor: "system",
    },
  ];
...
export function submitShippingInfo(state: CommerceState, info: ShipmentInfo): CommerceState {
  ...
  return {
    ...
    phase: derivePhase(state, { shipment_status: next }),
    shipment: info,
    timeline: [
      ...state.timeline,
      createEvent(
        "FULFILLMENT",
        "배송 라벨 생성",
        `운송사: ${info.carrier} | 운송장: ${info.tracking_number}`,
        "seller",
        "📦",
      ),
    ],
...
const SHIPMENT_EVENT_MAP: Record<
  string,
  { event: ShipmentEventType; title: string; detailFn: (s: CommerceState) => string; icon: string }
> = {
  LABEL_CREATED: {
    event: "ship",
    title: "발송 완료",
    detailFn: (s) => `${s.shipment!.carrier}에서 물품을 수거했습니다`,
    icon: "🚚",
  },
  IN_TRANSIT: {
    event: "out_for_delivery",
    title: "배달 출발",
    detailFn: () => "구매자 지역에서 배달이 시작되었습니다",
    icon: "🏠",
  },
  OUT_FOR_DELIVERY: {
    event: "deliver",
    title: "배송 완료",
    detailFn: () => "물품이 정상 배달되었습니다. 에스크로가 판매자에게 정산됩니다.",
    icon: "✅",
  },
};
...
export function advanceShipment(state: CommerceState): CommerceState {
  ...
  return {
    ...
    shipment_status: next,
    phase,
    seller_wallet: delivered
      ? { ...state.seller_wallet, balance: state.seller_wallet.balance + state.escrow_held }
      : state.seller_wallet,
  ...
  return {
    ...
    timeline: [
      ...
      createEvent(
        "DELIVERY",
        "배송 예외 발생",
        "운송사에서 배송 문제를 보고했습니다. 분쟁을 신청할 수 있습니다.",
        "system",
        "⚠️",
      ),
    ],
...
export function fileDispute(
  state: CommerceState,
  info: Omit<DisputeInfo, "resolution">,
): CommerceState {
  if (state.dispute_status !== "NONE" || state.phase === "COMPLETED") return state;
  const label =
    DISPUTE_REASON_OPTIONS.find((o) => o.code === info.reason_code)?.label ?? info.reason_code;
  return {
    ...state,
    phase: derivePhase(state, { dispute_status: "OPEN" }),
    dispute_status: "OPEN",
    dispute: info,
    timeline: [
      ...state.timeline,
  ...
  return {
    ...
    timeline: [
      ...
      createEvent(
        "IN_DISPUTE",
        "AI 심사 시작",
        "AI 분쟁 에이전트가 증거, 배송 데이터, 거래 내역을 분석 중입니다...",
        "ai",
        "🔍",
      ),
    ],
...
export function resolveDispute(
  ...
  const dEvent =
    outcome === "buyer_favor"
      ? ("resolve_buyer_favor" as const)
      : outcome === "seller_favor"
        ? ("resolve_seller_favor" as const)
        : ("resolve_partial_refund" as const);
  let cur = state.dispute_status as DisputeStatus;
  if (cur === "OPEN") {
    const r = transitionDisputeStatus(cur, "review");
    if (r) cur = r;
  }
  ...
  if (outcome === "buyer_favor") {
    ...
    resolution = {
      outcome: "구매자 승소",
      summary: "AI 심사 결과 구매자의 주장이 타당합니다. 전액 환불 처리됩니다.",
      refund_amount: refund,
    };
    sRel = Math.max(0, sRel - penalty);
  } else if (outcome === "seller_favor") {
    resolution = {
      outcome: "판매자 승소",
      summary: "AI 심사 결과 판매자가 의무를 이행했습니다. 환불 없음.",
    };
    bRel = Math.max(0, bRel - penalty);
  } else {
    refund = Math.round(net * 0.5);
    resolution = {
      outcome: "부분 환불",
      summary: "AI 심사: 양측 모두 책임이 있습니다. 50% 환불 처리됩니다.",
      refund_amount: refund,
    };
    sRel = Math.max(0, sRel - penalty);
  }
  ...
  return {
    ...
    dispute: state.dispute ? { ...state.dispute, resolution } : undefined,
    buyer_wallet: { ...state.buyer_wallet, balance: state.buyer_wallet.balance + refund },
    seller_wallet: {
      ...state.seller_wallet,
      balance: state.seller_wallet.balance + (state.escrow_held - refund),
    },
    escrow_held: 0,
    seller_received: state.escrow_held - refund,
    ...
    timeline: [
      ...
      createEvent(
        "IN_DISPUTE",
        "AI 심사 완료",
        "AI 분쟁 에이전트가 증거를 분석하고 판결을 내렸습니다",
        "ai",
        "🤖",
      ),
      createEvent(
        refund > 0 ? "REFUNDED" : "COMPLETED",
        `분쟁 해결: ${resolution.outcome}`,
        `${resolution.summary}${refund > 0 ? ` 환불: $${(refund / 100).toFixed(2)}` : ""}`,
        "system",
        outcome === "buyer_favor" ? "🏆" : outcome === "seller_favor" ? "🛡️" : "⚖️",
      ),
    ],
  };
}
...
export function updateNegotiation(
  state: CommerceState,
  patch: Partial<NegotiationResult>,
): CommerceState {
