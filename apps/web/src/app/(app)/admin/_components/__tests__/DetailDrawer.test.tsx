...
describe("<DetailDrawer />", () => {
  it("renders nothing when id is null", () => {
    const { container } = render(<DetailDrawer type="tag" id={null} onClose={() => {}} />);
  ...
  it("shows metadata when open and closes via backdrop + Escape", async () => {
    ...
    const { rerender } = render(
      <DetailDrawer type="tag" id="tag-1" onClose={onClose} fetchDetail={fetchDetail} />,
    );

    await waitFor(() => expect(screen.getByTestId("drawer-detail")).toBeInTheDocument());
    ...
    rerender(<DetailDrawer type="tag" id={null} onClose={onClose} fetchDetail={fetchDetail} />);
