...
export default function NotificationSettingsPage() {
  ...
  const loadPrefs = useCallback(async () => {
    const { preferences } = await notificationApi
      .getPreferences()
      .catch(() => ({ preferences: {} }));
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-ink-muted">Manage your notification preferences</p>
      </div>

      <div className="rounded-xl border border-line bg-surface-raised p-4 sm:p-6 space-y-5">
        {CATEGORIES.map((category) => (
          <div key={category}>
            <p className="mb-3 text-xs font-medium text-ink-muted uppercase tracking-wider">
