...
test.describe("Seller wizard publish side effect", () => {
  ...
  test("publish creates a negotiation_agents row for the seller", async ({ page }) => {
    await signIn(page);
    ...
    const beforeRows = ((await before.json()) as { agents: unknown[] }).agents.length;
    ...
    await page.getByLabel(/title/i).fill("E2E iPhone Pro");
