...
export const DEFAULT_DECIDE_TIMEOUT_MS = 180_000;
export const MAX_DECIDE_TIMEOUT_MS = 300_000;

function clampTimeoutMs(value: number): number {
  return Math.min(MAX_DECIDE_TIMEOUT_MS, Math.max(10_000, Math.round(value)));
}

/** Resolve the Decide LLM timeout. Explicit override is used as-is. Else env, then 180s. */
