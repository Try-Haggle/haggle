...
describe("SessionMemoryStore", () => {
  ...
  it("searchByCondition finds facts with matching condition keys", async () => {
    ...
    await store.saveRoundFact(
      sid,
      makeFact({ round: 2, conditions_changed: { shipping: "free" } }),
    );
  ...
  it("searchByCondition finds facts with matching condition values", async () => {
    await store.saveRoundFact(
      sid,
      makeFact({ round: 1, conditions_changed: { delivery: "express_shipping" } }),
    );
    await store.saveRoundFact(
      sid,
      makeFact({ round: 2, conditions_changed: { payment: "upfront" } }),
    );
