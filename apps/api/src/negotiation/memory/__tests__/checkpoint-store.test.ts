...
describe("CheckpointStore", () => {
  ...
  it("should revert from CLOSING to BARGAINING", async () => {
    await store.save({
      ...
      phase: "BARGAINING",
      version: 1,
      core_memory_snapshot: makeMemory("BARGAINING"),
      conditions_state: {},
    ...
    await store.save({
      ...
      phase: "CLOSING",
      version: 1,
      core_memory_snapshot: makeMemory("CLOSING"),
      conditions_state: {},
  ...
  it("should block revert from SETTLEMENT", async () => {
    await store.save({
      ...
      phase: "SETTLEMENT",
      version: 1,
      core_memory_snapshot: makeMemory("SETTLEMENT"),
      conditions_state: {},
      total_rounds_at_checkpoint: 12,
      both_agreed: true,
    });

    await expect(store.revert("sess-3", "CLOSING", DEFAULT_REVERT_POLICY)).rejects.toThrow(
      "blocked",
    );
  });
  ...
  it("should block invalid transition", async () => {
    ...
    await expect(store.revert("sess-4", "DISCOVERY", DEFAULT_REVERT_POLICY)).rejects.toThrow(
      "not allowed",
    );
  });
});
