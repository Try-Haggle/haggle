...
import type { BuddyDNA, CoreMemory, RoundFact } from "../../types.js";
...
function makeMemory(
  overrides: Partial<CoreMemory["session"]> = {},
  bOverrides: Partial<CoreMemory["boundaries"]> = {},
): CoreMemory {
  return {
    session: {
      session_id: "test",
...
describe("computeCoaching", () => {
  ...
  it("opens under target for a buyer and never over the ask for a seller", () => {
    const buyerCoaching = computeCoaching(
      makeMemory({ phase: "OPENING", role: "buyer" }),
      [],
      null,
      BUDDY,
    );
    const sellerCoaching = computeCoaching(
      makeMemory({ phase: "OPENING", role: "seller" }),
      [],
      null,
      BUDDY,
    );
    // A buyer's target is private, so anchoring under it is ordinary negotiation.
    expect(buyerCoaching.recommended_price).toBeLessThan(500);
    // A seller's target is the PUBLISHED ask, so quoting over it is not — it is the
    // "$120 listing, $130 counter" fault.
    expect(sellerCoaching.recommended_price).toBeLessThanOrEqual(500);
  });
  ...
  it("should classify opponent from facts", () => {
    ...
    const facts = [makeFact(1, 500, 700), makeFact(2, 510, 660), makeFact(3, 520, 620)];
  ...
  it("should warn on high time pressure", () => {
    const coaching = computeCoaching(
      makeMemory({ round: 13, rounds_remaining: 2 }),
      [],
      null,
      BUDDY,
    );
...
describe("computeCoachingAsync", () => {
  ...
  it("falls back to 0.5 when DB query fails", async () => {
    ...
    const withFrom = { from: vi.fn().mockReturnValue(withWhere) };
    const mockDb = {
      select: vi.fn().mockReturnValue(withFrom),
