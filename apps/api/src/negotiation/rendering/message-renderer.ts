...
/**
 * Money for chat text. Cents are shown only when they exist: a cheap item genuinely
 * negotiates in cents, but "$194.00" reads like a machine and — more importantly —
 * differed from the badge above it, which rounded the same value to a whole dollar.
 * The web chat uses the same rule, so one offer never appears as two numbers.
 */
...
export class TemplateMessageRenderer implements MessageRenderer {
  render(decision: EngineDecision, context: RenderContext): string {
    ...
    switch (locale) {
      case "ko":
        message = this.getKoreanTemplate(action, price, tone, role, phase);
        break;
      case "ja":
        message = this.getJapaneseTemplate(action, price, tone, role, phase);
        break;
      case "zh":
        message = this.getChineseTemplate(action, price, tone, role, phase);
        break;
      case "es":
        message = this.getSpanishTemplate(action, price, tone, role, phase);
        break;
      case "vi":
        message = this.getVietnameseTemplate(action, price, tone, role, phase);
        break;
      case "tl":
        message = this.getTagalogTemplate(action, price, tone, role, phase);
        break;
      default:
        message = this.getActionTemplate(action, price, tone, role, phase);
        break;
    }
    ...
    if (tone.signature_phrases && tone.signature_phrases.length > 0) {
      ...
      if (useSignature) {
        const phrase =
          tone.signature_phrases[Math.floor(Math.random() * tone.signature_phrases.length)]!;
        message += ` ${phrase}`;
      }
    }
  ...
  private counterTemplate(
    price: string,
    style: BuddyTone["style"],
    formality: BuddyTone["formality"],
    _role: string,
  ): string {
  ...
  private acceptTemplate(
    price: string,
    style: BuddyTone["style"],
    _formality: BuddyTone["formality"],
  ): string {
    if (style === "professional") return `Agreed${price ? ` at ${price}` : ""}. Let's proceed.`;
    if (style === "friendly")
      return `Deal${price ? ` at ${price}` : ""}! Glad we could work this out!`;
    if (style === "analytical")
      return `Accepted${price ? ` at ${price}` : ""}. This falls within optimal range.`;
  ...
  private rejectTemplate(style: BuddyTone["style"], _formality: BuddyTone["formality"]): string {
    if (style === "professional")
      return "I appreciate the offer, but I'll need to pass on those terms.";
    if (style === "friendly")
      return "Thanks for the offer, but I can't quite make that work. Can we try again?";
    if (style === "analytical")
      return "The offer doesn't meet the acceptable range. Let's reconsider the terms.";
  ...
  private holdTemplate(style: BuddyTone["style"], _formality: BuddyTone["formality"]): string {
    if (style === "professional") return "I'd like to take a moment to review the current terms.";
    if (style === "friendly")
      return "Let me think about this for a sec — I want to make sure this is right for both of us.";
    if (style === "analytical")
      return "Pausing to reassess. Need to verify some details before proceeding.";
  ...
  private discoverTemplate(
    ...
    if (role === "buyer") {
      ...
      if (style === "friendly")
        return "Hey! I'm really interested. Can you share more details about it?";
      if (style === "analytical")
        return "I'd like to understand the specific condition metrics — battery health, screen state, accessories?";
      if (style === "assertive")
        return "I need the full details: condition, battery, accessories, and verification status.";
      if (style === "casual") return "So what's the deal with this one? How's the condition?";
    } else {
      if (style === "professional")
        return "I'd be happy to provide details. What would you like to know?";
      if (style === "friendly") return "Sure thing! Ask me anything about it.";
      if (style === "analytical")
        return "I can provide detailed condition data. What specifics do you need?";
      if (style === "assertive")
        return "Here's what I can tell you — ask and I'll give you the facts.";
  ...
  private confirmTemplate(
    ...
    if (style === "professional")
      return `Confirming the agreement${price ? ` at ${price}` : ""}. Ready to proceed to settlement.`;
    if (style === "friendly")
      return `Awesome, we have a deal${price ? ` at ${price}` : ""}! Let's wrap this up.`;
    if (style === "analytical")
      return `Confirmed${price ? ` at ${price}` : ""}. All terms verified. Proceeding to settlement.`;
    if (style === "assertive")
      return `Confirmed${price ? ` at ${price}` : ""}. Let's close this out.`;
  ...
  private getKoreanTemplate(
    ...
    switch (action) {
      ...
      case "ACCEPT":
        if (style === "professional")
          return `${priceStr ? `${priceStr}에 ` : ""}합의합니다. 진행하겠습니다.`;
        if (style === "friendly")
          return `좋아요${priceStr ? `, ${priceStr}에` : ""} 거래 성사! 잘 됐네요!`;
        if (style === "analytical")
          return `${priceStr ? `${priceStr} ` : ""}수락합니다. 적정 범위 내입니다.`;
      ...
      case "REJECT":
        ...
        if (style === "friendly")
          return "감사하지만 좀 어려울 것 같아요. 다시 한번 제안해주실 수 있나요?";
        if (style === "analytical")
          return "제안이 수용 범위에 들지 않습니다. 조건을 재검토해주세요.";
      ...
      case "HOLD":
        ...
        if (style === "analytical")
          return "잠시 멈추고 재평가하겠습니다. 몇 가지 확인이 필요합니다.";
      ...
      case "CONFIRM":
        if (style === "professional")
          return `${priceStr ? `${priceStr}에 ` : ""}합의를 확인합니다. 결제를 진행하겠습니다.`;
        if (style === "friendly")
          return `${priceStr ? `${priceStr}에 ` : ""}거래 확정! 마무리하죠!`;
        if (style === "analytical")
          return `${priceStr ? `${priceStr} ` : ""}확인 완료. 모든 조건 검증됨. 결제로 진행합니다.`;
  ...
  private getJapaneseTemplate(
    ...
    switch (action) {
      case "COUNTER":
        return tone.style === "professional"
          ? `${p}をご提案いたします。`
          : `${p}はいかがでしょうか？`;
      case "ACCEPT":
        return tone.style === "professional"
      ...
      case "REJECT":
        return tone.style === "professional"
          ? "ご提案ありがとうございますが、この条件では難しいです。"
          : "すみません、ちょっと難しいです。別の価格はどうですか？";
      case "HOLD":
        return "少々お待ちください。条件を確認しています。";
      case "DISCOVER":
        return role === "buyer"
          ? "商品の状態について詳しく教えていただけますか？"
          : "何でもお聞きください。詳細をお伝えします。";
      case "CONFIRM":
        return `${p ? `${p}で` : ""}確定します。決済に進みます。`;
      default:
        return `決定: ${action}${p ? ` (${p})` : ""}`;
    }
  }
  ...
  private getChineseTemplate(
    ...
    switch (action) {
      case "COUNTER":
        return tone.style === "professional" ? `我提议${p}。` : `${p}怎么样？`;
      case "ACCEPT":
        return tone.style === "professional"
          ? `${p ? `以${p}` : ""}达成协议。`
          : `${p ? `${p}` : ""}成交！`;
      case "REJECT":
        return tone.style === "professional"
          ? "感谢您的报价，但这个条件我无法接受。"
          : "不好意思，这个价格不太合适。能换个价格吗？";
      case "HOLD":
        return "请稍等，我需要确认一下条件。";
      case "DISCOVER":
        return role === "buyer" ? "能详细介绍一下商品状况吗？" : "请随时提问，我会详细说明。";
      case "CONFIRM":
        return `${p ? `以${p}` : ""}确认成交，进入付款。`;
      default:
        return `决定: ${action}${p ? ` (${p})` : ""}`;
    }
  }
  ...
  private getSpanishTemplate(
    ...
    switch (action) {
      case "COUNTER":
        return tone.style === "professional"
          ? `Me gustaría proponer ${p}.`
          : `¿Qué te parece ${p}?`;
      case "ACCEPT":
        return tone.style === "professional"
          ? `De acuerdo${p ? ` en ${p}` : ""}. Procedamos.`
          : `¡Trato hecho${p ? ` en ${p}` : ""}!`;
      case "REJECT":
        return tone.style === "professional"
          ? "Agradezco la oferta, pero no puedo aceptar estos términos."
          : "Lo siento, ese precio no me funciona. ¿Puedes ofrecer algo mejor?";
      case "HOLD":
        return "Permíteme un momento para revisar las condiciones.";
      case "DISCOVER":
        return role === "buyer"
          ? "¿Podrías darme más detalles sobre el estado del producto?"
          : "¡Pregunta lo que quieras! Con gusto te informo.";
      case "CONFIRM":
        return `Confirmado${p ? ` en ${p}` : ""}. Procedemos al pago.`;
      default:
        return `Decisión: ${action}${p ? ` (${p})` : ""}`;
    }
  }
  ...
  private getVietnameseTemplate(
    ...
    switch (action) {
      case "COUNTER":
        return tone.style === "professional" ? `Tôi đề xuất mức giá ${p}.` : `${p} được không bạn?`;
      case "ACCEPT":
        return tone.style === "professional"
          ? `Đồng ý${p ? ` với giá ${p}` : ""}. Tiến hành giao dịch.`
          : `Deal${p ? ` ${p}` : ""}! Tuyệt vời!`;
      case "REJECT":
        return tone.style === "professional"
          ? "Cảm ơn bạn đã đề nghị, nhưng tôi không thể chấp nhận mức giá này."
          : "Xin lỗi, giá này hơi cao. Bạn có thể giảm được không?";
      case "HOLD":
        return "Xin chờ một chút, tôi cần xem xét lại điều kiện.";
      case "DISCOVER":
        return role === "buyer"
          ? "Bạn có thể cho tôi biết thêm về tình trạng sản phẩm không?"
          : "Bạn cứ hỏi, tôi sẽ trả lời chi tiết.";
      case "CONFIRM":
        return `Xác nhận${p ? ` giá ${p}` : ""}. Chuyển sang thanh toán.`;
      default:
        return `Quyết định: ${action}${p ? ` (${p})` : ""}`;
    }
  }
  ...
  private getTagalogTemplate(
    ...
    switch (action) {
      case "COUNTER":
        return tone.style === "professional" ? `Gusto kong mag-alok ng ${p}.` : `Paano kung ${p}?`;
      case "ACCEPT":
        return tone.style === "professional"
          ? `Payag ako${p ? ` sa ${p}` : ""}. Ituloy natin.`
          : `Deal${p ? ` sa ${p}` : ""}! Ayos!`;
      case "REJECT":
        return tone.style === "professional"
          ? "Salamat sa alok, pero hindi ko matanggap ang presyong ito."
          : "Pasensya na, medyo mataas. Pwede bang mas mababa?";
      case "HOLD":
        return "Sandali lang, ire-review ko muna ang mga kondisyon.";
      case "DISCOVER":
        return role === "buyer"
          ? "Pwede bang malaman ang condition ng product?"
          : "Tanong mo lang, sasagutin ko lahat!";
      case "CONFIRM":
        return `Confirmed${p ? ` sa ${p}` : ""}. Proceed na tayo sa payment.`;
      default:
        return `Desisyon: ${action}${p ? ` (${p})` : ""}`;
    }
  }
