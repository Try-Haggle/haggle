...
describe("Skill Registration & Resolution", () => {
  ...
  it("does not duplicate on re-registration", () => {
    ...
    expect(
      stack.getSkills().filter((s) => s.manifest.id === "electronics-knowledge-v1"),
    ).toHaveLength(1);
  });
...
// L1 (HAGGLE-9-A): item tags must come from the listing, not a phantom default.
describe("resolveItemTags (listing → skill tags)", () => {
  it('returns [] when no listing context (→ only "*" skills, no electronics leak)', () => {
    expect(resolveItemTags(undefined)).toEqual([]);
    expect(resolveItemTags(null)).toEqual([]);
    expect(resolveItemTags({})).toEqual([]);
  });

  it("uses category and tags, dropping empty/undefined", () => {
    expect(resolveItemTags({ category: "electronics" })).toEqual(["electronics"]);
    expect(resolveItemTags({ category: "fashion", tags: ["fashion/shoes/nike"] })).toEqual([
      "fashion",
      "fashion/shoes/nike",
    ]);
    expect(resolveItemTags({ category: "", tags: ["", "home/furniture"] })).toEqual([
      "home/furniture",
    ]);
  });

  it("normalizes casing/whitespace so canonical skill tags still match", () => {
    expect(resolveItemTags({ category: "  Electronics " })).toEqual(["electronics"]);

    clearRegistry();
    registerSkill(new ElectronicsKnowledgeSkill());
    const ids = SkillStack.fromTags(resolveItemTags({ category: "Electronics" }))
      .getSkills()
      .map((s) => s.manifest.id);
    expect(ids).toContain("electronics-knowledge-v1");
  });

  it("electronics listing still resolves the electronics knowledge skill (no regression)", () => {
    clearRegistry();
    registerSkill(new ElectronicsKnowledgeSkill());
    registerSkill(new FaratinCoachingSkill());

    const tags = resolveItemTags({ category: "electronics", tags: ["electronics/phones"] });
  ...
  it("non-electronics listing does NOT attach the electronics/IMEI skill", () => {
    clearRegistry();
    registerSkill(new ElectronicsKnowledgeSkill());
    registerSkill(new FaratinCoachingSkill());

    const tags = resolveItemTags({ category: "fashion", tags: ["fashion/shoes"] });
    const ids = SkillStack.fromTags(tags)
      .getSkills()
      .map((s) => s.manifest.id);

    expect(ids).not.toContain("electronics-knowledge-v1");
    expect(ids).toContain("faratin-coaching-v1"); // '*' skill still applies
  });

  it("missing category info falls back to universal skills only (fallback A)", () => {
    clearRegistry();
    registerSkill(new ElectronicsKnowledgeSkill());
    registerSkill(new FaratinCoachingSkill());

    const ids = SkillStack.fromTags(resolveItemTags(undefined))
      .getSkills()
      .map((s) => s.manifest.id);
...
describe("Faratin Coaching Skill", () => {
  ...
  it("concedes more as rounds progress (Faratin curve)", async () => {
    ...
    const earlyPrice = ((await skill.onHook(earlyCtx)).content as DecideHookResult["content"])
      .recommendedPrice!;
    const latePrice = ((await skill.onHook(lateCtx)).content as DecideHookResult["content"])
      .recommendedPrice!;
...
describe("RefereeBriefing (Facts Only)", () => {
  ...
  it("classifies opponent correctly", () => {
    const boulware: OpponentPattern = {
      aggression: 0.85,
      concession_rate: 0.005,
    } as OpponentPattern;
