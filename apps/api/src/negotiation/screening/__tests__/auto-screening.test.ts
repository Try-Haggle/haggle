...
describe("screenMessage", () => {
  it("should pass normal messages", () => {
    const result = screenMessage({
      messageText: "Is the phone still available? What condition is the screen in?",
    });
  ...
  it("should detect payment redirect spam", () => {
    const result = screenMessage({
      messageText: "Send money to paypal me now, wire transfer cash",
    });
  ...
  it("should detect off-platform redirect", () => {
    ...
    const result = screenMessage({
      messageText: "whatsapp me now, send money via wire transfer",
      senderTrustScore: 0.1,
    });
    expect(result.is_spam).toBe(true);
  });
