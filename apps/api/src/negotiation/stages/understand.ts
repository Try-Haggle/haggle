...
import { mapSignalsToFeatures } from "../features/signals-to-features.js";
import type {
  ConversationType,
  InformationLink,
 ...
 */
export function understand(input: UnderstandInput | UnderstandOutput): UnderstandOutput {
  ...
  const extractedFeatures = mapSignalsToFeatures(
    signals,
    input.sender_role === "buyer" ? "buyer_msg" : "seller_msg",
  );
...
function detectActionIntent(text: string): UnderstandOutput["action_intent"] {
  ...
  if (/\bcounter\b|\bhow about\b|\bwhat about\b|\bi('d| would) (like|offer)\b/.test(lower))
    return "COUNTER";
...
function classifyConversationType(
  ...
  const termEntities = new Set(
    signals.filter((s) => s.type === "term_preference").map((s) => s.entityType),
  );
  ...
  if (
    termEntities.has("shipping") ||
    termEntities.has("fulfillment") ||
    /shipping|deliver|pickup|meet locally/i.test(text)
  ) {
  ...
  if (signalTypes.has("product_identity") || signalTypes.has("demand_intent"))
    return "READINESS_DISCOVERY";
...
function inferMissingInformation(
  ...
  const hasLink = (connectsTo: InformationLink["connects_to"], entityType?: string) =>
    links.some(
      (link) => link.connects_to === connectsTo && (!entityType || link.entity_type === entityType),
    );
  ...
  if (
    (actionIntent === "OFFER" || actionIntent === "COUNTER") &&
    price === undefined &&
    !hasLink("pricing")
  ) {
    addNeed(needs, {
  ...
  if (
    /\b(unlocked|carrier|locked)\b/i.test(text) &&
    (actionIntent === "QUESTION" || !hasLink("product", "carrier"))
  ) {
    addNeed(needs, {
      slot: "carrier_lock",
      priority: "medium",
  ...
  if (
    (conditions.imei_mentioned ||
      conditions.find_my_mentioned ||
      /\b(serial|verification|authentic)\b/i.test(text)) &&
  ...
  if (
    conditions.warranty_mentioned &&
    (actionIntent === "QUESTION" || !/active|included|until|expires|expired/i.test(lower))
  ) {
    addNeed(needs, {
