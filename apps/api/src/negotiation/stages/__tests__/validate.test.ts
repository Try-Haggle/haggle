...
describe("Stage 4: validateStage", () => {
  ...
  it("detects V1 HARD violation — buyer price exceeds floor", () => {
    ...
    // Auto-fixed to the tightest bound that applies. V1 alone would stop at the buyer's
    // budget (95000), but the seller is only asking 90000 — bidding past the price on the
    // table gives money away, so the envelope (V8) pulls it down further.
    expect(result.auto_fix_applied).toBe(true);
    expect(result.final_decision.price).toBe(90000);
