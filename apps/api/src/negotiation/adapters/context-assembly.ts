...
function buildCoachingLayer(
  coaching: RefereeCoaching,
  level: "DETAILED" | "STANDARD" | "LIGHT",
): string {
  if (level === "LIGHT") {
  ...
  parts.push(
    `Acceptable range: $${toDollars(coaching.acceptable_range.min)}-$${toDollars(coaching.acceptable_range.max)}`,
  );
  ...
  if (level === "DETAILED") {
    ...
    parts.push(
      `Utility: price=${u.u_price.toFixed(2)} time=${u.u_time.toFixed(2)} total=${u.u_total.toFixed(2)}`,
    );
  ...
  return facts
    .map((f) => {
      ...
      if (Object.keys(f.conditions_changed).length > 0) {
        line +=
          " cond:" +
          Object.entries(f.conditions_changed)
            .map(([k, v]) => `${k}=${v}`)
            .join(",");
      }
      return line;
