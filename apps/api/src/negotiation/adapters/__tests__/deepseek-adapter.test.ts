...
describe("DeepSeekAdapter", () => {
    ...
    const facts: RoundFact[] = [
      {
        ...
        gap: 130,
        conditions_changed: {},
        coaching_given: { recommended: 520, tactic: "anchoring" },
  ...
  it("should parse valid JSON response and convert USD to minor units", () => {
    const raw =
      '{"action":"COUNTER","price":540,"reasoning":"Faratin curve","tactic_used":"anchoring"}';
  ...
  it("should include signals in prompt", () => {
    const prompt = adapter.buildUserPrompt(
      makeMemory(),
      [],
      ["competition_active", "time_critical"],
    );
    ...
    const prompt = adapter.buildUserPrompt(makeMemory(), [], undefined, undefined, {
      ...
      recent_turns: [
        { round: 1, sender: "BUYER", text: "Hi, would $370 work?", price_minor: 37000 },
        {
          round: 2,
          sender: "SELLER",
          text: "The battery is at 92% and it has minor scratches.",
          price_minor: 48000,
        },
      ],
    });
