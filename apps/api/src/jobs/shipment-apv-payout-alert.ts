...
export async function runShipmentApvPayoutAlert(
  db: Database,
  options: { now?: Date; fetchImpl?: typeof fetch; fixture?: {
    alertSource: string;
  ...
  if (options.fixture && !/^haggle-shipment-apv-payout-alert-fixture-[0-9a-f-]{36}$/.test(
    options.fixture.alertSource)) {
  ...
  const restorationRemediationHealth = await getShipmentApvInvoiceRestorationRemediationHealth(db, now);
  const cursorRetentionStatus = options.fixture?.cursorRetentionStatus
    ?? getShipmentApvRemediationCursorRetentionJobStatus();
  const cursorRetentionJob = cursorRetentionStatus.jobEnabled && cursorRetentionStatus.configured
    ? { ...cursorRetentionStatus,
      health: await getShipmentApvRemediationCursorRetentionJobHealth(db, now) }
    : undefined;
  let assessment = evaluateShipmentApvPayoutAlert(health, config, approvalHealth, invoiceStorageHealth,
    restorationStagingHealth, restorationRemediationHealth, cursorRetentionJob, now);
  ...
  if (!assessment.wouldAlert) {
    ...
    if (!deliveryState.incidentOpen) {
      return { status: "skipped" as const, reason: "healthy" as const,
        health, approvalHealth, invoiceStorageHealth, restorationStagingHealth, restorationRemediationHealth, cursorRetentionJob, assessment, deliveryState };
    }
  ...
  const cooldownKey = eventPrefix === "recovery"
  ...
  if (claim.outcome !== "acquired") {
    return { status: "skipped" as const, reason: "cooldown_or_in_progress" as const,
      health, approvalHealth, invoiceStorageHealth, restorationStagingHealth, restorationRemediationHealth, cursorRetentionJob, assessment };
  }
  try {
    ...
    if (alert.status === "delivered") {
      ...
      return { status: "delivered" as const, health, approvalHealth, invoiceStorageHealth, restorationStagingHealth, restorationRemediationHealth, cursorRetentionJob, assessment, alert };
    }
    await failWebhookEvent(db, claim);
    return { status: "failed" as const, health, approvalHealth, invoiceStorageHealth, restorationStagingHealth, restorationRemediationHealth, cursorRetentionJob, assessment, alert };
  } catch (error) {
    await failWebhookEvent(db, claim);
    throw error;
  }
