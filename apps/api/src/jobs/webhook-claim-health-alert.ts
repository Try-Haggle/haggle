...
export async function runWebhookClaimHealthAlert(
  ...
  try {
    const alert = await sendWebhookClaimAlert(health, assessment, {
      config,
      deliveryId: claim.eventId,
      fetchImpl: options.fetchImpl,
      now,
    ...
    if (alert.status === "delivered") {
      await completeWebhookEvent(db, claim, alert.httpStatus ?? 200);
      console.log(
        `[webhook-claim-health-alert] delivered severity=${assessment.severity} reasons=${assessment.reasons.join(",")}`,
      );
      return { status: "delivered" as const, health, assessment, alert };
    }
    await failWebhookEvent(db, claim);
    console.error(
      `[webhook-claim-health-alert] delivery failed status=${alert.httpStatus ?? "none"}`,
    );
