...
export async function runDisputeEvidenceProvenanceArchiveAlert(
  db: Database,
  options: {
    now?: Date;
  ...
  if (!assessment.wouldAlert) {
    const incident = await findLatestDeliveredDisputeEvidenceProvenanceArchiveIncident(
      db,
      claimSource,
    );
    if (!incident)
    ...
    const recovery = {
      ...
      reasons: ["evidence_provenance_archive_recovered"],
    };
    try {
      const alert = await sendDisputeEvidenceProvenanceArchiveAlert(health, recovery, {
        config,
  ...
  try {
    const alert = await sendDisputeEvidenceProvenanceArchiveAlert(health, assessment, {
      config,
      deliveryId: claim.eventId,
      fetchImpl: options.fetchImpl,
      now,
