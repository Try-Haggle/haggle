...
import {
  CONDITIONAL_SETTLEMENT_FINALITY_ALERT_SOURCE,
  type ConditionalSettlementFinalityAlertConfig,
...
import {
  type ConditionalSettlementFinalityHealth,
...
export async function runConditionalSettlementFinalityAlert(
  db: Database,
  options: {
    now?: Date;
  ...
  if (!assessment.wouldAlert) {
    ...
    const recovery = {
      ...
      reasons: ["conditional_settlement_finality_recovered"],
    };
    try {
      const alert = await sendConditionalSettlementFinalityAlert(health, recovery, {
        config,
      ...
      if (alert.status === "delivered") {
        ...
        return { status: "recovered" as const, health, assessment: recovery, alert };
      }
  ...
  const claim = await claimWebhookEvent(db, {
    source,
    eventId: `health_${createHash("sha256").update(key).digest("hex")}`,
  ...
  try {
    const alert = await sendConditionalSettlementFinalityAlert(health, assessment, {
      config,
      deliveryId: claim.eventId,
      fetchImpl: options.fetchImpl,
      now,
