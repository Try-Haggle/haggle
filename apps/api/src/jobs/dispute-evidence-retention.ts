...
): Promise<{ dry_run: boolean; eligible: number; claimed: number; deleted: number; failed: number; held: number }> {
  const policy = evidenceRetentionPolicy();
  const eligible = await countEligibleEvidenceRetention(db, policy);
  if (options.dryRun) return { dry_run: true, eligible, claimed: 0, deleted: 0, failed: 0, held: 0 };
