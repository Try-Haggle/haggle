...
import { getDisputeAiAuditArchiveHealth, getDisputeAiAuditDiscoveryFailureHealth,
  type DisputeAiAuditDiscoveryFailureHealth } from "../services/dispute-ai-audit-archive.service.js";
import {
  DISPUTE_AI_AUDIT_ARCHIVE_ALERT_SOURCE, evaluateDisputeAiAuditArchiveAlert,
  findLatestDeliveredDisputeAiAuditArchiveIncident, resolveDisputeAiAuditArchiveAlertConfigFromEnv,
  sendDisputeAiAuditArchiveAlert, type DisputeAiAuditArchiveAlertConfig,
...
export async function runDisputeAiAuditArchiveAlert(db: Database, options: {
  now?: Date; fetchImpl?: typeof fetch; config?: DisputeAiAuditArchiveAlertConfig; claimSource?: string;
  ...
  const [health, discoveryHealth] = await Promise.all([
    ...
    options.discoveryHealth ? Promise.resolve(options.discoveryHealth) : getDisputeAiAuditDiscoveryFailureHealth(db, now),
  ]);
  ...
  if (!assessment.wouldAlert) {
    ...
    const recoveryAssessment = { wouldAlert: true, severity: "recovery" as const, reasons: ["ai_audit_archive_recovered"] };
    try {
      const alert = await sendDisputeAiAuditArchiveAlert(health, recoveryAssessment, { config, deliveryId: claim.eventId, fetchImpl: options.fetchImpl, now, discoveryHealth });
      if (alert.status === "delivered") { await completeWebhookEvent(db, claim, alert.httpStatus ?? 200); return { status: "recovered" as const, health, assessment: recoveryAssessment, alert }; }
      await failWebhookEvent(db, claim); return { status: "failed" as const, phase: "recovery" as const, health, assessment: recoveryAssessment, alert };
  ...
  try {
    const alert = await sendDisputeAiAuditArchiveAlert(health, assessment, { config, deliveryId: claim.eventId, fetchImpl: options.fetchImpl, now, discoveryHealth });
