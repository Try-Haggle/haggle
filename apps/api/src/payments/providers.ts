...
export function createPaymentServiceFromEnv() {
  ...
  if (!facilitatorUrl || !feeWallet) {
    throw new Error(
      "real x402 mode requires HAGGLE_X402_FACILITATOR_URL and HAGGLE_X402_FEE_WALLET",
    );
  }
  ...
  const relayerPrivateKey = process.env.HAGGLE_ROUTER_RELAYER_PRIVATE_KEY as
    | `0x${string}`
    | undefined;
  const assetAddress = (process.env.HAGGLE_X402_USDC_ASSET_ADDRESS ?? "USDC") as Address | "USDC";
  const feeBps = readHaggleFeeBpsFromEnv();
  const defaultBuyerAuthMode = (process.env.HAGGLE_X402_DEFAULT_BUYER_AUTH_MODE ??
    "human_wallet") as BuyerAuthorizationMode;
  ...
  const viemReady = Boolean(
    rpcUrl && routerAddress && relayerPrivateKey && assetAddress !== "USDC",
  );
  const contractClientReady = Boolean(rpcUrl && relayerPrivateKey && assetAddress !== "USDC");
  const conditionalServerExecutionEnabled =
    process.env.HAGGLE_CONDITIONAL_SETTLEMENT_SERVER_EXECUTION === "true";

  const settlementRouter =
    serverSettlementRouterExecutionEnabled && viemReady
      ? (() => {
  ...
  const disputeRegistry =
    viemReady && disputeRegistryAddress
      ? (() => {
          ...
          const publicClient = createPublicClient({ chain, transport });
          const walletClient = createWalletClient({ account, chain, transport });
          return new ViemDisputeRegistryContract(
            network,
  ...
  const x402 = new RealX402Adapter({
    ...
    async resolve_seller_payout_target(sellerId: string) {
      return {
        seller_id: sellerId,
        wallet: paymentWalletFromMap(
          sellerId,
          sellerWalletMap[sellerId],
          walletNetwork,
          "external",
        ),
      };
    ...
    async resolve_buyer_authorization(intent: PaymentIntent) {
      return {
        ...
        wallet: paymentWalletFromMap(
          intent.buyer_id,
          buyerWalletMap[intent.buyer_id],
          walletNetwork,
          "external",
        ),
      };
...
export function getX402EnvConfig() {
  return {
    ...
    allowExactSettlementFallback:
      process.env.HAGGLE_X402_ALLOW_EXACT_SETTLEMENT_FALLBACK === "true",
