...
type FixtureStage =
  | "sender_lifecycle"
  | "finality_receiver_resilience"
  | "health_alert_delivery"
  | "health_receiver_resilience"
  | "health_receiver_burst"
  | "health_receiver_partial_failure"
  | "health_receiver_terminal"
  | "assertions"
  | "cleanup";
...
export async function runConditionalSettlementFinalityAlertFixture(db: Database) {
  ...
  const config = {
    url: "http://127.0.0.1:9999/finality",
    secret,
    timeoutMs: 1000,
    cooldownMinutes: 15,
  ...
  const receiverHealthAlertConfig: WebhookClaimAlertConfig = {
    url: "http://127.0.0.1:9999/webhook-claim-health",
    secret,
    timeoutMs: 1000,
    cooldownMinutes: 15,
    failedThreshold: 1,
  ...
  const receiverHealthAlertDeliveries: Array<{
    deliveryId: string;
    timestamp: string;
    signature: string;
    body: string;
  ...
  const receiverHealthAlertFetch = async (_url: string | URL | Request, init?: RequestInit) => {
    ...
    const item = {
      deliveryId: headers["x-haggle-alert-delivery-id"],
      timestamp: headers["x-haggle-alert-timestamp"],
      signature: headers["x-haggle-alert-signature"],
      body: String(init?.body ?? ""),
    };
    receiverHealthAlertDeliveries.push(item);
    const verification = verifyWebhookClaimHealthAlert({
      rawBody: item.body,
    ...
    const delivery = await claimVerifiedWebhookClaimHealthAlert(
      db,
      verification,
      receiverHealthAlertReceiverSource,
    );
    if (delivery.outcome !== "accepted")
      return new Response(JSON.stringify({ error: delivery.outcome }), { status: 409 });
    await completeWebhookEvent(db, delivery.claim, 202);
    receiverHealthAlertVerified += 1;
    const replay = await claimVerifiedWebhookClaimHealthAlert(
      db,
      verification,
      receiverHealthAlertReceiverSource,
    );
  ...
  const collectReceiverHealth = async () => {
    ...
    return {
      ...
      totals: {
        ...
        staleProcessing: health.staleProcessing,
        retryReady: health.retryReady,
      },
      sources: [
        {
          ...
          staleProcessing: health.staleProcessing,
          retryReady: health.retryReady,
          maxAttemptCount: health.maxAttemptCount,
          oldestUnfinishedAgeSeconds: health.oldestUnfinishedAgeSeconds,
        },
      ],
      recordedAt: health.recordedAt,
    };
  };
  const base: ConditionalSettlementFinalityHealth = {
    status: "healthy",
    total: 0,
    pending: 0,
    unavailable: 0,
  ...
  const fetchImpl = async (_url: string | URL | Request, init?: RequestInit) => {
    ...
    delivered.push(item);
    const verification = verifyConditionalSettlementFinalityAlert({
      rawBody: item.body,
      timestamp: item.timestamp,
      signature: item.signature,
      deliveryId: item.deliveryId,
    ...
    if (!verification.ok)
      return new Response(JSON.stringify({ error: verification.error }), { status: 401 });
    const receiverClaim = await claimVerifiedConditionalSettlementFinalityAlert(
      db,
      verification,
      receiverSource,
    );
    if (receiverClaim.outcome !== "accepted")
      return new Response(JSON.stringify({ error: receiverClaim.outcome }), { status: 409 });
    if (!(await completeWebhookEvent(db, receiverClaim.claim, 202)))
      return new Response(null, { status: 503 });
    receiverVerified += 1;
    const replay = await claimVerifiedConditionalSettlementFinalityAlert(
      db,
      verification,
      receiverSource,
    );
  ...
  const startStage = (stage: FixtureStage) => {
    diagnostics.stages.push({
      name: activeStage,
      durationMs: Math.max(0, Date.now() - activeStageStartedAt),
    });
    activeStage = stage;
  ...
  try {
    ...
    const firing = await runConditionalSettlementFinalityAlert(db, {
      ...common,
      collectHealth: async () => critical,
    });
    const duplicateFiring = await runConditionalSettlementFinalityAlert(db, {
      ...common,
      collectHealth: async () => critical,
    });
    const recovery = await runConditionalSettlementFinalityAlert(db, {
      ...common,
      now: new Date("2026-07-12T00:06:00.000Z"),
      collectHealth: async () => base,
    });
    const duplicateRecovery = await runConditionalSettlementFinalityAlert(db, {
      ...common,
      now: new Date("2026-07-12T00:06:00.000Z"),
      collectHealth: async () => base,
    });
    const signaturesValid = delivered.every(
      (item) => item.signature === signWebhookClaimAlertPayload(secret, item.timestamp, item.body),
    );
    ...
    if (conflictItem) {
      const previousSignature = signWebhookClaimAlertPayload(
        previousSecret,
        conflictItem.timestamp,
        conflictItem.body,
      );
      rotationOverlapAccepted = verifyConditionalSettlementFinalityAlert({
        rawBody: conflictItem.body,
        timestamp: conflictItem.timestamp,
        deliveryId: conflictItem.deliveryId,
        signature: previousSignature,
        secret: [secret, previousSecret],
        nowMs: Date.parse(conflictItem.timestamp),
      }).ok;
      retiredSecretRejected = !verifyConditionalSettlementFinalityAlert({
        rawBody: conflictItem.body,
        timestamp: conflictItem.timestamp,
        deliveryId: conflictItem.deliveryId,
        signature: previousSignature,
      ...
      changed.health = {
        ...(changed.health as Record<string, unknown>),
        recordedAt: "2026-07-12T00:00:01.000Z",
      };
      const conflictBody = JSON.stringify(changed);
      const conflictVerification = verifyConditionalSettlementFinalityAlert({
        rawBody: conflictBody,
        timestamp: conflictItem.timestamp,
        deliveryId: conflictItem.deliveryId,
        signature: signWebhookClaimAlertPayload(secret, conflictItem.timestamp, conflictBody),
        secret,
        nowMs: Date.parse(conflictItem.timestamp),
      });
      if (conflictVerification.ok) {
        payloadConflict =
          (
            await claimVerifiedConditionalSettlementFinalityAlert(
              db,
    ...
    const concurrencyBody = JSON.stringify({
      ...
      delivery_id: concurrencyDeliveryId,
      state: "firing",
      created_at: concurrencyTimestamp,
      severity: "warning",
      ...
      health: {
        ...
        unavailable: 1,
        rpcUnavailable: 1,
        recordedAt: concurrencyTimestamp,
      },
    });
    const concurrencyVerification = verifyConditionalSettlementFinalityAlert({
      rawBody: concurrencyBody,
      timestamp: concurrencyTimestamp,
      deliveryId: concurrencyDeliveryId,
      signature: signWebhookClaimAlertPayload(secret, concurrencyTimestamp, concurrencyBody),
      secret,
    ...
    const concurrentBlocked = concurrentClaims.filter(
      (claim) => claim.outcome === "in_progress",
    ).length;
    const concurrentCompleted =
      concurrentWinners.length === 1
        ? await completeWebhookEvent(db, concurrentWinners[0]!.claim, 202)
        : false;
    const takeoverDeliveryId = `health_${createHash("sha256").update(randomUUID()).digest("hex")}`;
    const takeoverBody = JSON.stringify({
      type: "conditional_settlement_finality.health",
      delivery_id: takeoverDeliveryId,
      state: "firing",
      created_at: concurrencyTimestamp,
      severity: "warning",
      reasons: ["configuration_blocked"],
      health: {
        ...base,
        status: "attention",
        total: 1,
    ...
    const takeoverVerification = verifyConditionalSettlementFinalityAlert({
      rawBody: takeoverBody,
      timestamp: concurrencyTimestamp,
      deliveryId: takeoverDeliveryId,
      signature: signWebhookClaimAlertPayload(secret, concurrencyTimestamp, takeoverBody),
      secret,
      nowMs: Date.parse(concurrencyTimestamp),
    });
    ...
    if (takeoverVerification.ok) {
      const staleOwner = await claimVerifiedConditionalSettlementFinalityAlert(
        db,
      ...
      if (staleOwner.outcome === "accepted") {
        const expired =
          (await db.execute(sql`UPDATE webhook_idempotency SET lease_expires_at = now() - interval '1 second'
          WHERE source = ${receiverSource} AND idempotency_key = ${takeoverDeliveryId}
            AND status = 'PROCESSING' AND claim_id = ${staleOwner.claim.claimId} RETURNING id`)) as unknown as Array<{
            id: string;
          }>;
        const takeover =
          expired.length === 1
        ...
        if (takeover?.outcome === "accepted") {
          ...
          try {
            await completeWebhookEvent(db, staleOwner.claim, 202);
          } catch (error) {
    ...
    const retryBody = JSON.stringify({
      ...
      created_at: concurrencyTimestamp,
      severity: "warning",
      reasons: ["confirmation_sla_overdue"],
      health: {
        ...base,
        status: "attention",
        total: 1,
    ...
    const retryVerification = verifyConditionalSettlementFinalityAlert({
      rawBody: retryBody,
      timestamp: concurrencyTimestamp,
      deliveryId: retryDeliveryId,
      signature: signWebhookClaimAlertPayload(secret, concurrencyTimestamp, retryBody),
      secret,
      nowMs: Date.parse(concurrencyTimestamp),
    });
    ...
    if (retryVerification.ok) {
      const failedDelivery = await claimVerifiedConditionalSettlementFinalityAlert(
        db,
        retryVerification,
        receiverSource,
      );
      if (failedDelivery.outcome === "accepted") {
        ...
        receiverHealthFiring = (
          await runWebhookClaimHealthAlert(db, {
            config: receiverHealthAlertConfig,
            claimSource: receiverHealthAlertSource,
            collectHealth: collectReceiverHealth,
            fetchImpl: receiverHealthAlertFetch as typeof fetch,
            now: new Date("2026-07-12T00:08:00.000Z"),
          })
        ).status;
        const earlyRetry = await claimVerifiedConditionalSettlementFinalityAlert(
          db,
          retryVerification,
          receiverSource,
        );
        retryBackoffBlocked =
          earlyRetry.outcome === "retry_backoff" && earlyRetry.retryAfterSeconds >= 1;
        const released =
          (await db.execute(sql`UPDATE webhook_idempotency SET next_attempt_at = now() - interval '1 second'
          WHERE source = ${receiverSource} AND idempotency_key = ${retryDeliveryId}
            AND status = 'FAILED' AND claim_id IS NULL RETURNING id`)) as unknown as Array<{
            id: string;
          }>;
        ...
        if (retried?.outcome === "accepted") {
          ...
          receiverHealthRecovery = (
            await runWebhookClaimHealthAlert(db, {
              config: receiverHealthAlertConfig,
              claimSource: receiverHealthAlertSource,
              collectHealth: collectReceiverHealth,
              fetchImpl: receiverHealthAlertFetch as typeof fetch,
              now: new Date("2026-07-12T00:09:00.000Z"),
            })
          ).status;
          const duplicateRecovery = await runWebhookClaimHealthAlert(db, {
            config: receiverHealthAlertConfig,
            claimSource: receiverHealthAlertSource,
            collectHealth: collectReceiverHealth,
            fetchImpl: receiverHealthAlertFetch as typeof fetch,
          ...
          receiverHealthDuplicateRecovery =
            duplicateRecovery.status === "skipped"
              ? duplicateRecovery.reason
              : duplicateRecovery.status;
        }
      }
    }
    ...
    if (receiverHealthAlertConflictItem) {
      ...
      changed.sources = sources.map((entry, index) =>
        index === 0
          ? { ...entry, max_attempt_count: Number(entry.max_attempt_count ?? 0) + 1 }
          : entry,
      );
      const body = JSON.stringify(changed);
      const verification = verifyWebhookClaimHealthAlert({
        rawBody: body,
        timestamp: receiverHealthAlertConflictItem.timestamp,
        deliveryId: receiverHealthAlertConflictItem.deliveryId,
        signature: signWebhookClaimAlertPayload(
          secret,
          receiverHealthAlertConflictItem.timestamp,
          body,
        ),
        secret,
      ...
      if (verification.ok) {
        receiverHealthAlertConflict =
          (
            await claimVerifiedWebhookClaimHealthAlert(
              db,
              verification,
              receiverHealthAlertReceiverSource,
            )
    ...
    if (receiverHealthAlertConflictItem) {
      const concurrencyDeliveryId = `health_${createHash("sha256").update(randomUUID()).digest("hex")}`;
      const concurrencyPayload = JSON.parse(receiverHealthAlertConflictItem.body) as Record<
        string,
        unknown
      >;
      concurrencyPayload.delivery_id = concurrencyDeliveryId;
      const concurrencyBody = JSON.stringify(concurrencyPayload);
      const concurrencyVerification = verifyWebhookClaimHealthAlert({
        rawBody: concurrencyBody,
        timestamp: receiverHealthAlertConflictItem.timestamp,
        deliveryId: concurrencyDeliveryId,
        signature: signWebhookClaimAlertPayload(
      ...
      if (concurrencyVerification.ok) {
        ...
        receiverHealthConcurrentBlocked = claims.filter(
          (claim) => claim.outcome === "in_progress",
        ).length;
        receiverHealthConcurrentCompleted =
          winners.length === 1 ? await completeWebhookEvent(db, winners[0]!.claim, 202) : false;
      }

      const takeoverDeliveryId = `health_${createHash("sha256").update(randomUUID()).digest("hex")}`;
      const takeoverPayload = JSON.parse(receiverHealthAlertConflictItem.body) as Record<
        string,
        unknown
      >;
      takeoverPayload.delivery_id = takeoverDeliveryId;
      const takeoverBody = JSON.stringify(takeoverPayload);
      const takeoverVerification = verifyWebhookClaimHealthAlert({
        rawBody: takeoverBody,
        timestamp: receiverHealthAlertConflictItem.timestamp,
        deliveryId: takeoverDeliveryId,
        signature: signWebhookClaimAlertPayload(
      ...
      if (takeoverVerification.ok) {
        const staleOwner = await claimVerifiedWebhookClaimHealthAlert(
          db,
          takeoverVerification,
          receiverHealthAlertReceiverSource,
        );
        if (staleOwner.outcome === "accepted") {
          ...
          if (takeover?.outcome === "accepted") {
            ...
            try {
              ...
              receiverHealthStaleOwnerFenced =
                error instanceof Error && error.message === "WEBHOOK_CLAIM_LOST";
            }
      ...
      const retryDeliveryId = `health_${createHash("sha256").update(randomUUID()).digest("hex")}`;
      const retryPayload = JSON.parse(receiverHealthAlertConflictItem.body) as Record<
        string,
        unknown
      >;
      retryPayload.delivery_id = retryDeliveryId;
      const retryBody = JSON.stringify(retryPayload);
      const retryVerification = verifyWebhookClaimHealthAlert({
        rawBody: retryBody,
        timestamp: receiverHealthAlertConflictItem.timestamp,
        deliveryId: retryDeliveryId,
        signature: signWebhookClaimAlertPayload(
      ...
      if (retryVerification.ok) {
        const failedDelivery = await claimVerifiedWebhookClaimHealthAlert(
          db,
          retryVerification,
          receiverHealthAlertReceiverSource,
        );
        if (failedDelivery.outcome === "accepted") {
          await failWebhookEvent(db, failedDelivery.claim);
          const earlyRetry = await claimVerifiedWebhookClaimHealthAlert(
            db,
            retryVerification,
            receiverHealthAlertReceiverSource,
          );
          receiverHealthRetryBackoffBlocked =
            earlyRetry.outcome === "retry_backoff" && earlyRetry.retryAfterSeconds >= 1;
      ...
      const burstClaims = await Promise.all(
        Array.from({ length: receiverHealthBurstRequests }, async () => {
          const burstDeliveryId = `health_${createHash("sha256").update(randomUUID()).digest("hex")}`;
          const burstPayload = JSON.parse(receiverHealthAlertConflictItem.body) as Record<
            string,
            unknown
          >;
          burstPayload.delivery_id = burstDeliveryId;
          const burstBody = JSON.stringify(burstPayload);
          const burstVerification = verifyWebhookClaimHealthAlert({
            rawBody: burstBody,
            timestamp: receiverHealthAlertConflictItem.timestamp,
            deliveryId: burstDeliveryId,
            signature: signWebhookClaimAlertPayload(
              secret,
              receiverHealthAlertConflictItem.timestamp,
              burstBody,
      ...
      const burstCompletions = await Promise.all(
        acceptedBurstClaims.map((claim) => completeWebhookEvent(db, claim.claim, 202)),
      );
      receiverHealthBurstCompleted = burstCompletions.filter(Boolean).length;

      startStage("health_receiver_partial_failure");
      const partialDeliveries = await Promise.all(
        Array.from({ length: 20 }, async (_, index) => {
          const deliveryId = `health_${createHash("sha256").update(randomUUID()).digest("hex")}`;
          const payload = JSON.parse(receiverHealthAlertConflictItem.body) as Record<
            string,
            unknown
          >;
      ...
      const acceptedPartialDeliveries = partialDeliveries.flatMap((item) =>
        item.verification.ok && item.claim?.outcome === "accepted"
          ? [
              {
                index: item.index,
                deliveryId: item.deliveryId,
      ...
      await Promise.all(
        acceptedPartialDeliveries.map(async (item) => {
          if (item.index % 5 === 0) {
      ...
      const partialReplays = await Promise.all(
        acceptedPartialDeliveries.map((item) =>
      ...
      receiverHealthPartialSuccessReplays = partialReplays.filter(
        (claim) => claim?.outcome === "replay_completed",
      ).length;
      receiverHealthPartialBackoffs = partialReplays.filter(
        (claim) => claim?.outcome === "retry_backoff" && claim.retryAfterSeconds >= 1,
      ).length;
      const failedPartialDeliveries = acceptedPartialDeliveries.filter(
        (item) => item.index % 5 === 0,
      );
      const releasedPartialRows = await Promise.all(
        failedPartialDeliveries.map(
          (item) =>
            db.execute(sql`
        ...
        WHERE source = ${receiverHealthAlertReceiverSource} AND idempotency_key = ${item.deliveryId}
          AND status = 'FAILED' AND claim_id IS NULL RETURNING id`) as unknown as Promise<
              Array<{ id: string }>
            >,
        ),
      );
      receiverHealthPartialReleased = releasedPartialRows.reduce(
        (total, rows) => total + rows.length,
        0,
      );
      const recoveredPartialClaimGroups = await Promise.all(
        failedPartialDeliveries.map(async (item) =>
          item.verification.ok
            ? Promise.all(
      ...
      const recoveredPartialWinners = recoveredPartialClaims.filter(
        (claim) => claim.outcome === "accepted",
      );
      receiverHealthPartialRecoveryWinners = recoveredPartialWinners.length;
      receiverHealthPartialRecoveryBlocked = recoveredPartialClaims.filter(
        (claim) => claim.outcome === "in_progress",
      ).length;
      receiverHealthPartialAttemptTwo = recoveredPartialWinners.filter(
        (claim) => claim?.outcome === "accepted" && claim.claim.attemptCount === 2,
      ).length;
      const recoveredPartialCompletions = await Promise.all(
        recoveredPartialWinners.map((claim) => completeWebhookEvent(db, claim.claim, 202)),
      );
      ...
      const interruptedPayload = JSON.parse(receiverHealthAlertConflictItem.body) as Record<
        string,
        unknown
      >;
      interruptedPayload.delivery_id = interruptedDeliveryId;
      const interruptedBody = JSON.stringify(interruptedPayload);
      const interruptedVerification = verifyWebhookClaimHealthAlert({
        rawBody: interruptedBody,
        timestamp: receiverHealthAlertConflictItem.timestamp,
      ...
      if (interruptedVerification.ok) {
        const firstAttempt = await claimVerifiedWebhookClaimHealthAlert(
          db,
        ...
        if (firstAttempt.outcome === "accepted") {
          ...
          if (secondAttempt?.outcome === "accepted") {
            ...
            const expired = (await db.execute(sql`UPDATE webhook_idempotency
              ...
              WHERE source = ${receiverHealthAlertReceiverSource} AND idempotency_key = ${interruptedDeliveryId}
                AND status = 'PROCESSING' AND claim_id = ${secondAttempt.claim.claimId} RETURNING id`)) as unknown as Array<{
              id: string;
            }>;
            const thirdAttempt =
              expired.length === 1
            ...
            if (thirdAttempt?.outcome === "accepted") {
              ...
              try {
                await completeWebhookEvent(db, secondAttempt.claim, 202);
              } catch (error) {
                receiverHealthInterruptedOldOwnerFenced =
                  error instanceof Error && error.message === "WEBHOOK_CLAIM_LOST";
              }
              receiverHealthInterruptedCompleted = await completeWebhookEvent(
                db,
                thirdAttempt.claim,
                202,
              );
              if (receiverHealthInterruptedCompleted) {
                const terminalReplays = await Promise.all(
                  Array.from({ length: 20 }, () =>
                    claimVerifiedWebhookClaimHealthAlert(
                      db,
                      interruptedVerification,
                ...
                receiverHealthTerminalReplays = terminalReplays.filter(
                  (claim) => claim.outcome === "replay_completed",
                ).length;
                const terminalRows =
                  (await db.execute(sql`SELECT status, attempt_count AS "attemptCount"
    ...
    const checks: Record<string, boolean> = {
      ...
      receiver_health_alert_receiver_concurrency_single_winner:
        receiverHealthConcurrentWinners === 1 &&
        receiverHealthConcurrentBlocked === 19 &&
        receiverHealthConcurrentCompleted,
      receiver_health_alert_receiver_stale_owner_fenced:
        receiverHealthStaleOwnerFenced &&
        receiverHealthTakeoverCompleted &&
        receiverHealthTakeoverAttemptCount === 2,
      receiver_health_alert_receiver_failure_backoff_recovered:
        receiverHealthRetryBackoffBlocked &&
        receiverHealthRetryReleased === 1 &&
        receiverHealthRetryCompleted &&
      ...
      receiver_health_alert_receiver_partial_failure_isolated:
        receiverHealthPartialCompleted === 16 &&
        receiverHealthPartialFailed === 4 &&
        receiverHealthPartialSuccessReplays === 16 &&
        receiverHealthPartialBackoffs === 4 &&
        receiverHealthPartialReleased === 4 &&
        receiverHealthPartialAttemptTwo === 4 &&
        receiverHealthPartialRecovered === 4,
      receiver_health_alert_receiver_partial_recovery_single_winner:
        receiverHealthPartialRecoveryRequests === 40 &&
        receiverHealthPartialRecoveryWinners === 4 &&
        receiverHealthPartialRecoveryBlocked === 36,
      receiver_health_alert_receiver_interrupted_retry_fenced:
        receiverHealthInterruptedAttemptTwo === 2 &&
        receiverHealthInterruptedAttemptThree === 3 &&
        receiverHealthInterruptedOldOwnerFenced &&
        receiverHealthInterruptedCompleted,
      receiver_health_alert_receiver_terminal_replay_converged:
        receiverHealthTerminalReplays === 20 &&
        receiverHealthTerminalStatus === "COMPLETED" &&
        receiverHealthTerminalAttemptCount === 3,
      ...
      aggregate_only: delivered.every(
        (item) => !/payment_id|order_id|tx_hash|block_hash/i.test(item.body),
      ),
      ...
      receiver_concurrency_single_winner:
        concurrentWinners.length === 1 && concurrentBlocked === 19 && concurrentCompleted,
      stale_owner_fenced_and_recovered:
        staleOwnerFenced && takeoverCompleted && takeoverAttemptCount === 2,
      receiver_failure_backoff_recovered:
        retryBackoffBlocked && retryReleased === 1 && retryCompleted && retryAttemptCount === 2,
      rotation_overlap_and_retirement: rotationOverlapAccepted && retiredSecretRejected,
      receiver_health_alert_firing_recovery:
        receiverHealthFiring === "delivered" && receiverHealthRecovery === "recovered",
      receiver_health_recovery_duplicate_blocked:
        receiverHealthDuplicateRecovery === "recovery_already_sent_or_in_progress",
      receiver_health_alert_signatures_valid:
        receiverHealthAlertDeliveries.length === 2 &&
        receiverHealthAlertDeliveries.every(
          (item) =>
            item.signature === signWebhookClaimAlertPayload(secret, item.timestamp, item.body),
        ),
    };
    finishActiveStage();
    checks.fixture_diagnostics_complete =
      diagnostics.stages.length === 8 && diagnostics.stages.every((stage) => stage.durationMs >= 0);
    result = {
      ...
      receiver: {
        ...
        replayBlocked: receiverReplayBlocked,
        payloadConflict,
      },
      concurrency: {
        requests: 20,
        winners: concurrentWinners.length,
        blocked: concurrentBlocked,
        completed: concurrentCompleted,
      },
      takeover: {
        staleOwnerFenced,
        completed: takeoverCompleted,
        attemptCount: takeoverAttemptCount,
      },
      retry: {
        backoffBlocked: retryBackoffBlocked,
        released: retryReleased,
        completed: retryCompleted,
        attemptCount: retryAttemptCount,
      },
      rotation: {
        overlapAccepted: rotationOverlapAccepted,
        retiredSecretRejected,
        overlapSecretCount: 2,
        retiredSecretCount: 1,
      },
      receiverHealthAlert: {
        firing: receiverHealthFiring,
        recovery: receiverHealthRecovery,
        duplicateRecovery: receiverHealthDuplicateRecovery,
        deliveries: receiverHealthAlertDeliveries.length,
        receiverVerified: receiverHealthAlertVerified,
        receiverReplayBlocked: receiverHealthAlertReplayBlocked,
        receiverConflict: receiverHealthAlertConflict,
        concurrency: {
          requests: 20,
          winners: receiverHealthConcurrentWinners,
          blocked: receiverHealthConcurrentBlocked,
          completed: receiverHealthConcurrentCompleted,
        },
        takeover: {
          staleOwnerFenced: receiverHealthStaleOwnerFenced,
          completed: receiverHealthTakeoverCompleted,
          attemptCount: receiverHealthTakeoverAttemptCount,
        },
        retry: {
          backoffBlocked: receiverHealthRetryBackoffBlocked,
          released: receiverHealthRetryReleased,
          completed: receiverHealthRetryCompleted,
          attemptCount: receiverHealthRetryAttemptCount,
        },
        burst: {
          requests: receiverHealthBurstRequests,
          accepted: receiverHealthBurstAccepted,
          completed: receiverHealthBurstCompleted,
        },
        partialBurst: {
          requests: 20,
          initiallyCompleted: receiverHealthPartialCompleted,
          initiallyFailed: receiverHealthPartialFailed,
          successfulReplays: receiverHealthPartialSuccessReplays,
          backoffs: receiverHealthPartialBackoffs,
          released: receiverHealthPartialReleased,
          attemptTwo: receiverHealthPartialAttemptTwo,
          recovered: receiverHealthPartialRecovered,
          recoveryRace: {
            requests: receiverHealthPartialRecoveryRequests,
            winners: receiverHealthPartialRecoveryWinners,
            blocked: receiverHealthPartialRecoveryBlocked,
          },
        },
        interruptedRetry: {
          attemptTwo: receiverHealthInterruptedAttemptTwo,
          ...
          terminal: {
            requests: 20,
            replays: receiverHealthTerminalReplays,
            status: receiverHealthTerminalStatus,
            attemptCount: receiverHealthTerminalAttemptCount,
          },
        },
      },
      firing: firing.status,
      duplicateFiring: duplicateFiring.status,
      recovery: recovery.status,
      duplicateRecovery: duplicateRecovery.status,
      diagnostics,
    };
  } catch (error) {
    ...
    try {
      const rows =
        (await db.execute(sql`DELETE FROM webhook_idempotency WHERE source = ${source} OR source = ${receiverSource}
        OR source = ${receiverHealthAlertSource} OR source = ${receiverHealthAlertReceiverSource} RETURNING id`)) as unknown as Array<{
          id: string;
        }>;
      const remaining =
        (await db.execute(sql`SELECT count(*) AS count FROM webhook_idempotency WHERE source = ${source}
        OR source = ${receiverSource} OR source = ${receiverHealthAlertSource}
        OR source = ${receiverHealthAlertReceiverSource}`)) as unknown as Array<{
          count: string | number;
        }>;
    ...
    const slowest = diagnostics.stages.reduce<{ name: FixtureStage; durationMs: number } | null>(
      (current, stage) => (!current || stage.durationMs > current.durationMs ? stage : current),
      null,
    );
  ...
  if (fixtureFailure)
    throw Object.assign(new Error("FINALITY_ALERT_FIXTURE_FAILED"), {
      code: "FINALITY_ALERT_FIXTURE_FAILED",
      stage: diagnostics.failureStage,
      diagnostics,
    });
