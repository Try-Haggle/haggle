...
type StoreResult =
  | { outcome: "stored" | "duplicate"; document: ShipmentApvInvoiceDocumentRecord }
  | {
      outcome:
        | "revision_not_found"
        | "evidence_not_bound"
...
function mapDocument(row: Record<string, unknown>): ShipmentApvInvoiceDocumentRecord {
  return {
    ...
    content_type: String(row.content_type) as ShipmentInvoiceContentType,
...
export async function storeShipmentApvInvoiceDocument(
  ...
  try {
    return await db.transaction(async (tx) => {
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtextextended(${`apv-invoice-document:${input.revisionId}`}, 0))`,
      );
    ...
    `)) as unknown as Array<Record<string, unknown>>;
      ...
      if (!revision.evidence_sha256 || !revision.provider_document_id)
        return { outcome: "evidence_not_bound" } as const;
      if (
        revision.evidence_sha256 !== sha256 ||
        revision.provider_document_id !== input.providerDocumentId
      ) {
...
export async function getShipmentApvInvoiceDocumentStorageHealth(
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (checkedBytes + metadata.size > HEALTH_MAX_HASH_BYTES) {
        scanTruncated = true;
        continue;
      }
  ...
  try {
    const rootEntries = options.revisionId
      ? await readdir(join(root, options.revisionId), { withFileTypes: true }).then((files) => [
          { name: options.revisionId!, files },
        ])
      : await readdir(root, { withFileTypes: true }).then(async (directories) => {
          ...
          for (const directory of directories) {
            ...
            filesystemEntries += 1;
            if (filesystemEntries > HEALTH_MAX_FILESYSTEM_ENTRIES) {
              scanTruncated = true;
              break;
            }
            if (
    ...
    for (const entry of rootEntries) {
      for (const file of entry.files) {
        ...
        if (!file.isFile() || file.isSymbolicLink() || !STORAGE_KEY_PATTERN.test(key))
          invalidEntries += 1;
