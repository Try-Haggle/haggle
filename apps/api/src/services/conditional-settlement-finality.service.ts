...
export type ConditionalSettlementFinalityResult =
  ...
  | {
      status: "unavailable";
      ready: false;
      reason: "RECEIPT_BLOCK_MISSING" | "RECEIPT_BLOCK_HASH_MISSING" | "CHAIN_HEAD_UNAVAILABLE" | "CHAIN_HEAD_BEHIND_RECEIPT"
        | "CANONICAL_BLOCK_UNAVAILABLE" | "RECEIPT_BLOCK_NOT_CANONICAL" | "INVALID_CONFIRMATION_POLICY";
...
export function getConditionalSettlementRequiredConfirmations(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.HAGGLE_CONDITIONAL_SETTLEMENT_CONFIRMATIONS;
  if (raw === undefined || raw === "") return DEFAULT_CONFIRMATIONS;
  if (!/^\d+$/.test(raw)) throw new Error("HAGGLE_CONDITIONAL_SETTLEMENT_CONFIRMATIONS must be an integer");
  const value = Number(raw);
  if (!Number.isSafeInteger(value) || value < MIN_CONFIRMATIONS || value > MAX_CONFIRMATIONS) {
    throw new Error(`HAGGLE_CONDITIONAL_SETTLEMENT_CONFIRMATIONS must be ${MIN_CONFIRMATIONS}..${MAX_CONFIRMATIONS}`);
  }
  return value;
}
...
export async function evaluateConditionalSettlementFinality(input: {
  ...
  if (!canonicalBlock.hash || canonicalBlock.hash.toLowerCase() !== input.receiptBlockHash.toLowerCase()) {
    return {
      status: "unavailable",
      ready: false,
...
export async function runConditionalSettlementFinalityFixture() {
  ...
  const evaluate = (receiptBlockNumber: bigint | undefined, head: bigint | Error, options: {
    ...
    evaluateConditionalSettlementFinality({
      ...
      client: {
        getBlockNumber: async () => { if (head instanceof Error) throw head; return head; },
        getBlock: async () => {
  ...
  const [oneShort, exact, headBehind, rpcFailure, missingBlock, invalidPolicy, missingHash, canonicalFailure, orphanedReceipt] = await Promise.all([
    evaluate(100n, 100n),
    ...
    evaluate(100n, 101n),
    evaluate(100n, 101n, { receiptBlockHash: receiptHash, canonicalHash: new Error("private canonical RPC detail") }),
  ...
  const checks = {
    ...
    head_behind_fail_closed: headBehind.status === "unavailable" && headBehind.reason === "CHAIN_HEAD_BEHIND_RECEIPT",
    rpc_failure_redacted: rpcFailure.status === "unavailable" && rpcFailure.reason === "CHAIN_HEAD_UNAVAILABLE"
      && !JSON.stringify(rpcFailure).includes("private upstream"),
    missing_receipt_block_fail_closed: missingBlock.status === "unavailable" && missingBlock.reason === "RECEIPT_BLOCK_MISSING",
    invalid_policy_fail_closed: invalidPolicy.status === "unavailable" && invalidPolicy.reason === "INVALID_CONFIRMATION_POLICY",
    missing_receipt_hash_fail_closed: missingHash.status === "unavailable" && missingHash.reason === "RECEIPT_BLOCK_HASH_MISSING",
    canonical_lookup_failure_redacted: canonicalFailure.status === "unavailable" && canonicalFailure.reason === "CANONICAL_BLOCK_UNAVAILABLE"
      && !JSON.stringify(canonicalFailure).includes("private canonical"),
    orphaned_receipt_blocked: orphanedReceipt.status === "unavailable" && orphanedReceipt.reason === "RECEIPT_BLOCK_NOT_CANONICAL",
  };
  return { pass: Object.values(checks).every(Boolean), policy: { default_confirmations: DEFAULT_CONFIRMATIONS }, checks,
    scenarios: { oneShort, exact, headBehind, rpcFailure, missingBlock, invalidPolicy, missingHash, canonicalFailure, orphanedReceipt } };
}
