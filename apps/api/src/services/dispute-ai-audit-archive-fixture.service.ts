...
import {
  claimVerifiedDisputeAiArchiveAlert,
  verifyDisputeAiAuditArchiveAlert,
} from "./dispute-ai-audit-archive-alert-verifier.service.js";
...
export async function runDisputeAiAuditArchiveFixture(db: Database) {
  ...
  const tryLockRows = (await db.transaction(async (transaction) =>
  ...
  const event = {
    ...
    policyVersion: "cycle62-policy-v1",
    model: "deepseek-v4-pro",
    contextHash: "c".repeat(64),
    requestedBy: "cycle62-fixture",
    forced: false,
  ...
  try {
    await db.transaction(async (transaction) => {
      ...
      const gapCoverage = await getDisputeAiAuditArchiveCoverage(
        fixtureDb,
        new Date(createdAt.getTime() + 10),
      );
      let revisionConflictBlocked = false;
      try {
        await appendDisputeAiAssessmentEvent(fixtureDb, {
          ...event,
          id: `cycle66_revision_${randomUUID()}`,
          revision: 3,
          versionId: `version_${randomUUID()}`,
        ...
        revisionConflictBlocked =
          error instanceof Error && error.message === "AI_AUDIT_REVISION_CONFLICT";
      }
      let supersedesConflictBlocked = false;
      try {
        await appendDisputeAiAssessmentEvent(fixtureDb, {
          ...event,
          id: `cycle66_supersedes_${randomUUID()}`,
          revision: 2,
          versionId: `version_${randomUUID()}`,
          supersedesAssessmentId: "wrong-assessment-id",
          createdAt: new Date(createdAt.getTime() + 2),
        });
      } catch (error) {
        supersedesConflictBlocked =
          error instanceof Error && error.message === "AI_AUDIT_SUPERSEDES_CONFLICT";
      }
      ...
      const coveredCoverage = await getDisputeAiAuditArchiveCoverage(
        fixtureDb,
        new Date(createdAt.getTime() + 20),
      );
      ...
      const rediscovery = await enqueuePendingDisputeAiAudits(fixtureDb, {
        privateKey,
        now: new Date(createdAt.getTime() + 23),
        limit: 100,
      });
      const retriedFailureRows = (await fixtureDb.execute(sql`
        SELECT attempt_count FROM dispute_ai_audit_discovery_failures
         WHERE dispute_id = ${poisonDisputeId}::uuid AND event_count = 1 AND status = 'OPEN'
      `)) as unknown as Array<{ attempt_count: number }>;
      const duplicate = await enqueueDisputeAiAuditArchive(fixtureDb, {
        disputeId,
        privateKey,
        now: createdAt,
      });
      ...
      const activeKey = normalizeDisputeAuditPublicKeyRecord({
        public_key_spki_base64: signedAudit.signature.public_key_spki_base64,
        status: "active",
      ...
      const retiredKey = normalizeDisputeAuditPublicKeyRecord({
        ...
        status: "retired",
        not_before: new Date(generatedAtMs - 1000).toISOString(),
        retired_at: new Date(generatedAtMs + 1000).toISOString(),
      });
      const revokedKey = normalizeDisputeAuditPublicKeyRecord({
        public_key_spki_base64: signedAudit.signature.public_key_spki_base64,
        status: "revoked",
        not_before: new Date(generatedAtMs - 1000).toISOString(),
        revoked_at: new Date(generatedAtMs + 1000).toISOString(),
      });
      ...
      const discoveryFailureHealth = await getDisputeAiAuditDiscoveryFailureHealth(
        fixtureDb,
        createdAt,
      );
      const cleanDiscoveryHealth = {
        ...discoveryFailureHealth,
      ...
      const mismatchFetch = (async () =>
        new Response(
          JSON.stringify({
            receipt_id: "cycle63-ai-audit-bad-receipt",
      ...
      const failedDispatch = await dispatchDisputeAiAuditArchives(fixtureDb, {
        ...
        now: new Date(createdAt.getTime() + 500),
      });
      const failedHealth = await getDisputeAiAuditArchiveHealth(
        fixtureDb,
        new Date(createdAt.getTime() + 600),
      );
      const alertBodies: Array<Record<string, unknown>> = [];
      const receiverChecks: boolean[] = [];
      const alertConfig = {
        url: "http://127.0.0.1:4177/mock-alert",
        secret: "cycle64-ai-archive-alert-secret",
        timeoutMs: 1000,
        cooldownMinutes: 15,
        staleThreshold: 1,
        ...
        discoveryUnresolvedThreshold: 1,
        discoveryTooLargeThreshold: 1,
        allowInsecureHttp: true,
        allowPrivateNetwork: true,
      };
      ...
      const alertFetch = (async (_url: string | URL | Request, init?: RequestInit) => {
        ...
        const verified = verifyDisputeAiAuditArchiveAlert({
          rawBody,
        ...
        if (!verified.ok) {
          receiverChecks.push(false);
          return new Response("invalid", { status: 401 });
        }
        const receiverSource = `${claimSource}-receiver`;
        const accepted = await claimVerifiedDisputeAiArchiveAlert(
          fixtureDb,
          verified,
          receiverSource,
        );
        ...
        const replay = await claimVerifiedDisputeAiArchiveAlert(
          fixtureDb,
          verified,
          receiverSource,
        );
      ...
      const firingAlert = await runDisputeAiAuditArchiveAlert(fixtureDb, {
        ...
        claimSource,
        discoveryHealth: cleanDiscoveryHealth,
        now: new Date(createdAt.getTime() + 650),
      });
      const requeued = await requeueDisputeAiAuditArchive(fixtureDb, {
        archiveId: first.archive.id,
        actorId: "99999999-9999-4999-8999-999999999999",
        reason: "Cycle 63 verified that the WORM receipt endpoint recovered.",
        now: new Date(createdAt.getTime() + 700),
      });
      const payloadPreserved =
        requeued.outcome === "requeued" &&
        requeued.archive.payloadSha256 === first.archive.payloadSha256;
      ...
      const recoveredHealth = await getDisputeAiAuditArchiveHealth(
        fixtureDb,
        new Date(createdAt.getTime() + 1100),
      );
      ...
      const pass =
        ...
        discovery.enqueued >= 1 &&
        firstFailureRows[0]?.attempt_count === 1 &&
        suppressed.discovered === 0 &&
        suppressed.enqueued === 0 &&
        suppressed.isolated === 0 &&
        retry.outcome === "retry_enabled" &&
        retryRequestedRows[0]?.status === "RETRY_REQUESTED" &&
        rediscovery.isolated === 1 &&
        retriedFailureRows[0]?.attempt_count === 2 &&
        discoveryAlertAssessment.wouldAlert &&
        discoveryAlertAssessment.severity === "warning" &&
        discoveryAlertAssessment.reasons.includes("ai_audit_discovery_failure_unresolved") &&
        duplicate.outcome === "duplicate" &&
        duplicate.archive.id === first.archive.id &&
        duplicate.archive.payloadSha256 === first.archive.payloadSha256 &&
        signatureValid &&
        failedDispatch.deadLettered === 1 &&
        failedHealth.deadLetter === baselineHealth.deadLetter + 1 &&
        failedHealth.status === "critical" &&
        requeued.outcome === "requeued" &&
        payloadPreserved &&
        dispatch.delivered === 1 &&
        archive.status === "DELIVERED" &&
        recoveredHealth.deadLetter === baselineHealth.deadLetter &&
        firingAlert.status === "delivered" &&
        alertBodies[0]?.state === "firing" &&
        recoveryAlert.status === "recovered" &&
        alertBodies[1]?.state === "recovered" &&
        duplicateRecovery.status === "skipped" &&
        duplicateRecovery.reason === "recovery_already_sent_or_in_progress" &&
        receiverChecks.length === 2 &&
        receiverChecks.every(Boolean) &&
        activeTrust.valid &&
        retiredTrust.valid &&
        !revokedTrust.valid &&
        revokedTrust.reason === "KEY_REVOKED" &&
        ...
        archive.receiptSha256 === archive.payloadSha256;
      throw new FixtureRollback({
        pass:
          pass &&
          advisoryLockSerialized &&
          advisoryTryLockRejected &&
          revisionConflictBlocked &&
          supersedesConflictBlocked,
        checks: pass ? "1/1" : "0/1",
        enqueued: first.outcome === "enqueued",
        poison_isolated: discovery.isolated >= 1 && firstFailureRows[0]?.attempt_count === 1,
        healthy_discovery_continued:
          discovery.enqueued >= 1 && first.archive.disputeId === disputeId,
        ...
        dead_letter_detected:
          failedDispatch.deadLettered === 1 && failedHealth.status === "critical",
  ...
  `);
  const remainingAlertClaims = await db.execute(
    sql`SELECT id FROM webhook_idempotency WHERE source IN (${claimSource}, ${`${claimSource}-receiver`})`,
  );
  ...
  rolledBackResult.pass =
    rolledBackResult.pass === true &&
    rolledBackResult.archive_cleanup === 1 &&
    rolledBackResult.event_cleanup === 2 &&
