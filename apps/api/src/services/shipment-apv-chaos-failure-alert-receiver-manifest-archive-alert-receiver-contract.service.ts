...
function parsePayloadContract(canonicalPayload: string) {
  try {
    ...
    if (Object.keys(payload).join("|") !== expectedKeys.join("|")
      ...
      || !reasons || reasons.length < 1 || reasons.length > ORDERED_REASONS.length
      || typeof payload.state_fingerprint !== "string"
      || !/^[0-9a-f]{64}$/.test(payload.state_fingerprint)
      || JSON.stringify(payload) !== canonicalPayload) return null;
    const reasonIndexes = reasons.map((reason) =>
      ORDERED_REASONS.indexOf(reason as typeof ORDERED_REASONS[number]));
    if (reasonIndexes.some((index) => index < 0)
      || reasonIndexes.some((index, position) => position > 0
        && index <= reasonIndexes[position - 1]!)) return null;
    const critical = reasonIndexes.some((index) => index <= 4);
    const semanticsValid = payload.action === "review_warning"
      ? payload.severity === "warning" && !critical
      : payload.action === "escalate_critical"
        && payload.severity === "critical" && critical;
    return semanticsValid ? { action: String(payload.action),
      severity: String(payload.severity), reasons,
      stateFingerprint: String(payload.state_fingerprint) } : null;
  } catch {
    return null;
  }
}
...
export async function verifyShipmentApvReceiverManifestArchiveAlertReceiverContract(
  ...
  const keyBindingVerified = String(row.intent_key_id)
      === String(row.signature_key_id)
  ...
  const intentAge = intentCreatedAt === null
    ? null : now.getTime() - intentCreatedAt;
  const freshnessVerified = signatureAge !== null && intentAge !== null
  ...
  const intentBindingVerified = String(row.intent_payload_signature_id)
    ...
    && signedAt !== null && intentCreatedAt !== null
    && outboxCreatedAt !== null && grantedAt !== null
    && cooldownExpiresAt !== null && cooldownClaimedAt !== null
    && cooldownClaimExpiresAt !== null && decidedAt !== null
    && requestCreatedAt !== null && requestExpiresAt !== null
    && keyEventCreatedAt !== null
    && requestExpiresAt > requestCreatedAt
    && requestExpiresAt <= requestCreatedAt + 15 * 60_000
    && decidedAt >= requestCreatedAt && decidedAt < requestExpiresAt
    ...
    && intentCreatedAt >= signedAt && intentCreatedAt < cooldownExpiresAt
    && now.getTime() < cooldownExpiresAt;

  if (!payloadContractVerified || !payloadHashVerified || !signatureVerified
    || !keyBindingVerified || !freshnessVerified || !intentBindingVerified) {
    throw new Error(
