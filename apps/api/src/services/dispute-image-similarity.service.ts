...
export async function computeImageSimilarityFingerprint(bytes: Buffer): Promise<ImageSimilarityFingerprint> {
  const image = sharp(bytes, { failOn: "error", limitInputPixels: 40_000_000 }).rotate();
  const [{ data: averageData, info: averageInfo }, { data: colorData, info: colorInfo }, dHash] = await Promise.all([
    image.clone().resize(8, 8, { fit: "fill" }).grayscale().raw().toBuffer({ resolveWithObject: true }),
    image.clone().resize(32, 32, { fit: "cover" }).removeAlpha().raw().toBuffer({ resolveWithObject: true }),
    computeImageDHash(bytes),
  ]);
  if (averageInfo.width !== 8 || averageInfo.height !== 8 || averageInfo.channels !== 1 || averageData.length !== 64) {
    throw new Error("INVALID_AHASH_IMAGE_BUFFER");
  }
  if (colorInfo.width !== 32 || colorInfo.height !== 32 || colorInfo.channels !== 3 || colorData.length !== 3072) {
...
function validColorHistogram(value: number[] | null | undefined): value is number[] {
  return Array.isArray(value) && value.length === 12 && value.every((item) => Number.isInteger(item) && item >= 0 && item <= 255);
}

export function colorHistogramDistance(left: number[], right: number[]): number {
  if (!validColorHistogram(left) || !validColorHistogram(right)) throw new Error("INVALID_COLOR_HISTOGRAM");
...
export function assessImageSimilarity(
  ...
  const observedSignals = [
    ...
    ...(aHashDistance !== null && aHashDistance <= CAMERA_SIMILARITY_REVIEW_DISTANCE ? ["ahash_near"] : []),
  ];
  const structuralDual = aHashDistance !== null
    && dHashDistance <= 10
    && aHashDistance <= CAMERA_SIMILARITY_REVIEW_DISTANCE;
  if (structuralDual) observedSignals.push("structure_dual_hash");
  const combined = aHashDistance !== null && colorDistance !== null
  ...
  const normalized = [
    ...
    ...(aHashDistance === null ? [] : [(dHashDistance / 10 + aHashDistance / CAMERA_SIMILARITY_REVIEW_DISTANCE) / 2]),
  ...
  return {
    reviewRequired: dHashDistance <= CAMERA_SIMILARITY_REVIEW_DISTANCE || structuralDual || combined,
    dHashDistance,
    aHashDistance,
    colorDistance,
