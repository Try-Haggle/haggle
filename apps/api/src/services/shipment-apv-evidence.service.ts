...
function mapEvidence(row: Record<string, unknown>): ShipmentApvEvidenceRecord {
  return {
    ...
    evidence_bound_at: row.evidence_bound_at instanceof Date
      ? row.evidence_bound_at.toISOString()
      : typeof row.evidence_bound_at === "string" ? row.evidence_bound_at : undefined,
  };
}
...
export async function bindShipmentApvRevisionEvidence(
  ...
  return db.transaction(async (tx) => {
    ...
    if (revision.evidence_sha256) {
      ...
      const same = existing.evidence_sha256 === input.documentSha256
    ...
    if (Math.abs(numeric(revision.delta_minor)) !== input.amountMinor) return { outcome: "amount_conflict" } as const;
