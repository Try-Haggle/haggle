...
function snapshot(
  probe: ConditionalSettlementPreflightResult,
): ConditionalSettlementPreflightAlertSnapshot {
...
export async function runConditionalSettlementPreflightAlertFixture(db: Database) {
  ...
  const config = {
    url: "http://127.0.0.1:4177/mock-preflight-alert",
    secret,
    timeoutMs: 1000,
    cooldownMinutes: 15,
  ...
  try {
    const fetchImpl = (async (_url: string | URL | Request, init?: RequestInit) => {
      ...
      const delivery = await claimVerifiedConditionalSettlementPreflightAlert(
        db,
        verification,
        receiverSource,
      );
      if (delivery.outcome !== "accepted") return new Response("conflict", { status: 409 });
      if (!(await completeWebhookEvent(db, delivery.claim, 202)))
        return new Response("incomplete", { status: 503 });
      const replay = await claimVerifiedConditionalSettlementPreflightAlert(
        db,
        verification,
        receiverSource,
      );
    ...
    const runConcurrent = async (args: {
      ...
      const runs = Array.from({ length: 20 }, () =>
        runConditionalSettlementPreflightAlert(db, {
          config,
          claimSource: args.source,
          now: args.now,
      ...
      try {
        ...
        return {
          delivered: results.filter(
            (result) => result.status === "delivered" || result.status === "recovered",
          ).length,
    ...
    const blockedProbe = {
      ...
      ready: false,
      checks: { ...readyProbe.checks, signer_matches: false },
      blocked_by: ["signer_matches"],
      checked_at: now.toISOString(),
    };
    const firing = await runConditionalSettlementPreflightAlert(db, {
      config,
      claimSource: senderSource,
      fetchImpl,
      now,
      collectSnapshot: async () => snapshot(blockedProbe),
    });
    ...
    const recovery = await runConditionalSettlementPreflightAlert(db, {
      ...
      collectSnapshot: async () =>
        snapshot({ ...readyProbe, checked_at: recoveryNow.toISOString() }),
    });
    ...
    const unavailableProbe = snapshot({
      ...readyProbe,
      status: "unavailable",
      ready: false,
      checks: {
    ...
    const retryRecovery = await runConditionalSettlementPreflightAlert(db, {
      config,
      claimSource: retrySenderSource,
      now: retryRecoveryNow,
      fetchImpl,
      collectSnapshot: async () =>
        snapshot({ ...readyProbe, checked_at: retryRecoveryNow.toISOString() }),
    });
    const concurrentNow = new Date(retryRecoveryNow.getTime() + 2000);
    const chainMismatch = snapshot({
      ...readyProbe,
      status: "blocked",
      ready: false,
      checks: { ...readyProbe.checks, chain_id_match: false },
      blocked_by: ["chain_id_match"],
      observed_chain_id: 8453,
      checked_at: concurrentNow.toISOString(),
    });
    const concurrentFiring = await runConcurrent({
      source: concurrentSenderSource,
      now: concurrentNow,
      health: chainMismatch,
    });
    ...
    if (firstDelivery) {
      ...
      if (changed.ok) {
        payloadConflict =
          (await claimVerifiedConditionalSettlementPreflightAlert(db, changed, receiverSource))
            .outcome === "payload_conflict";
      }
    }
    const checks = {
      firing_delivered: firing.status === "delivered" && states[0] === "firing",
      sender_duplicate_blocked:
        duplicateFiring.status === "skipped" &&
        duplicateFiring.reason === "cooldown_or_in_progress",
      recovery_delivered: recovery.status === "recovered" && states[1] === "recovered",
      duplicate_recovery_blocked:
      ...
      immediate_retry_respects_backoff:
        backoffRetry.status === "skipped" && backoffRetry.reason === "delivery_retry_backoff",
      fixture_backoff_release_scoped: released.length === 1,
      retry_attempt_delivered:
        deliveredRetry.status === "delivered" && retryAttemptRows[0]?.attemptCount === 2,
      retry_recovery_delivered: retryRecovery.status === "recovered",
      concurrent_firing_single_winner:
        concurrentFiring.delivered === 1 &&
        concurrentFiring.blocked === 19 &&
        concurrentFiring.fetchCalls === 1,
      concurrent_recovery_single_winner:
        concurrentRecovery.delivered === 1 &&
        concurrentRecovery.blocked === 19 &&
        concurrentRecovery.fetchCalls === 1,
    ...
    report = {
      ...
      checks,
      deliveries: {
        firing: states.filter((state) => state === "firing").length,
        recovery: states.filter((state) => state === "recovered").length,
      },
      receiver: {
        verified: signatures.length,
        replay_blocked: receiverReplays.filter(Boolean).length,
        payload_conflict: payloadConflict,
      },
      retry: {
        failed: failedDelivery.status === "failed",
        backoff_blocked: backoffRetry.reason === "delivery_retry_backoff",
        released: released.length,
        delivered: deliveredRetry.status === "delivered",
        attempt_count: retryAttemptRows[0]?.attemptCount ?? 0,
        recovered: retryRecovery.status === "recovered",
      },
  ...
  report.pass =
    Object.values(checks).every(Boolean) && cleanupCount === 12 && cleanupRemaining === 0;
  return report;
}
