...
export function createSignedDisputeSimilarityReviewAuditExport(input: {
  ...
  if (computedEventHash !== input.storedEventHash)
  ...
  const manifest = {
    ...
    event_payload_sha256: createHash("sha256")
      .update(canonicalDisputeAuditJson(input.event))
      .digest("hex"),
...
export function verifySignedDisputeSimilarityReviewAuditExport(
  ...
  if (
    ...
    value.manifest.event_payload_sha256 !==
      createHash("sha256").update(canonicalDisputeAuditJson(value.event)).digest("hex")
  )
    return false;
