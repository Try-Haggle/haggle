...
interface InternalRestorationCandidate {
  ...
  contentType: ShipmentInvoiceContentType;
}
...
function internalCandidate(row: Record<string, unknown>): InternalRestorationCandidate {
  const base = {
    ...
    integrityStatus: String(row.integrity_status) as "MISSING" | "QUARANTINED",
...
function publicCandidate(
  value: InternalRestorationCandidate,
): ShipmentApvInvoiceRestorationCandidate {
...
function mapRequest(row: Record<string, unknown>): ShipmentApvInvoiceRestorationRequest {
  return {
    ...
    replacement_sha256: String(row.replacement_sha256),
    replacement_byte_size: Number(row.replacement_byte_size),
    requester_id: String(row.requester_id),
    reason: String(row.reason),
    status: String(row.status) as ShipmentApvInvoiceRestorationRequest["status"],
    ...
    decision: row.decision
      ? (String(row.decision) as ShipmentApvInvoiceRestorationRequest["decision"])
      : undefined,
    ...
    staging_status: String(
      row.staging_status ?? "STAGED",
    ) as ShipmentApvInvoiceRestorationRequest["staging_status"],
    staging_disposed_at: row.staging_disposed_at
      ? new Date(String(row.staging_disposed_at)).toISOString()
      : undefined,
...
async function listInternalCandidates(db: Pick<Database, "execute">) {
  const rows =
    (await db.execute(sql`SELECT id, integrity_status, storage_key, sha256, byte_size, content_type
    FROM shipment_apv_invoice_documents WHERE integrity_status IN ('MISSING', 'QUARANTINED')
    ORDER BY integrity_updated_at ASC NULLS FIRST, created_at ASC LIMIT 1000`)) as unknown as Array<
...
export async function requestShipmentApvInvoiceRestoration(
  db: Database,
  input: {
  ...
  if (
    ...
    !validateShipmentApvInvoiceDocumentBytes(input.bytes, input.contentType)
  )
  ...
  try {
    return await db.transaction(async (tx) => {
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtextextended(${`apv-invoice-restore:${input.clientRequestId}`}, 0))`,
      );
      const existingRows =
        (await tx.execute(sql`SELECT * FROM shipment_apv_invoice_restoration_requests
      ...
      if (existingRows[0]) {
        ...
        if (
          ...
          existing.replacement_sha256 !== sha256
        ) {
      ...
      if (
        ...
        target.expectedSha256 !== sha256 ||
        target.expectedByteSize !== input.bytes.length
      )
        return { outcome: "replacement_mismatch" } as const;
      ...
      try {
        ...
        if (!existingBytes.equals(input.bytes))
          throw new Error("APV_INVOICE_RESTORATION_STAGING_CONFLICT");
      }
      const rows = (await tx.execute(sql`INSERT INTO shipment_apv_invoice_restoration_requests
        ...
        VALUES (${requestId}::uuid, ${input.clientRequestId}::uuid, ${target.candidateId}, ${target.documentId}::uuid,
          ...
          ${now.toISOString()}::timestamptz, ${now.toISOString()}::timestamptz) RETURNING *`)) as unknown as Array<
        Record<string, unknown>
      >;
      ...
      await appendEvent(tx, {
        ...
        version: 0,
        metadata: {
          source_integrity_status: target.integrityStatus,
          candidate_fingerprint: target.candidateId,
...
async function fileHash(path: string, maxBytes = 5 * 1024 * 1024) {
  ...
  if (!metadata.isFile() || metadata.isSymbolicLink())
    throw new Error("APV_INVOICE_RESTORE_NOT_REGULAR_FILE");
  if (metadata.size < 1 || metadata.size > maxBytes)
    throw new Error("APV_INVOICE_RESTORE_FILE_SIZE_OUT_OF_RANGE");
  const bytes = await readFile(path);
  return { sha256: createHash("sha256").update(bytes).digest("hex"), size: bytes.length };
}

export async function decideShipmentApvInvoiceRestoration(
  db: Database,
  input: {
    requestId: string;
  ...
  try {
    const claim = await db.transaction(async (tx) => {
      ...
      const candidateRows =
        (await tx.execute(sql`SELECT id, integrity_status, storage_key, sha256, byte_size, content_type
        FROM shipment_apv_invoice_documents WHERE id = ${row.document_id}::uuid FOR UPDATE`)) as unknown as Array<
          Record<string, unknown>
        >;
      if (
        !candidateRows[0] ||
        internalCandidate(candidateRows[0]).candidateId !== request.candidate_fingerprint
      ) {
    ...
    const preservePath = join(
      ...
      ".quarantine",
      input.requestId,
      `replacement-${basename(String(row.storage_key))}`,
    );
    ...
    try {
      stagingExists = (await fileHash(stagingPath)).sha256 === String(row.expected_sha256);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    try {
      ...
      destinationExists =
        destinationHash.sha256 === String(row.expected_sha256) &&
    ...
    if (stagingExists && !destinationExists) await rename(stagingPath, destination);
      ...
      throw new Error("APV_INVOICE_RESTORE_SOURCE_MISSING");
    else if (stagingExists && destinationExists)
      throw new Error("APV_INVOICE_RESTORE_DESTINATION_CONFLICT");
    return await db.transaction(async (tx) => {
      ...
      if (
        ...
        throw new Error("APV_INVOICE_RESTORE_APPLY_STATE_LOST");
      if (input.decision === "RESTORE") {
        ...
        if (documentRows.length !== 1)
          throw new Error("APV_INVOICE_RESTORE_DOCUMENT_STATE_CHANGED");
      }
      ...
      await appendEvent(tx, {
        ...
        metadata: { decision: input.decision, replacement_sha256: completed.replacement_sha256 },
        now,
      });
      await tx.execute(sql`INSERT INTO admin_action_log
        (actor_id, action_type, target_type, target_id, payload, created_at)
      ...
      return {
        outcome: input.decision === "RESTORE" ? "restored" : "preserved",
        request: completed,
      } as const;
    });
  } catch (error) {
...
function restorationPath(root: string, key: string) {
  ...
  if (!candidate.startsWith(`${rootPath}${sep}`))
    throw new Error("APV_INVOICE_RESTORATION_PATH_ESCAPE");
  return candidate;
}
...
export async function getShipmentApvInvoiceRestorationStagingHealth(
  db: Database,
  input: {
    storageRoot?: string;
    now?: Date;
    limit?: number;
    maxCheckedBytes?: number;
  } = {},
): Promise<ShipmentApvInvoiceRestorationStagingHealth> {
  const now = input.now ?? new Date();
  const limit = Math.min(Math.max(input.limit ?? 1000, 1), 1000);
  const maxCheckedBytes = Math.min(
    Math.max(input.maxCheckedBytes ?? 100 * 1024 * 1024, 1),
    100 * 1024 * 1024,
  );
  const root = input.storageRoot ?? resolveShipmentInvoiceDocumentRoot();
  const rows =
    (await db.execute(sql`SELECT id, status, staging_status, staging_key, replacement_sha256,
  ...
  for (const row of candidates) {
    ...
    if (
      row.staging_status === "MOVING" &&
      new Date(String(row.updated_at)).getTime() <= now.getTime() - 5 * 60 * 1000
    )
      staleMoving += 1;
    ...
    let path: string;
    try {
      path = restorationPath(root, String(row.staging_key));
    } catch {
      invalidEntries += 1;
      continue;
    }
    try {
      ...
      if (
        source.sha256 !== String(row.replacement_sha256) ||
        source.size !== Number(row.replacement_byte_size)
      )
        hashMismatches += 1;
    } catch (error) {
  ...
  return {
    ...
    trackedStaging: candidates.length,
    pendingDisposition,
    staleMoving,
    missingSources,
    hashMismatches,
...
export async function maintainShipmentApvInvoiceRestorationStaging(
  db: Database,
  input: {
    mode: "dry_run" | "apply";
    actorId: string;
  ...
  for (const candidate of candidates) {
    const claim = await db.transaction(async (tx) => {
      ...
      if (row.status === "PENDING") {
        ...
        await appendEvent(tx, {
          ...
          eventType: "EXPIRED",
          actorId: null,
          version: Number(expiredRows[0].version),
          now,
        });
        result.expired += 1;
    ...
    try {
      ...
      const destinationKey = join(
        ".quarantine",
        String(claim.id),
        `staged-${basename(String(claim.staging_key))}`,
      );
    ...
    try {
      source = await fileHash(stagingPath);
    } catch (error) {
    ...
    try {
      destination = await fileHash(destinationPath);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
        result.conflicts += 1;
        continue;
    ...
    const matches = (value: { sha256: string; size: number } | null) =>
      Boolean(
        value &&
          value.sha256 === String(claim.replacement_sha256) &&
          value.size === Number(claim.replacement_byte_size),
      );
    if (
    ...
    await db.transaction(async (tx) => {
      ...
      await appendEvent(tx, {
        requestId: String(claim.id),
        eventType: "STAGING_PRESERVED",
        actorId: input.actorId,
        version: Number(movedRows[0].version),
        metadata: {
          disposition: "QUARANTINE",
          replacement_sha256: String(claim.replacement_sha256),
        },
        now,
      });
      await tx.execute(sql`INSERT INTO admin_action_log
...
export async function deleteShipmentApvInvoiceRestorationFixtureRows(
  db: Database,
  requestIds: string[],
) {
