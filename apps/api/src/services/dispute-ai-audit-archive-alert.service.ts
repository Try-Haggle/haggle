...
import type {
  DisputeAiAuditArchiveHealth,
  DisputeAiAuditDiscoveryFailureHealth,
...
export interface DisputeAiAuditArchiveAlertConfig {
  ...
  discoveryUnresolvedThreshold: number;
  discoveryTooLargeThreshold: number;
...
export function getDisputeAiAuditArchiveAlertPolicyStatus() {
  ...
  return {
    ...
    cooldownMinutes: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_COOLDOWN_MINUTES,
      15,
      1,
      1440,
    ),
    staleThreshold: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_STALE_THRESHOLD,
      1,
      1,
      100_000,
    ),
    retryReadyThreshold: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_RETRY_READY_THRESHOLD,
      5,
      1,
      100_000,
    ),
    deadLetterThreshold: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_DEAD_LETTER_THRESHOLD,
      1,
      1,
      100_000,
    ),
    overdueUnfinishedThreshold: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_OVERDUE_UNFINISHED_THRESHOLD,
      1,
      1,
      100_000,
    ),
    discoveryUnresolvedThreshold: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_DISCOVERY_UNRESOLVED_THRESHOLD,
      1,
      1,
      100_000,
    ),
    discoveryTooLargeThreshold: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_DISCOVERY_TOO_LARGE_THRESHOLD,
      1,
      1,
      100_000,
    ),
...
export function resolveDisputeAiAuditArchiveAlertConfigFromEnv(): DisputeAiAuditArchiveAlertConfig | null {
  ...
  const secret = process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_SECRET ?? "";
  if (secret.length < 16)
    throw new Error("dispute AI audit archive alert secret must be at least 16 characters");
  const policy = getDisputeAiAuditArchiveAlertPolicyStatus();
  const config = {
    url,
    secret,
    timeoutMs: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_TIMEOUT_MS,
      5000,
      250,
      30_000,
    ),
...
export function evaluateDisputeAiAuditArchiveAlert(
  ...
  policy: Pick<
    DisputeAiAuditArchiveAlertConfig,
    | "staleThreshold"
    | "retryReadyThreshold"
    | "deadLetterThreshold"
    | "overdueUnfinishedThreshold"
    | "discoveryUnresolvedThreshold"
    | "discoveryTooLargeThreshold"
  >,
  ...
  const reasons = [
    ...
    ...(health.staleProcessing >= policy.staleThreshold
      ? ["ai_audit_archive_stale_processing"]
      : []),
    ...(health.retryReady >= policy.retryReadyThreshold
      ? ["ai_audit_archive_retry_ready_backlog"]
      : []),
    ...(health.overdueUnfinished >= policy.overdueUnfinishedThreshold
      ? ["ai_audit_archive_unfinished_too_old"]
      : []),
    ...(discoveryHealth && discoveryHealth.unresolved >= policy.discoveryUnresolvedThreshold
      ? ["ai_audit_discovery_failure_unresolved"]
      : []),
    ...(discoveryHealth && discoveryHealth.tooLarge >= policy.discoveryTooLargeThreshold
      ? ["ai_audit_discovery_failure_too_large"]
      : []),
  ];
  return {
    wouldAlert: reasons.length > 0,
    severity:
      reasons.includes("ai_audit_archive_dead_letter") ||
      reasons.includes("ai_audit_discovery_failure_too_large")
        ? ("critical" as const)
        : reasons.length
          ? ("warning" as const)
          : null,
...
export async function getDisputeAiAuditArchiveAlertDeliveryState(
  db: Database,
  source = DISPUTE_AI_AUDIT_ARCHIVE_ALERT_SOURCE,
) {
...
export async function findLatestDeliveredDisputeAiAuditArchiveIncident(
  db: Database,
  source = DISPUTE_AI_AUDIT_ARCHIVE_ALERT_SOURCE,
) {
  const rows = (await db.execute(sql`
...
export async function sendDisputeAiAuditArchiveAlert(
  health: DisputeAiAuditArchiveHealth,
  assessment: DisputeAiAuditArchiveAlertAssessment,
  options: {
  ...
  if (!/^(?:health|recovery)_[0-9a-f]{64}$/.test(options.deliveryId))
    throw new Error("invalid dispute AI audit archive alert delivery id");
  ...
  try {
    const response = await (options.fetchImpl ?? fetch)(options.config.url, {
      ...
      headers: {
        ...
        "x-haggle-alert-type": "dispute_ai_audit_archive.health",
