...
} from "./dispute-ai-audit-export.service.js";
...
export interface DisputeAiAuditArchiveCoverage {
  ...
  eligibleUnarchived: number;
  overdueEligibleUnarchived: number;
  blockedUnsealed: number;
  blockedOversized: number;
  coveragePercent: number;
  oldestUnarchivedAgeSeconds: number | null;
  coverageMaxAgeMinutes: number;
...
export function getDisputeAiAuditArchivePolicyStatus() {
  ...
  let configurationState: "not_configured" | "partial" | "invalid" | "valid" =
    !url && !signing ? "not_configured" : !url || !signing ? "partial" : "valid";
  ...
  return {
    ...
    maxExportEvents: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_MAX_EVENTS,
      10_000,
      1,
      100_000,
    ),
    unfinishedMaxAgeMinutes: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_UNFINISHED_MAX_AGE_MINUTES,
      15,
      1,
      10_080,
    ),
    coverageMaxAgeMinutes: boundedInteger(
      process.env.DISPUTE_AI_AUDIT_ARCHIVE_COVERAGE_MAX_AGE_MINUTES,
      15,
      1,
      10_080,
    ),
...
function mapRow(row: Record<string, unknown>): DisputeAiAuditArchiveRecord {
  return {
    ...
    eventsSha256: String(row.events_sha256),
    chainHeadEventHash: row.chain_head_event_hash ? String(row.chain_head_event_hash) : null,
    payload: row.payload as Record<string, unknown>,
    payloadSha256: String(row.payload_sha256),
    status: String(row.status) as DisputeAiAuditArchiveRecord["status"],
...
export async function getDisputeAiAuditArchiveHealth(
  db: Database,
  now = new Date(),
): Promise<DisputeAiAuditArchiveHealth> {
...
export async function getDisputeAiAuditArchiveCoverage(
  db: Database,
  now = new Date(),
): Promise<DisputeAiAuditArchiveCoverage> {
  const policy = getDisputeAiAuditArchivePolicyStatus();
  ...
  return {
    status:
      overdueEligibleUnarchived > 0 || blockedOversized > 0
        ? "critical"
        : eligibleUnarchived > 0 || blockedUnsealed > 0
          ? "attention"
          : "healthy",
    totalChains: Number(row.total_chains ?? 0),
    eligibleChains,
    archivedCurrent,
    eligibleUnarchived,
    overdueEligibleUnarchived,
    blockedUnsealed,
    blockedOversized,
    coveragePercent:
      eligibleChains === 0 ? 100 : Math.round((archivedCurrent / eligibleChains) * 10_000) / 100,
    oldestUnarchivedAgeSeconds:
      row.oldest_unarchived_age_seconds == null
        ? null
        : Math.max(0, Number(row.oldest_unarchived_age_seconds)),
    coverageMaxAgeMinutes: policy.coverageMaxAgeMinutes,
    maxExportEvents: policy.maxExportEvents,
    recordedAt: now.toISOString(),
  };
}
...
export async function getDisputeAiAuditDiscoveryFailureHealth(
  ...
  const retryRequested = Number(row.retry_requested ?? 0);
  const unresolved = Number(row.unresolved ?? 0);
  const tooLarge = Number(row.too_large ?? 0);
  return {
    status: tooLarge > 0 ? "critical" : unresolved > 0 ? "attention" : "healthy",
    open,
    retryRequested,
    unresolved,
    invalidChain: Number(row.invalid_chain ?? 0),
    tooLarge,
    unsealed: Number(row.unsealed ?? 0),
    resolvedLast24h: Number(row.resolved_last_24h ?? 0),
    oldestOpenAgeSeconds:
      row.oldest_open_age_seconds == null ? null : Math.max(0, Number(row.oldest_open_age_seconds)),
    recordedAt: now.toISOString(),
  };
}

interface DiscoveryFailureCursor {
  lastFailedAt: string;
  id: string;
}
function decodeDiscoveryFailureCursor(value: string): DiscoveryFailureCursor {
  try {
    ...
    ) as Partial<DiscoveryFailureCursor>;
    if (
    ...
    throw new Error("INVALID_AI_AUDIT_DISCOVERY_FAILURE_CURSOR");
  }
}
...
export async function listDisputeAiAuditDiscoveryFailures(
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 100)
    throw new Error("INVALID_AI_AUDIT_DISCOVERY_FAILURE_LIMIT");
  ...
  const items = selected.map((row) => ({
    ...
    attemptCount: Number(row.attempt_count),
    firstFailedAt: new Date(String(row.first_failed_at)).toISOString(),
    lastFailedAt: new Date(String(row.last_failed_at)).toISOString(),
    ageSeconds: Math.max(
      0,
      Math.floor((now.getTime() - new Date(String(row.first_failed_at)).getTime()) / 1000),
    ),
  }));
  ...
  return {
    ...
    nextCursor:
      hasMore && last
        ? encodeDiscoveryFailureCursor({ lastFailedAt: last.lastFailedAt, id: last.id })
        : null,
    recordedAt: now.toISOString(),
  };
}
...
export async function retryDisputeAiAuditDiscoveryFailure(
  db: Database,
  input: { disputeId: string; eventCount: number; actorId: string; reason: string; now?: Date },
) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (rows[0].status === "RETRY_REQUESTED")
      return { outcome: "retry_already_requested" as const };
...
function decodeFailureCursor(value: string): FailureCursor {
  try {
    ...
    throw new Error("INVALID_AI_AUDIT_ARCHIVE_FAILURE_CURSOR");
  }
}
...
export async function listDisputeAiAuditArchiveFailures(
  db: Database,
  input: { limit?: number; cursor?: string; now?: Date } = {},
) {
  const limit = input.limit ?? 20;
  if (!Number.isInteger(limit) || limit < 1 || limit > 100)
    throw new Error("INVALID_AI_AUDIT_ARCHIVE_FAILURE_LIMIT");
  ...
  return {
    items,
    nextCursor:
      hasMore && last ? encodeFailureCursor({ createdAt: last.createdAt, id: last.id }) : null,
    recordedAt: now.toISOString(),
  };
}
...
export async function requeueDisputeAiAuditArchive(
  ...
  return db.transaction(async (tx) => {
    const rows = (await tx.execute(
      sql`SELECT * FROM dispute_ai_audit_outbox WHERE id = ${input.archiveId}::uuid FOR UPDATE`,
    )) as unknown as Array<Record<string, unknown>>;
    if (!rows[0]) return { outcome: "not_found" as const };
...
async function buildDisputeAiAuditSnapshot(
  db: Database,
...
export async function enqueueDisputeAiAuditArchive(
  ...
  const now = input.now ?? new Date();
  const payload = await buildDisputeAiAuditSnapshot(db, {
    disputeId: input.disputeId,
    now,
    privateKey: input.privateKey,
  });
  ...
  const payloadSha256 = createHash("sha256")
    ...
    .digest("hex");
  const archiveKey = `dai_${createHash("sha256")
    .update(
      [
        input.disputeId,
        payload.manifest.event_count,
        payload.manifest.events_sha256,
  ...
  const existing = (await db.execute(
    sql`SELECT * FROM dispute_ai_audit_outbox WHERE archive_key = ${archiveKey}`,
  )) as unknown as Array<Record<string, unknown>>;
...
export async function claimDisputeAiAuditArchives(
  db: Database,
...
export async function deliverDisputeAiAuditArchive(
  ...
  assertDisputeModuleOutboundUrl(config.url, {
    label: "dispute AI audit archive",
...
export async function dispatchDisputeAiAuditArchives(
  db: Database,
  input: {
    config?: DisputeAiAuditArchiveConfig | null;
  ...
  for (const archive of claimed) {
    const result = await deliverDisputeAiAuditArchive(archive, config, {
