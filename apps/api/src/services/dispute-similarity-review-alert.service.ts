...
export function getDisputeSimilarityReviewAlertPolicyStatus() {
  ...
  const allowInsecureHttp = process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_ALLOW_INSECURE_HTTP === "true";
  const allowPrivateNetwork = process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_ALLOW_PRIVATE_NETWORK === "true";
  ...
  if (url || secret) {
    if (!url || secret.length < 16) configurationStatus = "partial";
      ...
      try {
        assertDisputeModuleOutboundUrl(url, { label: "dispute similarity review alert", allowInsecureHttp, allowPrivateNetwork });
        configurationStatus = "valid";
      } catch { configurationStatus = "invalid"; }
    }
  }
  return {
    ...
    cooldownMinutes: boundedInteger(process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_COOLDOWN_MINUTES, 15, 1, 1440),
    overdueThreshold: boundedInteger(process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_OVERDUE_THRESHOLD, 1, 1, 100_000),
    expiredThreshold: boundedInteger(process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_EXPIRED_THRESHOLD, 1, 1, 100_000),
    slaMinutes: boundedInteger(process.env.DISPUTE_SIMILARITY_REVIEW_SLA_MINUTES, 15, 1, 1440),
    dueSoonMinutes: boundedInteger(process.env.DISPUTE_SIMILARITY_REVIEW_DUE_SOON_MINUTES, 60, 1, 1440),
  };
}
...
export function resolveDisputeSimilarityReviewAlertConfigFromEnv(): DisputeSimilarityReviewAlertConfig | null {
  ...
  if (!url) return null;
  const secret = process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_SECRET ?? "";
  if (secret.length < 16) throw new Error("dispute similarity review alert secret must be at least 16 characters");
  const policy = getDisputeSimilarityReviewAlertPolicyStatus();
  const config = {
    url,
    secret,
    timeoutMs: boundedInteger(process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_TIMEOUT_MS, 5000, 250, 30_000),
    ...policy,
    allowInsecureHttp: process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_ALLOW_INSECURE_HTTP === "true",
    allowPrivateNetwork: process.env.DISPUTE_SIMILARITY_REVIEW_ALERT_ALLOW_PRIVATE_NETWORK === "true",
  };
...
export function evaluateDisputeSimilarityReviewAlert(
  ...
  const reasons = [
    ...
    ...(health.expiredUnresolved >= policy.expiredThreshold ? ["similarity_review_expired_unresolved"] : []),
  ];
  return {
    wouldAlert: reasons.length > 0,
    severity: reasons.includes("similarity_review_expired_unresolved") ? "critical" as const
      : reasons.length ? "warning" as const : null,
    reasons,
  };
}
