...
import { claimWebhookEvent, completeWebhookEvent } from "./webhook-event-claim.service.js";
...
const THRESHOLD_KEYS_V2 = ["retry_ready", "stale_processing", "exhausted", "expired_quarantined"];
const THRESHOLD_KEYS_V3 = [...THRESHOLD_KEYS_V2, "retention_blocked_expired"];
...
function boundedPositive(value: unknown) {
  return typeof value === "number" && Number.isSafeInteger(value) && value >= 1 && value <= 100_000;
}
...
function expectedReasons(
  ...
  return [
    ...
    ...(active && retentionJob?.last_run_status === "STALE_RUNNING"
      ...
      : []),
    ...(retention &&
    (retention.blocked_expired as number) >= (thresholds.retention_blocked_expired as number)
      ? ["alert_snapshot_retention_blocked"]
      : []),
    ...((totals.exhausted as number) >= (thresholds.exhausted as number)
      ? ["scan_retry_exhausted"]
      : []),
    ...((totals.expired_quarantined as number) >= (thresholds.expired_quarantined as number)
      ? ["scan_retry_expired_quarantine"]
      : []),
    ...((totals.stale_processing as number) >= (thresholds.stale_processing as number)
      ? ["scan_retry_stale_processing"]
      : []),
    ...
    ...(active && retentionJob?.last_run_status === "FAILED"
      ? ["alert_snapshot_retention_job_failed"]
      : []),
    ...(active && retentionJob?.overdue === true ? ["alert_snapshot_retention_job_overdue"] : []),
  ];
}
...
export function resolveDisputeEvidenceScanRetryAlertReceiverSecretsFromEnv() {
  const current = process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_SECRET?.trim() ?? "";
  const previousRaw = process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_PREVIOUS_SECRETS?.trim() ?? "";
  const previous = previousRaw ? previousRaw.split(",").map((value) => value.trim()) : [];
...
export function verifyDisputeEvidenceScanRetryAlert(input: {
  ...
  const isV3 = body.schema_version === "dispute-evidence-scan-retry-alert-v3";
  const isV2 = body.schema_version === "dispute-evidence-scan-retry-alert-v2";
  if (
  ...
  if (!exact(thresholds, thresholdKeys) || !exact(alertHealth, healthKeys)) {
    return { ok: false, error: "INVALID_ALERT_BODY" };
  }
  const totals = alertHealth.totals;
  const circuit = alertHealth.circuit;
  const retention = isV3 ? alertHealth.retention : undefined;
  const retentionJob = exact(retention, RETENTION_KEYS) ? retention.job : undefined;
  if (
    ...
    (isV3 &&
      (!exact(retention, RETENTION_KEYS) ||
        ...
        (retention.oldest_blocked_expired_age_seconds !== null &&
          !boundedNonnegative(retention.oldest_blocked_expired_age_seconds)) ||
        ...
        retentionJob.lease_stale !== (retentionJob.last_run_status === "STALE_RUNNING") ||
        retentionJob.status !==
          (!retentionJob.active
            ? "inactive"
  ...
  const critical =
    firingReasons.includes("scanner_circuit_open") ||
  ...
  if (secrets.length < 1 || secrets.length > 4 || new Set(secrets).size !== secrets.length) {
...
export async function claimVerifiedDisputeEvidenceScanRetryAlert(
  ...
  const claim = await claimDisputeEvidenceScanRetryAlert(db, verification, source);
