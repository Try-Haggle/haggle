...
export type ShipmentApvInvoiceAnomalyType =
  | "MISSING_FILE"
  | "SIZE_MISMATCH"
  | "HASH_MISMATCH"
  | "ORPHAN_FILE";
  ...
  return createHash("sha256")
    .update(
      [
        ...
        candidate.documentId ?? "orphan",
        candidate.expectedSha256 ?? "none",
        String(candidate.expectedByteSize ?? "none"),
      ].join(":"),
      "utf8",
    )
    .digest("hex");
...
function publicCandidate(value: InternalCandidate): ShipmentApvInvoiceReconciliationCandidate {
  return {
    ...
    documentBound: Boolean(value.documentId),
  };
}
...
function mapRequest(row: Record<string, unknown>): ShipmentApvInvoiceReconciliationRequest {
  return {
    ...
    document_bound: Boolean(row.document_id),
    requester_id: String(row.requester_id),
    reason: String(row.reason),
    status: String(row.status) as ShipmentApvInvoiceReconciliationRequest["status"],
...
async function discoverInternal(db: Executor, storageRoot: string) {
  const countRows =
    (await db.execute(sql`SELECT count(*) AS count FROM shipment_apv_invoice_documents
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (checkedBytes + metadata.size > MAX_HASH_BYTES) {
        scanTruncated = true;
        continue;
      }
  ...
  try {
    ...
    for (const directory of directories) {
      ...
      filesystemEntries += 1;
      if (filesystemEntries > MAX_FILESYSTEM_ENTRIES) {
        scanTruncated = true;
        break;
      }
      if (
        ...
        !UUID_PATTERN.test(directory.name)
      )
        continue;
      ...
      for (const file of files) {
        ...
        if (!file.isFile() || file.isSymbolicLink() || !STORAGE_KEY_PATTERN.test(storageKey))
          continue;
        if (!scanTruncated && !expectedKeys.has(storageKey)) {
          candidates.push(
            candidate({
              anomalyType: "ORPHAN_FILE",
              storageKey,
              documentId: null,
              expectedSha256: null,
...
export async function discoverShipmentApvInvoiceReconciliationCandidates(
  ...
  const result = await discoverInternal(
    db,
    options.storageRoot ?? resolveShipmentInvoiceDocumentRoot(),
  );
  return {
    candidates: result.candidates.map(publicCandidate),
...
export async function requestShipmentApvInvoiceReconciliation(
  ...
  try {
    return await db.transaction(async (tx) => {
      const existingRows =
        (await tx.execute(sql`SELECT * FROM shipment_apv_invoice_reconciliation_requests
      ...
      if (existingRows[0]) {
        ...
        return existing.target_fingerprint === target.candidateId &&
  ...
  try {
    const claim = await db.transaction(async (tx) => {
      ...
      if (now.getTime() >= new Date(request.expires_at).getTime()) {
        const expiredRows =
          (await tx.execute(sql`UPDATE shipment_apv_invoice_reconciliation_requests
          SET status = 'EXPIRED', decision_reason = 'Approval window expired', decided_at = ${now.toISOString()}::timestamptz,
              updated_at = ${now.toISOString()}::timestamptz, version = version + 1
      ...
      if (
        discovery.scanTruncated ||
        !discovery.candidates.some((item) => item.candidateId === request.target_fingerprint)
      ) {
      ...
      await appendEvent(tx, {
        ...
        metadata: { anomaly_type: request.anomaly_type },
        now,
      });
    ...
    if (anomalyType === "MISSING_FILE") {
      ...
      try {
        sourceExists = (await lstat(sourcePath)).isFile();
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
      }
      try {
        quarantineExists = (await lstat(quarantinePath)).isFile();
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
      }
      if (sourceExists && !quarantineExists) await rename(sourcePath, quarantinePath);
      else if (!sourceExists && !quarantineExists)
        throw new Error("APV_INVOICE_RECONCILIATION_SOURCE_MISSING");
      else if (sourceExists && quarantineExists)
        throw new Error("APV_INVOICE_RECONCILIATION_DESTINATION_CONFLICT");
    }
    return await db.transaction(async (tx) => {
      const rows = (await tx.execute(sql`SELECT * FROM shipment_apv_invoice_reconciliation_requests
        WHERE id = ${input.requestId}::uuid FOR UPDATE`)) as unknown as InternalRequestRow[];
      ...
      if (
        ...
        return { outcome: "apply_state_lost" } as const;
      if (current.document_id) {
        ...
        if (documentRows.length !== 1)
          throw new Error("APV_INVOICE_RECONCILIATION_DOCUMENT_STATE_CHANGED");
      }
      ...
      await appendEvent(tx, {
        ...
        metadata: {
          anomaly_type: approved.anomaly_type,
          quarantined: anomalyType !== "MISSING_FILE",
        },
        now,
      });
      await tx.execute(sql`INSERT INTO admin_action_log
...
export async function getShipmentApvInvoiceReconciliationTimeline(db: Database, requestId: string) {
  ...
  const requestRows =
    (await db.execute(sql`SELECT * FROM shipment_apv_invoice_reconciliation_requests
...
export async function deleteShipmentApvInvoiceReconciliationFixtureRows(
  db: Database,
  requestIds: string[],
) {
