...
function decodeFailureCursor(value: string): FailureCursor {
  try {
    ...
    throw new Error("INVALID_SIMILARITY_REVIEW_AUDIT_ARCHIVE_FAILURE_CURSOR");
  }
}
...
export async function listDisputeSimilarityReviewAuditArchiveFailures(
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 100)
    throw new Error("INVALID_SIMILARITY_REVIEW_AUDIT_ARCHIVE_FAILURE_LIMIT");
...
export async function enqueueDisputeSimilarityReviewAuditArchive(
  ...
  if (event.integrity !== "valid" || !event.eventHash)
    throw new Error("SIMILARITY_REVIEW_AUDIT_INTEGRITY_INVALID");
  const now = input.now ?? new Date();
  ...
  const payloadSha256 = createHash("sha256")
    .update(canonicalDisputeAuditJson(payload))
    .digest("hex");
...
export async function enqueuePendingDisputeSimilarityReviewAudits(
  db: Database,
  ...
  for (const row of rows) {
    const result = await enqueueDisputeSimilarityReviewAuditArchive(db, {
      eventId: row.id,
      now: input.now,
    });
...
export async function dispatchDisputeSimilarityReviewAuditArchives(
  ...
  for (const archive of claimed) {
    const result = await deliverDisputeSimilarityReviewAuditArchive(archive, config, {
