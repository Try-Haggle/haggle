...
const BODY_KEYS = new Set(["type", "delivery_id", "state", "created_at", "severity", "reasons", "totals", "sources"]);
const TOTAL_KEYS = new Set(["processing", "completed", "failed", "staleProcessing", "retryReady"]);
const SOURCE_KEYS = new Set(["source", "processing", "failed", "stale_processing", "retry_ready", "max_attempt_count", "oldest_unfinished_age_seconds"]);
...
export function resolveWebhookClaimAlertReceiverSecretsFromEnv() {
  ...
  if (!current && previous.length) throw new Error("webhook claim alert current secret is required when previous secrets are configured");
  const secrets = current ? [current, ...previous] : [];
  if (secrets.some((item) => item.length < MIN_SECRET_LENGTH || item.length > MAX_SECRET_LENGTH)) {
    throw new Error(`webhook claim alert receiver secrets must be ${MIN_SECRET_LENGTH}..${MAX_SECRET_LENGTH} characters`);
  }
  if (new Set(secrets).size !== secrets.length) throw new Error("webhook claim alert receiver secrets must be unique");
  if (secrets.length > WEBHOOK_CLAIM_ALERT_MAX_RECEIVER_SECRETS) {
    throw new Error(`webhook claim alert receiver accepts at most ${WEBHOOK_CLAIM_ALERT_MAX_RECEIVER_SECRETS} secrets`);
  }
  return secrets;
}
...
export function getWebhookClaimAlertReceiverPolicyStatus() {
  try {
    ...
    return { configured: secrets.length > 0, configurationState: secrets.length ? "valid" as const : "not_configured" as const,
      acceptedSecretCount: secrets.length, maxAcceptedSecretCount: WEBHOOK_CLAIM_ALERT_MAX_RECEIVER_SECRETS,
      timestampToleranceSeconds: 300 };
  } catch {
    return { configured: false, configurationState: "invalid" as const, acceptedSecretCount: 0,
      maxAcceptedSecretCount: WEBHOOK_CLAIM_ALERT_MAX_RECEIVER_SECRETS, timestampToleranceSeconds: 300 };
  }
}

export async function getWebhookClaimAlertReceiverHealth(db: Database, source = WEBHOOK_CLAIM_ALERT_RECEIVER_SOURCE) {
  ...
  return { status: staleProcessing > 0 ? "critical" as const : failed > 0 ? "warning" as const : "healthy" as const,
    ...
    retryReady: Number(row.retry_ready ?? 0), lastCompletedAt: row.last_completed_at ? new Date(row.last_completed_at).toISOString() : null,
...
function validAggregate(body: Record<string, unknown>) {
  ...
  if (!totals || typeof totals !== "object" || Array.isArray(totals)
    ...
    || [...TOTAL_KEYS].some((key) => !nonnegativeInteger((totals as Record<string, unknown>)[key]))) return false;
  const total = totals as Record<string, number>;
  if (total.staleProcessing > total.processing || total.retryReady > total.failed
    || !Array.isArray(sources) || sources.length > 100) return false;
  ...
  for (const rawSource of sources) {
    ...
    if (Object.keys(source).some((key) => !SOURCE_KEYS.has(key)) || typeof source.source !== "string"
      || !SOURCE_RE.test(source.source) || names.has(source.source)) return false;
    names.add(source.source);
    for (const key of ["processing", "failed", "stale_processing", "retry_ready", "max_attempt_count"]) {
      if (!nonnegativeInteger(source[key])) return false;
    }
    if (source.oldest_unfinished_age_seconds !== null && !nonnegativeInteger(source.oldest_unfinished_age_seconds)) return false;
    if ((source.stale_processing as number) > (source.processing as number)
      || (source.retry_ready as number) > (source.failed as number)
      || ((source.processing as number) + (source.failed as number) === 0) !== (source.oldest_unfinished_age_seconds === null)) return false;
    sums.processing += source.processing as number; sums.failed += source.failed as number;
    sums.staleProcessing += source.stale_processing as number; sums.retryReady += source.retry_ready as number;
  }
  return sums.processing === total.processing && sums.failed === total.failed
    && sums.staleProcessing === total.staleProcessing && sums.retryReady === total.retryReady;
}

export function verifyWebhookClaimHealthAlert(input: { rawBody: Buffer | string; timestamp?: string | string[];
  ...
  if (Object.keys(body).some((key) => !BODY_KEYS.has(key)) || body.type !== "webhook_claim.health"
    || body.delivery_id !== deliveryId || body.created_at !== timestamp || !validAggregate(body)
    || !Array.isArray(body.reasons) || body.reasons.some((reason) => typeof reason !== "string")) {
  ...
  const validRecovery = state === "recovered" && severity === "recovery" && reasons.length === 1
    && reasons[0] === "webhook_claim_recovered" && totals.staleProcessing === 0 && totals.failed === 0 && totals.retryReady === 0;
  const validFiring = state === "firing" && (severity === "warning" || severity === "critical")
    ...
    && reasons.every((reason) => reason === "stale_processing" ? totals.staleProcessing > 0
      : reason === "failed" ? totals.failed > 0 : totals.retryReady > 0)
  ...
  if (!secrets.length || secrets.length > WEBHOOK_CLAIM_ALERT_MAX_RECEIVER_SECRETS
...
export async function claimVerifiedWebhookClaimHealthAlert(db: Database, verification: Extract<Verification, { ok: true }>,
  source = WEBHOOK_CLAIM_ALERT_RECEIVER_SOURCE): Promise<{ outcome: "accepted"; claim: WebhookEventClaim }
