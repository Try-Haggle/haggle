...
export type DisputeEvidenceProvenanceArchiveHealth = Awaited<
  ReturnType<typeof getDisputeEvidenceProvenanceArchiveHealth>
>;
...
export const DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_SOURCE =
  "haggle-dispute-evidence-provenance-archive-alert";
...
export function getDisputeEvidenceProvenanceArchiveAlertPolicyStatus() {
  ...
  if (configurationState === "valid" && url) {
    try {
      assertDisputeModuleOutboundUrl(url, {
        label: "dispute evidence provenance archive alert",
        allowInsecureHttp,
        allowPrivateNetwork,
      });
    } catch {
  ...
  return {
    ...
    cooldownMinutes: boundedInteger(
      process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_COOLDOWN_MINUTES,
      15,
      1,
      1440,
    ),
    coverageGapThreshold: boundedInteger(
      process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_COVERAGE_GAP_THRESHOLD,
      1,
      1,
      100_000,
    ),
    staleThreshold: boundedInteger(
      process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_STALE_THRESHOLD,
      1,
      1,
      100_000,
    ),
    retryReadyThreshold: boundedInteger(
      process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_RETRY_READY_THRESHOLD,
      5,
      1,
      100_000,
    ),
    deadLetterThreshold: boundedInteger(
      process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_DEAD_LETTER_THRESHOLD,
      1,
      1,
      100_000,
    ),
...
export function resolveDisputeEvidenceProvenanceArchiveAlertConfigFromEnv(): DisputeEvidenceProvenanceArchiveAlertConfig | null {
  ...
  const secret = process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_SECRET ?? "";
  if (secret.length < 16)
    throw new Error("evidence provenance archive alert secret must be at least 16 characters");
  const policy = getDisputeEvidenceProvenanceArchiveAlertPolicyStatus();
  const config = {
    url,
    secret,
    timeoutMs: boundedInteger(
      process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_TIMEOUT_MS,
      5000,
      250,
      30_000,
    ),
    cooldownMinutes: policy.cooldownMinutes,
    coverageGapThreshold: policy.coverageGapThreshold,
...
export function evaluateDisputeEvidenceProvenanceArchiveAlert(
  ...
  policy: Pick<
    DisputeEvidenceProvenanceArchiveAlertConfig,
    "coverageGapThreshold" | "staleThreshold" | "retryReadyThreshold" | "deadLetterThreshold"
  >,
): DisputeEvidenceProvenanceArchiveAlertAssessment {
  const reasons = [
    ...(health.coverageGap >= policy.coverageGapThreshold
    ...
    ...(health.staleProcessing >= policy.staleThreshold
      ? ["evidence_provenance_archive_stale_processing"]
      : []),
    ...(health.retryReady >= policy.retryReadyThreshold
      ? ["evidence_provenance_archive_retry_ready_backlog"]
      : []),
  ];
  const critical =
    reasons.includes("evidence_provenance_archive_coverage_gap") ||
    reasons.includes("evidence_provenance_archive_dead_letter");
  return {
...
export async function findLatestDeliveredDisputeEvidenceProvenanceArchiveIncident(
  db: Database,
  source = DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_SOURCE,
) {
...
export async function sendDisputeEvidenceProvenanceArchiveAlert(
  ...
  options: {
    config: DisputeEvidenceProvenanceArchiveAlertConfig;
    deliveryId: string;
  ...
  if (!/^(?:health|recovery)_[0-9a-f]{64}$/.test(options.deliveryId))
    throw new Error("invalid evidence provenance archive alert delivery id");
