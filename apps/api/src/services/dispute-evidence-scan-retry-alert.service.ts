...
import type { getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth } from "../jobs/dispute-evidence-scan-retry-alert-snapshot-retention.js";
import type { getDisputeEvidenceScanRetryHealth } from "./dispute-evidence-scan-retry.service.js";
import type { getDisputeEvidenceScanRetryAlertSnapshotRetentionHealth } from "./dispute-evidence-scan-retry-alert-snapshot-retention.service.js";
import type { getDisputeEvidenceScannerCircuitHealth } from "./dispute-evidence-scanner-circuit.service.js";
...
export type DisputeEvidenceScanRetryHealth = Awaited<
  ReturnType<typeof getDisputeEvidenceScanRetryHealth>
>;
export type DisputeEvidenceScannerCircuitHealth = Awaited<
  ReturnType<typeof getDisputeEvidenceScannerCircuitHealth>
>;
...
export interface DisputeEvidenceScanRetryAlertSnapshot {
  schema_version: "dispute-evidence-scan-retry-alert-v2" | "dispute-evidence-scan-retry-alert-v3";
  ...
  health: {
    ...
    retention?: {
      ...
      job: {
        ...
        last_run_status: "NEVER" | "RUNNING" | "STALE_RUNNING" | "SUCCEEDED" | "FAILED";
        overdue: boolean;
        lease_stale: boolean;
...
export const DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_SOURCE = "haggle-dispute-evidence-scan-retry-alert";
...
export function getDisputeEvidenceScanRetryAlertPolicyStatus() {
  ...
  let configurationState: "not_configured" | "partial" | "invalid" | "valid" =
    !url && !secret
      ? "not_configured"
      : !url || secret.length < 16 || secret.length > 128
        ? "partial"
        : "valid";
...
export function resolveDisputeEvidenceScanRetryAlertConfigFromEnv(): DisputeEvidenceScanRetryAlertConfig | null {
  const url = process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_URL?.trim();
  if (!url) return null;
  const secret = process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_SECRET ?? "";
  if (secret.length < 16 || secret.length > 128) {
    throw new Error("dispute evidence scan retry alert secret must be 16..128 characters");
  }
  const allowInsecureHttp =
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_ALLOW_INSECURE_HTTP === "true";
  const allowPrivateNetwork =
...
export function evaluateDisputeEvidenceScanRetryAlert(
  ...
  policy: Pick<
    DisputeEvidenceScanRetryAlertConfig,
    | "retryReadyThreshold"
    | "staleThreshold"
    | "exhaustedThreshold"
    | "expiredThreshold"
    | "retentionBlockedThreshold"
  >,
  ...
  const retentionActive = retentionJob.policy.jobEnabled && retentionJob.policy.cronEnabled;
  const reasons = [
    ...(circuit.state === "OPEN" ? ["scanner_circuit_open"] : []),
    ...(circuit.state === "HALF_OPEN" ? ["scanner_circuit_half_open"] : []),
    ...
    ...(retention.blockedExpired >= policy.retentionBlockedThreshold
      ...
      : []),
    ...(health.totals.exhausted >= policy.exhaustedThreshold ? ["scan_retry_exhausted"] : []),
    ...
    ...(health.totals.staleProcessing >= policy.staleThreshold
      ...
      : []),
    ...(health.totals.retryReady >= policy.retryReadyThreshold ? ["scan_retry_ready_backlog"] : []),
    ...(retentionActive && retentionJob.lastRunStatus === "FAILED"
      ? ["alert_snapshot_retention_job_failed"]
      : []),
    ...(retentionActive && retentionJob.overdue ? ["alert_snapshot_retention_job_overdue"] : []),
  ];
  const critical =
    reasons.includes("scanner_circuit_open") ||
...
export async function getDisputeEvidenceScanRetryAlertDeliveryState(
  ...
  const incident = rows[0]?.lastIncidentAt ? new Date(rows[0].lastIncidentAt) : null;
  const recovery = rows[0]?.lastRecoveryAt ? new Date(rows[0].lastRecoveryAt) : null;
  return {
...
export async function getDisputeEvidenceScanRetryAlertSenderHealth(
  ...
  return {
    status:
      staleProcessing > 0 || bindingViolations > 0 || missingRetrySnapshots > 0
        ? ("critical" as const)
    ...
    oldestUnfinishedAgeSeconds:
      row.oldestUnfinishedAgeSeconds === null || row.oldestUnfinishedAgeSeconds === undefined
        ? null
...
function publicHealth(
  ...
  if (retention && retentionJob) {
    result.retention = {
      ...
      oldest_blocked_expired_age_seconds: retention.oldestBlockedExpiredAgeSeconds,
      job: {
        active: retentionJob.policy.jobEnabled && retentionJob.policy.cronEnabled,
...
function exactKeys(value: Record<string, unknown>, keys: string[]): boolean {
  ...
  return (
    actual.length === keys.length && [...keys].sort().every((key, index) => key === actual[index])
  );
}

function snapshotPayloadSha256(snapshot: DisputeEvidenceScanRetryAlertSnapshot): string {
...
export function createDisputeEvidenceScanRetryAlertSnapshot(
  ...
  if (
    !/^(?:health|recovery)_[0-9a-f]{64}$/.test(deliveryId) ||
    !assessment.severity ||
    !assessment.wouldAlert
  ) {
...
function normalizeDisputeEvidenceScanRetryAlertSnapshot(
  ...
  const numberKeys = (record: Record<string, unknown> | undefined, keys: string[]) =>
    Boolean(
      record &&
  ...
  const thresholdKeys = ["retry_ready", "stale_processing", "exhausted", "expired_quarantined"];
  const isV3 = input.schema_version === "dispute-evidence-scan-retry-alert-v3";
  const expectedThresholdKeys = isV3
    ? [...thresholdKeys, "retention_blocked_expired"]
    : thresholdKeys;
  ...
  const retentionJobKeys = [
    "active",
    "status",
    "last_run_status",
    "overdue",
    "lease_stale",
    "last_deleted_snapshots",
    "interval_seconds",
  ...
  if (
    ...
    !["dispute-evidence-scan-retry-alert-v2", "dispute-evidence-scan-retry-alert-v3"].includes(
    ...
    !thresholds ||
    !exactKeys(thresholds, expectedThresholdKeys) ||
    !numberKeys(thresholds, expectedThresholdKeys) ||
    !health ||
    !exactKeys(health, expectedHealthKeys) ||
    !totals ||
    !exactKeys(totals, totalKeys) ||
    ...
    (isV3 &&
      (!retention ||
        ...
        !retentionJob ||
        !exactKeys(retentionJob, retentionJobKeys) ||
        typeof retentionJob.active !== "boolean" ||
        !["inactive", "healthy", "attention", "critical"].includes(String(retentionJob.status)) ||
        !["NEVER", "RUNNING", "STALE_RUNNING", "SUCCEEDED", "FAILED"].includes(
          String(retentionJob.last_run_status),
        ) ||
        typeof retentionJob.overdue !== "boolean" ||
        typeof retentionJob.lease_stale !== "boolean" ||
        !numberKeys(retentionJob, [
          "last_deleted_snapshots",
          "interval_seconds",
          "max_start_delay_seconds",
        ])))
  ...
  const normalized: DisputeEvidenceScanRetryAlertSnapshot = {
    schema_version: input.schema_version as DisputeEvidenceScanRetryAlertSnapshot["schema_version"],
    type: "dispute_evidence_scan_retry.health",
    ...
    health: {
      totals: Object.fromEntries(totalKeys.map((key) => [key, Number(totals[key])])),
      oldest_unresolved_age_seconds: health.oldest_unresolved_age_seconds as number | null,
      circuit: {
  ...
  if (isV3 && retention && retentionJob) {
    normalized.thresholds.retention_blocked_expired = Number(thresholds.retention_blocked_expired);
    normalized.health.retention = {
      ...
      oldest_blocked_expired_age_seconds: retention.oldest_blocked_expired_age_seconds as
        | number
        | null,
      job: {
        active: retentionJob.active as boolean,
        status: retentionJob.status as NonNullable<
          DisputeEvidenceScanRetryAlertSnapshot["health"]["retention"]
        >["job"]["status"],
        last_run_status: retentionJob.last_run_status as NonNullable<
          DisputeEvidenceScanRetryAlertSnapshot["health"]["retention"]
        >["job"]["last_run_status"],
        ...
        max_start_delay_seconds: Number(retentionJob.max_start_delay_seconds),
      },
    };
  }
  ...
  if (
    !Object.values(normalized.thresholds).every((value) => positiveInteger(value, 100_000)) ||
    ...
    (normalized.health.oldest_unresolved_age_seconds !== null &&
      !nonnegativeInteger(normalized.health.oldest_unresolved_age_seconds, 31_536_000)) ||
    !nonnegativeInteger(normalized.health.circuit.consecutive_failures, 1_000) ||
    ...
    normalized.health.circuit.active_permits > normalized.health.circuit.max_concurrent ||
    ...
    (isV3 &&
      (!normalized.health.retention ||
        !nonnegativeInteger(normalized.health.retention.eligible_expired, 1_000_000_000) ||
        !nonnegativeInteger(normalized.health.retention.blocked_expired, 1_000_000_000) ||
        (normalized.health.retention.oldest_blocked_expired_age_seconds !== null &&
          !nonnegativeInteger(
        ...
        normalized.health.retention.job.status !==
          (!normalized.health.retention.job.active
            ? "inactive"
            : normalized.health.retention.job.lease_stale
              ? "critical"
  ...
  const expectedReasons = [
    ...(normalized.health.circuit.state === "OPEN" ? ["scanner_circuit_open"] : []),
    ...(normalized.health.circuit.state === "HALF_OPEN" ? ["scanner_circuit_half_open"] : []),
    ...(isV3 &&
    normalized.health.retention?.job.active &&
    normalized.health.retention.job.last_run_status === "STALE_RUNNING"
      ? ["alert_snapshot_retention_job_stale"]
    ...
    normalized.health.retention.blocked_expired >= normalized.thresholds.retention_blocked_expired
      ? ["alert_snapshot_retention_blocked"]
      : []),
    ...(normalized.health.totals.exhausted >= normalized.thresholds.exhausted
      ? ["scan_retry_exhausted"]
      : []),
    ...(normalized.health.totals.expired_quarantined >= normalized.thresholds.expired_quarantined
      ? ["scan_retry_expired_quarantine"]
      : []),
    ...(normalized.health.totals.stale_processing >= normalized.thresholds.stale_processing
      ? ["scan_retry_stale_processing"]
      : []),
    ...(normalized.health.totals.retry_ready >= normalized.thresholds.retry_ready
    ...
    normalized.health.retention.job.last_run_status === "FAILED"
      ? ["alert_snapshot_retention_job_failed"]
      : []),
    ...(isV3 && normalized.health.retention?.job.active && normalized.health.retention.job.overdue
      ? ["alert_snapshot_retention_job_overdue"]
      : []),
  ];
  const critical =
    expectedReasons.includes("scanner_circuit_open") ||
...
export async function persistDisputeEvidenceScanRetryAlertSnapshot(
  ...
  const normalizedInput = normalizeDisputeEvidenceScanRetryAlertSnapshot(snapshot);
  ...
  const normalized = normalizeDisputeEvidenceScanRetryAlertSnapshot(existing.payload);
  if (
...
export async function sendDisputeEvidenceScanRetryAlert(
  ...
  const snapshot =
    ...
    createDisputeEvidenceScanRetryAlertSnapshot(
      ...
      options.deliveryId,
      options.retention!,
      options.retentionJob!,
    );
  const verifiedSnapshot = normalizeDisputeEvidenceScanRetryAlertSnapshot(snapshot);
