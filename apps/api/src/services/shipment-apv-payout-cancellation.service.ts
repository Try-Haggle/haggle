...
export function canonicalShipmentApvCancellationAuditJson(value: unknown): string {
  ...
  if (Array.isArray(value)) return `[${value.map(canonicalShipmentApvCancellationAuditJson).join(",")}]`;
  const record = value as Record<string, unknown>;
  return `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${canonicalShipmentApvCancellationAuditJson(record[key])}`).join(",")}}`;
}

function canonicalLifecycleEvent(event: ShipmentApvPayoutCancellationEventRecord, previousEventHash: string | null) {
  return {
    id: event.id,
...
export function buildShipmentApvCancellationEventHash(
  ...
  return createHash("sha256")
    .update(canonicalShipmentApvCancellationAuditJson(canonicalLifecycleEvent(event, previousEventHash)))
    .digest("hex");
}

function legacyShipmentApvCancellationEventAnchor(event: ShipmentApvPayoutCancellationEventRecord) {
  return createHash("sha256")
    .update(`legacy:${canonicalShipmentApvCancellationAuditJson(canonicalLifecycleEvent(event, null))}`)
    .digest("hex");
}

export function verifyShipmentApvCancellationEventChain(events: ShipmentApvPayoutCancellationEventRecord[]) {
  let valid = true;
  ...
  for (const event of events) {
    ...
    if (!event.event_hash) {
      ...
      if (event.event_hash !== buildShipmentApvCancellationEventHash(event, event.previous_event_hash)) valid = false;
...
function decodePendingCancellationCursor(value: string): PendingCancellationCursor {
  try {
    const parsed = JSON.parse(Buffer.from(value, "base64url").toString("utf8")) as Partial<PendingCancellationCursor>;
...
async function expirePendingRequests(tx: Pick<Database, "execute">, nowIso: string, payoutOffsetId?: string) {
  ...
  for (const expired of expiredRows) {
    await appendLifecycleEvent(tx, {
      ...
      metadata: { expired_at: new Date(String(expired.expires_at)).toISOString(), reason: "Approval window expired" },
...
export async function requestShipmentApvPayoutCancellation(
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      if (!Number.isFinite(expiry.getTime()) || now.getTime() <= expiry.getTime()) return { outcome: "not_expired" } as const;
...
export async function decideShipmentApvPayoutCancellation(
  ...
  try {
    ...
    if (existing.payout_offset_id !== input.payoutOffsetId
      || existing.settlement_release_id !== input.settlementReleaseId) {
    ...
    if (existing.requester_id === input.approverId) return { outcome: "self_approval_forbidden" } as const;
    if (existing.version !== input.expectedVersion) return { outcome: "version_conflict" } as const;
    ...
    await appendLifecycleEvent(tx, {
      ...
      metadata: { decision_request_id: input.decisionRequestId, onchain_state: input.onchainState },
      createdAt: now,
    });
    return { outcome: input.decision === "APPROVE" ? "approved" : "rejected", request: mapRequest(updatedRows[0]) } as const;
    });
  } catch (error) {
...
export async function listPendingShipmentApvPayoutCancellations(
  ...
  return db.transaction(async (tx) => {
    ...
    const items = pageRows.map((row) => ({
      ...mapRequest(row),
      order_id: String(row.order_id), seller_id: String(row.seller_id), currency: String(row.currency),
      applied_offset_minor: Number(row.applied_offset_minor),
      signature_deadline: row.signature_deadline ? new Date(String(row.signature_deadline)).toISOString() : null,
    }));
