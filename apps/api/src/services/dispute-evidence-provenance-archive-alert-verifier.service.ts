...
export const DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_RECEIVER_SOURCE = "haggle-dispute-evidence-provenance-archive-alert-receiver";

export function resolveDisputeEvidenceProvenanceArchiveAlertReceiverSecretsFromEnv() {
  const candidates = [process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_SECRET,
    ...(process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_PREVIOUS_SECRETS?.split(",") ?? [])];
  return [...new Set(candidates.map((item) => item?.trim()).filter((item): item is string => Boolean(item && item.length >= 16)))];
}
export function getDisputeEvidenceProvenanceArchiveAlertReceiverPolicyStatus() {
...
export async function getDisputeEvidenceProvenanceArchiveAlertReceiverHealth(db: Database) {
  const rows = await db.execute(sql`
    ...
    FROM webhook_idempotency WHERE source = ${DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_RECEIVER_SOURCE}
...
export type DisputeEvidenceProvenanceArchiveAlertVerification =
...
export function verifyDisputeEvidenceProvenanceArchiveAlert(input: {
  rawBody: Buffer | string; timestamp?: string | string[]; signature?: string | string[];
  deliveryId?: string | string[]; secret: string | string[]; nowMs?: number; toleranceMs?: number;
}): DisputeEvidenceProvenanceArchiveAlertVerification {
  ...
  const timestampMs = Date.parse(timestamp); if (!Number.isFinite(timestampMs)) return { ok: false, error: "INVALID_ALERT_TIMESTAMP" };
  ...
  let body: Record<string, unknown>; try { body = JSON.parse(raw) as Record<string, unknown>; } catch { return { ok: false, error: "INVALID_ALERT_BODY" }; }
  const state = body.state; const severity = body.severity;
  if (body.type !== "dispute_evidence_provenance_archive.health" || body.created_at !== timestamp
    || (state !== "firing" && state !== "recovered") || (severity !== "critical" && severity !== "warning" && severity !== "recovery")
    || (state === "recovered" && severity !== "recovery") || (state === "firing" && severity === "recovery")) {
...
export async function claimVerifiedDisputeEvidenceProvenanceArchiveAlert(
  db: Database, verification: Extract<DisputeEvidenceProvenanceArchiveAlertVerification, { ok: true }>,
  source = DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_RECEIVER_SOURCE,
