...
export const DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_RECEIVER_SOURCE =
  "haggle-dispute-evidence-provenance-archive-alert-receiver";

export function resolveDisputeEvidenceProvenanceArchiveAlertReceiverSecretsFromEnv() {
  const candidates = [
    process.env.DISPUTE_EVIDENCE_PROVENANCE_ARCHIVE_ALERT_SECRET,
...
export function verifyDisputeEvidenceProvenanceArchiveAlert(input: {
  ...
  if (
    body.type !== "dispute_evidence_provenance_archive.health" ||
...
export async function claimVerifiedDisputeEvidenceProvenanceArchiveAlert(
  db: Database,
  verification: Extract<DisputeEvidenceProvenanceArchiveAlertVerification, { ok: true }>,
