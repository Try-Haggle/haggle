...
export async function runDisputeEvidenceScannerCircuitFixture(db: Database) {
  ...
  try {
    const initial = await Promise.all(Array.from({ length: 20 }, () =>
      acquireDisputeEvidenceScannerPermit(db, {
        now: startedAt, circuitKey, config: FIXTURE_CONFIG,
      })));
    const initialPermits = initial.filter((permit) => permit.acquired);
    const capacityBlocked = initial.filter((permit) =>
      !permit.acquired && permit.reason === "CAPACITY_BUSY");
    await Promise.all(initialPermits.map((permit) =>
      permit.acquired
        ? finalizeDisputeEvidenceScannerPermit(db, permit, {
          scannerOperational: false, now: startedAt, config: FIXTURE_CONFIG,
        })
    ...
    const whileOpen = await Promise.all(Array.from({ length: 20 }, () =>
      acquireDisputeEvidenceScannerPermit(db, {
        now: new Date(startedAt.getTime() + 1_000),
        circuitKey, config: FIXTURE_CONFIG,
      })));
    const probeAt = new Date(startedAt.getTime() + 61_000);
    const halfOpen = await Promise.all(Array.from({ length: 20 }, () =>
      acquireDisputeEvidenceScannerPermit(db, {
        now: probeAt, circuitKey, config: FIXTURE_CONFIG,
      })));
    const probes = halfOpen.filter((permit) =>
      permit.acquired && permit.kind === "PROBE");
    const probe = probes[0];
    if (!probe?.acquired) {
      throw new Error("SCANNER_CIRCUIT_FIXTURE_PROBE_MISSING");
    }
    const probeFinalized = await finalizeDisputeEvidenceScannerPermit(
      db, probe,
      { scannerOperational: true, now: probeAt, config: FIXTURE_CONFIG },
    );
    ...
    const checks = {
      bulkheadExactlyFour: initialPermits.length === 4
        && capacityBlocked.length === 16,
      failuresOpenedCircuit: openHealth.state === "OPEN"
        && openHealth.consecutiveFailures === 4,
      openCircuitBlockedAll: whileOpen.every((permit) =>
        !permit.acquired && permit.reason === "CIRCUIT_OPEN"),
      halfOpenSingleProbe: probes.length === 1
        && halfOpen.filter((permit) => !permit.acquired).length === 19,
      successfulProbeClosedCircuit: probeFinalized
        && closedHealth.state === "CLOSED"
        && closedHealth.consecutiveFailures === 0,
      infectedResponseCountsAsOperational: infectedFinalized,
      databaseRejectedPermitMutation: immutablePermitGuard,
      identifiersExcluded: openHealth.containsPermitTokens === false
        && openHealth.containsCircuitKey === false,
    };
