...
export interface ShipmentApvRevisionRecord {
  ...
  delta_minor: number;
  status: "APPLIED" | "REVIEW_REQUIRED" | "CREDIT_RECORDED" | "PENDING_REVIEW" | "WAIVED_TO_PLATFORM" | "CREDIT_APPLIED" | "ACKNOWLEDGED";
...
export type ShipmentApvRevisionResult =
  ...
  | { outcome: "not_found" | "identity_conflict" | "base_revision_missing" | "amount_conflict" | "payout_reserved" };

export function isValidShipmentApvRevisionAmount(input: Pick<ShipmentApvInput,
  "originalRateMinor" | "adjustedRateMinor" | "adjustmentMinor">): boolean {
  return Number.isSafeInteger(input.originalRateMinor)
...
function mapRevision(row: Record<string, unknown>): ShipmentApvRevisionRecord {
  return {
    ...
    buyer_effect_minor: 0,
    decision: typeof row.decision === "string" ? row.decision as ShipmentApvRevisionRecord["decision"] : undefined,
    ...
    provider_document_id: typeof row.provider_document_id === "string" ? row.provider_document_id : undefined,
    surcharge_category: typeof row.surcharge_category === "string" ? row.surcharge_category : undefined,
    surcharge_type: typeof row.surcharge_type === "string" ? row.surcharge_type : undefined,
    evidence_amount_minor: row.evidence_amount_minor === null || row.evidence_amount_minor === undefined
      ? undefined : numeric(row.evidence_amount_minor),
    evidence_currency: typeof row.evidence_currency === "string" ? row.evidence_currency : undefined,
...
export async function recordShipmentApvInvoiceRevision(
  ...
  return db.transaction(async (tx) => {
    ...
    if (adjustment.shipment_id !== input.shipmentId
    ...
    const duplicateRows = await tx.execute(sql`
      SELECT * FROM shipment_apv_adjustment_revisions
       ...
       LIMIT 1
    `) as unknown as Array<Record<string, unknown>>;
    if (duplicateRows[0]) return { outcome: "duplicate", revision: mapRevision(duplicateRows[0]) } as const;
