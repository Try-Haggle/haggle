...
export const DISPUTE_AI_ARCHIVE_ALERT_RECEIVER_SOURCE = "haggle-dispute-ai-audit-archive-alert-receiver";

export function resolveDisputeAiArchiveAlertReceiverSecretsFromEnv() {
  const candidates = [process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_SECRET,
    ...(process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_PREVIOUS_SECRETS?.split(",") ?? [])];
  return [...new Set(candidates.map((item) => item?.trim()).filter((item): item is string => Boolean(item && item.length >= 16)))];
}

export function getDisputeAiArchiveAlertReceiverPolicyStatus() {
...
export async function getDisputeAiArchiveAlertReceiverHealth(db: Database) {
  const rows = await db.execute(sql`
    ...
    FROM webhook_idempotency WHERE source = ${DISPUTE_AI_ARCHIVE_ALERT_RECEIVER_SOURCE}
  `) as unknown as Array<Record<string, string | number | Date | null>>;
  const row = rows[0] ?? {};
  const failed = Number(row.failed ?? 0); const staleProcessing = Number(row.stale_processing ?? 0);
...
export type DisputeAiArchiveAlertVerification =
...
export function verifyDisputeAiAuditArchiveAlert(input: {
...
}): DisputeAiArchiveAlertVerification {
  ...
  if (Math.abs((input.nowMs ?? Date.now()) - timestampMs) > (input.toleranceMs ?? 5 * 60_000)) return { ok: false, error: "ALERT_TIMESTAMP_OUT_OF_RANGE" };
  const raw = Buffer.isBuffer(input.rawBody) ? input.rawBody.toString("utf8") : input.rawBody;
  let body: Record<string, unknown>;
  try { body = JSON.parse(raw) as Record<string, unknown>; } catch { return { ok: false, error: "INVALID_ALERT_BODY" }; }
  const state = body.state; const severity = body.severity;
  if (body.type !== "dispute_ai_audit_archive.health" || body.created_at !== timestamp
...
export async function claimVerifiedDisputeAiArchiveAlert(
  db: Database, verification: Extract<DisputeAiArchiveAlertVerification, { ok: true }>,
  source = DISPUTE_AI_ARCHIVE_ALERT_RECEIVER_SOURCE,
