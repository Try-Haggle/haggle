...
export const DISPUTE_AI_ARCHIVE_ALERT_RECEIVER_SOURCE =
  "haggle-dispute-ai-audit-archive-alert-receiver";

export function resolveDisputeAiArchiveAlertReceiverSecretsFromEnv() {
  const candidates = [
    process.env.DISPUTE_AI_AUDIT_ARCHIVE_ALERT_SECRET,
...
export function verifyDisputeAiAuditArchiveAlert(input: {
  ...
  if (
    body.type !== "dispute_ai_audit_archive.health" ||
...
export async function claimVerifiedDisputeAiArchiveAlert(
  db: Database,
  verification: Extract<DisputeAiArchiveAlertVerification, { ok: true }>,
