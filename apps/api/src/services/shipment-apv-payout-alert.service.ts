import type { ShipmentApvRemediationCursorRetentionJobHealth } from "../jobs/shipment-apv-remediation-cursor-retention.js";
...
export function getShipmentApvPayoutAlertPolicyStatus() {
  return {
    configured: Boolean(
      process.env.SHIPMENT_APV_PAYOUT_ALERT_URL &&
    ...
    cooldownMinutes: boundedInteger(
      process.env.SHIPMENT_APV_PAYOUT_ALERT_COOLDOWN_MINUTES,
      15,
      1,
      1440,
    ),
    expiredThreshold: boundedInteger(
      process.env.SHIPMENT_APV_PAYOUT_ALERT_EXPIRED_THRESHOLD,
      1,
      1,
      100_000,
    ),
    approvalPendingThreshold: boundedInteger(
      process.env.SHIPMENT_APV_PAYOUT_ALERT_APPROVAL_PENDING_THRESHOLD,
      1,
      1,
      100_000,
    ),
    approvalMaxAgeMinutes: boundedInteger(
      process.env.SHIPMENT_APV_PAYOUT_ALERT_APPROVAL_MAX_AGE_MINUTES,
      15,
      1,
      29,
    ),
...
export function resolveShipmentApvPayoutAlertConfigFromEnv(): ShipmentApvPayoutAlertConfig | null {
  ...
  if (secret.length < 16)
    throw new Error("shipment APV payout alert secret must be at least 16 characters");
...
export function evaluateShipmentApvPayoutAlert(
  ...
  const approvalWaitingTooLong = Boolean(
    approvalHealth &&
  ...
  const cursorRetentionAssessment = evaluateShipmentApvCursorRetentionAlert(
    cursorRetentionJob,
    now,
  );
  const reasons = [
    ...
    ...(restorationStagingHealth?.pendingDisposition
      ? ["invoice_restoration_staging_pending"]
      : []),
    ...
    ...(restorationStagingHealth?.hashMismatches
      ? ["invoice_restoration_staging_hash_mismatch"]
      : []),
    ...(restorationStagingHealth?.invalidEntries
      ? ["invoice_restoration_staging_invalid_entry"]
      : []),
    ...(restorationStagingHealth?.scanTruncated
      ? ["invoice_restoration_staging_scan_truncated"]
      : []),
    ...(restorationRemediationHealth?.expiringSoonRequests
      ? ["invoice_restoration_remediation_expiring"]
      : []),
    ...(restorationRemediationHealth?.overduePendingRequests
      ? ["invoice_restoration_remediation_overdue"]
      : []),
  ...
  const critical =
    expiredReservation ||
    approvalWaitingTooLong ||
    Boolean(
      invoiceStorageHealth?.missingFiles ||
        invoiceStorageHealth?.hashMismatches ||
        invoiceStorageHealth?.missingDocuments,
    );
  const stagingCritical = Boolean(
    restorationStagingHealth?.missingSources ||
      restorationStagingHealth?.hashMismatches ||
      restorationStagingHealth?.invalidEntries,
  );
  const remediationCritical = Boolean(
    restorationRemediationHealth?.overduePendingRequests ||
      restorationRemediationHealth?.staleApplyingRequests ||
      restorationRemediationHealth?.staleApplyingOver15Minutes ||
      restorationRemediationHealth?.staleApplyingOver60Minutes ||
  ...
  return {
    ...
    severity:
      ...
      stagingCritical ||
      remediationCritical ||
      cursorRetentionAssessment.severity === "critical"
        ? "critical"
        : wouldAlert
          ? "warning"
          : null,
...
export function evaluateShipmentApvCursorRetentionAlert(
  ...
  if (!job?.jobEnabled || !job.configured)
  ...
  if (
    job.health.lastRunStatus === "STALE_RUNNING" ||
  ...
  const firstObservedMs = job.health.firstObservedAt
    ? new Date(job.health.firstObservedAt).getTime()
    : Number.NaN;
  const overdueAfterMs = (job.intervalSeconds + 2 * 60 * 60) * 1000;
  if (
    job.health.lastRunStatus === "NEVER" &&
  ...
  const lastSucceededMs = job.health.lastSucceededAt
    ? new Date(job.health.lastSucceededAt).getTime()
    : Number.NaN;
  if (
    job.health.lastRunStatus === "SUCCEEDED" &&
    Number.isFinite(lastSucceededMs) &&
