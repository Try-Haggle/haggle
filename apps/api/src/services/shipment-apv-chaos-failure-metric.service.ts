...
export function evaluateShipmentApvChaosFailurePolicy(counts: StageCountMap) {
  ...
  const status =
    criticalReasons.length > 0 ? "critical" : warningReasons.length > 0 ? "warning" : "healthy";
  return {
    status,
...
export async function getShipmentApvChaosFailureHealth(
  ...
  const stages = Object.fromEntries(
    SHIPMENT_APV_CHAOS_FAILURE_STAGES.map((stage) => {
      const row = byStage.get(stage);
      return [
        stage,
        {
          count: boundedCount(row?.failure_count),
  ...
  const total = Math.min(
    SHIPMENT_APV_CHAOS_FAILURE_STAGES.reduce((sum, stage) => sum + stages[stage].count, 0),
    MAX_METRIC_COUNT,
  );
  const latest =
    SHIPMENT_APV_CHAOS_FAILURE_STAGES.map((stage) => stages[stage].lastFailureAt)
  ...
  const earliest = (field: string) =>
    lifecycleRows
      .map((row) => isoTimestamp(row[field]))
      .filter((value): value is string => value !== null)
      .sort()
      .at(0) ?? null;
  const retainedLatestBucket =
    retainedRows
      .map((row) => isoTimestamp(row.retained_latest_bucket_start))
      .filter((value): value is string => value !== null)
      .sort()
      .at(-1) ?? null;
  const retainedLastFailure =
    retainedRows
      .map((row) => isoTimestamp(row.retained_last_failure_at))
      .filter((value): value is string => value !== null)
      .sort()
      .at(-1) ?? null;
  ...
  return {
    ...
    lifecycle: {
      ...
      warningObservedAt: earliest(
        lifecycleField("warning_observed_at", "retained_warning_observed_at"),
      ),
      criticalObservedAt: earliest(
        lifecycleField("critical_observed_at", "retained_critical_observed_at"),
      ),
      recoveredAt:
        total === 0 && retainedLatestBucket
