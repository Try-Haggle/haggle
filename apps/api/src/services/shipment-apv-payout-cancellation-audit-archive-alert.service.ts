...
export function getShipmentApvCancellationAuditArchiveAlertPolicyStatus() {
  ...
  return {
    ...
    cooldownMinutes: boundedInteger(process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_COOLDOWN_MINUTES, 15, 1, 1440),
    staleThreshold: boundedInteger(process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_STALE_THRESHOLD, 1, 1, 100_000),
    retryReadyThreshold: boundedInteger(process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_RETRY_READY_THRESHOLD, 5, 1, 100_000),
    deadLetterThreshold: boundedInteger(process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_DEAD_LETTER_THRESHOLD, 1, 1, 100_000),
    overdueUnfinishedThreshold: boundedInteger(process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_OVERDUE_UNFINISHED_THRESHOLD, 1, 1, 100_000),
  };
}
...
export function resolveShipmentApvCancellationAuditArchiveAlertConfigFromEnv(): ShipmentApvCancellationAuditArchiveAlertConfig | null {
  ...
  if (!url) return null;
  const secret = process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_SECRET ?? "";
  if (secret.length < 16) throw new Error("audit archive alert secret must be at least 16 characters");
  ...
  assertDisputeModuleOutboundUrl(config.url, { label: "audit archive alert", allowInsecureHttp: config.allowInsecureHttp, allowPrivateNetwork: config.allowPrivateNetwork });
  return config;
}

export function evaluateShipmentApvCancellationAuditArchiveAlert(
  health: ShipmentApvCancellationAuditArchiveHealth,
  policy: Pick<ShipmentApvCancellationAuditArchiveAlertConfig, "staleThreshold" | "retryReadyThreshold" | "deadLetterThreshold" | "overdueUnfinishedThreshold">,
) {
  const reasons = [
    ...
    ...(health.retryReady >= policy.retryReadyThreshold ? ["audit_archive_retry_ready_backlog"] : []),
    ...(health.overdueUnfinished >= policy.overdueUnfinishedThreshold ? ["audit_archive_unfinished_too_old"] : []),
  ];
  return { wouldAlert: reasons.length > 0, severity: reasons.includes("audit_archive_dead_letter") ? "critical" as const : reasons.length ? "warning" as const : null, reasons };
}
...
export async function sendShipmentApvCancellationAuditArchiveAlert(
  ...
  options: { config: ShipmentApvCancellationAuditArchiveAlertConfig; fetchImpl?: typeof fetch; now?: Date },
) {
  assertDisputeModuleOutboundUrl(options.config.url, { label: "audit archive alert", allowInsecureHttp: options.config.allowInsecureHttp, allowPrivateNetwork: options.config.allowPrivateNetwork });
  const timestamp = (options.now ?? new Date()).toISOString();
  const rawBody = JSON.stringify({ type: "shipment_apv_cancellation_audit_archive.health", created_at: timestamp, severity: assessment.severity, reasons: assessment.reasons, health });
  ...
  try {
    const response = await (options.fetchImpl ?? fetch)(options.config.url, {
      ...
      headers: { "content-type": "application/json", "x-haggle-alert-type": "shipment_apv_cancellation_audit_archive.health", "x-haggle-alert-timestamp": timestamp, "x-haggle-alert-signature": signWebhookClaimAlertPayload(options.config.secret, timestamp, rawBody) },
      body: rawBody,
    });
