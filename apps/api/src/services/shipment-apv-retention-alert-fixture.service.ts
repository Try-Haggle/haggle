...
export async function verifyShipmentApvRetentionAlertFixtureCleanupIsolation() {
  ...
  const restoreFailure = await coordinateFixtureCleanup({
    restoreState: async () => { throw new Error("sensitive restore failure"); },
    cleanupClaims: async () => ({ deleted: 2, remaining: 0 }),
    stopHeartbeat: () => undefined,
    releaseLease: async () => { releaseAfterRestoreFailure = true; return true; },
  });
  let releaseAfterClaimFailure = false;
  const claimFailure = await coordinateFixtureCleanup({
    restoreState: async () => true,
    cleanupClaims: async () => { throw new Error("sensitive claim failure"); },
    stopHeartbeat: () => undefined,
    releaseLease: async () => { releaseAfterClaimFailure = true; return true; },
  });
  ...
  const checks = {
    release_attempted_after_restore_failure: restoreFailure.cleanupFailed
      && releaseAfterRestoreFailure && restoreFailure.leaseReleased,
    claims_attempted_after_restore_failure: restoreFailure.claimsDeleted === 2
      && restoreFailure.claimsRemaining === 0,
    release_attempted_after_claim_failure: claimFailure.cleanupFailed
      && releaseAfterClaimFailure && claimFailure.leaseReleased,
    release_failure_not_masked: releaseFailure.cleanupFailed
      && releaseFailure.leaseReleased === false,
  };
  return { pass: Object.values(checks).every(Boolean), checks };
}

async function getFixtureReservationRollbackSnapshot(db: FixtureReservationExecutor, leaseKey: string) {
  ...
  `) as unknown as Array<{ state_snapshot: string | null; lease_count: number; lease_id: string | null }>;
  return {
...
async function coordinateFixtureRollbackExecution<T>(
  runCases: () => Promise<T>, cleanupActions: FixtureRollbackCleanupActions,
) {
  ...
  return { pass: !caseExecutionFailed && result !== null && cleanup.pass,
    caseExecutionFailed, result, cleanup };
}
...
export async function verifyShipmentApvFixtureRollbackFailureIsolation() {
  ...
  const caseFailure = await coordinateFixtureRollbackExecution(
    async () => { throw new Error("sensitive case database error"); },
    {
      deleteVerificationLeases: async () => { cleanupAfterCaseFailure = true; },
  ...
  const deleteFailure = await coordinateFixtureRollbackExecution(
    ...
    {
      deleteVerificationLeases: async () => { throw new Error("sensitive delete error"); },
      readRemainingVerificationLeases: async () => { readAfterDeleteFailure = true; return 1; },
    },
  );
  const readFailure = await coordinateFixtureRollbackExecution(
    async () => ({ cases: "complete" }),
    {
      ...
      readRemainingVerificationLeases: async () => { throw new Error("sensitive read error"); },
    },
  );
  const evidence = { caseFailure, deleteFailure, readFailure };
  const checks = {
    cleanup_attempted_after_case_failure: cleanupAfterCaseFailure
      && caseFailure.cleanup.deleteAttempted && caseFailure.cleanup.readAttempted,
    case_failure_not_masked: caseFailure.caseExecutionFailed && !caseFailure.pass,
    remaining_checked_after_delete_failure: readAfterDeleteFailure
      && deleteFailure.cleanup.readAttempted && deleteFailure.cleanup.remainingVerificationLeases === 1,
    delete_failure_not_masked: !deleteFailure.cleanup.deleteSucceeded && !deleteFailure.pass,
    read_failure_not_treated_as_zero: !readFailure.cleanup.readSucceeded
      && readFailure.cleanup.remainingVerificationLeases === -1 && !readFailure.pass,
...
export async function verifyShipmentApvFixtureReservationRollback(db: Database) {
  ...
  const execution = await coordinateFixtureRollbackExecution(async () => {
    ...
    try {
      await db.transaction(async (transaction) => {
        ...
        await reserveRetentionStateWithinTransaction(transaction, {
          leaseId: randomUUID(), reservationClaimId: randomUUID(), now, leaseKey: verificationKey,
        });
      });
    } catch (error) {
      busyRejected = error instanceof Error
    ...
    const blocker = await acquireShipmentApvChaosFixtureLeaseWithin(db, {
      leaseId: blockerLeaseId, ownerId: "shipment-apv-rollback-verification",
    }, now, verificationKey);
    ...
    try {
      ...
      collisionRejected = error instanceof Error
    ...
    return { checks: {
      ...
      collision_owner_preserved: collisionAfter.leaseCount === 1
        && collisionAfter.leaseId === blockerLeaseId,
      collision_state_unchanged: collisionAfter.stateSnapshot === baseline.stateSnapshot,
    } };
  }, {
    deleteVerificationLeases: async () => {
      await db.execute(sql`DELETE FROM payment_test_operation_leases WHERE key=${verificationKey}`);
    },
  ...
  const checks = { ...execution.result.checks,
...
async function reserveRetentionStateWithinTransaction(
  ...
  const lease = await acquireShipmentApvChaosFixtureLeaseWithin(transaction, {
    leaseId: input.leaseId, ownerId: "shipment-apv-retention-alert-fixture",
  }, input.now, input.leaseKey);
...
async function acquireFixtureLeaseAndReserveRetentionState(
  ...
  return db.transaction((transaction) => reserveRetentionStateWithinTransaction(transaction, input));
}
...
export async function runShipmentApvRetentionAlertFixture(db: Database) {
  ...
  if (!preflight.eligible) {
    throw new Error(FIXTURE_READINESS_ERROR_BY_REASON[preflight.reasons[0]!] ??
      "SHIPMENT_APV_RETENTION_ALERT_FIXTURE_PREFLIGHT_BLOCKED");
  }
  const rollbackIsolation = await verifyShipmentApvFixtureReservationRollback(db)
    .catch(() => { throw new Error("SHIPMENT_APV_FIXTURE_ROLLBACK_VERIFICATION_FAILED"); });
  ...
  const now = new Date();
  const reservation = await acquireFixtureLeaseAndReserveRetentionState(
    db, { leaseId, reservationClaimId, now });
  ...
  try {
    ...
    const fixtureOptions = { fetchImpl: fetchImpl as typeof fetch,
    ...
    const competingRetention = await runShipmentApvRemediationCursorRetention(db, {
      now: new Date(now.getTime() + 500), finishedAt: new Date(now.getTime() + 750),
      retentionDays: 30, limit: 1000,
    });
    const retention = await runShipmentApvRemediationCursorRetention(db, {
      now: new Date(now.getTime() + 1000), finishedAt: new Date(now.getTime() + 2000),
      retentionDays: 30, limit: 1000, fixtureLeaseId: leaseId,
    });
    const recoveredHealth = await getShipmentApvRemediationCursorRetentionJobHealth(
      db, new Date(now.getTime() + 2500));
    const recovery = await runShipmentApvPayoutAlert(db, {
      ...fixtureOptions, now: new Date(now.getTime() + 3000),
    });
    ...
    evidence = {
      ...
      competingRetention: { status: competingRetention.status,
        reason: "reason" in competingRetention ? competingRetention.reason : null },
      retention: { status: retention.status, reason: "reason" in retention ? retention.reason : null },
      recovery: { status: recovery.status, assessment: recovery.assessment },
      recoveredHealth: { lastRunStatus: recoveredHealth.lastRunStatus,
    ...
    const cleanupResult = await coordinateFixtureCleanup({
      restoreState: async () => {
        ...
        stateRestored = restored[0]?.status === saved.status
          && String(restored[0]?.claim_id ?? "") === String(saved.claim_id ?? "")
          && timestampMillis(restored[0]?.lease_expires_at) === timestampMillis(saved.lease_expires_at)
          && timestampMillis(restored[0]?.first_observed_at) === timestampMillis(saved.first_observed_at)
          && timestampMillis(restored[0]?.last_started_at) === timestampMillis(saved.last_started_at)
          && timestampMillis(restored[0]?.last_succeeded_at) === timestampMillis(saved.last_succeeded_at)
  ...
  const firing = evidence.firing as { status?: string; assessment?: { severity?: string; reasons?: string[] } };
  const competingRetention = evidence.competingRetention as { status?: string; reason?: string | null };
  ...
  const checks = {
    ...Object.fromEntries(Object.entries(preflight.checks)
      .map(([key, value]) => [`preflight_${key}`, value])),
    ...Object.fromEntries(Object.entries(rollbackIsolation.checks)
      .map(([key, value]) => [`rollback_${key}`, value])),
    ...Object.fromEntries(Object.entries(rollbackFailureIsolation.checks)
      .map(([key, value]) => [`rollback_failure_${key}`, value])),
    cross_instance_retention_blocked: competingRetention.status === "skipped"
      && competingRetention.reason === "in_progress",
    never_warning_delivered: firing.status === "delivered"
      && firing.assessment?.severity === "warning"
      && firing.assessment.reasons?.includes("invoice_restoration_cursor_retention_never_started") === true,
    retention_first_run_completed: ["completed", "skipped"].includes(retention.status ?? "")
      && (retention.status === "completed" || retention.reason === "healthy")
      && recoveredHealth.lastRunStatus === "SUCCEEDED",
    recovery_delivered: recovery.status === "delivered" && recovery.assessment?.severity === "recovery",
    signed_payloads_valid: deliveries.length === 2
      && deliveries.every((delivery) => delivery.signatureValid === true),
    ...
    ...Object.fromEntries(Object.entries(cleanupIsolation.checks)
      .map(([key, value]) => [`cleanup_${key}`, value])),
  };
  return { pass: Object.values(checks).every(Boolean), checks, preflight, ...evidence,
    cleanupIsolation, rollbackIsolation, rollbackFailureIsolation,
    cleanup: { claimsDeleted, claimsRemaining, stateRestored, leaseReleased: released,
      heartbeatRenewals: heartbeatState.renewals, heartbeatFailures: heartbeatState.failures } };
}
