...
export async function runDisputeEvidenceProvenanceFixture(db: Database) {
  const disputeRows = (await db.execute(sql`SELECT id FROM dispute_cases ORDER BY created_at DESC LIMIT 1`)) as unknown as Array<{ id: string }>;
  ...
  const provenance = createSignedDisputeEvidenceProvenance({
    disputeId, evidenceId, sourceContentSha256, verifierProvider: "fixture-vision",
  ...
  try {
    ...
    `) as unknown as Array<{ derivedArtifacts: DisputeEvidenceDerivedArtifact[]; sourceContentSha256: string; provenance: unknown }>;
    const row = rows[0];
    stored = Boolean(row && row.derivedArtifacts.length === 1 && row.sourceContentSha256 === sourceContentSha256);
    if (row) {
      trusted = verifyTrustedDisputeEvidenceProvenance({
        provenance: row.provenance, artifacts: row.derivedArtifacts, disputeId, evidenceId,
        sourceContentSha256: row.sourceContentSha256, keys: [activeKey],
      }).valid;
      ...
      artifactTamperBlocked = !verifyTrustedDisputeEvidenceProvenance({
        provenance: row.provenance, artifacts: tampered, disputeId, evidenceId,
        sourceContentSha256: row.sourceContentSha256, keys: [activeKey],
      }).valid;
      sourceSwapBlocked = !verifyTrustedDisputeEvidenceProvenance({
        provenance: row.provenance, artifacts: row.derivedArtifacts, disputeId, evidenceId,
        sourceContentSha256: "b".repeat(64), keys: [activeKey],
      }).valid;
      revokedKeyBlocked = !verifyTrustedDisputeEvidenceProvenance({
        provenance: row.provenance, artifacts: row.derivedArtifacts, disputeId, evidenceId,
        sourceContentSha256: row.sourceContentSha256, keys: [revokedKey],
      }).valid;
      try {
        await db.execute(sql`UPDATE dispute_evidence SET derived_artifacts = NULL WHERE id = ${evidenceId}::uuid`);
      } catch (error) {
        appendOnlyUpdateBlocked = error instanceof Error && error.message.includes("dispute_evidence is append-only");
      }
      const evidence = {
        id: evidenceId, dispute_id: disputeId, submitted_by: "buyer" as const, type: "image" as const,
        text: "provenance fixture", derived_artifacts: row.derivedArtifacts,
      ...
      const firstArchive = await enqueueDisputeEvidenceProvenanceArchive(db, { evidence, now: generatedAt });
      const duplicateArchive = await enqueueDisputeEvidenceProvenanceArchive(db, { evidence, now: generatedAt });
      archiveId = firstArchive.archive.id;
      archiveEnqueued = firstArchive.outcome === "enqueued";
      archiveDuplicate = duplicateArchive.outcome === "duplicate" && duplicateArchive.archive.id === archiveId;
      try {
        await db.execute(sql`
          UPDATE dispute_evidence_provenance_archive_outbox SET payload = '{}'::jsonb WHERE id = ${archiveId}::uuid
        `);
      } catch (error) {
        archivePayloadImmutable = error instanceof Error
          && error.message.includes("dispute evidence provenance archive payload is immutable");
      }
      const archiveConfig = { url: "http://127.0.0.1:4177/mock-worm", timeoutMs: 1000, maxAttempts: 1,
        allowInsecureHttp: true, allowPrivateNetwork: true };
      const mismatchFetch = (async () => new Response(JSON.stringify({
        receipt_id: "cycle74-bad-receipt", stored_sha256: "0".repeat(64),
      ...
      const failed = await dispatchDisputeEvidenceProvenanceArchives(db, {
        config: archiveConfig, fetchImpl: mismatchFetch, now: new Date(generatedAt.getTime() + 10), archiveId,
      });
      receiptMismatchDeadLettered = failed.deadLettered === 1;
      const alertConfig = { url: "http://127.0.0.1:4177/mock-ops-alert",
        secret: "cycle76-provenance-alert-secret", timeoutMs: 1000, cooldownMinutes: 15,
        coverageGapThreshold: 1, staleThreshold: 1, retryReadyThreshold: 1, deadLetterThreshold: 1,
        allowInsecureHttp: true, allowPrivateNetwork: true };
      const alertStates: string[] = [];
      const alertFetch = (async (_url: string | URL | Request, init?: RequestInit) => {
        const headers = new Headers(init?.headers); const rawBody = String(init?.body ?? "");
        ...
        const accepted = await claimVerifiedDisputeEvidenceProvenanceArchiveAlert(db, verification, alertReceiverSource);
        if (accepted.outcome !== "accepted") return new Response("conflict", { status: 409 });
        await completeWebhookEvent(db, accepted.claim, 202);
        const replay = await claimVerifiedDisputeEvidenceProvenanceArchiveAlert(db, verification, alertReceiverSource);
        receiverReplayBlocked = receiverReplayBlocked || replay.outcome === "replay_or_in_progress";
        const parsed = JSON.parse(rawBody) as { state?: string }; alertStates.push(String(parsed.state ?? ""));
        return new Response("accepted", { status: 202 });
      }) as typeof fetch;
      const firing = await runDisputeEvidenceProvenanceArchiveAlert(db, { config: alertConfig,
        fetchImpl: alertFetch, claimSource: alertClaimSource, now: new Date(generatedAt.getTime() + 12) });
      firingAlertDelivered = firing.status === "delivered" && firing.assessment.severity === "critical"
        && firing.assessment.reasons.includes("evidence_provenance_archive_dead_letter") && alertStates[0] === "firing";
      const failures = await listDisputeEvidenceProvenanceArchiveFailures(db, { limit: 100,
        now: new Date(generatedAt.getTime() + 15) });
      failureQueueDetected = failures.items.some((item) => item.archiveId === archiveId
        && item.status === "DEAD_LETTER" && item.payloadSha256 === firstArchive.archive.payloadSha256);
      const requeued = await requeueDisputeEvidenceProvenanceArchive(db, {
        archiveId, actorId: "99999999-9999-4999-8999-999999999999",
      ...
      archiveRequeued = requeued.outcome === "requeued" && duplicateRequeue.outcome === "already_queued"
      ...
      const receiptFetch = (async (_url: string | URL | Request, init?: RequestInit) => {
        ...
        return new Response(JSON.stringify({ receipt_id: "cycle74-provenance-receipt",
      ...
      const delivered = await dispatchDisputeEvidenceProvenanceArchives(db, {
        config: { ...archiveConfig, maxAttempts: 3 }, fetchImpl: receiptFetch,
        now: new Date(generatedAt.getTime() + 30), archiveId,
      });
      ...
      `) as unknown as Array<{ status: string; payload_sha256: string; receipt_sha256: string | null }>;
      ...
      const recovery = await runDisputeEvidenceProvenanceArchiveAlert(db, { config: alertConfig,
        fetchImpl: alertFetch, claimSource: alertClaimSource, now: new Date(generatedAt.getTime() + 40) });
      const duplicateRecovery = await runDisputeEvidenceProvenanceArchiveAlert(db, { config: alertConfig,
        fetchImpl: alertFetch, claimSource: alertClaimSource, now: new Date(generatedAt.getTime() + 41) });
      recoveryAlertDelivered = recovery.status === "recovered" && alertStates[1] === "recovered";
      duplicateRecoveryBlocked = duplicateRecovery.status === "skipped"
      ...
      const rollbackArtifacts: DisputeEvidenceDerivedArtifact[] = [{
        ...artifacts[0]!, id: `${rollbackEvidenceId}:visual:1`, source_evidence_id: rollbackEvidenceId,
      }];
      const rollbackProvenance = createSignedDisputeEvidenceProvenance({
        disputeId, evidenceId: rollbackEvidenceId, sourceContentSha256,
        verifierProvider: "fixture-vision", artifacts: rollbackArtifacts, generatedAt, privateKey,
      });
      try {
        await db.transaction(async (transaction) => {
          ...
          await tx.execute(sql`
            ...
            VALUES (${rollbackEvidenceId}::uuid, ${disputeId}::uuid, 'buyer', 'image', 'rollback fixture',
              ...
              ${generatedAt.toISOString()}::timestamptz)
          `);
          await enqueueDisputeEvidenceProvenanceArchive(tx, { evidence: {
            id: rollbackEvidenceId, dispute_id: disputeId, submitted_by: "buyer", type: "image",
            text: "rollback fixture", derived_artifacts: rollbackArtifacts,
            source_content_sha256: sourceContentSha256, derived_artifacts_provenance: rollbackProvenance,
            created_at: generatedAt.toISOString(),
          }, now: generatedAt });
          throw new Error("CYCLE74_FORCE_ATOMIC_ROLLBACK");
        });
      } catch (error) {
        if (!(error instanceof Error) || error.message !== "CYCLE74_FORCE_ATOMIC_ROLLBACK") throw error;
      }
      ...
      atomicRollbackClean = rollbackRows[0]?.evidence_count === 0 && rollbackRows[0]?.archive_count === 0;
    }
  } finally {
    const deleted = await db.execute(sql`DELETE FROM dispute_evidence WHERE id = ${evidenceId}::uuid RETURNING id`) as unknown as unknown[];
    const archiveDeleted = archiveId
      ? await db.execute(sql`DELETE FROM dispute_evidence_provenance_archive_outbox WHERE id = ${archiveId}::uuid RETURNING id`) as unknown as unknown[]
      : [];
    ...
    `) as unknown as unknown[];
    cleanup = deleted.length + archiveDeleted.length + auditDeleted.length + alertClaimsDeleted.length;
  }
  const checks = { stored, trusted, artifact_tamper_blocked: artifactTamperBlocked,
    source_swap_blocked: sourceSwapBlocked, revoked_key_blocked: revokedKeyBlocked,
    append_only_update_blocked: appendOnlyUpdateBlocked, archive_enqueued: archiveEnqueued,
    archive_duplicate_idempotent: archiveDuplicate, archive_payload_immutable: archivePayloadImmutable,
    receipt_mismatch_dead_lettered: receiptMismatchDeadLettered, archive_delivered: archiveDelivered,
    receipt_matched: receiptMatched, archive_survived_evidence_delete: archiveSurvivedEvidenceDelete,
    atomic_rollback_clean: atomicRollbackClean,
    failure_queue_detected: failureQueueDetected, archive_requeued: archiveRequeued,
    requeue_audit_once: requeueAuditOnce, firing_alert_delivered: firingAlertDelivered,
    recovery_alert_delivered: recoveryAlertDelivered, receiver_replay_blocked: receiverReplayBlocked,
    duplicate_recovery_blocked: duplicateRecoveryBlocked, cleanup: cleanup === 6 };
  ...
  return {
    ...
    trust_boundary: "DB content is trusted only when the artifact digest, source file hash, signature, and registry key state all match.",
  };
}
