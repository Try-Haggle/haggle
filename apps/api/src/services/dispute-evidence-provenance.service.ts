...
import {
  type DisputeAuditPublicKeyRecord,
  disputeAuditKeyId,
...
export function createSignedDisputeEvidenceProvenance(input: {
  ...
  artifacts: DisputeEvidenceDerivedArtifact[];
  generatedAt: Date;
  privateKey?: KeyObject;
  privateKeyBase64?: string;
}) {
  if (!/^[0-9a-f]{64}$/.test(input.sourceContentSha256))
    throw new Error("INVALID_EVIDENCE_SOURCE_CONTENT_SHA256");
  if (!input.artifacts.length || input.artifacts.length > 20)
    throw new Error("INVALID_EVIDENCE_DERIVED_ARTIFACT_COUNT");
  ...
  const artifactsSha256 = createHash("sha256")
    .update(canonicalDisputeAuditJson(input.artifacts))
    .digest("hex");
  ...
  return {
    ...
    signature: {
      ...
      key_id: disputeAuditKeyId(publicKeyDer),
      public_key_spki_base64: publicKeyDer.toString("base64"),
      value_base64: sign(
        null,
        Buffer.from(canonicalDisputeAuditJson(manifest)),
        privateKey,
      ).toString("base64"),
    },
  };
...
export function verifyTrustedDisputeEvidenceProvenance(input: {
  ...
  try {
    ...
    if (
      manifest?.schema !== "haggle.dispute-evidence-derived-artifacts.v1" ||
      manifest.dispute_id !== input.disputeId ||
      manifest.evidence_id !== input.evidenceId ||
      manifest.source_content_sha256 !== input.sourceContentSha256 ||
      typeof manifest.verifier_provider !== "string" ||
      !manifest.verifier_provider ||
      manifest.verifier_provider.length > 120 ||
      manifest.artifact_count !== input.artifacts.length ||
      input.artifacts.some(
        (artifact) =>
          artifact.source_evidence_id !== input.evidenceId ||
          (artifact.kind === "image_visual_observation" &&
            artifact.metadata?.provider !== manifest.verifier_provider),
      ) ||
      createHash("sha256").update(canonicalDisputeAuditJson(input.artifacts)).digest("hex") !==
    ...
    if (disputeAuditKeyId(publicKeyDer) !== signature.key_id)
      return { valid: false as const, reason: "PROVENANCE_KEY_ID_MISMATCH" as const };
    ...
    if (key.status === "revoked")
      return { valid: false as const, reason: "PROVENANCE_KEY_REVOKED" as const };
    const generatedAt = Date.parse(manifest.generated_at);
    if (
      !Number.isFinite(generatedAt) ||
      generatedAt < Date.parse(key.not_before) ||
    ...
    return {
      ...
      reason:
        key.status === "retired"
          ? ("TRUSTED_RETIRED_KEY" as const)
          : ("TRUSTED_ACTIVE_KEY" as const),
      keyId: key.key_id,
      artifactsSha256: manifest.artifacts_sha256,
    };
  } catch {
    return { valid: false as const, reason: "PROVENANCE_INVALID" as const };
  }
}
