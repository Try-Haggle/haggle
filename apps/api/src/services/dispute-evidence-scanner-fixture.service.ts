...
export async function runDisputeEvidenceScannerSecurityFixture() {
  ...
  const cleanFetch: typeof fetch = async (input, init) => {
    ...
    authenticatedRequest =
      url.hostname === "scanner.fixture.invalid" &&
      new Headers(init?.headers).get("authorization") === `Bearer ${FIXTURE_TOKEN}`;
  ...
  try {
    await resolveDisputeModuleOutboundTarget(fixtureConfig().url, outboundPolicy, {
  ...
  try {
    await resolveDisputeModuleOutboundTarget(fixtureConfig().url, outboundPolicy, {
      lookupImpl: async () =>
        Array.from({ length: DISPUTE_MODULE_MAX_RESOLVED_ADDRESSES + 1 }, (_, index) => ({
  ...
  const dnsConnectionPinned = await new Promise<boolean>((resolve) => {
    ...
    const callback: FixtureLookupCallback = (error, addresses) =>
      resolve(!error && addresses?.length === 1 && addresses[0]?.address === "8.8.8.8");
    (
    ...
    )(publicTarget.url.hostname, { all: true }, callback);
  });

  const checks = {
    integritySizeMismatchBlocked:
      sizeMismatch.status === "INFECTED" && sizeMismatch.detail === "FILE_SIZE_MISMATCH",
    integrityTypeMismatchBlocked:
      typeMismatch.status === "INFECTED" && typeMismatch.detail === "CONTENT_TYPE_MISMATCH",
    unconfiguredQuarantined:
      unconfigured.status === "PENDING" && unconfigured.detail === "MALWARE_SCANNER_NOT_CONFIGURED",
    authenticatedRequest,
    redirectsBlocked,
    cleanResponseAccepted: clean.status === "CLEAN" && clean.detail === "CLEAN",
    infectedResponseRejected:
      infected.status === "INFECTED" && infected.detail === "EICAR_FIXTURE_DETECTED",
    nonJsonRejected:
      nonJson.status === "FAILED" && nonJson.detail === "INVALID_SCANNER_CONTENT_TYPE",
    oversizedResponseRejected:
      oversized.status === "FAILED" && oversized.detail === "SCANNER_RESPONSE_TOO_LARGE",
    httpFailureQuarantined:
      httpFailure.status === "FAILED" && httpFailure.detail === "SCANNER_HTTP_503",
    privateNetworkRejected,
    productionOverrideRejected,
    dnsPublicResolutionAccepted:
      publicTarget.addresses.length === 1 && publicTarget.addresses[0]?.address === "8.8.8.8",
  ...
  const passed = Object.entries(checks).filter(([key, value]) =>
    key === "realNetworkCalled" ? value === false : value,
  ).length;
