...
import { deleteShipmentApvInvoiceDocumentForFixture, getShipmentApvInvoiceDocumentStorageHealth,
  readShipmentApvInvoiceDocumentBytes, runShipmentApvInvoiceDocumentReconciliationDryRun,
  storeShipmentApvInvoiceDocument } from "./shipment-apv-invoice-document.service.js";
import {
...
import { runShipmentApvRetentionAlertFixture } from
  "./shipment-apv-retention-alert-fixture.service.js";
...
export async function runShipmentApvChaos(db: Database) {
  ...
  const baseInput: Omit<ShipmentApvInput, "providerInvoiceId" | "originalRateMinor" | "adjustedRateMinor" | "adjustmentMinor"> = {
  ...
  try {
    ...
    const sellerReview = await submitShipmentApvSellerReview(db, {
      ...
      reason: "The carrier adjustment does not match the package and address information supplied at purchase.",
    });
    ...
    const reviewDecisions = await Promise.all(Array.from({ length: 10 }, () => decideShipmentApvReview(db, {
      adjustmentId: overBuffer.id,
      reviewerId: randomUUID(),
      requestId: reviewDecisionRequestId,
      decision: "WAIVED",
      reason: "The available carrier record is insufficient to hold the seller responsible for this correction fee.",
    ...
    const decisionConflicts = reviewDecisions.filter((item) => item.outcome === "version_conflict" || item.outcome === "invalid_state");
    const finalReview = decisionWinners[0] && "record" in decisionWinners[0] ? decisionWinners[0].record : null;
    ...
    const concurrentRevisions = await Promise.all(
      Array.from({ length: 10 }, () => recordShipmentApvInvoiceRevision(db, concurrentRevisionInput)),
    );
    ...
    if (!("revision" in revisionTwo) || !("revision" in revisionThree) || !revisionFour || !("revision" in revisionFour)) {
    ...
    const revisionTwoDocumentBytes = Buffer.from(JSON.stringify({ provider: "easypost", invoice_id: invoiceId,
      revision: 2, adjusted_rate_minor: 1125, currency: "USD" }));
    const bindEvidence = (revisionId: string, amountMinor: number, suffix: string) => bindShipmentApvRevisionEvidence(db, {
      revisionId,
      actorId: randomUUID(),
      documentSha256: suffix === "revision-2"
    ...
    const revisionTwoEvidenceReplay = await bindEvidence(revisionTwo.revision.id, 100, "revision-2");
    ...
    const invoiceDocumentInput = { revisionId: revisionTwo.revision.id,
      providerDocumentId: `${invoiceId}:revision-2`, contentType: "application/json" as const,
      bytes: revisionTwoDocumentBytes, uploadedBy: randomUUID(), storageRoot: invoiceDocumentRoot };
    ...
    const invoiceDocumentConflict = await storeShipmentApvInvoiceDocument(db, {
      ...invoiceDocumentInput, contentType: "text/csv",
    });
    const invoiceDocumentReadBytes = await readShipmentApvInvoiceDocumentBytes(db,
      revisionTwo.revision.id, invoiceDocumentRoot);
    const invoiceDocumentHealthBefore = await getShipmentApvInvoiceDocumentStorageHealth(db, {
      revisionId: revisionTwo.revision.id, storageRoot: invoiceDocumentRoot,
    });
    ...
    const invoiceDocumentReconciliation = await runShipmentApvInvoiceDocumentReconciliationDryRun(db, {
      revisionId: revisionTwo.revision.id, storageRoot: invoiceDocumentRoot,
    });
    const healthyPayout = { status: "healthy" as const, expiredReserved: 0, signedExpired: 0,
      unsignedExpired: 0, affectedSellers: 0, appliedOffsetMinor: 0, oldestExpiredAgeSeconds: null,
      recordedAt: new Date().toISOString() };
    const invoiceDocumentAlert = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, invoiceDocumentHealthWithOrphan);
    ...
    const orphanCandidate = reconciliationCandidates.candidates.find((item) => item.anomalyType === "ORPHAN_FILE");
    ...
    const reconciliationRequest = await requestShipmentApvInvoiceReconciliation(db, {
      clientRequestId: reconciliationClientRequestId, candidateId: orphanCandidate.candidateId,
    ...
    const reconciliationSelfApproval = await decideShipmentApvInvoiceReconciliation(db, {
      requestId: reconciliationRequestRecord.id, decisionRequestId: randomUUID(),
      approverId: reconciliationMakerId, decision: "APPROVE",
      reason: "The request maker must not approve the same quarantine action.", expectedVersion: 0,
      storageRoot: invoiceDocumentRoot,
    });
    const reconciliationDecisionRequestId = randomUUID();
    const reconciliationApprovalInput = { requestId: reconciliationRequestRecord.id,
      decisionRequestId: reconciliationDecisionRequestId, approverId: reconciliationCheckerId,
      decision: "APPROVE" as const,
      reason: "A separate checker verified the orphan and approved non-destructive quarantine.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot };
    const reconciliationApproval = await decideShipmentApvInvoiceReconciliation(db, reconciliationApprovalInput);
    const reconciliationApprovalReplay = await decideShipmentApvInvoiceReconciliation(db, reconciliationApprovalInput);
    const reconciliationTimeline = await getShipmentApvInvoiceReconciliationTimeline(db,
      reconciliationRequestRecord.id);
    const quarantinePath = join(invoiceDocumentRoot, ".quarantine", reconciliationRequestRecord.id,
      `${orphanHash}.json`);
    ...
    const invoiceDocumentAlertAfter = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, invoiceDocumentHealthAfter);
    const invoiceDocumentSha256 = createHash("sha256").update(revisionTwoDocumentBytes).digest("hex");
    const invoiceDocumentPath = join(invoiceDocumentRoot, revisionTwo.revision.id, `${invoiceDocumentSha256}.json`);
    ...
    const corruptedCandidate = corruptedCandidates.candidates.find((item) => item.anomalyType === "HASH_MISMATCH");
    ...
    const corruptRequest = await requestShipmentApvInvoiceReconciliation(db, {
      clientRequestId: randomUUID(), candidateId: corruptedCandidate.candidateId, requesterId: randomUUID(),
    ...
    const corruptDecisionInput = { requestId: corruptRequest.request.id, decisionRequestId: randomUUID(),
      approverId: randomUUID(), decision: "APPROVE" as const,
      reason: "A separate checker verified the hash mismatch and approved non-destructive quarantine.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot };
    const corruptApproval = await decideShipmentApvInvoiceReconciliation(db, corruptDecisionInput);
    const corruptQuarantinePath = join(invoiceDocumentRoot, ".quarantine", corruptRequest.request.id,
    ...
    const restorationCandidate = restorationCandidates.candidates.find((item) => item.integrityStatus === "QUARANTINED");
    ...
    const restorationMismatch = await requestShipmentApvInvoiceRestoration(db, {
      clientRequestId: randomUUID(), candidateId: restorationCandidate.candidateId, requesterId: randomUUID(),
      reason: "Reject a replacement that does not match the immutable carrier evidence hash.",
      contentType: "application/json", bytes: Buffer.from('{"different":true}'), storageRoot: invoiceDocumentRoot,
    });
    const restorationMakerId = randomUUID();
    const restorationClientRequestId = randomUUID();
    const restorationInput = { clientRequestId: restorationClientRequestId,
      candidateId: restorationCandidate.candidateId, requesterId: restorationMakerId,
      reason: "Stage a re-collected invoice that exactly matches the immutable carrier evidence hash.",
      contentType: "application/json" as const, bytes: revisionTwoDocumentBytes, storageRoot: invoiceDocumentRoot };
    const restorationRequest = await requestShipmentApvInvoiceRestoration(db, restorationInput);
    const restorationRequestReplay = await requestShipmentApvInvoiceRestoration(db, restorationInput);
    ...
    const restorationSelfApproval = await decideShipmentApvInvoiceRestoration(db, {
      requestId: restorationRequest.request.id, decisionRequestId: randomUUID(), approverId: restorationMakerId,
      decision: "PRESERVE", reason: "The restoration maker must not approve the same replacement.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot,
    });
    const preservationDecisionInput = { requestId: restorationRequest.request.id,
      decisionRequestId: randomUUID(), approverId: randomUUID(), decision: "PRESERVE" as const,
      reason: "A separate checker verified the replacement hash and approved permanent preservation.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot };
    const preservationApproval = await decideShipmentApvInvoiceRestoration(db, preservationDecisionInput);
    const preservationApprovalReplay = await decideShipmentApvInvoiceRestoration(db, preservationDecisionInput);
    const preservationTimeline = await getShipmentApvInvoiceRestorationTimeline(db, restorationRequest.request.id);
    const preservedReplacementPath = join(invoiceDocumentRoot, ".quarantine", restorationRequest.request.id,
      `replacement-${invoiceDocumentSha256}.json`);
    const preservedReplacementBytes = await readFile(preservedReplacementPath).catch(() => null);
    const restoreCandidateAfterPreserve = (await listShipmentApvInvoiceRestorationCandidates(db)).candidates
      .find((item) => item.integrityStatus === "QUARANTINED");
    if (!restoreCandidateAfterPreserve) throw new Error("APV_INVOICE_RESTORE_AFTER_PRESERVE_CANDIDATE_MISSING");
    ...
    const remediatedRestorationRequest = await requestShipmentApvInvoiceRestoration(db, {
      clientRequestId: randomUUID(), candidateId: restoreCandidateAfterPreserve.candidateId,
      requesterId: remediatedRestorationMakerId,
      reason: "Stage an exact replacement before simulating terminal staging corruption.",
      contentType: "application/json", bytes: revisionTwoDocumentBytes, storageRoot: invoiceDocumentRoot,
    });
    ...
    const remediatedStagingPath = join(invoiceDocumentRoot, ".restoration",
      remediatedRestorationRequest.request.id, `${invoiceDocumentSha256}.json`);
    ...
    const remediatedRestorationRejection = await decideShipmentApvInvoiceRestoration(db, {
      requestId: remediatedRestorationRequest.request.id, decisionRequestId: randomUUID(), approverId: randomUUID(),
      decision: "REJECT", reason: "A checker rejected the replacement while preserving the staged evidence.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot,
    });
    ...
    const remediationCandidates = await listShipmentApvInvoiceRestorationRemediationCandidates(
      db, invoiceDocumentRoot);
    const remediationCandidate = remediationCandidates.candidates.find((item) => item.issueType === "HASH_MISMATCH");
    ...
    const remediationInput = { clientRequestId: remediationClientRequestId,
      candidateId: remediationCandidate.candidateId, requesterId: remediationMakerId,
      reason: "Quarantine the verified mismatched staging bytes without deleting evidence.",
      storageRoot: invoiceDocumentRoot };
    const remediationRequest = await requestShipmentApvInvoiceRestorationRemediation(db, remediationInput);
    const remediationRequestReplay = await requestShipmentApvInvoiceRestorationRemediation(db, remediationInput);
    ...
    const remediationSelfApproval = await decideShipmentApvInvoiceRestorationRemediation(db, {
      requestId: remediationRequest.request.id, decisionRequestId: randomUUID(), approverId: remediationMakerId,
      decision: "APPROVE", reason: "The remediation maker must not approve the same evidence disposition.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot,
    });
    const remediationDecisionInput = { requestId: remediationRequest.request.id,
      decisionRequestId: randomUUID(), approverId: randomUUID(), decision: "APPROVE" as const,
      reason: "A separate checker verified the mismatch and approved non-destructive quarantine.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot };
    const remediationApproval = await decideShipmentApvInvoiceRestorationRemediation(db,
      remediationDecisionInput);
    const remediationApprovalReplay = await decideShipmentApvInvoiceRestorationRemediation(db,
      remediationDecisionInput);
    const remediationTimeline = await getShipmentApvInvoiceRestorationRemediationTimeline(db,
      remediationRequest.request.id);
    const remediatedRestorationTimeline = await getShipmentApvInvoiceRestorationTimeline(db,
      remediatedRestorationRequest.request.id);
    const remediatedQuarantinePath = join(invoiceDocumentRoot, ".quarantine", remediationRequest.request.id,
    ...
    const expiryStagingPath = join(invoiceDocumentRoot, ".restoration", expiryRestorationRequest.request.id,
      `${invoiceDocumentSha256}.json`);
    await rm(expiryStagingPath, { force: true });
    const expiryRestorationRejection = await decideShipmentApvInvoiceRestoration(db, {
      requestId: expiryRestorationRequest.request.id, decisionRequestId: randomUUID(), approverId: randomUUID(),
      decision: "REJECT", reason: "A checker rejected the replacement after its source became unavailable.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot,
    });
    const missingRemediationCandidate = (await listShipmentApvInvoiceRestorationRemediationCandidates(
      db, invoiceDocumentRoot)).candidates.find((item) => item.issueType === "SOURCE_MISSING");
    if (!missingRemediationCandidate) throw new Error("APV_INVOICE_REMEDIATION_EXPIRY_CANDIDATE_MISSING");
    const expiryRemediationRequest = await requestShipmentApvInvoiceRestorationRemediation(db, {
      clientRequestId: randomUUID(), candidateId: missingRemediationCandidate.candidateId,
      requesterId: randomUUID(), reason: "Record the missing staging source for checker disposition.",
      storageRoot: invoiceDocumentRoot,
    });
    ...
    const remediationQueueHealthBefore = await getShipmentApvInvoiceRestorationRemediationHealth(db);
    const remediationQueueAlertBefore = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, undefined, remediationHealthAfter, remediationQueueHealthBefore);
    ...
    const expiredRemediationTimeline = await getShipmentApvInvoiceRestorationRemediationTimeline(db,
    ...
    const cleanupRemediationRequest = await requestShipmentApvInvoiceRestorationRemediation(db, {
      clientRequestId: randomUUID(), candidateId: missingRemediationCandidate.candidateId,
      requesterId: randomUUID(), reason: "Finalize the still-missing staging source after request expiry.",
      storageRoot: invoiceDocumentRoot, now: cleanupRequestNow,
    });
    ...
    await db.execute(sql`INSERT INTO shipment_apv_invoice_restoration_remediation_events
      ...
      (${randomUUID()}::uuid,${invoicePaginationRestorationRemediationRequestId}::uuid,'APPLYING',
        ...
        ${paginationRemediationUpdatedAt.toISOString()}::timestamptz)`);
    const paginationRecoveryFirstPage = await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
      approverId: cleanupCheckerId, limit: 1,
    });
    const paginationRecoverySecondPage = await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
      approverId: cleanupCheckerId, limit: 1, cursor: paginationRecoveryFirstPage.nextCursor ?? undefined,
    });
    let paginationInvalidCursorBlocked = false;
    try {
      await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
        approverId: cleanupCheckerId, cursor: Buffer.from("{}", "utf8").toString("base64url"),
      });
    } catch (error) {
      paginationInvalidCursorBlocked = error instanceof Error
        && error.message === "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_CURSOR";
    }
    let paginationExpiredCursorBlocked = false;
    try {
      await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
        approverId: cleanupCheckerId, cursor: paginationRecoveryFirstPage.nextCursor ?? undefined,
        now: new Date(new Date(paginationRecoveryFirstPage.recordedAt).getTime() + 16 * 60_000),
      });
    } catch (error) {
      paginationExpiredCursorBlocked = error instanceof Error
        && error.message === "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_CURSOR_EXPIRED";
    }
    const paginationRemediationCleanup = await deleteShipmentApvInvoiceRestorationRemediationFixtureRows(
      db, [invoicePaginationRestorationRemediationRequestId]);
    const cleanupRecoveryQueue = await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
      approverId: cleanupCheckerId,
    });
    const foreignRecoveryQueue = await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
      approverId: randomUUID(),
    });
    const cleanupRecoveryHealth = await getShipmentApvInvoiceRestorationRemediationHealth(db);
    const cleanupRecoveryAlert = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, undefined, remediationHealthAfter, cleanupRecoveryHealth);
    const wrongCheckerAcknowledgment = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db, {
      requestId: cleanupRemediationRequest.request.id, clientRequestId: randomUUID(),
      decisionRequestId: cleanupDecisionRequestId, checkerId: randomUUID(),
      action: "ACKNOWLEDGED", expectedVersion: 1,
    });
    const incidentBeforeAcknowledgment = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db, {
      requestId: cleanupRemediationRequest.request.id, clientRequestId: randomUUID(),
      decisionRequestId: cleanupDecisionRequestId, checkerId: cleanupCheckerId,
      action: "INCIDENT_LINKED", expectedVersion: 1,
      incidentReference: `PREMATURE-${randomUUID()}`,
    });
    const acknowledgmentClientRequestId = randomUUID();
    const recoveryAcknowledgmentInput = { requestId: cleanupRemediationRequest.request.id,
      clientRequestId: acknowledgmentClientRequestId, decisionRequestId: cleanupDecisionRequestId,
      checkerId: cleanupCheckerId, action: "ACKNOWLEDGED" as const, expectedVersion: 1 };
    const recoveryAcknowledgment = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db,
      recoveryAcknowledgmentInput);
    const recoveryAcknowledgmentReplay = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db,
      recoveryAcknowledgmentInput);
    const acknowledgedOnlyRecoveryHealth = await getShipmentApvInvoiceRestorationRemediationHealth(db);
    const overdueIncidentNow = new Date(Date.now() + 16 * 60_000);
    const overdueIncidentRecoveryHealth = await getShipmentApvInvoiceRestorationRemediationHealth(db,
      overdueIncidentNow);
    const overdueIncidentRecoveryAlert = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, undefined, remediationHealthAfter, overdueIncidentRecoveryHealth);
    const incidentReference = `APV-INCIDENT-${randomUUID()}`;
    const incidentClientRequestId = randomUUID();
    const incidentLinkInput = { requestId: cleanupRemediationRequest.request.id,
      clientRequestId: incidentClientRequestId, decisionRequestId: cleanupDecisionRequestId,
      checkerId: cleanupCheckerId, action: "INCIDENT_LINKED" as const, expectedVersion: 1,
      incidentReference };
    const recoveryIncidentLink = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db,
      incidentLinkInput);
    const recoveryIncidentReplay = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db,
      incidentLinkInput);
    const recoveryIncidentAlreadyRecorded = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db, {
      ...incidentLinkInput, clientRequestId: randomUUID(),
    });
    const recoveryIncidentConflict = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db, {
      ...incidentLinkInput, clientRequestId: randomUUID(), incidentReference: `DIFFERENT-${randomUUID()}`,
    });
    const acknowledgedRecoveryQueue = await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
      approverId: cleanupCheckerId,
    });
    const acknowledgedRecoveryHealth = await getShipmentApvInvoiceRestorationRemediationHealth(db);
    const acknowledgedRecoveryAlert = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, undefined, remediationHealthAfter, acknowledgedRecoveryHealth);
    const agedAcknowledgmentNow = new Date(Date.now() + 31 * 60_000);
    const agedAcknowledgmentRecoveryHealth = await getShipmentApvInvoiceRestorationRemediationHealth(
      db, agedAcknowledgmentNow);
    const agedAcknowledgmentRecoveryAlert = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, undefined, remediationHealthAfter, agedAcknowledgmentRecoveryHealth);
    ...
    const remediationStateAfterAcknowledgment = await db.execute(sql`SELECT status,version
      ...
      WHERE id=${cleanupRemediationRequest.request.id}::uuid`) as unknown as Array<Record<string, unknown>>;
    const cleanupWrongChecker = await decideShipmentApvInvoiceRestorationRemediation(db, {
      requestId: cleanupRemediationRequest.request.id, decisionRequestId: cleanupDecisionRequestId,
      approverId: randomUUID(), decision: "APPROVE",
      reason: "A different checker must not take over a stale applying decision.",
      expectedVersion: 1, storageRoot: invoiceDocumentRoot,
    });
    const cleanupRemediationApproval = await decideShipmentApvInvoiceRestorationRemediation(db, {
      requestId: cleanupRemediationRequest.request.id, decisionRequestId: cleanupDecisionRequestId,
      approverId: cleanupCheckerId,
      decision: "APPROVE", reason: "A separate checker confirmed the source remains missing and closed it safely.",
      expectedVersion: 1, storageRoot: invoiceDocumentRoot,
    });
    const cleanupRemediationTimeline = await getShipmentApvInvoiceRestorationRemediationTimeline(db,
      cleanupRemediationRequest.request.id);
    const expiryRestorationTimeline = await getShipmentApvInvoiceRestorationTimeline(db,
    ...
    const remediationQueueAlertAfter = evaluateShipmentApvPayoutAlert(healthyPayout,
      { expiredThreshold: 1 }, undefined, undefined, remediationStagingHealthAfter, remediationQueueHealthAfter);
    ...
    const rejectedRestorationDecision = await decideShipmentApvInvoiceRestoration(db, {
      requestId: rejectedRestorationRequest.request.id, decisionRequestId: randomUUID(), approverId: randomUUID(),
      decision: "REJECT", reason: "A separate checker rejected restoration but retained supplied bytes for audit.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot,
    });
    const stagingMaintenanceDryRun = await maintainShipmentApvInvoiceRestorationStaging(db, {
      mode: "dry_run", actorId: randomUUID(), storageRoot: invoiceDocumentRoot,
    });
    ...
    const stagingAlertBefore = evaluateShipmentApvPayoutAlert(healthyPayout, { expiredThreshold: 1 },
      undefined, undefined, stagingHealthBefore);
    const stagingMaintenanceJob = await runShipmentApvInvoiceRestorationStagingMaintenance(db, {
      actorId: randomUUID(), storageRoot: invoiceDocumentRoot,
    });
    ...
    const stagingMaintenanceReplayJob = await runShipmentApvInvoiceRestorationStagingMaintenance(db, {
      actorId: randomUUID(), storageRoot: invoiceDocumentRoot,
    });
    ...
    const rejectedRestorationTimeline = await getShipmentApvInvoiceRestorationTimeline(db,
      rejectedRestorationRequest.request.id);
    const retainedRejectedPath = join(invoiceDocumentRoot, ".quarantine", rejectedRestorationRequest.request.id,
      `staged-${invoiceDocumentSha256}.json`);
    const retainedRejectedBytes = await readFile(retainedRejectedPath).catch(() => null);
    const restoreCandidateAfterReject = (await listShipmentApvInvoiceRestorationCandidates(db)).candidates
      .find((item) => item.integrityStatus === "QUARANTINED");
    if (!restoreCandidateAfterReject) throw new Error("APV_INVOICE_RESTORE_AFTER_REJECT_CANDIDATE_MISSING");
    const expiredRestorationRequest = await requestShipmentApvInvoiceRestoration(db, {
      clientRequestId: randomUUID(), candidateId: restoreCandidateAfterReject.candidateId,
      requesterId: randomUUID(),
    ...
    const expirationMaintenance = await maintainShipmentApvInvoiceRestorationStaging(db, {
      mode: "apply", actorId: randomUUID(), storageRoot: invoiceDocumentRoot,
    });
    const expiredRestorationTimeline = await getShipmentApvInvoiceRestorationTimeline(db,
      expiredRestorationRequest.request.id);
    const retainedExpiredPath = join(invoiceDocumentRoot, ".quarantine", expiredRestorationRequest.request.id,
      `staged-${invoiceDocumentSha256}.json`);
    const retainedExpiredBytes = await readFile(retainedExpiredPath).catch(() => null);
    const restoreCandidateAfterExpiry = (await listShipmentApvInvoiceRestorationCandidates(db)).candidates
      .find((item) => item.integrityStatus === "QUARANTINED");
    if (!restoreCandidateAfterExpiry) throw new Error("APV_INVOICE_RESTORE_AFTER_EXPIRY_CANDIDATE_MISSING");
    const finalRestorationMakerId = randomUUID();
    const finalRestorationRequest = await requestShipmentApvInvoiceRestoration(db, {
      clientRequestId: randomUUID(), candidateId: restoreCandidateAfterExpiry.candidateId,
    ...
    const restorationDecisionInput = { requestId: finalRestorationRequest.request.id,
      decisionRequestId: randomUUID(), approverId: randomUUID(), decision: "RESTORE" as const,
      reason: "A separate checker verified the replacement hash and approved restoration.",
      expectedVersion: 0, storageRoot: invoiceDocumentRoot };
    const restorationApproval = await decideShipmentApvInvoiceRestoration(db, restorationDecisionInput);
    const restorationApprovalReplay = await decideShipmentApvInvoiceRestoration(db, restorationDecisionInput);
    const restorationTimeline = await getShipmentApvInvoiceRestorationTimeline(db, finalRestorationRequest.request.id);
    const restoredDocumentBytes = await readShipmentApvInvoiceDocumentBytes(db,
      revisionTwo.revision.id, invoiceDocumentRoot);
    ...
    const stagingAlertAfter = evaluateShipmentApvPayoutAlert(healthyPayout, { expiredThreshold: 1 },
      undefined, undefined, stagingHealthAfter);
    ...
    const revisionTwoApplication = await applyShipmentApvInvoiceRevision(db, {
      ...
      reviewerId: randomUUID(),
      decision: "WAIVED",
      reason: "The revised carrier charge is not supported by enough evidence to assign it to the seller.",
      expectedVersion: 0,
    });
    ...
    const revisionThreeApplication = await applyShipmentApvInvoiceRevision(db, {
      ...
      reason: "Apply the carrier credit to the parties that currently bear the corresponding adjustment.",
      expectedVersion: 0,
    });
    const revisionFourDecisionRequestId = randomUUID();
    const concurrentApplications = await Promise.all(Array.from({ length: 10 }, () => applyShipmentApvInvoiceRevision(db, {
      revisionId: revisionFour.revision.id,
      requestId: revisionFourDecisionRequestId,
      reviewerId: randomUUID(),
      decision: "UPHELD",
      reason: "The revised carrier evidence supports assigning this additional package charge to the seller.",
      expectedVersion: 0,
    })));
    const appliedApplications = concurrentApplications.filter((item) => item.outcome === "applied");
    const duplicateApplications = concurrentApplications.filter((item) => item.outcome === "duplicate");
    ...
    if (creditClaim.outcome !== "acquired") throw new Error(`APV_CHAOS_CREDIT_${creditClaim.outcome}`);
    ...
    const acquiredClaims = concurrentClaims.filter((item): item is ShipmentApvClaim => item.outcome === "acquired");
    ...
    if (recoveryReserve.outcome !== "reserved") throw new Error(`APV_RECOVERY_RESERVE_${recoveryReserve.outcome}`);
    const recoveryNotExpired = await cancelExpiredShipmentApvPayoutOffset(db, {
      settlementReleaseId: carryReleaseId,
      payoutOffsetId: recoveryReserve.offset.id,
      actorId: randomUUID(),
      reason: "A live payout reservation must remain unavailable for cancellation before its deadline.",
      onchainState: "FUNDED",
    });
    ...
    const recoveryStateBlocked = await cancelExpiredShipmentApvPayoutOffset(db, {
      ...
      payoutOffsetId: recoveryReserve.offset.id,
      actorId: randomUUID(),
      reason: "The expired release signature was not executed and the chain state must be verified.",
    ...
    const cancellationReason = "The expired release signature was not executed and the settlement remains funded.";
    ...
    if (!("request" in cancellationRequest)) throw new Error(`APV_CANCEL_REQUEST_${cancellationRequest.outcome}`);
    ...
    const paginationRequest = await requestShipmentApvPayoutCancellation(db, {
      ...
      requesterId: cancellationRequesterId,
      reason: "A second pending request verifies stable opaque cursor pagination and automatic expiry auditing.",
      now: paginationRequestNow,
    });
    if (!("request" in paginationRequest)) throw new Error(`APV_CANCEL_PAGINATION_${paginationRequest.outcome}`);
    ...
    const pendingCancellationPageOne = await listPendingShipmentApvPayoutCancellations(db, { limit: 1 });
    const pendingCancellationPageTwo = pendingCancellationPageOne.nextCursor
      ? await listPendingShipmentApvPayoutCancellations(db, { limit: 1, cursor: pendingCancellationPageOne.nextCursor })
    ...
    const approvalResults = await Promise.all(Array.from({ length: 10 }, () => decideShipmentApvPayoutCancellation(db, {
      requestId: cancellationRequestRecord.id,
      payoutOffsetId: recoveryReserve.offset.id,
      settlementReleaseId: carryReleaseId,
      ...
      approverId: cancellationApproverId,
      decision: "APPROVE",
      reason: "A different administrator verified the expired signature and funded onchain state.",
      expectedVersion: 0,
      onchainState: "FUNDED",
    })));
    ...
    const paginationRetryRequest = await requestShipmentApvPayoutCancellation(db, {
      ...
      settlementReleaseId: paginationReleaseId,
      requesterId: cancellationRequesterId,
      reason: "The expired approval is retried through a new maker-checker request without reusing its decision.",
      now: paginationRetryNow,
    });
    if (!("request" in paginationRetryRequest)) throw new Error(`APV_CANCEL_RETRY_${paginationRetryRequest.outcome}`);
    ...
    const paginationRetryDecision = await decideShipmentApvPayoutCancellation(db, {
      ...
      approverId: cancellationApproverId,
      decision: "APPROVE",
      reason: "A different administrator approved the replacement request after the first approval expired.",
      expectedVersion: 0,
      onchainState: "FUNDED",
    ...
    `) as unknown as Array<Record<string, unknown>>;
    const lifecycleTimeline = await getShipmentApvPayoutCancellationTimeline(db, cancellationRequestRecord.id);
    ...
    const signedLifecycleExportValid = verifySignedShipmentApvPayoutCancellationAuditExport(signedLifecycleExport);
    ...
    const deliveredAuditArchive = await getShipmentApvCancellationAuditArchiveStatus(db, cancellationRequestRecord.id);
    const auditArchiveHealthAfter = await getShipmentApvCancellationAuditArchiveHealth(db);
    const retryTimeline = await getShipmentApvPayoutCancellationTimeline(db, paginationRetryRecord.id);
    ...
    const deadLetterDispatch = await dispatchShipmentApvCancellationAuditArchives(db, {
      config: { url: "http://127.0.0.1/audit-archive", timeoutMs: 5_000, maxAttempts: 1, allowInsecureHttp: true, allowPrivateNetwork: true },
      fetchImpl: async () => new Response(JSON.stringify({ receipt_id: "mismatch", stored_sha256: "0".repeat(64) }), { status: 201 }),
      now: new Date(), limit: 10,
    });
    const deadLetterHealth = await getShipmentApvCancellationAuditArchiveHealth(db);
    const archiveFailureQueueBefore = await listShipmentApvCancellationAuditArchiveFailures(db, { limit: 20 });
    ...
    const recoveryDispatch = await dispatchShipmentApvCancellationAuditArchives(db, {
      config: { url: "http://127.0.0.1/audit-archive", timeoutMs: 5_000, maxAttempts: 3, allowInsecureHttp: true, allowPrivateNetwork: true },
      fetchImpl: async (_url, init) => {
        const headers = init?.headers as Record<string, string>;
        return new Response(JSON.stringify({ receipt_id: "worm_recovered_receipt", stored_sha256: headers["x-haggle-content-sha256"] }), { status: 201 });
      },
      now: new Date(), limit: 10,
    });
    const recoveredArchiveHealth = await getShipmentApvCancellationAuditArchiveHealth(db);
    const archiveFailureQueueAfter = await listShipmentApvCancellationAuditArchiveFailures(db, { limit: 20 });
    ...
    `);
    const tamperedLifecycleTimeline = await getShipmentApvPayoutCancellationTimeline(db, cancellationRequestRecord.id);
    ...
    const recoveredRecoveryQueue = await listExpiredShipmentApvPayoutReservations(db, { limit: 20 });
    ...
    const checks = {
      ...Object.fromEntries(Object.entries(retentionAlertFixture.checks)
        .map(([key, value]) => [`cursor_retention_alert_${key}`, value])),
      buffer_capped: overBuffer.buffer_applied_minor === 150,
      seller_liability_recorded: overBuffer.seller_liability_minor === 250 && overBuffer.status === "REVIEW_REQUIRED",
      ...
      seller_review_recorded: sellerReview.outcome === "updated" && sellerReview.record.review_status === "PENDING",
      ...
      review_decision_single_winner: decisionWinners.length === 1
        && decisionDuplicates.length + decisionConflicts.length === 9,
      review_decision_retry_idempotent: decisionReplay.outcome === "duplicate",
      waived_liability_balanced: finalReview?.review_status === "WAIVED"
      ...
      revision_positive_delta: revisionTwo.outcome === "recorded"
        && revisionTwo.revision.revision_number === 2 && revisionTwo.revision.delta_minor === 100,
      revision_replay_idempotent: revisionTwoReplay.outcome === "duplicate",
      revision_negative_delta: revisionThree.outcome === "recorded"
        && revisionThree.revision.revision_number === 3 && revisionThree.revision.delta_minor === -50,
      revision_concurrent_serialized: recordedRevisions.length === 1 && duplicateRevisions.length === 9,
      revision_money_effect_zero: revisionTwo.outcome === "recorded"
        && revisionThree.outcome === "recorded"
        && revisionTwo.revision.buyer_effect_minor === 0
        && revisionThree.revision.buyer_effect_minor === 0,
      revision_waiver_applied: revisionTwoApplication.outcome === "applied"
      ...
      revision_credit_applied: revisionThreeApplication.outcome === "applied"
        && revisionThreeApplication.revision.carrier_credit_minor === 50,
      revision_application_single_winner: appliedApplications.length === 1 && duplicateApplications.length === 9,
      revision_aggregate_balanced: Number(finalAdjustment?.adjusted_rate_minor) === 1175
      ...
      revision_evidence_bound: revisionTwoEvidence.outcome === "bound"
        && revisionThreeEvidence.outcome === "bound" && revisionFourEvidence.outcome === "bound",
      ...
      revision_invoice_document_bytes_verified: invoiceDocumentReadBytes?.equals(revisionTwoDocumentBytes) === true,
      revision_invoice_storage_healthy_before: invoiceDocumentHealthBefore.status === "healthy",
      revision_invoice_storage_orphan_detected: invoiceDocumentHealthWithOrphan.status === "warning"
        && invoiceDocumentHealthWithOrphan.orphanFiles === 1,
      revision_invoice_storage_dry_run_safe: invoiceDocumentReconciliation.dryRun
      ...
      revision_invoice_storage_warning_alerted: invoiceDocumentAlert.severity === "warning"
      ...
      revision_invoice_reconciliation_candidate_opaque: reconciliationCandidates.candidates.length >= 1
        && orphanCandidate.candidateId.length === 64 && !JSON.stringify(orphanCandidate).includes("storage"),
      revision_invoice_reconciliation_requested: reconciliationRequest.outcome === "requested"
      ...
      revision_invoice_reconciliation_checker_approved: reconciliationApproval.outcome === "approved",
      revision_invoice_reconciliation_idempotent: reconciliationApprovalReplay.outcome === "duplicate",
      revision_invoice_reconciliation_quarantined: quarantinedOrphanBytes?.equals(orphanBytes) === true,
      revision_invoice_reconciliation_lifecycle: reconciliationTimeline?.events.map((event) => event.event_type).join(",")
        === "REQUESTED,APPLYING,APPROVED",
      revision_invoice_corruption_detected: corruptedDocumentHealth.status === "critical"
        && corruptedDocumentHealth.hashMismatches === 1,
      revision_invoice_corruption_quarantined: corruptApproval.outcome === "approved"
        && corruptQuarantineBytes?.equals(corruptedDocumentBytes) === true,
      revision_invoice_quarantine_remains_visible: quarantinedDocumentHealth.status === "warning"
        && quarantinedDocumentHealth.quarantinedDocuments === 1,
      revision_invoice_restoration_candidate_opaque: restorationCandidate.candidateId.length === 64
        && !JSON.stringify(restorationCandidate).includes("document"),
      revision_invoice_restoration_mismatch_blocked: restorationMismatch.outcome === "replacement_mismatch",
      revision_invoice_restoration_request_idempotent: restorationRequest.outcome === "requested"
      ...
      revision_invoice_restoration_preserve_idempotent: preservationApprovalReplay.outcome === "duplicate",
      ...
      revision_invoice_restoration_preserve_lifecycle:
        preservationTimeline?.events.map((event) => event.event_type).join(",") === "REQUESTED,APPLYING,PRESERVED",
      revision_invoice_remediation_corruption_detected: remediationHealthBefore.status === "critical"
        && remediationHealthBefore.hashMismatches === 1,
      revision_invoice_remediation_candidate_opaque: remediationCandidate.candidateId.length === 64
        && !JSON.stringify(remediationCandidate).includes("request")
        && !JSON.stringify(remediationCandidate).includes("sha256"),
      revision_invoice_remediation_request_idempotent: remediationRequest.outcome === "requested"
        && remediationRequestReplay.outcome === "duplicate",
      revision_invoice_remediation_pending_queue: pendingRemediations.some((item) =>
        item.id === remediationRequest.request.id && item.status === "PENDING"),
      ...
      revision_invoice_remediation_decision_idempotent: remediationApprovalReplay.outcome === "duplicate",
      ...
      revision_invoice_remediation_health_recovered: remediationHealthAfter.status === "healthy"
        && remediationHealthAfter.trackedStaging === 0,
      ...
      revision_invoice_remediation_expiry_worker_converged:
        remediationExpiryReplay.status === "skipped" && remediationExpiryReplay.reason === "healthy",
      revision_invoice_remediation_expiry_lifecycle:
        expiredRemediationTimeline?.events.map((event) => event.event_type).join(",") === "REQUESTED,EXPIRED",
      ...
      revision_invoice_remediation_recovery_pagination_second_page:
        ...
        && paginationRecoverySecondPage.items[0]?.requestId === invoicePaginationRestorationRemediationRequestId
      ...
      revision_invoice_remediation_recovery_queue_opaque:
        !/path|sha256|candidate|restorationRequestId|requester|reason/i.test(JSON.stringify(cleanupRecoveryQueue)),
      ...
      revision_invoice_remediation_recovery_incident_hash_bound:
        ...
        && recoveryAcknowledgmentRows.find((row) => row.action === "INCIDENT_LINKED")?.incident_reference_hash
      ...
      revision_invoice_remediation_recovery_alert_aggregate_only:
        ...
        && cleanupRecoveryAlert.reasons.includes("invoice_restoration_remediation_stale_applying_15m")
        && cleanupRecoveryAlert.reasons.includes("invoice_restoration_remediation_stale_applying_60m")
        && cleanupRecoveryAlert.reasons.includes("invoice_restoration_remediation_unacknowledged_60m")
        && cleanupRecoveryAlert.reasons.includes("invoice_restoration_remediation_incident_unlinked_60m")
      ...
      revision_invoice_remediation_recovery_handling_alert_cleared:
        ...
        && !acknowledgedRecoveryAlert.reasons.includes("invoice_restoration_remediation_unacknowledged_60m")
        && !acknowledgedRecoveryAlert.reasons.includes("invoice_restoration_remediation_incident_unlinked_60m"),
      revision_invoice_remediation_recovery_base_stale_alert_preserved:
        acknowledgedRecoveryAlert.severity === "critical"
        && acknowledgedRecoveryAlert.reasons.includes("invoice_restoration_remediation_stale_applying")
        && acknowledgedRecoveryAlert.reasons.includes("invoice_restoration_remediation_stale_applying_15m")
        && acknowledgedRecoveryAlert.reasons.includes("invoice_restoration_remediation_stale_applying_60m"),
      ...
      revision_invoice_remediation_recovery_incident_sla_overdue:
        ...
        && !/requestId|decisionRequestId|approverId|path|sha256|candidate/i.test(
          JSON.stringify({ health: overdueIncidentRecoveryHealth, alert: overdueIncidentRecoveryAlert })),
      revision_invoice_remediation_recovery_aged_ack_realerted:
        ...
        && agedAcknowledgmentRecoveryAlert.reasons.includes(
          "invoice_restoration_remediation_incident_linked_still_applying_30m")
        && !/requestId|decisionRequestId|approverId|path|sha256|candidate/i.test(
          JSON.stringify({ health: agedAcknowledgmentRecoveryHealth, alert: agedAcknowledgmentRecoveryAlert })),
      ...
      revision_invoice_remediation_expiry_alert_cleared: remediationQueueAlertAfter.wouldAlert === false,
      revision_invoice_restoration_rejected: rejectedRestorationDecision.outcome === "rejected",
      revision_invoice_restoration_staging_dry_run: !("outcome" in stagingMaintenanceDryRun)
        && stagingMaintenanceDryRun.eligible === 1 && stagingMaintenanceDryRun.preserved === 0,
      revision_invoice_restoration_staging_preserved: !("outcome" in stagingMaintenance)
        && stagingMaintenance.preserved === 1 && stagingMaintenance.resumed === 1
        && stagingMaintenance.conflicts === 0
        && retainedRejectedBytes?.equals(revisionTwoDocumentBytes) === true,
      revision_invoice_restoration_staging_maintenance_idempotent: !("outcome" in stagingMaintenanceReplay)
        && stagingMaintenanceReplay.eligible === 0 && stagingMaintenanceReplay.preserved === 0,
      revision_invoice_restoration_staging_worker_completed: stagingMaintenanceJob.status === "completed",
      revision_invoice_restoration_staging_worker_converged: stagingMaintenanceReplayJob.status === "skipped"
      ...
      revision_invoice_restoration_expiration_dry_run: !("outcome" in expirationMaintenanceDryRun)
        && expirationMaintenanceDryRun.eligible === 1 && expirationMaintenanceDryRun.preserved === 0,
      revision_invoice_restoration_expired_staging_preserved: !("outcome" in expirationMaintenance)
        && expirationMaintenance.expired === 1 && expirationMaintenance.preserved === 1
      ...
      revision_invoice_restoration_staging_health_warning: stagingHealthBefore.status === "warning"
        && stagingHealthBefore.pendingDisposition === 1 && stagingHealthBefore.staleMoving === 1,
      revision_invoice_restoration_staging_alert_firing: stagingAlertBefore.severity === "warning"
        && stagingAlertBefore.reasons.includes("invoice_restoration_staging_stale"),
      revision_invoice_restoration_staging_health_recovered: stagingHealthAfter.status === "healthy"
        && stagingHealthAfter.trackedStaging === 0,
      ...
      revision_invoice_restoration_decision_idempotent: restorationApprovalReplay.outcome === "duplicate",
      revision_invoice_restoration_bytes_verified: restoredDocumentBytes?.equals(revisionTwoDocumentBytes) === true,
      revision_invoice_restoration_lifecycle: restorationTimeline?.events.map((event) => event.event_type).join(",")
      ...
      payout_offset_reserved: payoutOffset.outcome === "reserved"
      ...
      payout_completion_idempotent: payoutApplied.outcome === "applied"
        && payoutAppliedReplay.outcome === "duplicate" && payoutTxConflict.outcome === "snapshot_conflict",
      payout_freezes_new_apv: frozenClaim.outcome === "payout_reserved" && frozenRevision.outcome === "payout_reserved",
      payout_carry_queue_partial: carryQueueBefore.some((item) => item.status === "PARTIAL" && item.remaining_amount_minor === 40),
      payout_carry_forward_reserved: carryPayout.outcome === "reserved"
        && carryPayout.offset.applied_offset_minor === 40 && carryPayout.offset.unapplied_liability_minor === 0,
      payout_carry_forward_settled: carryPayoutApplied.outcome === "applied"
        && carryQueueAfter.some((item) => item.status === "SETTLED" && item.remaining_amount_minor === 0),
      ...
      payout_recovery_requires_funded_state: recoveryStateBlocked.outcome === "onchain_state_conflict",
      payout_recovery_cancelled_and_retried: recoveryCancelled.outcome === "cancelled"
        && recoveryCancelled.offset.status === "CANCELLED" && carryPayout.outcome === "reserved",
      payout_expired_health_detected: expiredReservationHealth.status === "attention"
        && expiredReservationHealth.expiredReserved === 1 && expiredReservationHealth.signedExpired === 1,
      payout_expired_health_recovered: recoveredReservationHealth.status === "healthy"
        && recoveredReservationHealth.expiredReserved === 0,
      payout_recovery_audit_atomic: recoveryAuditCount === 1,
      payout_recovery_queue_detected: expiredRecoveryQueue.items.length === 1
      ...
      payout_cancel_request_idempotent: cancellationRequest.outcome === "requested"
        && cancellationRequestReplay.outcome === "duplicate" && cancellationRequestConflict.outcome === "request_conflict",
      payout_cancel_self_approval_blocked: selfApproval.outcome === "self_approval_forbidden",
      payout_cancel_checker_revalidates_chain: approvalStateBlocked.outcome === "onchain_state_conflict",
      payout_cancel_single_checker_winner: approvalWinners.length === 1 && approvalReplays.length === 9,
      payout_cancel_pending_queue_visible: pendingCancellationQueue.items.some((item) => item.id === cancellationRequestRecord.id),
      payout_cancel_queue_cursor_stable: pendingCancellationPageOne.items.length === 1
      ...
      payout_cancel_approval_health_detected: pendingCancellationHealth.status === "attention"
        && pendingCancellationHealth.pendingRequests === 2,
      payout_cancel_approval_health_recovered: recoveredCancellationHealth.status === "healthy"
        && recoveredCancellationHealth.pendingRequests === 0,
      payout_cancel_expiry_system_audited: paginationExpired.outcome === "expired"
        && lifecycleEventRows.some((event) => event.cancellation_request_id === paginationRequestRecord.id
          && event.event_type === "EXPIRED" && event.actor_id === null && Number(event.request_version) === 1),
      payout_cancel_expiry_retry_approved: paginationRetryRequest.outcome === "requested"
        && paginationRetryDecision.outcome === "approved",
      payout_cancel_lifecycle_complete: lifecycleEventRows.length === 6
        ...
        && lifecycleEventRows.some((event) => event.cancellation_request_id === cancellationRequestRecord.id
          && event.event_type === "APPROVED" && event.actor_id === cancellationApproverId),
      payout_cancel_hash_chain_valid: lifecycleTimeline.integrity.valid
      ...
      payout_cancel_signed_export_valid: signedLifecycleExportValid
      ...
      payout_cancel_archive_idempotent: auditArchiveEnqueue.outcome === "enqueued"
        && auditArchiveReplay.outcome === "duplicate"
        && auditArchiveEnqueue.archive.id === auditArchiveReplay.archive.id,
      payout_cancel_archive_receipt_verified: archiveDispatch.status === "processed"
        && archiveDispatch.delivered === 1 && archiveDispatch.failed === 0
        && deliveredAuditArchive?.status === "DELIVERED"
        && deliveredAuditArchive.receiptSha256 === deliveredAuditArchive.payloadSha256,
      payout_cancel_archive_health_detected: auditArchiveHealthBefore.pending === 1
        && auditArchiveHealthBefore.oldestUnfinishedAgeSeconds !== null,
      payout_cancel_archive_overdue_detected: auditArchiveHealthBefore.status === "attention"
        && auditArchiveHealthBefore.overdueUnfinished === 1
        && (auditArchiveHealthBefore.oldestUnfinishedAgeSeconds ?? 0) >= 15 * 60,
      payout_cancel_archive_health_recovered: auditArchiveHealthAfter.pending === 0
        && auditArchiveHealthAfter.processing === 0 && auditArchiveHealthAfter.failed === 0
        && auditArchiveHealthAfter.deadLetter === 0,
      payout_cancel_archive_dead_letter_detected: deadLetterDispatch.status === "processed"
        && deadLetterDispatch.deadLettered === 1 && deadLetterHealth.status === "critical"
        && deadLetterHealth.deadLetter === 1,
      payout_cancel_archive_failure_queue_detected: archiveFailureQueueBefore.items.length === 1
      ...
      payout_cancel_archive_requeue_audited: archiveRequeue.outcome === "requeued"
        && archiveRequeueReplay.outcome === "already_queued" && archiveRequeueAuditCount === 1,
      payout_cancel_archive_requeue_recovered: recoveryDispatch.status === "processed"
        && recoveryDispatch.delivered === 1 && recoveredArchiveHealth.status === "healthy"
    ...
    return {
      ...
      revisions: {
        ...
        concurrent: {
          ...
          revision: recordedRevisions[0] && "revision" in recordedRevisions[0]
        ...
        applications: {
          ...
          aggregate: finalAdjustment ? {
            ...
            assessed_seller_liability_minor: Number(finalAdjustment.assessed_seller_liability_minor),
        ...
        evidence: {
          ...
          invoiceDocument: { stored: invoiceDocumentStored, replay: invoiceDocumentReplay.outcome,
            conflict: invoiceDocumentConflict.outcome, bytesVerified: invoiceDocumentReadBytes?.equals(revisionTwoDocumentBytes) === true,
            storageHealth: { before: invoiceDocumentHealthBefore, withOrphan: invoiceDocumentHealthWithOrphan,
              dryRun: invoiceDocumentReconciliation, after: invoiceDocumentHealthAfter,
              alert: invoiceDocumentAlert, alertAfter: invoiceDocumentAlertAfter,
              reconciliation: { candidates: reconciliationCandidates, request: reconciliationRequest,
                selfApproval: reconciliationSelfApproval.outcome, approval: reconciliationApproval,
                replay: reconciliationApprovalReplay.outcome, timeline: reconciliationTimeline,
                quarantinedBytesVerified: quarantinedOrphanBytes?.equals(orphanBytes) === true },
              restoration: { corruptedHealth: corruptedDocumentHealth, corruptRequest,
                corruptApproval, corruptBytesVerified: corruptQuarantineBytes?.equals(corruptedDocumentBytes) === true,
                quarantinedHealth: quarantinedDocumentHealth,
                candidates: restorationCandidates, mismatch: restorationMismatch.outcome,
                request: restorationRequest, requestReplay: restorationRequestReplay.outcome,
                selfApproval: restorationSelfApproval.outcome,
                preservation: { approval: preservationApproval, replay: preservationApprovalReplay.outcome,
                  timeline: preservationTimeline,
                  bytesVerified: preservedReplacementBytes?.equals(revisionTwoDocumentBytes) === true },
                remediation: { restorationRejection: remediatedRestorationRejection,
                  healthBefore: remediationHealthBefore, candidates: remediationCandidates,
                  request: remediationRequest, requestReplay: remediationRequestReplay.outcome,
                  pendingQueueCount: pendingRemediations.length,
                  selfApproval: remediationSelfApproval.outcome, approval: remediationApproval,
                  approvalReplay: remediationApprovalReplay.outcome, timeline: remediationTimeline,
                  restorationTimeline: remediatedRestorationTimeline,
                  bytesQuarantined: remediatedQuarantineBytes?.equals(corruptedStagingBytes) === true,
                  healthAfter: remediationHealthAfter,
                  expiry: { restorationRejection: expiryRestorationRejection,
                    healthBefore: remediationQueueHealthBefore, alertBefore: remediationQueueAlertBefore,
                    job: remediationExpiryJob,
                    replay: remediationExpiryReplay, timeline: expiredRemediationTimeline,
                    cleanupApproval: cleanupRemediationApproval,
                    recovery: { queue: cleanupRecoveryQueue, foreignQueue: foreignRecoveryQueue,
                      pagination: { first: paginationRecoveryFirstPage, second: paginationRecoverySecondPage,
                      ...
                      health: cleanupRecoveryHealth, alert: cleanupRecoveryAlert,
                    ...
                    healthAfter: remediationQueueHealthAfter, alertAfter: remediationQueueAlertAfter,
                    stagingHealthAfter: remediationStagingHealthAfter } },
                stagingRetention: { rejected: rejectedRestorationDecision,
                  dryRun: stagingMaintenanceDryRun, apply: stagingMaintenance,
                  replay: stagingMaintenanceReplay, job: { first: stagingMaintenanceJob.status,
                    replay: stagingMaintenanceReplayJob.status, replayReason: stagingMaintenanceReplayJob.reason },
                  timeline: rejectedRestorationTimeline,
                  bytesVerified: retainedRejectedBytes?.equals(revisionTwoDocumentBytes) === true,
                  expiration: { request: expiredRestorationRequest, dryRun: expirationMaintenanceDryRun,
                    apply: expirationMaintenance, timeline: expiredRestorationTimeline,
                    bytesVerified: retainedExpiredBytes?.equals(revisionTwoDocumentBytes) === true },
                  health: { before: stagingHealthBefore, alertBefore: stagingAlertBefore,
                    after: stagingHealthAfter, alertAfter: stagingAlertAfter } },
                finalRequest: finalRestorationRequest, approval: restorationApproval,
                approvalReplay: restorationApprovalReplay.outcome, timeline: restorationTimeline,
                restoredBytesVerified: restoredDocumentBytes?.equals(revisionTwoDocumentBytes) === true,
      ...
      payout: {
        ...
        signedSplit: payoutOffset.outcome === "reserved" ? {
          gross_minor: 10000,
        ...
        carryForward: {
          recovery: {
            ...
            makerChecker: {
              ...
              approvals: { requests: approvalResults.length, approved: approvalWinners.length, replay: approvalReplays.length },
              ...
              archive: deliveredAuditArchive ? {
                ...
                receiptMatches: deliveredAuditArchive.receiptSha256 === deliveredAuditArchive.payloadSha256,
    ...
    const reconciliationRequestIds = [invoiceReconciliationRequestId, invoiceCorruptReconciliationRequestId]
      .filter((value): value is string => Boolean(value));
    const restorationRemediationRequestIds = [invoiceRestorationRemediationRequestId,
    ...
    const deletedInvoiceRestorationRemediation = restorationRemediationRequestIds.length
      ? await deleteShipmentApvInvoiceRestorationRemediationFixtureRows(db, restorationRemediationRequestIds)
      : { acknowledgments: 0, events: 0, requests: 0 };
    const restorationRequestIds = [invoicePreservationRequestId, invoiceRemediatedRestorationRequestId,
    ...
    const deletedInvoiceDocument = invoiceDocumentRevisionId
      ? await deleteShipmentApvInvoiceDocumentForFixture(db, invoiceDocumentRevisionId, invoiceDocumentRoot)
      : false;
    ...
    const deletedLiabilities = await db.execute(sql`
      ...
      RETURNING id
    `) as unknown as Array<Record<string, unknown>>;
    const deletedAdjustments = await db.delete(shipmentApvAdjustments)
      .where(eq(shipmentApvAdjustments.shipmentId, shipmentId)).returning({ id: shipmentApvAdjustments.id });
    const deletedShipments = await db.delete(shipments)
      .where(eq(shipments.id, shipmentId)).returning({ id: shipments.id });
    ...
    Object.assign(cleanup, {
      ...
      succeeded: deletedOffsets.length === 4
