...
export const getPriceBand = _getPriceBand;
 ...
 * Delegates to a category-aware registry (Step 62). The default builder
 * is a verbatim copy of the previous inline implementation, so behavior
 * is unchanged for uncategorized listings.
 */
export function buildEmbeddingInput(snapshot: Record<string, unknown>): string {
  return resolveEmbeddingBuilder(snapshot)(snapshot);
}
