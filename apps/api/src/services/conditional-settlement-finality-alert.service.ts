...
export const CONDITIONAL_SETTLEMENT_FINALITY_ALERT_SOURCE = "haggle-conditional-settlement-finality-alert";
...
function strictInteger(raw: string | undefined, fallback: number, min: number, max: number, label: string): number {
...
export function assertConditionalSettlementFinalityAlertTimingSafe(timeoutMs: number) {
  ...
  if (timeoutMs > maxSafeTimeoutMs) throw new Error(`finality alert timeout must be <= ${maxSafeTimeoutMs}ms`);
  return { timeoutMs, claimLeaseSeconds, maxSafeTimeoutMs, safetyMarginMs: CLAIM_LEASE_SAFETY_MARGIN_MS, timingSafe: true };
}
...
export function getConditionalSettlementFinalityAlertPolicyStatus() {
  ...
  let timingSafe = true;
  try {
    timeoutMs = strictInteger(process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_TIMEOUT_MS, 5000, 250, 30_000, "finality alert timeout");
  ...
  if (configurationState === "valid" && url) {
    try {
      assertDisputeModuleOutboundUrl(url, { label: "conditional settlement finality alert",
        allowInsecureHttp: process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_ALLOW_INSECURE_HTTP === "true",
        allowPrivateNetwork: process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_ALLOW_PRIVATE_NETWORK === "true" });
  ...
  return { configured: configurationState === "valid", configurationState,
    ...
    cooldownMinutes: boundedInteger(process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_COOLDOWN_MINUTES, 15, 1, 1440),
    timeoutMs, claimLeaseSeconds: getWebhookEventClaimLeaseSeconds(), timingSafe };
}
...
export function resolveConditionalSettlementFinalityAlertConfigFromEnv(): ConditionalSettlementFinalityAlertConfig | null {
  ...
  const timeoutMs = strictInteger(process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_TIMEOUT_MS, 5000, 250, 30_000, "finality alert timeout");
  assertConditionalSettlementFinalityAlertTimingSafe(timeoutMs);
  const config = { url, secret, timeoutMs,
    cooldownMinutes: boundedInteger(process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_COOLDOWN_MINUTES, 15, 1, 1440),
    allowInsecureHttp: process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_ALLOW_INSECURE_HTTP === "true",
    allowPrivateNetwork: process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_ALLOW_PRIVATE_NETWORK === "true" };
  assertDisputeModuleOutboundUrl(url, { label: "conditional settlement finality alert",
...
export function evaluateConditionalSettlementFinalityAlert(health: ConditionalSettlementFinalityHealth): ConditionalSettlementFinalityAlertAssessment {
  const reasons = [
  ...
  return { wouldAlert: reasons.length > 0, severity: reasons.includes("orphaned_receipt") ? "critical" : reasons.length ? "warning" : null, reasons };
}

export async function findLatestDeliveredConditionalSettlementFinalityIncident(db: Database, source = CONDITIONAL_SETTLEMENT_FINALITY_ALERT_SOURCE) {
...
export async function getConditionalSettlementFinalityAlertDeliveryState(db: Database, source = CONDITIONAL_SETTLEMENT_FINALITY_ALERT_SOURCE) {
...
export async function sendConditionalSettlementFinalityAlert(health: ConditionalSettlementFinalityHealth,
  assessment: ConditionalSettlementFinalityAlertAssessment,
  options: { config: ConditionalSettlementFinalityAlertConfig; deliveryId: string; fetchImpl?: typeof fetch; now?: Date }) {
  assertConditionalSettlementFinalityAlertTimingSafe(options.config.timeoutMs);
  if (!/^(?:health|recovery)_[0-9a-f]{64}$/.test(options.deliveryId)) throw new Error("invalid finality alert delivery id");
  assertDisputeModuleOutboundUrl(options.config.url, { label: "conditional settlement finality alert",
    allowInsecureHttp: options.config.allowInsecureHttp, allowPrivateNetwork: options.config.allowPrivateNetwork });
  const timestamp = (options.now ?? new Date()).toISOString();
  const rawBody = JSON.stringify({ type: "conditional_settlement_finality.health", delivery_id: options.deliveryId,
  ...
  try {
    const response = await (options.fetchImpl ?? fetch)(options.config.url, { method: "POST", redirect: "error", signal: controller.signal,
      headers: { "content-type": "application/json", "x-haggle-alert-type": "conditional_settlement_finality.health",
  ...
  } catch { return { status: "failed" as const, error: "request_failed" as const }; }
