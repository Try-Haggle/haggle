...
} from "./dispute-evidence-scan-retry-alert-snapshot-retention.service.js";
import {
...
export async function runDisputeEvidenceScanRetryAlertSnapshotRetentionFixture(
  db: Database,
) {
  const fixtureId = randomUUID();
  const source = `haggle-scan-retry-retention-fixture-${fixtureId}`;
  const deliveryIds = ["completed", "failed", "orphan"].map((kind) =>
    `health_${createHash("sha256").update(`${fixtureId}:${kind}`).digest("hex")}`);
  ...
  try {
    ...
    const completedClaim = await claimWebhookEvent(db, {
      ...
      payloadSha256: createHash("sha256")
        .update(JSON.stringify(snapshot(deliveryIds[0]!))).digest("hex"),
    });
    ...
    const failedClaim = await claimWebhookEvent(db, {
      ...
      payloadSha256: createHash("sha256")
        .update(JSON.stringify(snapshot(deliveryIds[1]!))).digest("hex"),
    });
    ...
    const before =
      await getDisputeEvidenceScanRetryAlertSnapshotRetentionHealth(db, source);
    ...
    await lockAcquired;
    const competitors = await Promise.all(Array.from({ length: 19 }, () =>
      runDisputeEvidenceScanRetryAlertSnapshotRetentionJob(db, {
        batchSize: 100,
      })));
    releaseLock();
    const winner = await winnerPromise;
    const lockBlocked = competitors.filter((item) =>
      item.status === "skipped"
      && item.reason === "retention_run_in_progress").length;
    const completedJobHealth =
      await getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth(db);
    const after =
      await getDisputeEvidenceScanRetryAlertSnapshotRetentionHealth(db, source);
    ...
    await db.execute(sql`
      UPDATE dispute_evidence_scan_retry_alert_snapshot_retention_state
         SET status = 'RUNNING', claim_id = ${staleClaimId}::uuid,
             lease_expires_at = ${new Date(staleNow.getTime() - 1_000)
               .toISOString()}::timestamptz,
             last_started_at = ${new Date(staleNow.getTime() - 901_000)
               .toISOString()}::timestamptz,
    ...
    const recoveredJobHealth =
      await getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth(db);
    const serialized = JSON.stringify({ before, after, winner, competitors,
      completedJobHealth, recoveredJobHealth });
    const checks = {
      expiredClassificationExact: before.eligibleExpired === 1
        && before.blockedExpired === 2 && before.status === "attention",
      distributedLockSingleWinner: winner.status === "executed"
        && winner.deleted === 1 && lockBlocked === 19,
      ...
      postRunHealthAccurate: after.eligibleExpired === 0
        && after.blockedExpired === 2 && after.status === "attention",
      boundedBatchApplied: winner.status === "executed"
        && winner.batchSize === 100,
      persistentJobRunRecorded:
        completedJobHealth.lastRunStatus === "SUCCEEDED"
        && completedJobHealth.lastDeletedSnapshots === 1,
      staleJobLeaseReclaimed: reclaimed.status === "executed"
        && reclaimed.deleted === 0
        && recoveredJobHealth.lastRunStatus === "SUCCEEDED",
      staleJobOwnerFenced: staleOwnerRows.length === 0
        && recoveredJobHealth.lastDeletedSnapshots === 0,
      identifiersExcluded: !serialized.includes(fixtureId)
    ...
    report = {
      ...
      execution: {
        ...
        persistentJobRuns: completedJobHealth.lastRunStatus === "SUCCEEDED"
          ? 1 : 0,
      ...
      await db.execute(sql`
        UPDATE dispute_evidence_scan_retry_alert_snapshot_retention_state
           SET status = ${String(savedJob.status)},
               claim_id = ${savedJob.claim_id == null
                 ? null : String(savedJob.claim_id)}::uuid,
               lease_expires_at = ${savedJob.lease_expires_at == null
                 ? null : new Date(String(savedJob.lease_expires_at)).toISOString()}::timestamptz,
               first_observed_at = ${new Date(String(savedJob.first_observed_at))
                 .toISOString()}::timestamptz,
               last_started_at = ${savedJob.last_started_at == null
                 ? null : new Date(String(savedJob.last_started_at)).toISOString()}::timestamptz,
               last_succeeded_at = ${savedJob.last_succeeded_at == null
                 ? null : new Date(String(savedJob.last_succeeded_at)).toISOString()}::timestamptz,
               last_failed_at = ${savedJob.last_failed_at == null
                 ? null : new Date(String(savedJob.last_failed_at)).toISOString()}::timestamptz,
               last_deleted_snapshots = ${Number(
                 savedJob.last_deleted_snapshots ?? 0)},
               last_failure_code = ${savedJob.last_failure_code == null
                 ? null : String(savedJob.last_failure_code)},
               updated_at = ${new Date(String(savedJob.updated_at))
                 .toISOString()}::timestamptz
         WHERE job_key = 'snapshot_retention'
      `);
