...
const FIRING_REASONS = new Set([
  "orphaned_receipt",
  "rpc_unavailable",
  "configuration_blocked",
  "confirmation_sla_overdue",
]);
export const CONDITIONAL_SETTLEMENT_FINALITY_ALERT_RECEIVER_SOURCE =
  "haggle-conditional-settlement-finality-alert-receiver";
...
export function resolveConditionalSettlementFinalityAlertReceiverSecretsFromEnv() {
  ...
  if (!current && previous.length)
    throw new Error(
      "finality alert current secret is required when previous secrets are configured",
    );
  const secrets = current ? [current, ...previous] : [];
  if (secrets.some((item) => item.length < MIN_SECRET_LENGTH || item.length > MAX_SECRET_LENGTH)) {
    throw new Error(
      `finality alert receiver secrets must be ${MIN_SECRET_LENGTH}..${MAX_SECRET_LENGTH} characters`,
    );
  }
  if (new Set(secrets).size !== secrets.length)
    throw new Error("finality alert receiver secrets must be unique");
  if (secrets.length > CONDITIONAL_SETTLEMENT_FINALITY_ALERT_MAX_RECEIVER_SECRETS) {
    throw new Error(
      `finality alert receiver accepts at most ${CONDITIONAL_SETTLEMENT_FINALITY_ALERT_MAX_RECEIVER_SECRETS} secrets`,
    );
  }
  return secrets;
}
...
export function getConditionalSettlementFinalityAlertReceiverPolicyStatus() {
  try {
    ...
    return {
      ...
      maxAcceptedSecretCount: CONDITIONAL_SETTLEMENT_FINALITY_ALERT_MAX_RECEIVER_SECRETS,
...
export async function getConditionalSettlementFinalityAlertReceiverHealth(
  db: Database,
...
function validHealth(value: unknown) {
  ...
  if (
    ...
    !nonnegativeInteger(health.oldestPendingAgeSeconds)
  )
    return false;
  return (
    (health.pendingSlaSeconds as number) > 0 &&
    health.total === (health.pending as number) + (health.unavailable as number) &&
    (health.oldestPendingAgeSeconds === null) === (health.pending === 0) &&
    typeof health.recordedAt === "string" &&
    Number.isFinite(Date.parse(health.recordedAt))
  );
}

function reasonsMatchHealth(reasons: string[], health: Record<string, unknown>) {
  return (
    reasons.includes("orphaned_receipt") === (health.orphanedReceipts as number) > 0 &&
...
export function verifyConditionalSettlementFinalityAlert(input: {
  ...
  if (
    ...
    body.type !== "conditional_settlement_finality.health" ||
    body.delivery_id !== deliveryId ||
    body.created_at !== timestamp ||
    !validHealth(body.health) ||
  ...
  const validRecovery =
    ...
    severity === "recovery" &&
    reasons.length === 1 &&
    reasons[0] === "conditional_settlement_finality_recovered" &&
    health.status === "healthy" &&
    health.orphanedReceipts === 0 &&
    health.rpcUnavailable === 0 &&
    health.configurationBlocked === 0 &&
    health.overduePending === 0;
  const validFiring =
    state === "firing" &&
  ...
  if (
    !verificationSecrets.length ||
    verificationSecrets.length > CONDITIONAL_SETTLEMENT_FINALITY_ALERT_MAX_RECEIVER_SECRETS ||
    verificationSecrets.some(
      (item) => item.length < MIN_SECRET_LENGTH || item.length > MAX_SECRET_LENGTH,
    )
  ) {
...
export async function claimVerifiedConditionalSettlementFinalityAlert(
  db: Database,
