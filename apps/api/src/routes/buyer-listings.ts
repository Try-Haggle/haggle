...
export function registerBuyerListingsRoutes(app: FastifyInstance, db: Database) {
  ...
  app.post<{
    ...
    if (!publicId) {
      return reply.status(400).send({ ok: false, error: "publicId is required" });
    }
    ...
    const existing = await db.query.buyerListings.findFirst({
      where: (fields, ops) =>
        ops.and(ops.eq(fields.userId, userId), ops.eq(fields.publishedListingId, published.id)),
    });
  ...
  app.get("/api/viewed", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    const rows = await db
      ...
      .from(buyerListings)
      .innerJoin(listingsPublished, eq(listingsPublished.id, buyerListings.publishedListingId))
