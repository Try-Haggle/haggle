...
import { createPublicClient, decodeEventLog, http, isAddress, type Address, type Hex } from "viem";
...
import { calculateSellerFeeSplit, MAX_HAGGLE_FEE_BPS, readHaggleFeeBpsFromEnv } from "../payments/fee-policy.js";
...
const payoutCancellationArchiveRetrySchema = z.object({ reason: z.string().trim().min(12).max(500) });
const payoutCancellationArchiveFailureQuerySchema = z.object({
  limit: z.coerce.number().int().min(1).max(100).default(20),
  cursor: z.string().min(1).max(1024).optional(),
});

function cancellationAuditArchiveSummary(archive: Awaited<ReturnType<typeof getShipmentApvCancellationAuditArchiveStatus>>) {
...
function getStripeOnrampContext(providerContext: Record<string, unknown>) {
  return isRecord(providerContext.stripe_onramp)
    ? providerContext.stripe_onramp
    : {};
}
...
function conditionalSettlementSettlementRail(
  ...
  return typeof stripeOnramp.status === "string" ? "stripe" : intentRail === "stripe" ? "stripe" : "x402";
}
...
function expectedConditionalReleaseSellerWallet(
  ...
  return normalizeAddress(conditionalContext.seller_wallet)
...
function expectedConditionalReleaseAmounts(
  ...
  if (sellerAmountMinor !== null && feeAmountMinor !== null) {
    return {
      sellerAmountMinor,
      feeAmountMinor,
      feeBps: typeof conditionalContext.release_fee_bps === "number"
...
export function registerSettlementReleaseRoutes(app: FastifyInstance, db: Database) {
  ...
  app.post("/settlement-releases/:id/apply-adjustment", { preHandler: [requireAdmin] }, async (_request, reply) => {
  ...
  app.post("/settlement-releases/:id/conditional-release-request", { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    const expectedSellerWallet = expectedConditionalReleaseSellerWallet(providerContext, conditionalContext);
    ...
    if (requestedSellerWallet && requestedSellerWallet !== expectedSellerWallet) {
      return reply.code(409).send({
        ...
        message: "conditional release seller wallet must match the funded settlement seller wallet",
      });
    }
    ...
    const settlementAmount = toSettlementAssetMoney(intent.amount, "USDC");
    const baseSplit = calculateSellerFeeSplit(settlementAmount.amount_minor, readHaggleFeeBpsFromEnv());
    const maxContractFeeMinor = Math.floor(settlementAmount.amount_minor * MAX_HAGGLE_FEE_BPS / 10_000);
    ...
    if (payoutOffsetResult.outcome === "snapshot_conflict" || payoutOffsetResult.outcome === "request_conflict") {
  ...
  app.post("/settlement-releases/:id/conditional-release-execution", { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    const expectedContractAddress = normalizeAddress(process.env.HAGGLE_CONDITIONAL_SETTLEMENT_ADDRESS);
    if (
      parsed.data.contract_address
      && (!expectedContractAddress || normalizeAddress(parsed.data.contract_address) !== expectedContractAddress)
    ) {
      return reply.code(400).send({ error: "CONDITIONAL_RELEASE_CONTRACT_MISMATCH" });
    }
    if (parsed.data.chain_id && Number(parsed.data.chain_id) !== expectedConditionalSettlementChainId()) {
  ...
  app.post("/settlement-releases/:id/conditional-release-confirmation", { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    const sellerWallet = expectedConditionalReleaseSellerWallet(providerContext, conditionalContext);
    if (!sellerWallet) {
      return reply.code(400).send({ error: "SELLER_WALLET_NOT_RESOLVED" });
    }
    const feeWallet = normalizeAddress(conditionalContext.release_fee_wallet)
    ...
    if (!receipt) {
      ...
      return reply.header("Retry-After", String(CONDITIONAL_SETTLEMENT_RETRY_AFTER_SECONDS)).code(202).send({
        release,
        intent,
        conditional_settlement: pendingContext,
    ...
    if (!finality.ready) {
      const pendingContext = {
        ...conditionalContext,
        release_tx_hash: txHash,
        status: finality.status === "pending" ? "RELEASE_CONFIRMATIONS_PENDING" : "RELEASE_FINALITY_UNAVAILABLE",
        release_checked_at: new Date().toISOString(),
        finality,
      };
      ...
      return reply.code(statusCode).send({ release, intent, conditional_settlement: pendingContext,
        ...(statusCode === 202 ? { retry: conditionalSettlementConfirmationRetry() } : {}) });
    }

    const releaseEvent = receipt.status === "success"
      ? (() => {
    ...
    const payoutOffsetId = typeof conditionalContext.apv_payout_offset_id === "string"
      ? conditionalContext.apv_payout_offset_id
      : undefined;
    const payoutOffsetResult = receipt.status === "success" && payoutOffsetId
      ...
      : null;
    if (payoutOffsetResult
      && (payoutOffsetResult.outcome === "snapshot_conflict" || payoutOffsetResult.outcome === "request_conflict")) {
      return reply.code(409).send({ error: "APV_PAYOUT_OFFSET_CONFIRMATION_CONFLICT" });
    }
    const confirmedPayoutOffset = payoutOffsetResult && "offset" in payoutOffsetResult
      ? payoutOffsetResult.offset
      : null;
  ...
  app.post("/settlement-releases/:id/apv-payout-offsets/:offsetId/cancel-expired", { preHandler: [requireAdmin] }, async (request, reply) => {
    return reply.code(410).send({
  ...
  app.post("/settlement-releases/:id/apv-payout-offsets/:offsetId/cancellation-requests", { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION", issues: parsed.error.issues });
    ...
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_PAYOUT_OFFSET_NOT_FOUND" });
    if (result.outcome === "not_expired") return reply.code(409).send({ error: "APV_PAYOUT_OFFSET_NOT_EXPIRED" });
    if (result.outcome === "invalid_state") return reply.code(409).send({ error: "APV_PAYOUT_OFFSET_STATE_CONFLICT" });
    ...
    if (!("request" in result)) return reply.code(400).send({ error: "APV_PAYOUT_CANCELLATION_REQUEST_REJECTED" });
    return reply.send({ cancellation_request: result.request, idempotent: result.outcome === "duplicate" });
  });

  app.get("/admin/settlement-releases/apv-payout-cancellation-requests/pending", { preHandler: [requireAdmin] }, async (request, reply) => {
    const parsed = payoutCancellationQueueQuerySchema.safeParse(request.query ?? {});
    if (!parsed.success) {
      return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_QUEUE_QUERY", issues: parsed.error.issues });
    }
    try {
  ...
  app.get("/admin/settlement-releases/apv-payout-cancellation-requests/:requestId/timeline", { preHandler: [requireAdmin] }, async (request, reply) => {
    const parsed = payoutCancellationTimelineParamsSchema.safeParse(request.params ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_REQUEST_ID" });
    const timeline = await getShipmentApvPayoutCancellationTimeline(db, parsed.data.requestId);
    if (!timeline) return reply.code(404).send({ error: "APV_PAYOUT_CANCELLATION_REQUEST_NOT_FOUND" });
    return reply.send({ cancellation_timeline: timeline });
  });

  app.get("/admin/settlement-releases/apv-payout-cancellation-requests/:requestId/audit-export", { preHandler: [requireAdmin] }, async (request, reply) => {
    const parsed = payoutCancellationTimelineParamsSchema.safeParse(request.params ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_REQUEST_ID" });
    const timeline = await getShipmentApvPayoutCancellationTimeline(db, parsed.data.requestId);
    if (!timeline) return reply.code(404).send({ error: "APV_PAYOUT_CANCELLATION_REQUEST_NOT_FOUND" });
    ...
    try {
      ...
      reply.header("Content-Disposition", `attachment; filename="haggle-apv-cancellation-${parsed.data.requestId}-audit.json"`);
      ...
      if (error instanceof ShipmentApvCancellationAuditSigningNotConfiguredError) {
        return reply.code(503).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_SIGNING_NOT_CONFIGURED" });
      }
      request.log.error({ error, cancellation_request_id: parsed.data.requestId }, "Failed to sign APV cancellation audit export");
      return reply.code(500).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_SIGNING_FAILED" });
    }
  });

  app.post("/admin/settlement-releases/apv-payout-cancellation-requests/:requestId/audit-archive", { preHandler: [requireAdmin] }, async (request, reply) => {
    const parsed = payoutCancellationTimelineParamsSchema.safeParse(request.params ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_REQUEST_ID" });
    const timeline = await getShipmentApvPayoutCancellationTimeline(db, parsed.data.requestId);
    if (!timeline) return reply.code(404).send({ error: "APV_PAYOUT_CANCELLATION_REQUEST_NOT_FOUND" });
    if (!timeline.integrity.valid) return reply.code(409).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_CHAIN_INVALID" });
    if (timeline.request.status === "PENDING") return reply.code(409).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_NOT_FINAL" });
    try {
      const result = await enqueueShipmentApvCancellationAuditArchive(db, {
        cancellationRequestId: parsed.data.requestId,
        events: timeline.events,
      });
      return reply.send({ audit_archive: cancellationAuditArchiveSummary(result.archive), idempotent: result.outcome === "duplicate" });
    } catch (error) {
      if (error instanceof ShipmentApvCancellationAuditSigningNotConfiguredError) {
        return reply.code(503).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_SIGNING_NOT_CONFIGURED" });
      }
  ...
  app.get("/admin/settlement-releases/apv-payout-cancellation-audit-archives/health", { preHandler: [requireAdmin] }, async (_request, reply) => {
    return reply.send({
  ...
  app.get("/admin/settlement-releases/apv-payout-cancellation-audit-archives/failures", { preHandler: [requireAdmin] }, async (request, reply) => {
    const parsed = payoutCancellationArchiveFailureQuerySchema.safeParse(request.query ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_AUDIT_ARCHIVE_FAILURE_QUERY" });
    try {
      return reply.send({ audit_archive_failures: await listShipmentApvCancellationAuditArchiveFailures(db, parsed.data) });
    } catch (error) {
      if (error instanceof Error && error.message === "INVALID_APV_AUDIT_ARCHIVE_FAILURE_CURSOR") {
        return reply.code(400).send({ error: error.message });
      }
      throw error;
    }
  ...
  app.get("/admin/settlement-releases/apv-payout-cancellation-requests/:requestId/audit-archive", { preHandler: [requireAdmin] }, async (request, reply) => {
    const parsed = payoutCancellationTimelineParamsSchema.safeParse(request.params ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_REQUEST_ID" });
    const archive = await getShipmentApvCancellationAuditArchiveStatus(db, parsed.data.requestId);
    if (!archive) return reply.code(404).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_ARCHIVE_NOT_FOUND" });
    return reply.send({ audit_archive: cancellationAuditArchiveSummary(archive) });
  });

  app.post("/admin/settlement-releases/apv-payout-cancellation-requests/:requestId/audit-archive/retry", { preHandler: [requireAdmin] }, async (request, reply) => {
    ...
    if (!params.success || !body.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_AUDIT_ARCHIVE_RETRY" });
    const result = await requeueShipmentApvCancellationAuditArchive(db, {
      cancellationRequestId: params.data.requestId,
      actorId: request.user!.id,
      reason: body.data.reason,
    });
    if (result.outcome === "invalid_reason") return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_AUDIT_ARCHIVE_RETRY" });
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_ARCHIVE_NOT_FOUND" });
    if (result.outcome === "already_delivered") return reply.code(409).send({ error: "APV_PAYOUT_CANCELLATION_AUDIT_ARCHIVE_ALREADY_DELIVERED", audit_archive: cancellationAuditArchiveSummary(result.archive) });
    return reply.send({ audit_archive: cancellationAuditArchiveSummary(result.archive), outcome: result.outcome });
  });

  app.post("/settlement-releases/:id/apv-payout-offsets/:offsetId/cancellation-requests/:requestId/decision", { preHandler: [requireAdmin] }, async (request, reply) => {
    const { id, offsetId, requestId } = request.params as { id: string; offsetId: string; requestId: string };
    const parsed = payoutCancellationDecisionSchema.safeParse(request.body ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_PAYOUT_CANCELLATION_DECISION", issues: parsed.error.issues });
    ...
    if (parsed.data.decision === "APPROVE") {
      ...
      const intent = await getPaymentIntentById(db, release.payment_intent_id);
      const intentRow = intent ? await getPaymentIntentRowById(db, release.payment_intent_id) : null;
      if (!intent || !intentRow) return reply.code(404).send({ error: "PAYMENT_INTENT_NOT_FOUND" });
      const providerContext = intentRow.providerContext && typeof intentRow.providerContext === "object"
        && !Array.isArray(intentRow.providerContext) ? intentRow.providerContext : {};
      const settlementId = normalizeHex(getConditionalSettlementContext(providerContext).settlement_id);
      if (!settlementId) return reply.code(409).send({ error: "CONDITIONAL_SETTLEMENT_ID_REQUIRED" });
      const client = createConditionalReleaseReceiptClient();
      const contractAddress = normalizeAddress(process.env.HAGGLE_CONDITIONAL_SETTLEMENT_ADDRESS);
      if (!client || !contractAddress) return reply.code(503).send({ error: "CONDITIONAL_RELEASE_RECEIPT_RPC_NOT_CONFIGURED" });
      ...
      if (stateValue === null) return reply.code(503).send({ error: "CONDITIONAL_SETTLEMENT_STATE_UNAVAILABLE" });
      state = (["NONE", "FUNDED", "RELEASED", "REFUNDED", "DISPUTED"] as const)[Number(stateValue)] ?? "NONE";
    }
    ...
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_PAYOUT_CANCELLATION_REQUEST_NOT_FOUND" });
    if (result.outcome === "self_approval_forbidden") return reply.code(403).send({ error: "APV_PAYOUT_CANCELLATION_SELF_APPROVAL_FORBIDDEN" });
    if (["request_conflict", "decision_conflict", "version_conflict", "invalid_state", "expired", "onchain_state_conflict"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_PAYOUT_CANCELLATION_${result.outcome.toUpperCase()}`, onchain_state: state });
    }
    if (!("request" in result)) return reply.code(400).send({ error: "APV_PAYOUT_CANCELLATION_DECISION_REJECTED" });
    return reply.send({ cancellation_request: result.request, idempotent: result.outcome === "duplicate", onchain_state: state });
  });
