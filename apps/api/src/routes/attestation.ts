...
import { createAttestationUploadUrl } from "../services/supabase-storage.service.js";
...
export function registerAttestationRoutes(app: FastifyInstance, db: Database) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return reply.code(400).send({ error: "INVALID_PATH", message: (err as Error).message });
      }
  ...
  app.post("/api/attestation/commit", { preHandler: [requireAuth] }, async (request, reply) => {
    const parsed = commitSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.code(400).send({ error: "INVALID_COMMIT_REQUEST", issues: parsed.error.issues });
    }
    const { listingId, imei, batteryHealthPct, findMyOff, photoStoragePaths } = parsed.data;
    ...
    try {
      ...
      if (err instanceof AttestationValidationError || err instanceof AttestationStorageError) {
