// biome-ignore-all lint/suspicious/noImplicitAnyLet: Guarded assignments retain claim result types.
...
import {
  ...
  getDisputeAiArchiveAlertReceiverHealth,
  getDisputeAiArchiveAlertReceiverPolicyStatus,
  resolveDisputeAiArchiveAlertReceiverSecretsFromEnv,
...
export function registerOpsAlertRoutes(app: FastifyInstance, db: Database) {
  app.post(
    "/internal/ops/alerts/webhook-claim-health",
    { config: { rawBody: true } },
    async (request, reply) => {
      let secrets: string[];
      try {
        secrets = resolveWebhookClaimAlertReceiverSecretsFromEnv();
      } catch {
        return reply.code(503).send({ error: "OPS_ALERT_RECEIVER_INVALID_CONFIGURATION" });
      }
      if (!secrets.length)
        return reply.code(503).send({ error: "OPS_ALERT_RECEIVER_NOT_CONFIGURED" });
      ...
      if (!verification.ok)
        return reply
          .code(BAD_REQUEST_ERRORS.has(verification.error) ? 400 : 401)
      ...
      if (delivery.outcome === "payload_conflict")
        return reply.code(409).send({ error: "ALERT_DELIVERY_PAYLOAD_CONFLICT" });
      if (delivery.outcome === "replay_completed")
        return reply.send({
          accepted: false,
          replayed: true,
      ...
      if (delivery.outcome === "retry_backoff")
        return reply
          .header("retry-after", String(delivery.retryAfterSeconds))
          .code(503)
      ...
      try {
        ...
        try {
          await failWebhookEvent(db, delivery.claim);
        } catch {
          /* Lease takeover remains the recovery path. */
        }
        return reply
          .header("retry-after", "2")
          .code(503)
          .send({ error: "ALERT_CLAIM_COMPLETION_FAILED", retry_after_seconds: 2 });
      }
  ...
  app.get(
    "/admin/ops/alerts/webhook-claim-health/health",
    { preHandler: [requireAdmin] },
    async (_request, reply) =>
      reply.send({
        receiver_kind: "webhook_claim_health",
        receiver_health: await getWebhookClaimAlertReceiverHealth(db),
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (delivery.outcome === "retry_later") {
        return reply.header("retry-after", String(delivery.retryAfterSeconds)).code(503).send({
    ...
    async (request, reply) => {
      ...
      try {
        return reply.send({
          ...
          receiver_health: await getDisputeEvidenceScanRetryAlertReceiverHealth(db),
          receiver_policy: getDisputeEvidenceScanRetryAlertReceiverPolicyStatus(),
        });
      } catch {
        request.log.error(
          {
  ...
  app.post(
    "/internal/ops/alerts/conditional-settlement-finality",
    { config: { rawBody: true } },
    async (request, reply) => {
      let secrets: string[];
      try {
        secrets = resolveConditionalSettlementFinalityAlertReceiverSecretsFromEnv();
      } catch {
        return reply.code(503).send({ error: "OPS_ALERT_RECEIVER_INVALID_CONFIGURATION" });
      }
      if (!secrets.length)
      ...
      try {
        ...
        try {
          ...
          /* A later lease takeover remains the recovery path. */
        }
        return reply
          .header("retry-after", "2")
          .code(503)
  ...
  app.get(
    "/admin/ops/alerts/conditional-settlement-finality/health",
    { preHandler: [requireAdmin] },
    async (_request, reply) =>
      reply.send({
        receiver_kind: "conditional_settlement_finality",
        receiver_health: await getConditionalSettlementFinalityAlertReceiverHealth(db),
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (delivery.outcome === "payload_conflict")
        return reply.code(409).send({ error: "ALERT_DELIVERY_PAYLOAD_CONFLICT" });
      if (delivery.outcome === "replay_or_in_progress")
        return reply.send({
          accepted: false,
          replayed: true,
          delivery_id: verification.deliveryId,
  ...
  app.get("/.well-known/haggle-dispute-audit-keys.json", async (_request, reply) => {
    try {
      ...
      if (!keys.length)
        return reply.code(503).send({ error: "DISPUTE_AUDIT_KEY_REGISTRY_NOT_CONFIGURED" });
      const document = disputeAuditPublicKeyRegistryDocument(keys);
      return reply
        .header("cache-control", "public, max-age=300")
        .header("etag", `"${document.registry_sha256}"`)
        .send(document);
    } catch {
  ...
  app.get(
    ...
    async (_request, reply) =>
      reply.send({
        receiver_kind: "evidence_provenance_archive",
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const rawBody = (request as unknown as { rawBody?: Buffer }).rawBody;
      if (!rawBody) return reply.code(400).send({ error: "MISSING_RAW_BODY" });
      const verification = verifyDisputeAiAuditArchiveAlert({
        rawBody,
        timestamp: request.headers["x-haggle-alert-timestamp"],
        signature: request.headers["x-haggle-alert-signature"],
        deliveryId: request.headers["x-haggle-alert-delivery-id"],
  ...
  app.get(
    ...
    async (_request, reply) =>
      reply.send({
        receiver_kind: "ai_audit_archive",
        receiver_health: await getDisputeAiArchiveAlertReceiverHealth(db),
