...
export function registerNotificationRoutes(app: FastifyInstance, db: Database): void {
        ...
        if (createdAtCursor && idCursor) {
          return ops.and(
            ...
            ops.or(
              ...
              ops.and(ops.eq(fields.createdAt, createdAtCursor), ops.lt(fields.id, idCursor)),
            ),
          );
        }
        return base;
  ...
  app.put<{ Body: { category: string; channel: string; enabled: boolean } }>(
    ...
    async (request, reply) => {
      ...
      if (
        !NOTIFICATION_CATEGORIES.includes(category as never) ||
        !NOTIFICATION_CHANNELS.includes(channel as never)
      ) {
