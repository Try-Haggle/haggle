...
import { listShipmentApvInvoiceRevisions, recordShipmentApvInvoiceRevision } from "../services/shipment-apv-revision.service.js";
...
import { evaluateShipmentApvCursorRetentionAlert } from
  "../services/shipment-apv-payout-alert.service.js";
...
const recordEventSchema = z.object({
  ...
  event_type: z.enum(["label_create", "ship", "out_for_delivery", "deliver", "exception", "return_ship", "return_complete"]),
...
const apvRevisionEvidenceSchema = z.object({
  ...
  surcharge_category: z.enum([
    "ADDRESS_CORRECTION", "CLEARANCE", "COMMODITY_TYPE", "DANGEROUS_GOODS",
    "DELIVERY_CONVENIENCE", "DEMAND", "EXCEPTION", "FUEL", "HANDLING", "HAZMAT",
    "INTERNATIONAL_HANDLING", "OTHER", "PAYMENT_TYPE", "REBATE", "RETURNS", "SECURITY",
    "SERVICE_AREA_ADJUSTMENT", "SHIPMENT_VALUE", "SIGNATURE_SERVICE", "SPECIAL_SERVICE",
    "VALUE_ADD", "WEEKEND", "UNKNOWN",
  ]),
...
const apvInvoiceDocumentSchema = z.object({
  ...
  content_base64: z.string().min(4).max(7_000_000).regex(/^[A-Za-z0-9+/]+={0,2}$/),
});

const apvInvoiceReconciliationRequestSchema = z.object({
  client_request_id: z.string().uuid(),
  candidate_id: z.string().regex(/^[0-9a-f]{64}$/),
  reason: z.string().trim().min(12).max(500),
}).strict();

const apvInvoiceReconciliationDecisionSchema = z.object({
...
const apvInvoiceRestorationRequestSchema = z.object({
  client_request_id: z.string().uuid(),
  candidate_id: z.string().regex(/^[0-9a-f]{64}$/),
  content_type: z.enum(["application/pdf", "text/csv", "application/json"]),
  content_base64: z.string().min(4).max(7_000_000).regex(/^[A-Za-z0-9+/]+={0,2}$/),
...
const apvInvoiceRestorationDecisionSchema = z.object({
  decision_request_id: z.string().uuid(),
...
const apvInvoiceRestorationStagingMaintenanceSchema = z.object({
  mode: z.enum(["dry_run", "apply"]),
  limit: z.number().int().min(1).max(1000).optional(),
}).strict();

const apvInvoiceRestorationRemediationRequestSchema = z.object({
  client_request_id: z.string().uuid(),
  candidate_id: z.string().regex(/^[0-9a-f]{64}$/),
  reason: z.string().trim().min(12).max(500),
}).strict();

const apvInvoiceRestorationRemediationDecisionSchema = z.object({
  decision_request_id: z.string().uuid(),
  decision: z.enum(["APPROVE", "REJECT"]),
  reason: z.string().trim().min(12).max(500),
  expected_version: z.number().int().nonnegative(),
...
const apvInvoiceRestorationRemediationAcknowledgmentSchema = z.object({
  client_request_id: z.string().uuid(),
  decision_request_id: z.string().uuid(),
  action: z.enum(["ACKNOWLEDGED", "INCIDENT_LINKED"]),
  expected_version: z.number().int().min(1),
  incident_reference: z.string().trim().min(4).max(128)
    .regex(/^[\x20-\x7e]+$/).optional(),
}).strict().superRefine((value, context) => {
  if ((value.action === "INCIDENT_LINKED") !== Boolean(value.incident_reference)) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["incident_reference"],
...
function preparedShipmentRatesFromMetadata(metadata: Record<string, unknown> | null | undefined): PreparedShipmentRate[] {
...
function preparedRateNotFoundResponse(rateId: string) {
  return {
    ...
    message: "Run POST /shipments/:id/prepare and purchase one of the returned rates for this shipment.",
    rate_id: rateId,
  };
}
...
function normalizeRateAddress(address: Record<string, unknown>): Record<string, string> {
  ...
  for (const key of ["street1", "street2", "city", "state", "zip", "country"]) {
    const value = normalizeQuoteString(address[key], key === "state" || key === "country" || key === "zip");
...
export function registerShipmentRoutes(app: FastifyInstance, db: Database) {
  ...
  app.get("/shipments/apv-adjustments/:adjustmentId", { preHandler: [requireAuth] }, async (request, reply) => {
    const { adjustmentId } = request.params as { adjustmentId: string };
    if (!z.string().uuid().safeParse(adjustmentId).success) return reply.code(400).send({ error: "INVALID_ADJUSTMENT_ID" });
  ...
  app.post("/shipments/apv-adjustments/:adjustmentId/seller-review", { preHandler: [requireAuth] }, async (request, reply) => {
    const { adjustmentId } = request.params as { adjustmentId: string };
    if (!z.string().uuid().safeParse(adjustmentId).success) return reply.code(400).send({ error: "INVALID_ADJUSTMENT_ID" });
    const parsed = apvSellerReviewSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_REVIEW_REQUEST", issues: parsed.error.issues });
    ...
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_ADJUSTMENT_NOT_FOUND" });
    if (result.outcome === "forbidden") return reply.code(403).send({ error: "FORBIDDEN" });
    if (result.outcome === "invalid_state") return reply.code(409).send({ error: "APV_REVIEW_NOT_AVAILABLE" });
    if (result.outcome === "request_conflict") return reply.code(409).send({ error: "APV_REVIEW_ALREADY_SUBMITTED" });
    if (result.outcome === "version_conflict") return reply.code(409).send({ error: "APV_REVIEW_VERSION_CONFLICT" });
    ...
    if (result.outcome === "updated") {
      await auditShipmentAction(db, request, "shipment.apv_seller_review", {
        ...
        metadata: { adjustment_id: result.record.id, review_version: result.record.review_version },
      });
    }
    return reply.send({ adjustment: result.record, idempotent: result.outcome === "duplicate" });
  });

  app.post("/shipments/apv-adjustments/:adjustmentId/decision", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_REVIEW_DECISION", issues: parsed.error.issues });
    ...
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_ADJUSTMENT_NOT_FOUND" });
    if (result.outcome === "invalid_state") return reply.code(409).send({ error: "APV_REVIEW_NOT_PENDING" });
    if (result.outcome === "version_conflict") return reply.code(409).send({ error: "APV_REVIEW_VERSION_CONFLICT" });
    if (result.outcome === "forbidden" || result.outcome === "request_conflict") {
      return reply.code(409).send({ error: "APV_REVIEW_DECISION_CONFLICT" });
    }
    if (!("record" in result)) return reply.code(409).send({ error: "APV_REVIEW_DECISION_CONFLICT" });
    if (result.outcome === "updated") {
  ...
  app.get("/shipments/apv-adjustments/:adjustmentId/revisions", { preHandler: [requireAuth] }, async (request, reply) => {
    const { adjustmentId } = request.params as { adjustmentId: string };
    if (!z.string().uuid().safeParse(adjustmentId).success) return reply.code(400).send({ error: "INVALID_ADJUSTMENT_ID" });
    const adjustment = await getShipmentApvReview(db, adjustmentId);
    if (!adjustment) return reply.code(404).send({ error: "APV_ADJUSTMENT_NOT_FOUND" });
  ...
  app.post("/shipments/apv-revisions/:revisionId/decision", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { revisionId } = request.params as { revisionId: string };
    if (!z.string().uuid().safeParse(revisionId).success) return reply.code(400).send({ error: "INVALID_APV_REVISION_ID" });
    const parsed = apvRevisionDecisionSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_REVISION_DECISION", issues: parsed.error.issues });
    ...
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_REVISION_NOT_FOUND" });
    if (result.outcome === "invalid_decision") return reply.code(409).send({ error: "APV_REVISION_DECISION_MISMATCH" });
    if (result.outcome === "evidence_required") return reply.code(409).send({ error: "APV_REVISION_EVIDENCE_REQUIRED" });
    if (result.outcome === "predecessor_pending") return reply.code(409).send({ error: "APV_REVISION_PREDECESSOR_PENDING" });
    if (result.outcome === "aggregate_conflict") return reply.code(409).send({ error: "APV_REVISION_RECONCILIATION_REQUIRED" });
    if (result.outcome === "payout_reserved") return reply.code(409).send({ error: "APV_PAYOUT_ALREADY_RESERVED" });
    if (result.outcome === "invalid_state") return reply.code(409).send({ error: "APV_REVISION_STATE_CONFLICT" });
    if (result.outcome === "request_conflict") return reply.code(409).send({ error: "APV_REVISION_REQUEST_CONFLICT" });
    if (!("revision" in result)) return reply.code(409).send({ error: "APV_REVISION_APPLICATION_CONFLICT" });
  ...
  app.post("/shipments/apv-revisions/:revisionId/evidence", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { revisionId } = request.params as { revisionId: string };
    if (!z.string().uuid().safeParse(revisionId).success) return reply.code(400).send({ error: "INVALID_APV_REVISION_ID" });
    const parsed = apvRevisionEvidenceSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_REVISION_EVIDENCE", issues: parsed.error.issues });
    ...
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_REVISION_NOT_FOUND" });
    if (result.outcome === "invalid_state") return reply.code(409).send({ error: "APV_REVISION_EVIDENCE_STATE_CONFLICT" });
    if (result.outcome === "amount_conflict") return reply.code(409).send({ error: "APV_REVISION_EVIDENCE_AMOUNT_CONFLICT" });
    if (result.outcome === "evidence_conflict") return reply.code(409).send({ error: "APV_REVISION_EVIDENCE_CONFLICT" });
    if (!("evidence" in result)) return reply.code(409).send({ error: "APV_REVISION_EVIDENCE_UNAVAILABLE" });
  ...
  app.post("/shipments/apv-revisions/:revisionId/invoice-document", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { revisionId } = request.params as { revisionId: string };
    if (!z.string().uuid().safeParse(revisionId).success) return reply.code(400).send({ error: "INVALID_APV_REVISION_ID" });
    const parsed = apvInvoiceDocumentSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_DOCUMENT", issues: parsed.error.issues });
    ...
    let result;
    try {
      result = await storeShipmentApvInvoiceDocument(db, { revisionId,
        providerDocumentId: parsed.data.provider_document_id, contentType: parsed.data.content_type,
        bytes, uploadedBy: request.user!.id });
    } catch {
      return reply.code(503).send({ error: "APV_INVOICE_DOCUMENT_STORAGE_UNAVAILABLE" });
    }
    if (result.outcome === "revision_not_found") return reply.code(404).send({ error: "APV_REVISION_NOT_FOUND" });
    if (result.outcome === "evidence_not_bound") return reply.code(409).send({ error: "APV_REVISION_EVIDENCE_REQUIRED" });
    if (result.outcome === "evidence_mismatch") return reply.code(409).send({ error: "APV_INVOICE_DOCUMENT_EVIDENCE_MISMATCH" });
    if (result.outcome === "document_conflict") return reply.code(409).send({ error: "APV_INVOICE_DOCUMENT_CONFLICT" });
    if (result.outcome === "invalid_document") return reply.code(400).send({ error: "INVALID_APV_INVOICE_DOCUMENT_BYTES" });
    if (!("document" in result)) return reply.code(409).send({ error: "APV_INVOICE_DOCUMENT_UNAVAILABLE" });
    if (result.outcome === "stored") {
      await auditShipmentAction(db, request, "shipment.apv_invoice_document", {
        ...
        metadata: { revision_id: result.document.revision_id,
          provider_document_id: result.document.provider_document_id,
          content_type: result.document.content_type, byte_size: result.document.byte_size,
          sha256: result.document.sha256 },
      });
    }
    return reply.code(result.outcome === "stored" ? 201 : 200)
      .send({ document: result.document, idempotent: result.outcome === "duplicate" });
  });

  app.get("/admin/shipments/apv-invoice-documents/health", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    try {
  ...
  app.post("/admin/shipments/apv-invoice-documents/reconcile", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const parsed = z.object({ dry_run: z.literal(true) }).strict().safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "APV_INVOICE_DOCUMENT_DRY_RUN_REQUIRED" });
    try {
      return reply.send({ reconciliation: await runShipmentApvInvoiceDocumentReconciliationDryRun(db) });
    } catch {
      return reply.code(503).send({ error: "APV_INVOICE_DOCUMENT_RECONCILIATION_UNAVAILABLE" });
    }
  });

  app.get("/admin/shipments/apv-invoice-documents/reconciliation-candidates", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    try {
      return reply.send({ reconciliation_candidates: await discoverShipmentApvInvoiceReconciliationCandidates(db) });
    } catch {
      return reply.code(503).send({ error: "APV_INVOICE_RECONCILIATION_SCAN_UNAVAILABLE" });
    }
  });

  app.post("/admin/shipments/apv-invoice-documents/reconciliation-requests", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const parsed = apvInvoiceReconciliationRequestSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RECONCILIATION_REQUEST" });
    const result = await requestShipmentApvInvoiceReconciliation(db, {
      clientRequestId: parsed.data.client_request_id, candidateId: parsed.data.candidate_id,
      requesterId: request.user!.id, reason: parsed.data.reason,
    });
    if (result.outcome === "candidate_not_found") return reply.code(404).send({ error: "APV_INVOICE_RECONCILIATION_CANDIDATE_NOT_FOUND" });
    if (result.outcome === "scan_truncated") return reply.code(409).send({ error: "APV_INVOICE_RECONCILIATION_SCAN_TRUNCATED" });
    if (result.outcome === "pending_conflict") return reply.code(409).send({ error: "APV_INVOICE_RECONCILIATION_ALREADY_PENDING" });
    if (result.outcome === "request_conflict") return reply.code(409).send({ error: "APV_INVOICE_RECONCILIATION_REQUEST_CONFLICT" });
    ...
    return reply.code(result.outcome === "requested" ? 201 : 200)
      .send({ reconciliation_request: result.request, idempotent: result.outcome === "duplicate" });
  });

  app.get("/admin/shipments/apv-invoice-documents/reconciliation-requests/pending", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    return reply.send({ reconciliation_requests: await listPendingShipmentApvInvoiceReconciliations(db) });
  });

  app.get("/admin/shipments/apv-invoice-documents/reconciliation-requests/:requestId/timeline", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    if (!z.string().uuid().safeParse(requestId).success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RECONCILIATION_ID" });
  ...
  app.post("/admin/shipments/apv-invoice-documents/reconciliation-requests/:requestId/decision", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    if (!z.string().uuid().safeParse(requestId).success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RECONCILIATION_ID" });
    const parsed = apvInvoiceReconciliationDecisionSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RECONCILIATION_DECISION" });
    const result = await decideShipmentApvInvoiceReconciliation(db, {
      requestId, decisionRequestId: parsed.data.decision_request_id, approverId: request.user!.id,
      decision: parsed.data.decision, reason: parsed.data.reason, expectedVersion: parsed.data.expected_version,
    });
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_INVOICE_RECONCILIATION_NOT_FOUND" });
    if (result.outcome === "self_approval_forbidden") return reply.code(403).send({ error: "APV_INVOICE_RECONCILIATION_SELF_APPROVAL_FORBIDDEN" });
    if (["candidate_changed", "version_conflict", "invalid_state", "decision_conflict", "apply_state_lost"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_INVOICE_RECONCILIATION_${result.outcome.toUpperCase()}` });
    }
    if (result.outcome === "apply_failed") return reply.code(503).send({ error: "APV_INVOICE_RECONCILIATION_APPLY_FAILED" });
    if (result.outcome === "invalid_request" || !("request" in result)) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RECONCILIATION_DECISION" });
    }
    return reply.send({ reconciliation_request: result.request, idempotent: result.outcome === "duplicate" });
  });

  app.get("/admin/shipments/apv-invoice-documents/restoration-candidates", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    return reply.send({ restoration_candidates: await listShipmentApvInvoiceRestorationCandidates(db) });
  });

  app.post("/admin/shipments/apv-invoice-documents/restoration-requests", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const parsed = apvInvoiceRestorationRequestSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REQUEST" });
    const bytes = Buffer.from(parsed.data.content_base64, "base64");
    if (bytes.toString("base64") !== parsed.data.content_base64) {
    ...
    const result = await requestShipmentApvInvoiceRestoration(db, {
      clientRequestId: parsed.data.client_request_id, candidateId: parsed.data.candidate_id,
      requesterId: request.user!.id, reason: parsed.data.reason,
      contentType: parsed.data.content_type, bytes,
    });
    if (result.outcome === "candidate_not_found") return reply.code(404).send({ error: "APV_INVOICE_RESTORATION_CANDIDATE_NOT_FOUND" });
    if (result.outcome === "replacement_mismatch") return reply.code(409).send({ error: "APV_INVOICE_RESTORATION_REPLACEMENT_MISMATCH" });
    if (["pending_conflict", "request_conflict", "staging_conflict"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_INVOICE_RESTORATION_${result.outcome.toUpperCase()}` });
    }
    if (result.outcome === "invalid_request" || !("request" in result)) {
  ...
  app.get("/admin/shipments/apv-invoice-documents/restoration-requests/pending", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    return reply.send({ restoration_requests: await listPendingShipmentApvInvoiceRestorations(db) });
  });

  app.get("/admin/shipments/apv-invoice-documents/restoration-requests/:requestId/timeline", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    if (!z.string().uuid().safeParse(requestId).success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_ID" });
  ...
  app.post("/admin/shipments/apv-invoice-documents/restoration-requests/:requestId/decision", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    if (!z.string().uuid().safeParse(requestId).success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_ID" });
    const parsed = apvInvoiceRestorationDecisionSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_DECISION" });
    const result = await decideShipmentApvInvoiceRestoration(db, { requestId,
      decisionRequestId: parsed.data.decision_request_id, approverId: request.user!.id,
      decision: parsed.data.decision, reason: parsed.data.reason, expectedVersion: parsed.data.expected_version });
    if (result.outcome === "not_found") return reply.code(404).send({ error: "APV_INVOICE_RESTORATION_NOT_FOUND" });
    if (result.outcome === "self_approval_forbidden") return reply.code(403).send({ error: "APV_INVOICE_RESTORATION_SELF_APPROVAL_FORBIDDEN" });
    if (["candidate_changed", "version_conflict", "invalid_state", "decision_conflict"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_INVOICE_RESTORATION_${result.outcome.toUpperCase()}` });
    }
    if (result.outcome === "apply_failed") return reply.code(503).send({ error: "APV_INVOICE_RESTORATION_APPLY_FAILED" });
    if (result.outcome === "invalid_request" || !("request" in result)) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_DECISION" });
    }
    return reply.send({ restoration_request: result.request, idempotent: result.outcome === "duplicate" });
  });

  app.post("/admin/shipments/apv-invoice-documents/restoration-staging/maintenance", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const parsed = apvInvoiceRestorationStagingMaintenanceSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_STAGING_MAINTENANCE" });
    const result = await maintainShipmentApvInvoiceRestorationStaging(db, {
      mode: parsed.data.mode, limit: parsed.data.limit, actorId: request.user!.id,
    });
    if ("outcome" in result) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_STAGING_MAINTENANCE" });
    return reply.send({ restoration_staging_maintenance: result });
  });

  app.get("/admin/shipments/apv-invoice-documents/restoration-staging/health", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    return reply.send({ restoration_staging_health: await getShipmentApvInvoiceRestorationStagingHealth(db),
      restoration_staging_maintenance: getShipmentApvInvoiceRestorationStagingMaintenanceJobStatus(),
      restoration_remediation_health: await getShipmentApvInvoiceRestorationRemediationHealth(db),
      restoration_remediation_expiry: getShipmentApvInvoiceRestorationRemediationExpiryJobStatus() });
  });

  app.get("/admin/shipments/apv-invoice-documents/restoration-remediation-candidates", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    return reply.send({ restoration_remediation_candidates:
      await listShipmentApvInvoiceRestorationRemediationCandidates(db) });
  });

  app.post("/admin/shipments/apv-invoice-documents/restoration-remediation-requests", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const parsed = apvInvoiceRestorationRemediationRequestSchema.safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_REQUEST" });
    const result = await requestShipmentApvInvoiceRestorationRemediation(db, {
      clientRequestId: parsed.data.client_request_id, candidateId: parsed.data.candidate_id,
      requesterId: request.user!.id, reason: parsed.data.reason,
    });
    if (result.outcome === "candidate_not_found") {
      return reply.code(404).send({ error: "APV_INVOICE_RESTORATION_REMEDIATION_CANDIDATE_NOT_FOUND" });
    }
    if (["pending_conflict", "request_conflict"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_INVOICE_RESTORATION_REMEDIATION_${result.outcome.toUpperCase()}` });
    }
    if (result.outcome === "invalid_request" || !("request" in result)) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_REQUEST" });
    }
    return reply.code(result.outcome === "requested" ? 201 : 200)
      .send({ restoration_remediation_request: result.request, idempotent: result.outcome === "duplicate" });
  });

  app.get("/admin/shipments/apv-invoice-documents/restoration-remediation-requests/pending", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    return reply.send({ restoration_remediation_requests:
      await listPendingShipmentApvInvoiceRestorationRemediations(db) });
  });

  app.get("/admin/shipments/apv-invoice-documents/restoration-remediation-recovery-queue", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const parsed = z.object({ limit: z.coerce.number().int().min(1).max(100).default(20),
      cursor: z.string().min(1).max(512).regex(/^[A-Za-z0-9_-]+$/).optional() }).strict()
      .safeParse(request.query ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_QUERY" });
    try {
      const queue = await listStaleShipmentApvInvoiceRestorationRemediationRecoveries(db, {
        approverId: request.user!.id, limit: parsed.data.limit, cursor: parsed.data.cursor,
      });
      const cursorHealth = await getShipmentApvInvoiceRestorationRemediationRecoveryCursorHealth(db)
        .catch(() => null);
      const retentionJobHealth = await getShipmentApvRemediationCursorRetentionJobHealth(db)
        .catch(() => null);
      const retentionJobStatus = getShipmentApvRemediationCursorRetentionJobStatus();
      return reply.send({ restoration_remediation_recovery_queue: queue,
        restoration_remediation_recovery_cursor_health: cursorHealth,
        restoration_remediation_recovery_cursor_retention_job: {
          ...retentionJobStatus, health: retentionJobHealth,
          alertAssessment: retentionJobHealth
            ? evaluateShipmentApvCursorRetentionAlert({ ...retentionJobStatus, health: retentionJobHealth }) : null,
        } });
    } catch (error) {
      if (error instanceof Error
        && ["INVALID_APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_CURSOR",
          "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_CURSOR_EXPIRED"].includes(error.message)) {
        const reason = error.message.endsWith("_EXPIRED") ? "EXPIRED" : "INVALID";
        await recordShipmentApvInvoiceRestorationRemediationRecoveryCursorRejection(db, { reason })
          .catch(() => undefined);
  ...
  app.post("/admin/shipments/apv-invoice-documents/restoration-remediation-recovery-cursor-metrics/maintenance", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    ...
    const retentionJobHealth = await getShipmentApvRemediationCursorRetentionJobHealth(db)
      .catch(() => null);
    const retentionJobStatus = getShipmentApvRemediationCursorRetentionJobStatus();
    return reply.send({ restoration_remediation_recovery_cursor_maintenance: maintenance,
      restoration_remediation_recovery_cursor_retention_job: {
        ...retentionJobStatus, health: retentionJobHealth,
        alertAssessment: retentionJobHealth
          ? evaluateShipmentApvCursorRetentionAlert({ ...retentionJobStatus, health: retentionJobHealth }) : null,
  ...
  app.post("/admin/shipments/apv-invoice-documents/restoration-remediation-requests/:requestId/recovery-actions", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    ...
    if (!parsed.success) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_ACTION" });
    }
    const result = await recordShipmentApvInvoiceRestorationRemediationAcknowledgment(db, {
      requestId, clientRequestId: parsed.data.client_request_id,
      decisionRequestId: parsed.data.decision_request_id, checkerId: request.user!.id,
      action: parsed.data.action, expectedVersion: parsed.data.expected_version,
    ...
    if (result.outcome === "not_stale_enough") {
      return reply.code(409).send({ error: "APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_NOT_STALE_ENOUGH" });
    }
    if (result.outcome === "acknowledgment_required") {
      return reply.code(409).send({ error: "APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_ACKNOWLEDGMENT_REQUIRED" });
    }
    if (["invalid_state", "request_conflict", "incident_conflict", "action_conflict"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_INVOICE_RESTORATION_REMEDIATION_RECOVERY_${result.outcome.toUpperCase()}` });
    }
  ...
  app.get("/admin/shipments/apv-invoice-documents/restoration-remediation-requests/:requestId/timeline", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    if (!z.string().uuid().safeParse(requestId).success) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_ID" });
    ...
    if (!timeline) return reply.code(404).send({ error: "APV_INVOICE_RESTORATION_REMEDIATION_NOT_FOUND" });
    return reply.send({ restoration_remediation_timeline: timeline });
  });

  app.post("/admin/shipments/apv-invoice-documents/restoration-remediation-requests/:requestId/decision", { preHandler: [requireAuth] }, async (request, reply) => {
    if (request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
    const { requestId } = request.params as { requestId: string };
    if (!z.string().uuid().safeParse(requestId).success) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_ID" });
    ...
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_DECISION" });
    ...
    if (result.outcome === "self_approval_forbidden") {
      return reply.code(403).send({ error: "APV_INVOICE_RESTORATION_REMEDIATION_SELF_APPROVAL_FORBIDDEN" });
    }
    if (["candidate_changed", "version_conflict", "invalid_state", "decision_conflict"].includes(result.outcome)) {
      return reply.code(409).send({ error: `APV_INVOICE_RESTORATION_REMEDIATION_${result.outcome.toUpperCase()}` });
    }
    ...
    if (result.outcome === "invalid_request" || !("request" in result)) {
      return reply.code(400).send({ error: "INVALID_APV_INVOICE_RESTORATION_REMEDIATION_DECISION" });
    }
    return reply.send({ restoration_remediation_request: result.request,
      idempotent: result.outcome === "duplicate" });
  });

  app.get("/shipments/apv-liabilities", { preHandler: [requireAuth] }, async (request, reply) => {
    const query = z.object({ seller_id: z.string().uuid().optional() }).safeParse(request.query ?? {});
    if (!query.success) return reply.code(400).send({ error: "INVALID_APV_LIABILITY_QUERY", issues: query.error.issues });
    if (query.data.seller_id && request.user?.role !== "admin") return reply.code(403).send({ error: "ADMIN_REQUIRED" });
  ...
  app.post("/shipments/:id/prepare", { preHandler: [requireAuth, requireShipmentOwner({ role: "seller" })] }, async (request, reply) => {
    ...
    const quotedAt = new Date().toISOString();
    const mockRates = [
      { id: "rate_mock_ground", carrier: "USPS", service: "GroundAdvantage", rate: "5.50", rate_minor: 550, est_delivery_days: 5, source: "mock" as const, quoted_at: quotedAt },
      { id: "rate_mock_priority", carrier: "USPS", service: "Priority", rate: "8.25", rate_minor: 825, est_delivery_days: 3, source: "mock" as const, quoted_at: quotedAt },
      { id: "rate_mock_express", carrier: "USPS", service: "Express", rate: "26.35", rate_minor: 2635, est_delivery_days: 1, source: "mock" as const, quoted_at: quotedAt },
      { id: "rate_mock_ups", carrier: "UPS", service: "Ground", rate: "9.50", rate_minor: 950, est_delivery_days: 5, source: "mock" as const, quoted_at: quotedAt },
      { id: "rate_mock_fedex", carrier: "FedEx", service: "Ground", rate: "9.75", rate_minor: 975, est_delivery_days: 5, source: "mock" as const, quoted_at: quotedAt },
    ];
    ...
    if (easypostApiKey && rate_id.startsWith("rate_") && !rate_id.startsWith("rate_mock_")) {
      if (preparedRate.source !== "easypost" || !preparedRate.easypost_shipment_id) {
        const responseBody = {
          ...
          message: "Selected EasyPost rate does not belong to a prepared EasyPost shipment quote.",
          rate_id,
        };
        await completeShipmentOperation(db, "shipment.purchase_label", idempotency, 400, responseBody);
        return reply.code(400).send(responseBody);
      ...
      try {
        ...
        if (!rateToBuy) {
          const responseBody = {
            ...
            message: "The prepared EasyPost rate is no longer available on the prepared shipment. Run /prepare again.",
            rate_id,
            easypost_shipment_id: preparedRate.easypost_shipment_id,
          };
          await completeShipmentOperation(db, "shipment.purchase_label", idempotency, 409, responseBody);
          return reply.code(409).send(responseBody);
        }

        const boughtShipment = await client.Shipment.buy(preparedRate.easypost_shipment_id, rateToBuy);
  ...
  app.post("/shipments/:id/refund-label", { preHandler: [requireAuth, requireShipmentOwner({ role: "seller" })] }, async (request, reply) => {
    const shipmentId = (request.params as { id: string }).id;
    const shipment = await getShipmentById(db, shipmentId);
    if (!shipment) return reply.code(404).send({ error: "SHIPMENT_NOT_FOUND" });
    const parsed = refundLabelSchema.safeParse(request.body ?? {});
    if (!parsed.success) return reply.code(400).send({ error: "INVALID_LABEL_REFUND_REQUEST", issues: parsed.error.issues });

    const idempotency = await beginShipmentOperationIdempotency(db, request, reply, "shipment.refund_label", shipmentId);
    if (idempotency.replayed) return;
    ...
    if (claim.outcome !== "acquired") {
      ...
      const responseBody = {
        ...
        message: claim.outcome === "invalid_status"
          ? "Only an unscanned LABEL_CREATED shipment can request a label refund"
          : "The existing label refund state was preserved",
      };
      await completeShipmentOperation(db, "shipment.refund_label", idempotency, mapped.code, responseBody);
      return reply.code(mapped.code).send(responseBody);
    }

    const providerShipmentId = typeof shipment.metadata?.easypost_shipment_id === "string"
    ...
    try {
      ...
      if (shipment.carrier === "mock" && !requiresRealShippingProvider()) {
        ...
        if (!easypostApiKey || !providerShipmentId) {
          ...
          const responseBody = realShippingUnavailable();
          await completeShipmentOperation(db, "shipment.refund_label", idempotency, 503, responseBody);
          return reply.code(503).send(responseBody);
        }
        const EasyPost = (await import("@easypost/api")).default;
        const client = new EasyPost(easypostApiKey);
      ...
      if (!providerStatus) {
        await failShipmentLabelRefund(db, claim);
        const responseBody = { error: "LABEL_REFUND_PROVIDER_STATUS_INVALID", message: "Shipping provider returned an unknown refund state" };
        await completeShipmentOperation(db, "shipment.refund_label", idempotency, 502, responseBody);
        return reply.code(502).send(responseBody);
      }
      const completed = await completeShipmentLabelRefund(db, claim, providerStatus, providerShipmentId ?? `mock:${shipmentId}`);
      if (!completed) {
        const responseBody = { error: "LABEL_REFUND_CLAIM_LOST", message: "Label refund state changed while the provider request was running" };
        await completeShipmentOperation(db, "shipment.refund_label", idempotency, 409, responseBody);
        return reply.code(409).send(responseBody);
      }
      await updateCommerceOrderStatus(
        db,
        shipment.order_id,
        providerStatus === "SUBMITTED" || providerStatus === "REFUNDED" ? "FULFILLMENT_PENDING" : "FULFILLMENT_ACTIVE",
      );
      ...
      const responseBody = {
        ...
        money_effect: "NONE",
        message: providerStatus === "SUBMITTED"
      ...
      await completeShipmentOperation(db, "shipment.refund_label", idempotency, responseCode, responseBody as Record<string, unknown>);
      return reply.code(responseCode).send(responseBody);
    } catch (error) {
      await failShipmentLabelRefund(db, claim);
      console.error("EasyPost label refund failed:", safeRedactShippingLog(error));
      const responseBody = { error: "LABEL_REFUND_PROVIDER_FAILED", message: "Shipping provider could not process the label refund request" };
      await completeShipmentOperation(db, "shipment.refund_label", idempotency, 502, responseBody);
      return reply.code(502).send(responseBody);
    }
  });
  ...
  app.post("/shipments/:id/refund-label/status", { preHandler: [requireAuth, requireShipmentOwner({ role: "seller" })] }, async (request, reply) => {
    const shipmentId = (request.params as { id: string }).id;
    const shipment = await getShipmentById(db, shipmentId);
    if (!shipment) return reply.code(404).send({ error: "SHIPMENT_NOT_FOUND" });
    if (shipment.label_refund_status !== "SUBMITTED") {
      return reply.send({ shipment, refund_status: shipment.label_refund_status ?? "NONE", refreshed: false });
    }
    const providerShipmentId = typeof shipment.metadata?.easypost_shipment_id === "string"
      ? shipment.metadata.easypost_shipment_id
      : null;
    if (!easypostApiKey || !providerShipmentId) return reply.code(503).send(realShippingUnavailable());
    try {
      const EasyPost = (await import("@easypost/api")).default;
      const client = new EasyPost(easypostApiKey);
      ...
      if (!providerStatus) {
        return reply.code(502).send({ error: "LABEL_REFUND_PROVIDER_STATUS_INVALID", message: "Shipping provider returned an unknown refund state" });
      }
      ...
      return reply.send({ shipment: await getShipmentById(db, shipmentId), refund_status: providerStatus, refreshed: true });
    } catch (error) {
      console.error("EasyPost label refund status failed:", safeRedactShippingLog(error));
      return reply.code(502).send({ error: "LABEL_REFUND_STATUS_FAILED", message: "Shipping provider refund status is temporarily unavailable" });
    }
  });
  ...
  app.post("/shipments/:id/event", { preHandler: [requireAuth, requireShipmentOwner({ role: "seller" })] }, async (request, reply) => {
    ...
    if (shipment.label_refund_status === "REQUESTING" || shipment.label_refund_status === "SUBMITTED" || shipment.label_refund_status === "REFUNDED") {
      return reply.code(409).send({
        error: "LABEL_REFUND_BLOCKS_SHIPMENT_EVENT",
        message: "This label cannot be advanced while its refund is requesting, submitted, or refunded",
      });
    }
    ...
    try {
      const payload = parsed.data.payload as Partial<Pick<import("@haggle/shipping-core").ShipmentEvent, "carrier_raw_status" | "message" | "location">> | undefined;
  ...
  app.post("/shipments/webhooks/easypost", {
    ...
    if (invoice) {
      let acquiredApvClaim: Extract<Awaited<ReturnType<typeof claimShipmentApvAdjustment>>, { outcome: "acquired" }> | null = null;
      try {
        ...
        if (!release) {
          console.warn("APV invoice: no settlement release for order", { order_id: shipmentRow.order_id });
          await completeWebhookEvent(db, webhookClaim, 200);
          return reply.send({ accepted: true, skipped: true, reason: "settlement release not found" });
        }

        const providerShipmentId = typeof shipmentRow.metadata?.easypost_shipment_id === "string"
          ? shipmentRow.metadata.easypost_shipment_id
          : null;
        if (!providerShipmentId) {
          await completeWebhookEvent(db, webhookClaim, 409);
          return reply.code(409).send({ accepted: false, error: "APV_PROVIDER_SHIPMENT_ID_MISSING" });
        }
        ...
        if (invoice.invoice_event === "updated") {
          ...
          if (revision.outcome === "not_found" || revision.outcome === "base_revision_missing") {
            ...
            return reply.code(503).send({ accepted: false, error: "APV_REVISION_BASE_NOT_READY" });
          }
          if (revision.outcome === "identity_conflict") {
            await completeWebhookEvent(db, webhookClaim, 409);
            return reply.code(409).send({ accepted: false, error: "APV_REVISION_IDENTITY_CONFLICT" });
          }
          if (revision.outcome === "amount_conflict") {
            await completeWebhookEvent(db, webhookClaim, 409);
            return reply.code(409).send({ accepted: false, error: "APV_REVISION_AMOUNT_CONFLICT" });
          }
