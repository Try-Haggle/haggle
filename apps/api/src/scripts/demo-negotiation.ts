...
console.log(
  `│  💰 구매자 목표: $${buyerStrategy.p_target} / 판매자 목표: $${sellerStrategy.p_target}            │`,
);
console.log(
  `│  🚫 구매자 한계: $${buyerStrategy.p_limit} / 판매자 한계: $${sellerStrategy.p_limit}              │`,
);
console.log(
  `│  ⏱️  데드라인: ${buyerStrategy.t_deadline / 60}분                                         │`,
);
console.log("└─────────────────────────────────────────────────────────────┘");
console.log("");
...
function printRound(
  ...
  console.log(
    `  │ V_p=${result.utility.v_p.toFixed(3)} V_t=${result.utility.v_t.toFixed(3)} V_r=${result.utility.v_r.toFixed(3)} V_s=${result.utility.v_s.toFixed(3)}`,
  );
  console.log(
    `  │ 결정:     ${actionIcon} ${action}${action === "COUNTER" || action === "NEAR_DEAL" ? ` → $${responsePrice}` : ""}`,
  );
  console.log("");
}
...
function printDeal(price: number, acceptedBy: string, round: number, elapsed: number) {
  ...
  console.log(
    `  ║  📊 시장가 대비: ${savings >= 0 ? `-$${savings} (${savingsPct}% 절약)` : `+$${Math.abs(savings)}`}`.padEnd(
      60,
    ) + "║",
  );
  console.log(
    `  ║  🤝 수락 측: ${acceptedBy === "BUYER" ? "Alice (구매자)" : "Bob (판매자)"}`.padEnd(60) +
      "║",
  );
...
function printSummary(
  ...
  console.log(
    `│  양보 없는 라운드: 구매자 ${buyer.rounds_no_concession}회 / 판매자 ${seller.rounds_no_concession}회`,
  );
  console.log(`│  ⏱️  총 소요: ${Math.floor(elapsed / 60)}분`);
  console.log(
    `│  🧮 엔진 연산: ${buyer.current_round + seller.current_round}회 (LLM 0회, 순수 수학)`,
  );
  ...
  if (dealMade) {
    ...
    console.log(
      `│  💡 판매자: 한계가 대비 $${sellerGained} 추가 수익 (limit $${sellerStrategy.p_limit})`,
    );
