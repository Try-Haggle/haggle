...
function reconstructMemory(buyerPrice: number): CoreMemory {
  ...
  const beta =
    s.opponent_pattern === "BOULWARE" ? 2.0 : s.opponent_pattern === "CONCEDER" ? 1.5 : 1.0;
  const faratin = s.seller_target + (s.seller_floor - s.seller_target) * t ** (1 / beta);
  ...
  const u_total = u_price * 0.55 + u_time * 0.25 + 0.2; // simplified

  const warnings: string[] = [];
  if (timePressure > 0.7) warnings.push("⏰ 라운드 부족 — 마감 압박");
  if (gap / range < 0.1) warnings.push("🤏 갭 10% 미만 — NEAR_DEAL 영역");
  if (s.convergence_rate < 0.01 && s.history.length >= 3)
    warnings.push("🔄 정체 감지 — 전략 전환 필요");
  ...
  return {
    ...
    coaching: {
      ...
      suggested_tactic:
        gap / range < 0.1
          ? "near_deal_acceptance"
          : timePressure > 0.7
            ? "time_pressure_close"
            : s.opponent_pattern === "BOULWARE"
              ? "nibble"
...
function skillEvaluate(mem: CoreMemory, buyerPrice: number): ProtocolDecision {
  ...
  if (buyerPrice >= session.seller_target) {
    return {
      action: "ACCEPT",
      price: buyerPrice,
      reasoning: `Buyer offer $${buyerPrice} >= target $${session.seller_target}. Auto-accept.`,
      tactic_used: "auto_accept",
    };
  }
  ...
  if (buyerPrice < session.seller_floor * 0.9) {
    return {
      action: "REJECT",
      reasoning: `Buyer offer $${buyerPrice} is 10%+ below floor $${session.seller_floor}. Unacceptable.`,
      tactic_used: "floor_protection",
    };
  }
  ...
  if (gapRatio < 0.05 && buyerPrice >= session.seller_floor) {
    return {
      action: "ACCEPT",
      price: buyerPrice,
      reasoning: `Gap $${b.gap} is < 5% of range. Near-deal acceptance.`,
...
function validateDecision(
  ...
  _mem: CoreMemory,
): Array<{ rule: string; severity: string; passed: boolean; detail: string }> {
...
function renderMessage(decision: ProtocolDecision, _phase: NegotiationPhase): string {
...
async function callLLMIfNeeded(
  ...
  try {
    ...
    const finalPrice =
      llmPrice && llmPrice >= session.seller_floor && llmPrice <= session.seller_target
        ? llmPrice
        : skillDecision.price;
...
async function handleRound(buyerPrice: number, buyerMessage: string): Promise<RoundResponse> {
  ...
  if (gap / range < 0.1 && session.round >= 3) session.phase = "CLOSING";
    ...
    pipeline.push({
      ...
      data: {
        ...
        cost_usd: llmResult.tokens
          ? (llmResult.tokens.input * PRICE_INPUT_PER_M +
              llmResult.tokens.output * PRICE_OUTPUT_PER_M) /
            1_000_000
          : 0,
...
<div class="header">
  ...
  <span class="info" id="round-info">R0/` +
  MAX_ROUNDS +
  `</span>
</div>
  ...
  <div class="chat-panel">
    <div class="chat-messages" id="chat">
      <div class="msg system"><div class="msg-bubble" style="background:transparent;">
        판매자 Bob이 <span class="msg-price">$` +
  session.seller_target +
  `</span>에 ` +
  ITEM +
  `을 올렸습니다.<br>
