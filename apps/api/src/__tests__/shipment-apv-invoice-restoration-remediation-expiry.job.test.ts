...
describe("shipment APV restoration remediation expiry job", () => {
  it("expires a bounded batch without accepting a file or actor", async () => {
    vi.mocked(expireShipmentApvInvoiceRestorationRemediations).mockResolvedValueOnce({
      scanned: 2, expired: 2, limit: 25, truncated: false, recordedAt: "2026-07-12T00:00:00.000Z",
    });
    await expect(runShipmentApvInvoiceRestorationRemediationExpiry({} as Database, { limit: 25 }))
      .resolves.toMatchObject({ status: "completed", expiry: { expired: 2, limit: 25 } });
    expect(expireShipmentApvInvoiceRestorationRemediations).toHaveBeenCalledWith(expect.anything(),
      expect.objectContaining({ limit: 25 }));
  });

  it("converges to a healthy no-op", async () => {
    vi.mocked(expireShipmentApvInvoiceRestorationRemediations).mockResolvedValueOnce({
      scanned: 0, expired: 0, limit: 100, truncated: false, recordedAt: "2026-07-12T00:00:00.000Z",
    });
    await expect(runShipmentApvInvoiceRestorationRemediationExpiry({} as Database))
