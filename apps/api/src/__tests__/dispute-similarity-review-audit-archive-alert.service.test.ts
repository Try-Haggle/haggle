...
const config = {
  ...
  secret: "similarity-archive-alert-secret",
  timeoutMs: 5000,
  cooldownMinutes: 15,
  staleThreshold: 1,
...
describe("similarity review audit archive alerts", () => {
  afterEach(() => {
    for (const key of Object.keys(process.env).filter((key) =>
      key.startsWith("DISPUTE_SIMILARITY_REVIEW_AUDIT_ARCHIVE_ALERT_"),
    ))
  ...
  it("classifies dead letters as critical and stale leases as warning", () => {
    ...
    expect(
      ...
      wouldAlert: true,
      severity: "warning",
      reasons: ["similarity_audit_archive_stale_processing"],
    });
  });
  ...
  it("sends an HMAC-signed aggregate payload without archive identifiers", async () => {
    ...
    await expect(
      sendDisputeSimilarityReviewAuditArchiveAlert(
        health,
        evaluateDisputeSimilarityReviewAuditArchiveAlert(health, config),
        { config, deliveryId, fetchImpl: fetchMock, now: new Date(health.recordedAt) },
      ),
  ...
  it("marks recovery payloads and derives an identifier-free delivery state", async () => {
    ...
    await sendDisputeSimilarityReviewAuditArchiveAlert(
      ...
      {
        config,
        deliveryId: `recovery_${"b".repeat(64)}`,
        fetchImpl: fetchMock,
    ...
    expect(JSON.parse(String((fetchMock.mock.calls[0]?.[1] as RequestInit).body))).toMatchObject({
      state: "recovered",
      severity: "recovery",
      reasons: ["similarity_audit_archive_recovered"],
    });
    const db = {
      execute: vi
        .fn()
    ...
    await expect(
      findLatestDeliveredDisputeSimilarityReviewAuditArchiveIncident(db),
  ...
  it("rejects unsafe targets and reports configuration state", async () => {
    await expect(
      sendDisputeSimilarityReviewAuditArchiveAlert(
        health,
        evaluateDisputeSimilarityReviewAuditArchiveAlert(health, config),
        { config, deliveryId: "invalid" },
      ),
    ).rejects.toThrow("invalid similarity review audit archive alert delivery id");
    await expect(
      sendDisputeSimilarityReviewAuditArchiveAlert(
        health,
        evaluateDisputeSimilarityReviewAuditArchiveAlert(health, config),
        { config: { ...config, url: "https://127.0.0.1/alerts" }, deliveryId },
      ),
    ).rejects.toThrow("private network");
    expect(getDisputeSimilarityReviewAuditArchiveAlertPolicyStatus().configurationState).toBe(
      "not_configured",
    );
    ...
    expect(resolveDisputeSimilarityReviewAuditArchiveAlertConfigFromEnv()).toMatchObject({
      url: config.url,
    });
    expect(getDisputeSimilarityReviewAuditArchiveAlertPolicyStatus()).toMatchObject({
      configured: true,
