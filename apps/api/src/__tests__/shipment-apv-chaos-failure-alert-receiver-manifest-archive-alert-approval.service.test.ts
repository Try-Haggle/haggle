...
describe("shipment APV receiver manifest archive alert approval request", () => {
  it("persists one append-only maker request bound to the current warning preview", async () => {
    const { db, execute, transaction } = database([[], [], [healthRow()], [approvalRow()]]);
  ...
  it("returns exact replay before consulting current health", async () => {
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertApprovalRequest(db, {
        clientRequestId,
        stateFingerprint: fingerprint,
        requestedBy: actor,
        now,
  ...
  it("rejects a healthy state and a changed warning fingerprint before writing", async () => {
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertApprovalRequest(healthy.db, {
        clientRequestId,
        stateFingerprint: fingerprint,
        requestedBy: actor,
        now,
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertApprovalRequest(changed.db, {
        clientRequestId,
        stateFingerprint: "a".repeat(64),
        requestedBy: actor,
        now,
      }),
    ).rejects.toThrow("ALERT_STATE_CHANGED");
