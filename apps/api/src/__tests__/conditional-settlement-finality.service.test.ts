...
describe("conditional settlement receipt finality", () => {
  it("uses a bounded strict confirmation policy", () => {
    ...
    expect(
      getConditionalSettlementRequiredConfirmations({
        HAGGLE_CONDITIONAL_SETTLEMENT_CONFIRMATIONS: "12",
      }),
    ).toBe(12);
    for (const value of ["0", "65", "2.5", " 2", "no"]) {
      expect(() =>
        getConditionalSettlementRequiredConfirmations({
          HAGGLE_CONDITIONAL_SETTLEMENT_CONFIRMATIONS: value,
        }),
      ).toThrow();
    }
  });
  ...
  it("does not query canonical membership before the threshold and blocks an orphaned receipt at the threshold", async () => {
    ...
    const client = {
      getBlockNumber: vi.fn().mockResolvedValueOnce(100n).mockResolvedValueOnce(101n),
      getBlock,
    };
    await expect(
      evaluateConditionalSettlementFinality({ receiptBlockNumber: 100n, receiptBlockHash, client }),
