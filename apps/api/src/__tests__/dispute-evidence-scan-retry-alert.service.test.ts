...
const circuit = (state: "CLOSED" | "OPEN" | "HALF_OPEN" = "CLOSED") => ({
  ...
  probeExpiresAt: state === "HALF_OPEN" ? "2026-07-14T08:30:30.000Z" : null,
  lastSuccessAt: null,
...
describe("dispute evidence scan retry alert", () => {
  it("classifies backlog and terminal failures without identifiers", () => {
    ...
    expect(
      evaluateDisputeEvidenceScanRetryAlert(
        health({ staleProcessing: 1, exhausted: 1 }),
        circuit(),
        config,
        retention(),
        retentionJob(),
  ...
  it("classifies blocked retention and active job failures independently", () => {
    expect(
      ...
      wouldAlert: true,
      severity: "critical",
      reasons: ["alert_snapshot_retention_job_stale", "alert_snapshot_retention_blocked"],
    });
    expect(
      evaluateDisputeEvidenceScanRetryAlert(
        health(),
      ...
      wouldAlert: true,
      severity: "warning",
      reasons: ["alert_snapshot_retention_job_failed", "alert_snapshot_retention_job_overdue"],
    });
    expect(
      evaluateDisputeEvidenceScanRetryAlert(
        health(),
  ...
  it("reports aggregate-only sender delivery failures and stale claims", async () => {
    ...
    const snapshotRow = {
      snapshotCount: 4,
      retryableSnapshots: 2,
      orphanedSnapshots: 1,
      missingRetrySnapshots: 1,
      bindingViolations: 0,
    };
    const execute = vi
      .fn()
    ...
    const serialized = JSON.stringify(
      await getDisputeEvidenceScanRetryAlertSenderHealth(
        { execute } as unknown as Database,
        "fixture-alert-source",
      ),
    );
  ...
  it("fails closed for partial, weak, or unsafe configuration", () => {
    ...
    expect(getDisputeEvidenceScanRetryAlertPolicyStatus()).toMatchObject({
      configurationState: "partial",
      configured: false,
    });
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_SECRET = "short";
    expect(() => resolveDisputeEvidenceScanRetryAlertConfigFromEnv()).toThrow("16..128");
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_SECRET = config.secret;
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_URL = "http://127.0.0.1/scan-retry";
    expect(getDisputeEvidenceScanRetryAlertPolicyStatus()).toMatchObject({
      configurationState: "invalid",
      configured: false,
    });
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_URL = config.url;
    expect(resolveDisputeEvidenceScanRetryAlertConfigFromEnv()).toMatchObject({
      url: config.url,
      retryReadyThreshold: 10,
    });
  });
  ...
  it("sends a signed aggregate-only payload and verifies it", async () => {
    ...
    const assessment = evaluateDisputeEvidenceScanRetryAlert(
      health({ exhausted: 1, failed: 1, quarantined: 1 }),
      circuit(),
      config,
      retention(),
      retentionJob(),
    ...
    expect(
      verifyDisputeEvidenceScanRetryAlert({
        ...
        timestamp: headers.get("x-haggle-alert-timestamp") ?? "",
        signature: signWebhookClaimAlertPayload(config.secret, now.toISOString(), legacyRawBody),
        deliveryId,
        secret: config.secret,
        nowMs: now.getTime(),
      }),
  ...
  it("rejects a hashable snapshot whose reasons contradict its aggregate", async () => {
    ...
    await expect(
      sendDisputeEvidenceScanRetryAlert(health({ exhausted: 1 }), circuit(), assessment, {
        config,
  ...
  it("supports bounded receiver secret rotation", () => {
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_SECRET = config.secret;
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_PREVIOUS_SECRETS = "previous-scan-retry-secret";
    expect(resolveDisputeEvidenceScanRetryAlertReceiverSecretsFromEnv()).toEqual([
      config.secret,
      "previous-scan-retry-secret",
    ]);
    expect(getDisputeEvidenceScanRetryAlertReceiverPolicyStatus()).toMatchObject({
      configurationState: "valid",
      acceptedSecretCount: 2,
    });
    process.env.DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_PREVIOUS_SECRETS = `${config.secret}`;
    expect(getDisputeEvidenceScanRetryAlertReceiverPolicyStatus()).toMatchObject({
      configurationState: "invalid",
  ...
  it("redacts transport failures", async () => {
    const fetchImpl = vi.fn().mockRejectedValue(new Error("https://secret-host?token=private"));
    await expect(
      sendDisputeEvidenceScanRetryAlert(
        health({ retryReady: 10 }),
        circuit(),
        ...
        {
          config,
          deliveryId: `health_${"c".repeat(64)}`,
          retention: retention(),
          retentionJob: retentionJob(),
          fetchImpl: fetchImpl as typeof fetch,
        },
