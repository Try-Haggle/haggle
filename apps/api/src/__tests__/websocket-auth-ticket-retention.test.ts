...
describe("WebSocket ticket retention", () => {
  it("deletes one bounded batch while holding the distributed lock", async () => {
    ...
    await expect(runWebSocketAuthTicketRetention(db as never, { batchSize: 25 })).resolves.toEqual({
      acquired: true,
      deleted: 3,
      batchSize: 25,
    });
  ...
  it("returns a non-mutating skip when another instance owns the lock", async () => {
    ...
    await expect(runWebSocketAuthTicketRetention(db as never)).resolves.toEqual({
      acquired: false,
      deleted: 0,
      batchSize: 1000,
    });
  ...
  it("validates fixture scope and batch bounds before storage access", async () => {
    const db = transactionDb([]);
    await expect(runWebSocketAuthTicketRetention(db as never, { batchSize: 0 })).rejects.toThrow(
      "INVALID_WEBSOCKET_TICKET_RETENTION_BATCH_SIZE",
    );
    await expect(
      runWebSocketAuthTicketRetention(db as never, { fixtureUserId: "not-uuid" }),
  ...
  it("registers a five-minute run-on-start job and reports scheduler state", () => {
    ...
    expect(
      buildJobRegistry().find((job) => job.name === "websocket-auth-ticket-retention"),
    ).toMatchObject({
      enabled: true,
      runOnStart: true,
      intervalMs: 300_000,
