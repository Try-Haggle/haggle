...
vi.mock("../jobs/websocket-auth-ticket-retention.js", () => ({
  runWebSocketAuthTicketRetention: vi.fn().mockResolvedValue({
    acquired: true, deleted: 0, batchSize: 1_000,
  }),
}));

const mockMaintain = vi.mocked(maintainShipmentApvInvoiceRestorationRemediationRecoveryCursorMetrics);
...
describe("shipment APV remediation cursor retention job", () => {
  it("reports only bounded non-secret job configuration", () => {
    ...
    expect(getShipmentApvRemediationCursorRetentionJobStatus()).toEqual({
      jobEnabled: true, configured: true, retentionDays: 45, limit: 250, intervalSeconds: 86_400,
    });
  });
  ...
  it("runs one apply batch and reports completion", async () => {
    ...
    mockMaintain.mockResolvedValueOnce({ dryRun: false, retentionDays: 30, limit: 1000,
      eligibleBuckets: undefined, deletedBuckets: 2, expiredBuckets: 1, invalidBuckets: 1,
    ...
    await expect(runShipmentApvRemediationCursorRetention(db, {
      now, finishedAt, retentionDays: 30, limit: 1000,
    ...
    expect(mockMaintain).toHaveBeenCalledWith(expect.anything(), {
      retentionDays: 30, limit: 1000, dryRun: false, now,
    });
  ...
  it("records a fixed failure state and rethrows the maintenance error", async () => {
    ...
    await expect(runShipmentApvRemediationCursorRetention(db))
      .rejects.toThrow("database unavailable with private detail");
    expect(execute).toHaveBeenCalledTimes(2);
  });

  it("returns aggregate stale-run health without exposing claim identifiers", async () => {
    const { db } = databaseWithResults([{
      status: "RUNNING", lease_expires_at: "2026-07-13T00:14:59.000Z",
      last_started_at: "2026-07-13T00:00:00.000Z", last_succeeded_at: null,
      last_failed_at: null, last_deleted_buckets: 2, last_expired_buckets: 1,
      last_invalid_buckets: 1, last_truncated: false, last_failure_code: null,
    }]);
    const health = await getShipmentApvRemediationCursorRetentionJobHealth(
      db, new Date("2026-07-13T00:15:00.000Z"));
    expect(health).toMatchObject({ lastRunStatus: "STALE_RUNNING", leaseStale: true,
      lastDeletedBuckets: 2, lastExpiredBuckets: 1, lastInvalidBuckets: 1 });
  ...
  it("returns the persisted first observation for a job that has never started", async () => {
    const { db } = databaseWithResults([{
      status: "NEVER", lease_expires_at: null, first_observed_at: "2026-07-12T00:00:00.000Z",
      ...
      last_truncated: false, last_failure_code: null,
    }]);
    await expect(getShipmentApvRemediationCursorRetentionJobHealth(
      db, new Date("2026-07-13T00:00:00.000Z")))
      .resolves.toMatchObject({ lastRunStatus: "NEVER",
        firstObservedAt: "2026-07-12T00:00:00.000Z", lastStartedAt: null });
  });
