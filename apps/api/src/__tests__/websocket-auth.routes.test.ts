...
function dbWith(executeResults: unknown[][], error?: Error) {
  ...
  const transaction = vi.fn(async (fn: (tx: { execute: typeof execute }) => unknown) => fn({ execute }));
...
describe("WebSocket ticket HTTP boundary", () => {
  it("issues a no-store notification ticket for an authenticated user", async () => {
    const app = Fastify();
    app.addHook("onRequest", async (request) => { request.user = { id: USER_ID, role: "authenticated" }; });
    registerWebSocketAuthRoutes(app, dbWith([[], [], [],
      [{ expires_at: new Date(Date.now() + 30_000) }]]));
    const response = await app.inject({
      method: "POST", url: "/auth/websocket-tickets", payload: { channel: "notification" },
    });
  ...
  it("rejects unauthenticated, extra-field, and non-participant requests", async () => {
    ...
    expect((await anonymous.inject({ method: "POST", url: "/auth/websocket-tickets",
      payload: { channel: "notification" } })).statusCode).toBe(401);
    await anonymous.close();

    const app = Fastify();
    app.addHook("onRequest", async (request) => { request.user = { id: USER_ID }; });
    registerWebSocketAuthRoutes(app, dbWith([[]]));
    expect((await app.inject({ method: "POST", url: "/auth/websocket-tickets",
      payload: { channel: "notification", session_id: USER_ID } })).statusCode).toBe(400);
    expect((await app.inject({ method: "POST", url: "/auth/websocket-tickets",
      payload: { channel: "negotiation", session_id: "11111111-1111-4111-8111-111111111111" } })).statusCode).toBe(403);
    await app.close();
  });
  ...
  it("consumes a matching ticket before the handler runs", async () => {
    const app = Fastify();
    app.get("/probe", {
      preValidation: createWebSocketTicketPreValidation(dbWith([[{ user_id: USER_ID }]]), "notification"),
  ...
  it("rejects an untrusted browser origin before consuming its ticket", async () => {
    ...
    const allowed = await app.inject({ method: "GET", url: "/probe",
      headers: { origin: "http://127.0.0.1:4177",
        "sec-websocket-protocol": protocol } });
  ...
  it("fails invalid, replayed, and unavailable stores before upgrade", async () => {
    ...
    const replayed = await invalid.inject({ method: "GET", url: "/probe",
      headers: { "sec-websocket-protocol": `haggle-ticket.${"a".repeat(43)}` } });
    ...
    unavailable.get("/probe", {
      preValidation: createWebSocketTicketPreValidation(dbWith([], new Error("postgresql://secret")), "notification"),
    }, async () => ({ ok: true }));
    const failed = await unavailable.inject({ method: "GET", url: "/probe",
