...
vi.mock("../services/dispute-evidence-scanner-circuit.service.js", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("../services/dispute-evidence-scanner-circuit.service.js")
    >();
  return {
    ...actual,
...
describe("scanner circuit integration", () => {
  it("acquires and finalizes a permit around a valid scanner response", async () => {
    vi.mocked(acquireDisputeEvidenceScannerPermit).mockResolvedValue(permit);
    ...
    expect(finalizeDisputeEvidenceScannerPermit).toHaveBeenCalledWith(
      db,
      permit,
      expect.objectContaining({ scannerOperational: true }),
    );
  });
  ...
  it("fails closed when permit finalization cannot be persisted", async () => {
    ...
    ).rejects.toThrow("SCANNER_CIRCUIT_FINALIZE_FAILED");
  });
});
