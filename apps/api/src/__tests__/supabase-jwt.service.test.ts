...
describe("Supabase JWT verification boundary", () => {
  it("accepts a valid ES256 token and exposes only aggregate policy", async () => {
    ...
    expect(verifier.policyStatus()).toEqual(
  ...
  it("rejects HS256 algorithm confusion before requesting JWKS", async () => {
    const fetcher = vi.fn();
    const verifier = new SupabaseJwtVerifier(config(), fetcher as typeof fetch);
    const token = jwt.sign({ sub: "user-123", iss: issuer, aud: audience }, "not-a-public-key", {
      algorithm: "HS256",
    });
  ...
  it("rejects a signed non-UUID subject before it reaches UUID-backed routes", async () => {
    ...
    await expect(verifier.verify(signedToken(key, "user-123"))).rejects.toThrow(
  ...
  it("rejects duplicate key ids, non-JSON, and oversized JWKS responses", async () => {
    ...
    const duplicate = new SupabaseJwtVerifier(
      config(),
      vi.fn().mockResolvedValue(response([key.jwk, key.jwk])) as typeof fetch,
    );
    const nonJson = new SupabaseJwtVerifier(
      config(),
      vi
        .fn()
        .mockResolvedValue(response([key.jwk], { "content-type": "text/html" })) as typeof fetch,
    );
    const oversized = new SupabaseJwtVerifier(
