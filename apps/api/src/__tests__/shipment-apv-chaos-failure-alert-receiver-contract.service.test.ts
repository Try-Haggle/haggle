...
const signatureBase64 = signer.signMessage(
  Buffer.from(`haggle.shipment-apv-failure-alert.payload-sha256.v1:${payloadSha256}`, "utf8"),
);
...
describe("shipment APV local receiver contract", () => {
  it("verifies the local no-network receiver envelope", async () => {
    await expect(
      verifyShipmentApvFailureAlertReceiverContract(database([row()]), {
  ...
  it("rejects a canonical payload hash mismatch", async () => {
    await expect(
      verifyShipmentApvFailureAlertReceiverContract(
  ...
  it("rejects a signed hash whose payload violates the receiver schema", async () => {
    ...
    const invalidSignature = signer.signMessage(
      Buffer.from(`haggle.shipment-apv-failure-alert.payload-sha256.v1:${invalidHash}`, "utf8"),
    );
    await expect(
      verifyShipmentApvFailureAlertReceiverContract(
        database([
  ...
  it("rejects an invalid Ed25519 signature", async () => {
    await expect(
      verifyShipmentApvFailureAlertReceiverContract(
        database([row({ signature_base64: Buffer.alloc(64).toString("base64") })]),
        { deliveryIntentId: intentId, now },
      ),
    ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CONTRACT_REJECTED");
  });
  ...
  it("rejects stale and future signatures", async () => {
    for (const signedAt of ["2026-07-13T17:54:59.999Z", "2026-07-13T18:00:05.001Z"]) {
      await expect(
        verifyShipmentApvFailureAlertReceiverContract(database([row({ signed_at: signedAt })]), {
          deliveryIntentId: intentId,
          now,
        }),
      ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CONTRACT_REJECTED");
  ...
  it("rejects a non-current or rebound registry key", async () => {
    for (const change of [
      ...
      await expect(
        verifyShipmentApvFailureAlertReceiverContract(database([row(change)]), {
          deliveryIntentId: intentId,
          now,
        }),
      ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CONTRACT_REJECTED");
  ...
  it("rejects a missing intent and any intent that claims an HTTP side effect", async () => {
    await expect(
      verifyShipmentApvFailureAlertReceiverContract(database([]), {
        deliveryIntentId: intentId,
        now,
      }),
    ...
    await expect(
      verifyShipmentApvFailureAlertReceiverContract(
        database([row({ http_request_created: true })]),
        { deliveryIntentId: intentId, now },
      ),
    ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CONTRACT_REJECTED");
  });
