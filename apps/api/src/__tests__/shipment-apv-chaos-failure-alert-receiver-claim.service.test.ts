...
function claim(overrides: Record<string, unknown> = {}) {
  return {
    ...
    received_at: "2026-07-13T18:00:00.000Z",
    inserted: true,
    ...overrides,
  };
}
...
describe("shipment APV local receiver claims", () => {
  it("stores one persistent replay-protected dry-run claim", async () => {
    ...
    await expect(
      createShipmentApvFailureAlertReceiverClaim(db, { deliveryIntentId: intentId }),
  ...
  it("fails closed when receiver contract verification fails", async () => {
    ...
    await expect(
      createShipmentApvFailureAlertReceiverClaim(database([]), { deliveryIntentId: intentId }),
  ...
  it("rejects a delivery id or payload collision returned by a concurrent insert", async () => {
    await expect(
      createShipmentApvFailureAlertReceiverClaim(
        database([], [binding], [claim({ payload_sha256: "d".repeat(64), inserted: false })]),
        { deliveryIntentId: intentId },
      ),
    ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CLAIM_CONFLICT");
  });
  ...
  it("fails closed when a concurrent winner cannot be reloaded", async () => {
    await expect(
      createShipmentApvFailureAlertReceiverClaim(database([], [binding], [], []), {
        deliveryIntentId: intentId,
      }),
    ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CLAIM_UNAVAILABLE");
  });
});
