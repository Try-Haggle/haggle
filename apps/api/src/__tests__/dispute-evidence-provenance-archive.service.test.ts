...
function archive(): DisputeEvidenceProvenanceArchiveRecord {
  return {
    ...
    payload: {
      schema: "haggle.dispute-evidence-provenance-archive.v1",
...
describe("dispute evidence provenance archive", () => {
  it("requires complete signed provenance before touching the database", async () => {
    ...
    await expect(
      enqueueDisputeEvidenceProvenanceArchive(db as never, {
        evidence: {
  ...
  it("accepts only a receipt with the exact stored payload hash", async () => {
    ...
    await expect(
      deliverDisputeEvidenceProvenanceArchive(value, config, { fetchImpl }),
    ).resolves.toMatchObject({
      status: "delivered",
      receiptId: "worm-receipt-1",
      receiptSha256: value.payloadSha256,
    });
    expect(fetchImpl).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          "idempotency-key": value.archiveKey,
          "x-haggle-content-sha256": value.payloadSha256,
        }),
      }),
    );
  });
  ...
  it("rejects a successful HTTP response with a mismatched receipt hash", async () => {
    ...
    await expect(
      deliverDisputeEvidenceProvenanceArchive(archive(), config, { fetchImpl }),
    ).resolves.toMatchObject({
      status: "failed",
