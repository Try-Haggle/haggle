...
vi.mock("../services/dispute-evidence-scan-retry-alert.service.js",
...
const health = (exhausted = 1) => ({
  ...
  totals: { quarantined: exhausted, pending: 0, failed: exhausted,
    processing: 0, staleProcessing: 0, retryReady: 0, exhausted,
...
const circuit = (state: "CLOSED" | "OPEN" = "CLOSED") => ({
  ...
  state, consecutiveFailures: state === "CLOSED" ? 0 : 3,
  activePermits: 0,
  ...
  probeExpiresAt: null, lastSuccessAt: null,
...
DisputeEvidenceScanRetryAlertSnapshotRetentionHealth => ({
  ...
  eligibleExpired: 0, blockedExpired,
...
const retentionJob = (
  ...
  status: "healthy", lastRunStatus: "SUCCEEDED", overdue: false,
  leaseStale: false, firstObservedAt: "2026-07-13T08:30:00.000Z",
  lastStartedAt: "2026-07-14T08:29:00.000Z",
  lastSucceededAt: "2026-07-14T08:30:00.000Z", lastFailedAt: null,
...
describe("dispute evidence scan retry alert job", () => {
  beforeEach(() => {
    vi.mocked(getDisputeEvidenceScanRetryAlertSnapshotRetentionHealth)
      .mockResolvedValue(retention());
    vi.mocked(getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth)
      .mockResolvedValue(retentionJob());
    vi.mocked(findRetryableDisputeEvidenceScanRetryAlertSnapshot)
      .mockResolvedValue(null);
    vi.mocked(hasRecentDeliveredDisputeEvidenceScanRetryIncident)
      .mockResolvedValue(false);
    vi.mocked(persistDisputeEvidenceScanRetryAlertSnapshot)
      .mockImplementation(async (_db, _source, snapshot) => ({
        snapshot,
  ...
  it("retries the immutable snapshot before creating a new cooldown alert", async () => {
    ...
    vi.mocked(getDisputeEvidenceScannerCircuitHealth)
    ...
    const saved = createDisputeEvidenceScanRetryAlertSnapshot(
      ...
      }, config, `health_${"9".repeat(64)}`,
      retention(), retentionJob(),
    );
    vi.mocked(findRetryableDisputeEvidenceScanRetryAlertSnapshot)
      .mockResolvedValueOnce({ snapshot: saved, payloadSha256: "e".repeat(64) });
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({
      outcome: "acquired", source: "source", eventId: saved.delivery_id,
      claimId: crypto.randomUUID(), attemptCount: 2,
    });
    vi.mocked(sendDisputeEvidenceScanRetryAlert).mockResolvedValueOnce({
      status: "delivered", httpStatus: 204,
    });

    await expect(runDisputeEvidenceScanRetryAlert({} as Database, {
      config, now: new Date("2026-07-14T09:31:00.000Z"),
    })).resolves.toMatchObject({
      status: "retried", phase: "incident", attemptCount: 2,
    });
  ...
  it("delivers one claimed incident and suppresses another server", async () => {
    ...
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "acquired", source: "source",
        eventId: `health_${"a".repeat(64)}`, claimId: crypto.randomUUID(),
        attemptCount: 1 })
      .mockResolvedValueOnce({ outcome: "duplicate", source: "source",
        eventId: `health_${"a".repeat(64)}` });
    vi.mocked(sendDisputeEvidenceScanRetryAlert).mockResolvedValueOnce({
      status: "delivered", httpStatus: 204,
    });
    await expect(runDisputeEvidenceScanRetryAlert({} as Database, { config }))
      .resolves.toMatchObject({ status: "delivered" });
    await expect(runDisputeEvidenceScanRetryAlert({} as Database, { config }))
      .resolves.toMatchObject({
        status: "skipped", reason: "cooldown_or_in_progress",
      });
  ...
  it("marks failed delivery for bounded claim retry", async () => {
    ...
    vi.mocked(getDisputeEvidenceScannerCircuitHealth)
      .mockResolvedValue(circuit());
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({
      outcome: "acquired", source: "source",
      eventId: `health_${"b".repeat(64)}`, claimId: crypto.randomUUID(),
      attemptCount: 1,
    });
    vi.mocked(sendDisputeEvidenceScanRetryAlert).mockResolvedValueOnce({
  ...
  it("distinguishes sender retry backoff and claim conflicts", async () => {
    ...
    vi.mocked(getDisputeEvidenceScannerCircuitHealth)
      .mockResolvedValue(circuit());
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "retry_later", source: "source",
        eventId: `health_${"f".repeat(64)}`, retryAfterSeconds: 2 })
      .mockResolvedValueOnce({ outcome: "payload_conflict", source: "source",
        eventId: `health_${"f".repeat(64)}` });
    await expect(runDisputeEvidenceScanRetryAlert({} as Database, { config }))
      .resolves.toMatchObject({
        status: "skipped", reason: "delivery_retry_backoff",
      });
    await expect(runDisputeEvidenceScanRetryAlert({} as Database, { config }))
      .resolves.toMatchObject({
        status: "skipped", reason: "delivery_claim_payload_conflict",
      });
  ...
  it("sends one recovery only after a delivered incident", async () => {
    ...
    vi.mocked(findLatestDeliveredDisputeEvidenceScanRetryIncident)
      .mockResolvedValueOnce({ eventId: `health_${"c".repeat(64)}`,
    ...
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "acquired", source: "source",
        ...
        claimId: crypto.randomUUID(), attemptCount: 1 })
      .mockResolvedValueOnce({ outcome: "duplicate", source: "source",
        eventId: `recovery_${"d".repeat(64)}` });
    vi.mocked(sendDisputeEvidenceScanRetryAlert).mockResolvedValueOnce({
      status: "delivered", httpStatus: 204,
  ...
  it("stays silent when healthy without an incident", async () => {
    ...
    await expect(runDisputeEvidenceScanRetryAlert({} as Database, { config }))
      .resolves.toMatchObject({
        status: "skipped", reason: "healthy_no_delivered_incident",
      });
  ...
  it("alerts on an open circuit before a retry backlog exists", async () => {
    vi.mocked(getDisputeEvidenceScanRetryHealth).mockResolvedValue(health(0));
    vi.mocked(getDisputeEvidenceScannerCircuitHealth)
      .mockResolvedValue(circuit("OPEN"));
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({
      outcome: "acquired", source: "source",
      eventId: `health_${"e".repeat(64)}`, claimId: crypto.randomUUID(),
      attemptCount: 1,
    });
    vi.mocked(sendDisputeEvidenceScanRetryAlert).mockResolvedValueOnce({
      status: "delivered", httpStatus: 204,
  ...
  it("alerts when expired snapshots are blocked without a retry backlog", async () => {
    ...
    vi.mocked(getDisputeEvidenceScannerCircuitHealth)
      .mockResolvedValue(circuit());
    vi.mocked(getDisputeEvidenceScanRetryAlertSnapshotRetentionHealth)
      .mockResolvedValue(retention(1));
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({
      outcome: "acquired", source: "source",
      eventId: `health_${"7".repeat(64)}`, claimId: crypto.randomUUID(),
      attemptCount: 1,
    });
    vi.mocked(sendDisputeEvidenceScanRetryAlert).mockResolvedValueOnce({
      status: "delivered", httpStatus: 204,
