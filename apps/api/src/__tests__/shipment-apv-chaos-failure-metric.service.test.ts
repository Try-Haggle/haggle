...
describe("shipment APV chaos failure metrics", () => {
  ...
  it("rejects an unknown stage before touching the database", async () => {
    const execute = vi.fn();
    await expect(recordShipmentApvChaosFailure(
  ...
  it("returns a fixed identifier-free 24 hour stage aggregate", async () => {
    const execute = vi.fn().mockResolvedValue([
      { stage: "rollback_verification", failure_count: "3",
      ...
      { stage: "fixture_execution", failure_count: "2147483648",
      ...
      { stage: "secret_database_stage", failure_count: "99",
        last_failure_at: "2026-07-13T13:00:00.000Z" },
    ]);
    await expect(getShipmentApvChaosFailureHealth(
      { execute } as unknown as Pick<Database, "execute">,
      ...
      stages: {
        ...
        fixture_execution: { count: 2_147_483_647,
  ...
  it("uses fixed warning and critical boundaries per stage", () => {
    const evaluate = (rollback: number, isolation: number, execution: number) =>
      evaluateShipmentApvChaosFailurePolicy({ rollback_verification: rollback,
        rollback_failure_isolation: isolation, fixture_execution: execution });
    expect(evaluate(0, 0, 0)).toMatchObject({ status: "healthy", reasons: [] });
    expect(evaluate(1, 0, 0)).toMatchObject({ status: "warning",
      reasons: ["rollback_verification_warning"] });
    expect(evaluate(3, 0, 0)).toMatchObject({ status: "critical",
      reasons: ["rollback_verification_critical"] });
    expect(evaluate(0, 0, 2)).toMatchObject({ status: "healthy", reasons: [] });
    expect(evaluate(0, 0, 3)).toMatchObject({ status: "warning",
      reasons: ["fixture_execution_warning"] });
    expect(evaluate(0, 0, 10)).toMatchObject({ status: "critical",
  ...
  it("derives a recovered lifecycle from the latest retained hourly bucket", async () => {
    const execute = vi.fn().mockResolvedValue([{
      stage: "rollback_verification", failure_count: "0",
    ...
    expect(health).toMatchObject({ status: "healthy", total: 0, lifecycle: {
      phase: "recovered",
