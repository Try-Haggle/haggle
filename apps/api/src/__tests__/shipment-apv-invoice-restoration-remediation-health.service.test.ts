    ...
    execute: vi.fn().mockResolvedValue([
      {
        pending_requests: 0,
        applying_requests: 0,
        expiring_soon_requests: 0,
        overdue_pending_requests: 0,
        stale_applying_requests: 0,
        stale_applying_over_15_minutes: 0,
        stale_applying_over_60_minutes: 0,
        unacknowledged_stale_over_60_minutes: 0,
        incident_unlinked_stale_over_60_minutes: 0,
        ...
        oldest_applying_age_seconds: null,
        ...row,
      },
    ]),
...
describe("shipment APV restoration remediation health", () => {
  it.each([
    ...
    const health = await getShipmentApvInvoiceRestorationRemediationHealth(
      dbRow({ oldest_applying_age_seconds: age }),
  ...
  it("returns aggregate escalation counts without request identifiers", async () => {
    const health = await getShipmentApvInvoiceRestorationRemediationHealth(
      dbRow({
        applying_requests: 3,
        stale_applying_requests: 3,
        stale_applying_over_15_minutes: 2,
        stale_applying_over_60_minutes: 1,
    ...
    expect(health).toMatchObject({
      status: "critical",
      applyingRequests: 3,
      staleApplyingRequests: 3,
      staleApplyingOver15Minutes: 2,
    ...
    expect(JSON.stringify(health)).not.toMatch(
      /requestId|decisionRequestId|approverId|path|sha256/i,
    );
  });
