...
const config = {
  ...
  secret: "ai-archive-alert-secret",
  timeoutMs: 5000,
  cooldownMinutes: 15,
  staleThreshold: 1,
...
describe("dispute AI audit archive alerts", () => {
  afterEach(() => {
    for (const key of Object.keys(process.env).filter((key) =>
      key.startsWith("DISPUTE_AI_AUDIT_ARCHIVE_ALERT_"),
    ))
      delete process.env[key];
  });

  it("classifies dead letters as critical and stale leases as warning", () => {
    expect(evaluateDisputeAiAuditArchiveAlert(health, config)).toEqual({
      wouldAlert: true,
      severity: "critical",
      reasons: [
        "ai_audit_archive_dead_letter",
    ...
    expect(
      evaluateDisputeAiAuditArchiveAlert(
        { ...health, deadLetter: 0, overdueUnfinished: 0 },
        config,
      ),
  ...
  it("alerts independently on retry-ready and overdue backlogs", () => {
    expect(
      evaluateDisputeAiAuditArchiveAlert(
        { ...health, deadLetter: 0, staleProcessing: 0, retryReady: 5, overdueUnfinished: 0 },
        config,
      ),
    ).toMatchObject({ reasons: ["ai_audit_archive_retry_ready_backlog"] });
    expect(
      evaluateDisputeAiAuditArchiveAlert(
        { ...health, deadLetter: 0, staleProcessing: 0, retryReady: 0 },
        config,
      ),
  ...
  it("alerts on unresolved discovery failures and escalates oversized chains", () => {
    ...
    expect(
      evaluateDisputeAiAuditArchiveAlert(healthyArchive, config, discoveryHealth),
    ).toMatchObject({ severity: "warning", reasons: ["ai_audit_discovery_failure_unresolved"] });
    expect(
      evaluateDisputeAiAuditArchiveAlert(healthyArchive, config, {
        ...discoveryHealth,
        status: "critical",
        tooLarge: 1,
      }),
      ...
      severity: "critical",
      reasons: ["ai_audit_discovery_failure_unresolved", "ai_audit_discovery_failure_too_large"],
    });
  });
  ...
  it("sends an HMAC-signed aggregate payload without archive identifiers", async () => {
    ...
    await expect(
      sendDisputeAiAuditArchiveAlert(health, evaluateDisputeAiAuditArchiveAlert(health, config), {
        config,
        deliveryId,
        fetchImpl: fetchMock,
    ...
    expect(String(request.body)).not.toMatch(
      /dispute_id|archive_id|archive_key|payload_sha256|receipt_id/,
    );
    expect(JSON.parse(String(request.body)).discovery_failure_health).toMatchObject({
      unresolved: 1,
      invalidChain: 1,
    });
  });
  ...
  it("reports recovery lifecycle without claim identifiers", async () => {
    const db = {
      execute: vi.fn().mockResolvedValue([
        {
          last_incident_at: "2026-07-12T00:00:00.000Z",
          last_recovery_at: "2026-07-12T00:05:00.000Z",
        },
      ]),
    } as any;
    await expect(getDisputeAiAuditArchiveAlertDeliveryState(db)).resolves.toEqual({
      incidentOpen: false,
  ...
  it("rejects unsafe targets and reports configuration state", async () => {
    ...
    ).rejects.toThrow("invalid dispute AI");
    await expect(
      sendDisputeAiAuditArchiveAlert(health, evaluateDisputeAiAuditArchiveAlert(health, config), {
