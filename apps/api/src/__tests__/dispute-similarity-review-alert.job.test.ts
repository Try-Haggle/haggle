...
vi.mock("../services/dispute-record.service.js", () => ({
  getDisputeEvidenceSimilarityReviewHealth: vi.fn(),
}));
vi.mock("../services/dispute-similarity-review-alert.service.js", async (importOriginal) => {
  const original =
    await importOriginal<typeof import("../services/dispute-similarity-review-alert.service.js")>();
...
describe("dispute similarity review alert job", () => {
  ...
  it("skips when unconfigured or healthy", async () => {
    await expect(runDisputeSimilarityReviewAlert({} as Database)).resolves.toEqual({
      status: "skipped",
    ...
    await expect(runDisputeSimilarityReviewAlert({} as Database)).resolves.toMatchObject({
      status: "skipped",
      reason: "healthy",
    });
  });
  ...
  it("delivers one claimed alert and suppresses a duplicate server", async () => {
    ...
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({
        outcome: "acquired",
        source: "haggle-dispute-similarity-review-alert",
        eventId: "health",
    ...
    vi.mocked(sendDisputeSimilarityReviewAlert).mockResolvedValueOnce({
      status: "delivered",
      httpStatus: 200,
    });
    await expect(runDisputeSimilarityReviewAlert({} as Database)).resolves.toMatchObject({
