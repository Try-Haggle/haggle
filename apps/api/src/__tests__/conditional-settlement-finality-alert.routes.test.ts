...
describe("conditional settlement finality alert receiver routes", () => {
  ...
  it("accepts and completes a valid current-secret delivery", async () => {
    ...
    const response = await app.inject({
      method: "POST",
      url: "/internal/ops/alerts/conditional-settlement-finality",
  ...
  it("bounds and validates the complete receiver rotation set", () => {
    delete process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_SECRET;
    process.env.CONDITIONAL_SETTLEMENT_FINALITY_ALERT_PREVIOUS_SECRETS = previousSecret;
    expect(() => resolveConditionalSettlementFinalityAlertReceiverSecretsFromEnv()).toThrow(
    ...
    expect(getConditionalSettlementFinalityAlertReceiverPolicyStatus()).toMatchObject({
      configurationState: "valid",
      acceptedSecretCount: 4,
    ...
    expect(
      verifyConditionalSettlementFinalityAlert({
        ...
        timestamp,
        deliveryId,
        signature: headers(raw, timestamp)["x-haggle-alert-signature"],
        secret: [
          secret,
          previousSecret,
          "previous-secret-02",
  ...
  it("keeps in-progress and failed deliveries retryable", async () => {
    ...
    expect(inProgress.statusCode).toBe(503);
    expect(inProgress.headers["retry-after"]).toBe("2");
  ...
  it("rejects signed payloads with unapproved fields", async () => {
    ...
    const app = await makeApp();
    const timestamp = new Date().toISOString();
    const raw = body(timestamp, { payment_id: "pi_secret" });
    const response = await app.inject({
      method: "POST",
      url: "/internal/ops/alerts/conditional-settlement-finality",
      headers: headers(raw, timestamp),
    ...
    expect(response.json()).toEqual({ error: "INVALID_ALERT_BODY" });
    expect(claimWebhookEvent).not.toHaveBeenCalled();
    await app.close();
  });
  ...
  it("fails closed when unconfigured or stale and does not acknowledge failed completion", async () => {
    ...
    const freshRaw = body(fresh);
    expect(
      (
        await app.inject({
          method: "POST",
          url: "/internal/ops/alerts/conditional-settlement-finality",
          headers: headers(freshRaw, fresh),
          payload: freshRaw,
        })
      ).statusCode,
    ).toBe(503);
  ...
  it("returns aggregate receiver health only to admins", async () => {
    ...
    const response = await admin.inject({
      method: "GET",
      url: "/admin/ops/alerts/conditional-settlement-finality/health",
    });
