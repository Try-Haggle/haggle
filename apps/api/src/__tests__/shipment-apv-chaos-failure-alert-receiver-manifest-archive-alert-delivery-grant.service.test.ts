...
function bindingRow(overrides: Record<string, unknown> = {}) {
  return { id: null, client_grant_id: null, approval_decision_id: decisionId,
    state_fingerprint: fingerprint, status: "GRANTED_DRY_RUN",
    granted_by: checker, granted_at: "2026-07-14T00:04:00.000Z",
    cooldown_expires_at: "2026-07-14T00:19:00.000Z", inserted: false,
    decision: "APPROVED", decision_reason: "checker_approved_snapshot",
    decided_by: checker, decided_at: "2026-07-14T00:04:00.000Z",
    approval_request_id: requestId, preview_schema_version: previewSchema,
    approval_state_fingerprint: fingerprint, preview_action: "review_warning",
    ...
    prior_grant_id: null, prior_client_grant_id: null, prior_granted_by: null,
    ...overrides };
}

function grantRow(overrides: Record<string, unknown> = {}) {
  return { ...bindingRow(), id: grantId, client_grant_id: clientGrantId,
    granted_at: now.toISOString(), cooldown_expires_at: "2026-07-14T00:20:00.000Z",
...
const input = { approvalDecisionId: decisionId, clientGrantId,
  grantedBy: checker, now, grantId };
...
describe("receiver manifest archive alert delivery grant", () => {
  it("claims the state cooldown and records a non-delivering grant", async () => {
    const { db, execute, transaction } = database([
      [], [], [bindingRow()], [healthRow()], [grantRow()],
    ]);
    const result =
      await createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(db, input);
    expect(result).toMatchObject({
      schemaVersion:
        "shipment-apv-failure-alert-receiver-manifest-archive-alert-delivery-grant-v1",
      deliveryGrantId: grantId, clientGrantId, approvalDecisionId: decisionId,
      stateFingerprint: fingerprint, status: "GRANTED_DRY_RUN",
  ...
  it("returns an exact immutable replay before decision and health reads", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(db, input))
  ...
  it("fails closed on replay key rebinding or malformed stored data", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(
      rebound.db, input)).rejects.toThrow("GRANT_REPLAY_CONFLICT");

    const malformed = database([[], [grantRow({ status: "DELIVERED" })]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(
      malformed.db, input)).rejects.toThrow("GRANT_REPLAY_CONFLICT");
  });

  it("requires an approved decision and the original checker", async () => {
    const rejected = database([[], [], [bindingRow({ decision: "REJECTED",
      decision_reason: "checker_rejected_snapshot" })]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(
      rejected.db, input)).rejects.toThrow("DECISION_NOT_APPROVED");

    const wrongActor = database([[], [], [bindingRow()]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(
      wrongActor.db, { ...input, grantedBy: maker }))
  ...
  it("blocks expired approvals and current-state changes", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(
      expired.db, input)).rejects.toThrow("APPROVAL_REQUEST_EXPIRED");
  ...
  it("rejects invalid decision bindings before current-state evaluation", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryGrant(
      invalid.db, input)).rejects.toThrow("DECISION_INVALID");
