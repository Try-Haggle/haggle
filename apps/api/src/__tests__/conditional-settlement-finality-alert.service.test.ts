...
const health = (patch: Partial<ConditionalSettlementFinalityHealth> = {}): ConditionalSettlementFinalityHealth => ({
  status: "healthy", total: 0, pending: 0, unavailable: 0, orphanedReceipts: 0, rpcUnavailable: 0,
  configurationBlocked: 0, overduePending: 0, oldestPendingAgeSeconds: null, pendingSlaSeconds: 120,
  recordedAt: "2026-07-12T00:00:00.000Z", ...patch,
});
...
describe("conditional settlement finality alert", () => {
  it("classifies orphaned receipts as critical and backlog as warning", () => {
    expect(evaluateConditionalSettlementFinalityAlert(health({ orphanedReceipts: 1 }))).toMatchObject({ severity: "critical", reasons: ["orphaned_receipt"] });
    expect(evaluateConditionalSettlementFinalityAlert(health({ rpcUnavailable: 1, overduePending: 2 }))).toMatchObject({
      severity: "warning", reasons: ["confirmation_sla_overdue", "rpc_unavailable"],
    });
    expect(evaluateConditionalSettlementFinalityAlert(health())).toMatchObject({ wouldAlert: false, severity: null });
  });
  ...
  it("sends only aggregate health with HMAC metadata", async () => {
    ...
    const result = await sendConditionalSettlementFinalityAlert(health({ orphanedReceipts: 1, total: 1 }),
      { wouldAlert: true, severity: "critical", reasons: ["orphaned_receipt"] }, {
        config: { url: "http://127.0.0.1:9999/alert", secret: "fixture-secret-1234", timeoutMs: 1000,
          cooldownMinutes: 15, allowInsecureHttp: true, allowPrivateNetwork: true }, deliveryId, fetchImpl,
    ...
    expect(JSON.parse(request.body)).toMatchObject({ type: "conditional_settlement_finality.health", delivery_id: deliveryId,
  ...
  it("reports configuration state without exposing endpoint or secret", () => {
    ...
    expect(policy).toMatchObject({ configured: true, configurationState: "valid", timingSafe: true });
