...
import type { Checkpoint, CoreMemory, NegotiationPhase, RoundFact } from "../negotiation/types.js";
...
function makeCheckpoint(phase: NegotiationPhase, version: number): Checkpoint {
  return {
    ...
    core_memory_snapshot: {
      ...
      coaching: {
        recommended_price: 82000,
        acceptable_range: { min: 78000, max: 86000 },
        suggested_tactic: "anchor",
        hint: "",
        opponent_pattern: "LINEAR" as const,
...
describe("PgCheckpointPersistence", () => {
  ...
  beforeEach(async () => {
    ...
    const { PgCheckpointPersistence } = await import(
      "../negotiation/memory/pg-checkpoint-persistence.js"
    );
...
describe("PgRoundFactSink", () => {
  ...
  it("flush() hash chain verification: computeFactHash produces deterministic result", async () => {
    ...
    const { PgRoundFactSink: PgRoundFactSink2 } = await import(
      "../negotiation/memory/pg-round-fact-sink.js"
    );
