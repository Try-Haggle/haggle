...
describe("camera challenge verification", () => {
  it("keeps camera evidence pending when no verifier is configured", async () => {
    ...
    await expect(verifyCameraChallenge({
  ...
  it("returns the verifier confidence for a matching challenge", async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      ...
      visual_observations: [
        { category: "visible_damage", observation: "  Dent on the lower-left corner\n", confidence: 0.87 },
        { category: "unknown", observation: "ignored", confidence: 1 },
        { category: "item_condition", observation: "ignored invalid confidence", confidence: 2 },
      ],
    ...
    expect(result).toMatchObject({
      ...
      visualObservations: [{ category: "visible_damage", observation: "Dent on the lower-left corner", confidence: 0.87 }],
    });
    expect(fetchMock).toHaveBeenCalledWith("https://vision.example/verify", expect.objectContaining({
      method: "POST",
