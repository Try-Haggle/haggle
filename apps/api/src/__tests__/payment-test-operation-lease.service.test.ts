...
describe("payment test operation lease", () => {
  it("acquires the global finality fixture lease", async () => {
    ...
    const execute = vi.fn().mockResolvedValue([
      {
        key: FINALITY_ALERT_FIXTURE_LEASE_KEY,
        leaseId: "11111111-1111-4111-8111-111111111111",
        ownerId: "admin-1",
        expiresAt: new Date(now.getTime() + PAYMENT_TEST_OPERATION_LEASE_SECONDS * 1000),
      },
    ]);
    const lease = await acquireFinalityAlertFixtureLease(
  ...
  it("returns null while another owner holds the lease", async () => {
    const execute = vi.fn().mockResolvedValue([]);
    await expect(
      acquireFinalityAlertFixtureLease({ execute } as unknown as Database, {
  ...
  it("renews only the active global lease token", async () => {
    ...
    await expect(
      renewFinalityAlertFixtureLease(
        { execute } as unknown as Database,
        "11111111-1111-4111-8111-111111111111",
        new Date("2026-07-12T18:04:00.000Z"),
      ),
    ).resolves.toBe(true);
  });
  ...
  it("acquires, renews, and releases the shipment APV fixture lease", async () => {
    ...
    const execute = vi
      ...
      .mockResolvedValueOnce([
        {
          key: SHIPMENT_APV_CHAOS_FIXTURE_LEASE_KEY,
          leaseId,
          ownerId: "shipment-apv-fixture",
          expiresAt: now,
        },
      ])
    ...
    await expect(
      acquireShipmentApvChaosFixtureLease(db, { leaseId, ownerId: "shipment-apv-fixture" }, now),
  ...
  it("verifies expiry takeover and old-owner fencing", async () => {
    ...
    const result = await runFinalityAlertFixtureLeaseVerification(
      { execute } as unknown as Database,
      {
        keySuffix: "test",
        firstLeaseId: "first",
