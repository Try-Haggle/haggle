...
import type { BuddyDNA, CoreMemory, NegotiationPhase, RoundFact } from "../negotiation/types.js";
...
describe("computeCoaching", () => {
  ...
  it("OPENING: seller anchor never exceeds the published asking price", () => {
    // A seller's `my_target` IS the price on the listing. The raw anchor is
    // `my_target * (1 + margin)`, which is how a $120 listing answered a $95 offer with
    // $130 — a number the buyer can see is above the ask. The envelope caps it.
    const memory = makeMemory({ phase: "OPENING", role: "seller", my_target: 500, my_floor: 400 });
    const coaching = computeCoaching(memory, [], null, makeBuddyDna());
    expect(coaching.recommended_price).toBeLessThanOrEqual(500);
    expect(coaching.recommended_price).toBeGreaterThanOrEqual(400);
    expect(coaching.acceptable_range.max).toBeLessThanOrEqual(500);
  });
  ...
  it("convergence_rate positive when gap is shrinking", () => {
    const facts = [makeFact(1, 500, 700), makeFact(2, 520, 680), makeFact(3, 540, 660)];
    const memory = makeMemory();
