...
function body(timestamp: string, overrides: Record<string, unknown> = {}) {
  return JSON.stringify({ type: "webhook_claim.health", delivery_id: deliveryId, state: "firing", created_at: timestamp,
    severity: "critical", reasons: ["stale_processing", "failed", "retry_ready"],
    totals: { processing: 1, completed: 10, failed: 1, staleProcessing: 1, retryReady: 1 },
    sources: [{ source: "conditional_settlement_finality_receiver", processing: 1, failed: 1,
      stale_processing: 1, retry_ready: 1, max_attempt_count: 2, oldest_unfinished_age_seconds: 90 }], ...overrides });
}
...
describe("webhook claim health alert receiver routes", () => {
  ...
  it("accepts and completes a valid aggregate firing delivery", async () => {
    ...
    const response = await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(raw, timestamp), payload: raw });
    expect(response.statusCode).toBe(202);
    expect(response.json()).toMatchObject({ accepted: true, state: "firing", severity: "critical" });
  ...
  it("accepts a recovery signed by an overlap secret", async () => {
    ...
    const raw = body(timestamp, { state: "recovered", severity: "recovery", reasons: ["webhook_claim_recovered"],
      totals: { processing: 0, completed: 11, failed: 0, staleProcessing: 0, retryReady: 0 }, sources: [] });
    const response = await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(raw, timestamp, previousSecret), payload: raw });
    expect(response.statusCode).toBe(202); expect(response.json()).toMatchObject({ state: "recovered", severity: "recovery" });
    await app.close();
  });
  ...
  it("returns completed replay and isolates payload conflicts", async () => {
    ...
    const app = await makeApp(); const timestamp = new Date().toISOString(); const raw = body(timestamp);
    expect((await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
  ...
  it("keeps in-progress and failed deliveries retryable", async () => {
    ...
    const app = await makeApp(); const timestamp = new Date().toISOString(); const raw = body(timestamp);
    const processing = await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(raw, timestamp), payload: raw });
    expect(processing.statusCode).toBe(503); expect(processing.headers["retry-after"]).toBe("2");
    const backoff = await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(raw, timestamp), payload: raw });
  ...
  it("rejects signed unknown fields and contradictory aggregates before claiming", async () => {
    ...
    expect((await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(extra, timestamp), payload: extra })).statusCode).toBe(400);
    ...
    expect((await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(mismatch, timestamp), payload: mismatch })).statusCode).toBe(400);
  ...
  it("rejects stale, invalid signatures, and invalid receiver configuration", async () => {
    process.env.WEBHOOK_CLAIM_ALERT_SECRET = secret;
    const app = await makeApp(); const stale = new Date(Date.now() - 301_000).toISOString(); const staleRaw = body(stale);
    expect((await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(staleRaw, stale), payload: staleRaw })).statusCode).toBe(401);
    const current = new Date().toISOString(); const currentRaw = body(current);
    expect((await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(currentRaw, current, "wrong-secret-with-length"), payload: currentRaw })).statusCode).toBe(401);
    process.env.WEBHOOK_CLAIM_ALERT_PREVIOUS_SECRETS = "short";
    const invalid = await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(currentRaw, current), payload: currentRaw });
    expect(invalid.statusCode).toBe(503); expect(invalid.json()).toEqual({ error: "OPS_ALERT_RECEIVER_INVALID_CONFIGURATION" });
    await app.close();
  });
  ...
  it("marks completion failures retryable", async () => {
    ...
    const response = await app.inject({ method: "POST", url: "/internal/ops/alerts/webhook-claim-health",
      headers: headers(raw, timestamp), payload: raw });
    expect(response.statusCode).toBe(503); expect(response.headers["retry-after"]).toBe("2");
    expect(failWebhookEvent).toHaveBeenCalledOnce(); await app.close();
  });
  ...
  it("returns aggregate receiver health only to admins", async () => {
    ...
    const response = await admin.inject({ method: "GET", url: "/admin/ops/alerts/webhook-claim-health/health" });
    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ receiver_kind: "webhook_claim_health",
    ...
    expect((await user.inject({ method: "GET", url: "/admin/ops/alerts/webhook-claim-health/health" })).statusCode).toBe(403);
    await user.close();
  });
});
