...
function row(overrides: Record<string, unknown> = {}) {
  ...
  const id = randomUUID();
  return { id, status: "PENDING", staging_status: "STAGED",
    staging_key: `.restoration/${id}/invoice.json`, replacement_sha256: createHash("sha256").update(bytes).digest("hex"),
    replacement_byte_size: bytes.length, expires_at: "2026-07-12T12:30:00.000Z",
    updated_at: "2026-07-12T11:59:00.000Z", created_at: "2026-07-12T11:59:00.000Z",
    bytes, ...overrides };
}
...
function dbWith(rows: Array<Record<string, unknown>>) {
  return { execute: vi.fn().mockResolvedValue(rows.map(({ bytes: _bytes, ...value }) => value)) } as unknown as Database;
}
...
describe("shipment APV invoice restoration staging health", () => {
  it("returns healthy aggregate without identifiers when no staging is tracked", async () => {
    const health = await getShipmentApvInvoiceRestorationStagingHealth(dbWith([]), { storageRoot: await root(), now });
    expect(health).toMatchObject({ status: "healthy", trackedStaging: 0, missingSources: 0,
      hashMismatches: 0, invalidEntries: 0, scanTruncated: false });
  ...
  it("classifies stale, missing, tampered, and escaping staging without exposing paths", async () => {
    ...
    const valid = row({ status: "REJECTED", staging_status: "MOVING", updated_at: "2026-07-12T11:54:00.000Z" });
    ...
    for (const item of [valid, tampered]) {
      ...
      await writeFile(path, item === tampered ? Buffer.alloc(Number(item.replacement_byte_size), 0x78) : item.bytes as Buffer);
    }
    const health = await getShipmentApvInvoiceRestorationStagingHealth(
      dbWith([valid, tampered, missing, escaping]), { storageRoot, now });
    expect(health).toMatchObject({ status: "critical", trackedStaging: 4, pendingDisposition: 4,
      staleMoving: 1, missingSources: 1, hashMismatches: 1, invalidEntries: 1 });
  ...
  it("stops hashing at the configured byte budget and reports truncation", async () => {
    ...
    const health = await getShipmentApvInvoiceRestorationStagingHealth(dbWith([candidate]), {
      storageRoot, now, maxCheckedBytes: 1,
    });
    expect(health).toMatchObject({ status: "warning", trackedStaging: 1,
      pendingDisposition: 1, checkedBytes: 0, scanTruncated: true });
  });
});
