...
describe("shipment APV cancellation audit archive delivery", () => {
  ...
  it("accepts a receipt only when the write-once store confirms the exact payload hash", async () => {
    ...
    await expect(deliverShipmentApvCancellationAuditArchive(archive, config, { fetchImpl: fetchMock }))
      .resolves.toEqual({
        status: "delivered",
        httpStatus: 201,
  ...
  it("blocks private archive destinations unless explicitly allowed", async () => {
    await expect(deliverShipmentApvCancellationAuditArchive(archive, {
      ...config,
  ...
  it("refuses to archive a non-terminal lifecycle", async () => {
    await expect(enqueueShipmentApvCancellationAuditArchive({} as Database, {
  ...
  it("requeues only delivery state while preserving the signed payload hash and auditing once", async () => {
    const row = {
      id: archive.id, archive_key: archive.archiveKey, cancellation_request_id: archive.cancellationRequestId,
      payload: archive.payload, payload_sha256: archive.payloadSha256, status: "DEAD_LETTER", attempt_count: 3,
      next_attempt_at: archive.nextAttemptAt, lease_token: null, lease_expires_at: null, last_error: "HTTP 503",
      http_status: 503, receipt_id: null, receipt_sha256: null, delivered_at: null,
      created_at: archive.createdAt, updated_at: archive.updatedAt,
    };
    const execute = vi.fn()
      .mockResolvedValueOnce([row])
      .mockResolvedValueOnce([{ ...row, status: "PENDING", attempt_count: 0, last_error: null, http_status: null }])
      .mockResolvedValueOnce([]);
    const db = { transaction: (callback: (tx: { execute: typeof execute }) => unknown) => callback({ execute }) } as unknown as Database;
    ...
    expect(result).toMatchObject({ outcome: "requeued", archive: { status: "PENDING", attemptCount: 0, payloadSha256: archive.payloadSha256, payload: archive.payload } });
    expect(execute).toHaveBeenCalledTimes(3);
  });

  it("marks an old pending archive as attention before it dead-letters", async () => {
    const execute = vi.fn().mockResolvedValue([{ pending: 1, processing: 0, failed: 0, dead_letter: 0, stale_processing: 0, retry_ready: 0, overdue_unfinished: 1, oldest_unfinished_age_seconds: 901 }]);
    const result = await getShipmentApvCancellationAuditArchiveHealth({ execute } as unknown as Database, new Date("2026-07-12T01:00:00.000Z"));
    expect(result).toMatchObject({ status: "attention", pending: 1, overdueUnfinished: 1, unfinishedMaxAgeMinutes: 15, oldestUnfinishedAgeSeconds: 901 });
  });

  it("distinguishes missing, invalid, and valid delivery configuration", () => {
    expect(getShipmentApvCancellationAuditArchiveDeliveryPolicyStatus().configurationState).toBe("not_configured");
    process.env.HAGGLE_AUDIT_ARCHIVE_URL = "http://archive.example/audits";
    expect(getShipmentApvCancellationAuditArchiveDeliveryPolicyStatus().configurationState).toBe("invalid");
    process.env.HAGGLE_AUDIT_ARCHIVE_URL = "https://archive.example/audits";
    expect(getShipmentApvCancellationAuditArchiveDeliveryPolicyStatus()).toMatchObject({ configured: true, configurationState: "valid" });
  });

  it("lists failed archives with an opaque cursor and without signed payloads", async () => {
    const row = (id: string, createdAt: string) => ({
      id, archive_key: `apvca_${"a".repeat(64)}`, cancellation_request_id: archive.cancellationRequestId,
      payload: { secret: "not returned" }, payload_sha256: archive.payloadSha256, status: "DEAD_LETTER", attempt_count: 3,
      next_attempt_at: createdAt, lease_token: null, lease_expires_at: null, last_error: "HTTP 503", http_status: 503,
      receipt_id: null, receipt_sha256: null, delivered_at: null, created_at: createdAt, updated_at: createdAt,
    });
    ...
    const result = await listShipmentApvCancellationAuditArchiveFailures({ execute } as unknown as Database, { limit: 1, now: new Date("2026-07-12T01:00:00.000Z") });
  ...
  it("rejects a malformed failure cursor before querying the database", async () => {
    const execute = vi.fn();
    await expect(listShipmentApvCancellationAuditArchiveFailures({ execute } as unknown as Database, { cursor: "broken" }))
