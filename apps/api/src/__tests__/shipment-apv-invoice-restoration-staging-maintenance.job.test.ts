...
const result = { mode: "apply" as const, scanned: 1, eligible: 1, expired: 1,
  preserved: 1, resumed: 0, sourceMissing: 0, conflicts: 0, truncated: false };
...
describe("shipment APV invoice restoration staging maintenance job", () => {
  it("skips without an explicit system actor", async () => {
    await expect(runShipmentApvInvoiceRestorationStagingMaintenance({} as Database)).resolves.toEqual({
  ...
  it("applies bounded maintenance and reports completed work", async () => {
    ...
    await expect(runShipmentApvInvoiceRestorationStagingMaintenance({} as Database, {
      actorId, limit: 25,
    })).resolves.toEqual({ status: "completed", reason: undefined, maintenance: result });
    expect(maintainShipmentApvInvoiceRestorationStaging).toHaveBeenCalledWith(expect.anything(),
  ...
  it("converges to a healthy no-op after the queue is empty", async () => {
    vi.mocked(maintainShipmentApvInvoiceRestorationStaging).mockResolvedValueOnce({
      ...result, scanned: 0, eligible: 0, expired: 0, preserved: 0,
    });
    await expect(runShipmentApvInvoiceRestorationStagingMaintenance({} as Database, { actorId }))
      .resolves.toMatchObject({ status: "skipped", reason: "healthy", maintenance: { eligible: 0 } });
  });
});
