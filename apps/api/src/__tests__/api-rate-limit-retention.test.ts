...
const original = {
  ...
  HAGGLE_API_RATE_LIMIT_HMAC_SECRET: process.env.HAGGLE_API_RATE_LIMIT_HMAC_SECRET,
};
...
describe("API rate-limit retention", () => {
  it("deletes only a bounded batch and returns aggregate counts", async () => {
    const db = {
      execute: vi.fn().mockResolvedValue([{ deleted: 1 }, { deleted: 1 }, { deleted: 1 }]),
    };
    await expect(
      runApiRateLimitRetention(db as never, {
        retentionHours: 48,
        batchSize: 25,
      }),
    ).resolves.toEqual({
  ...
  it("registers hourly run-on-start retention only in distributed mode", () => {
    ...
    expect(buildJobRegistry().find((job) => job.name === "api-rate-limit-retention")).toMatchObject(
      {
        enabled: false,
        runOnStart: true,
