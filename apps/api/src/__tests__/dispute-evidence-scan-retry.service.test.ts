...
const claim: DisputeEvidenceScanRetryClaim = {
  ...
  storagePath:
    "dispute-evidence/22222222-2222-4222-8222-222222222222/evidence.png",
...
describe("dispute evidence scan retry", () => {
  it("validates retry policy and caps exponential backoff", () => {
    ...
    expect(() => resolveDisputeEvidenceScanRetryConfigFromEnv())
      .toThrow(/batch size/);
  ...
  it("maps an atomic database claim without exposing SQL state", async () => {
    ...
    const claims = await claimDisputeEvidenceScanRetries(db, {
      now: new Date("2026-07-14T00:00:00.000Z"), config,
    });
    expect(claims).toHaveLength(1);
    expect(claims[0]).toMatchObject({
      uploadId: claim.uploadId, attemptCount: 2,
    });
  ...
  it("fences stale finalizers and accepts only the current lease owner", async () => {
    ...
    const result = {
      ...
      provider: "scanner.test", detail: "CLEAN", sha256: "a".repeat(64),
    };
    expect(await finalizeDisputeEvidenceScanRetry(
      db, claim, result, { config },
    )).toBe(false);
    expect(await finalizeDisputeEvidenceScanRetry(
      db, claim, result, { config },
    )).toBe(true);
  ...
  it("does not consume the retry budget while the shared circuit is open", async () => {
    ...
    const db = { execute } as unknown as Database;
    expect(await finalizeDisputeEvidenceScanRetry(db, claim, {
  ...
  it("returns identifier-free aggregate retry health", async () => {
    ...
    const db = { execute: vi.fn().mockResolvedValue([{
      quarantined: 4, pending: 1, failed: 1, processing: 1,
      staleProcessing: 1, retryReady: 2, exhausted: 1,
      expiredQuarantined: 0, oldestUnresolvedAgeSeconds: 300,
    }]) } as unknown as Database;
  ...
  it("skips without scanner configuration before claiming database work", async () => {
    ...
    const result = await runDisputeEvidenceScanRetry(
      { transaction } as unknown as Database,
      { retryConfig: config, scannerConfig: null },
    );
    expect(result).toMatchObject({
      status: "skipped", reason: "SCANNER_NOT_CONFIGURED", claimed: 0,
      realNetworkCalled: false, storageRead: false,
    });
  ...
  it("runs an injected retry without real network or storage", async () => {
    const execute = vi.fn()
      .mockResolvedValueOnce([{
        uploadId: claim.uploadId, disputeId: claim.disputeId,
        storagePath: claim.storagePath, contentType: claim.contentType,
        fileSizeBytes: claim.fileSizeBytes, attemptCount: 1,
        leaseToken: claim.leaseToken,
    ...
    const scannerConfig = {
      url: "https://scanner.test/scan", token: "scanner-secret-123",
      timeoutMs: 1_000, maxResponseBytes: 16_384,
    ...
    const result = await runDisputeEvidenceScanRetry(db, {
      ...
      scan: vi.fn().mockResolvedValue({
        status: "CLEAN", provider: "scanner.test", detail: "CLEAN",
        sha256: "a".repeat(64),
      }),
    });
    expect(result).toMatchObject({
      status: "completed", claimed: 1, clean: 1,
      realNetworkCalled: false, storageRead: false,
    });
  });
});
