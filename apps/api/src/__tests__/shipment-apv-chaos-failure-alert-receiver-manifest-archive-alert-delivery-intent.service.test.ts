...
import { SHIPMENT_APV_RECEIVER_MANIFEST_ARCHIVE_ALERT_SIGNING_DOMAIN } from
...
const signatureBase64 = signer.signMessage(Buffer.from(
  ...
  "utf8"));
const blockers = ["independent_trust_anchor_missing",
...
function bindingRow(overrides: Record<string, unknown> = {}) {
  return {
    intent_id: null, client_delivery_intent_id: null,
    intent_payload_signature_id: null, intent_payload_outbox_id: null,
    intent_payload_sha256: null, intent_key_id: null, intent_status: null,
    blocking_reasons: null, http_request_created: null,
    delivery_attempted: null, intent_requested_by: null, intent_created_at: null,
    signature_id: signatureId, signature_payload_outbox_id: outboxId,
    ...
    signed_by: checker, signed_at: "2026-07-14T00:07:00.000Z",
...
function intentRow(overrides: Record<string, unknown> = {}) {
  return bindingRow({ intent_id: intentId,
    client_delivery_intent_id: clientIntentId,
    intent_payload_signature_id: signatureId,
    intent_payload_outbox_id: outboxId,
    intent_payload_sha256: payloadSha256, intent_key_id: signer.keyId,
    intent_status: "BLOCKED_CONFIGURATION_DRY_RUN",
    blocking_reasons: blockers, http_request_created: false,
    delivery_attempted: false, intent_requested_by: checker,
    intent_created_at: now.toISOString(),
    current_registry_public_key: null, current_registry_event_type: null,
    current_registry_event_created_at: null, current_cooldown_grant_id: null,
    current_cooldown_claimed_at: null, current_cooldown_expires_at: null,
    ...overrides });
}
...
const input = { payloadSignatureId: signatureId,
  clientDeliveryIntentId: clientIntentId, requestedBy: checker, now };
...
describe("receiver manifest archive alert blocked delivery intent", () => {
  it("records a persistent non-executable intent with fixed blockers", async () => {
    const fixture = database([
      [], [], [bindingRow()], [healthRow()], [], [intentRow()],
    ]);
    const result =
      await createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
        fixture.db, input);
    expect(result).toMatchObject({
      schemaVersion:
        "shipment-apv-failure-alert-receiver-manifest-archive-alert-delivery-intent-v1",
      deliveryIntentId: intentId, clientDeliveryIntentId: clientIntentId,
      payloadSignatureId: signatureId, payloadOutboxId: outboxId,
      status: "BLOCKED_CONFIGURATION_DRY_RUN", blockingReasons: blockers,
  ...
  it("returns exact historical replay without current state or key reads", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
      fixture.db, input)).resolves.toMatchObject({ replayed: true,
  ...
  it("fails closed on replay rebinding and persisted intent mutation", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
      rebound.db, input)).rejects.toThrow("DELIVERY_INTENT_REPLAY_CONFLICT");
    const unsafe = database([[], [intentRow({ delivery_attempted: true })]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
      unsafe.db, input)).rejects.toThrow("DELIVERY_INTENT_REPLAY_CONFLICT");
  });

  it("requires the signing checker and active original cooldown", async () => {
    const actor = database([[], [], [bindingRow()]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
      actor.db, { ...input, requestedBy: maker }))
  ...
  it("blocks a second client intent for the same signed payload", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
      fixture.db, input)).rejects.toThrow("DELIVERY_INTENT_ALREADY_CREATED");
  });
  ...
  it("rejects malformed signature ancestry before planning delivery", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertDeliveryIntent(
      fixture.db, input)).rejects.toThrow("SIGNATURE_INVALID");
