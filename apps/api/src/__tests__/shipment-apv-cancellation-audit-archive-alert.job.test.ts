...
vi.mock(
  "../services/shipment-apv-payout-cancellation-audit-archive-alert.service.js",
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import("../services/shipment-apv-payout-cancellation-audit-archive-alert.service.js")
    >()),
    sendShipmentApvCancellationAuditArchiveAlert: vi.fn(),
  }),
);
...
describe("APV cancellation audit archive alert job", () => {
  ...
  it("skips when unconfigured or healthy", async () => {
    await expect(
      runShipmentApvCancellationAuditArchiveAlert({} as Database),
    ).resolves.toMatchObject({ status: "skipped", reason: "not_configured" });
    configure();
    vi.mocked(getShipmentApvCancellationAuditArchiveHealth).mockResolvedValueOnce({
      ...health,
      status: "healthy",
      deadLetter: 0,
    });
  ...
  it("delivers once and suppresses the cooldown duplicate", async () => {
    configure();
    vi.mocked(getShipmentApvCancellationAuditArchiveHealth).mockResolvedValue(health);
    ...
    vi.mocked(sendShipmentApvCancellationAuditArchiveAlert).mockResolvedValueOnce({
      status: "delivered",
      httpStatus: 200,
    });
    await expect(
