...
vi.mock(
  "../services/conditional-settlement-preflight-alert.service.js",
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import("../services/conditional-settlement-preflight-alert.service.js")
    >()),
...
const env = {
  ...
  HAGGLE_BASE_RPC_URL: "https://rpc.example",
  HAGGLE_CONDITIONAL_SETTLEMENT_ADDRESS: `0x${"11".repeat(20)}`,
  HAGGLE_X402_USDC_ASSET_ADDRESS: `0x${"22".repeat(20)}`,
  HAGGLE_ROUTER_RELAYER_PRIVATE_KEY: `0x${"33".repeat(32)}`,
  HAGGLE_X402_FEE_WALLET: `0x${"44".repeat(20)}`,
} as const;
...
describe("conditional settlement preflight alert job", () => {
  it("skips an unconfigured transport", async () => {
    ...
    await expect(runConditionalSettlementPreflightAlert({} as Database)).resolves.toMatchObject({
  ...
  it("delivers one firing alert and suppresses a duplicate cooldown claim", async () => {
    ...
    vi.mocked(claimWebhookEvent)
      ...
      .mockResolvedValueOnce({ outcome: "duplicate", source: "x", eventId: acquired.eventId });
    vi.mocked(sendConditionalSettlementPreflightAlert).mockResolvedValueOnce({
      status: "delivered",
      httpStatus: 200,
    });
    const runProbe = vi.fn().mockResolvedValue({
      ...
      status: "blocked",
      ready: false,
      checks: { ...probe.checks, signer_matches: false },
      blocked_by: ["signer_matches"],
    });
    await expect(
      runConditionalSettlementPreflightAlert({} as Database, { config, runProbe }),
    ).resolves.toMatchObject({ status: "delivered" });
    await expect(
      runConditionalSettlementPreflightAlert({} as Database, { config, runProbe }),
    ).resolves.toMatchObject({ reason: "cooldown_or_in_progress" });
    expect(completeWebhookEvent).toHaveBeenCalledOnce();
  });

  it("delivers one recovery and leaves a failed delivery retryable", async () => {
    Object.assign(process.env, env);
    vi.mocked(findLatestDeliveredConditionalSettlementPreflightIncident).mockResolvedValue({
      eventId: `health_${"c".repeat(64)}`,
      completedAt: probe.checked_at,
    });
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce(acquired)
      .mockResolvedValueOnce({ ...acquired, eventId: `health_${"d".repeat(64)}` });
    vi.mocked(sendConditionalSettlementPreflightAlert)
      .mockResolvedValueOnce({ status: "delivered", httpStatus: 200 })
    ...
    const failingProbe = vi.fn().mockResolvedValue({
      ...probe,
      status: "unavailable",
      ready: false,
    ...
    await expect(
      runConditionalSettlementPreflightAlert({} as Database, { config, runProbe: failingProbe }),
  ...
  it("uses an injected snapshot without reading or mutating global onchain configuration", async () => {
    ...
    await expect(
      runConditionalSettlementPreflightAlert({} as Database, { config, collectSnapshot, runProbe }),
    ).resolves.toMatchObject({ status: "delivered" });

    expect(collectSnapshot).toHaveBeenCalledOnce();
    expect(runProbe).not.toHaveBeenCalled();
    expect(Object.fromEntries(Object.keys(env).map((key) => [key, process.env[key]]))).toEqual(
      before,
    );
  });
  ...
  it("distinguishes delivery retry backoff from a cooldown duplicate", async () => {
    ...
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({
      outcome: "retry_later",
      source: "x",
      eventId: acquired.eventId,
    });

    await expect(
      runConditionalSettlementPreflightAlert({} as Database, { config, collectSnapshot }),
  ...
  it("rejects an unsafe directly injected timeout before acquiring a DB claim", async () => {
    await expect(
      runConditionalSettlementPreflightAlert({} as Database, {
