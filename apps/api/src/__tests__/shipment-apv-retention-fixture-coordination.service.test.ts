...
import { runShipmentApvRemediationCursorRetention } from "../jobs/shipment-apv-remediation-cursor-retention.js";
...
describe("shipment APV retention fixture coordination", () => {
  it("gates both singleton insert and conflict-update claims on the fixture lease", async () => {
    ...
    await expect(
      runShipmentApvRemediationCursorRetention({ execute } as unknown as Database, { now }),
    ...
    expect(query.params.filter((value) => value === "shipment-apv-chaos-fixture")).toHaveLength(2);
  ...
  it("binds the exact fixture lease id for the owner-only retention run", async () => {
    ...
    await expect(
      runShipmentApvRemediationCursorRetention({ execute } as unknown as Database, {
        now,
        fixtureLeaseId,
      }),
