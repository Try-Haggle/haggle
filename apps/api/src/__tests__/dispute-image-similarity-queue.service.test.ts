...
function row(id: string, createdAt: string) {
  return {
    id, dispute_id: "33333333-3333-4333-8333-333333333333", uploaded_by: "buyer",
    content_type: "image/jpeg", file_size_bytes: 1234, storage_path: `private/${id}.jpg`,
    similarity_distance: 4,
    similarity_signals: { distances: { dhash: 4, ahash: 2, color: 5 }, matched_signals: ["dhash_near"] },
    matched_upload_id: "44444444-4444-4444-8444-444444444444",
    matched_storage_path: "private/reference.jpg",
    expires_at: "2026-07-13T00:00:00.000Z", created_at: createdAt,
  };
}
...
describe("dispute image similarity review queue", () => {
  ...
  it("rejects malformed cursors before querying", async () => {
    const execute = vi.fn();
    await expect(listDisputeEvidenceSimilarityReviews({ execute } as unknown as Database, { cursor: "broken" }))
