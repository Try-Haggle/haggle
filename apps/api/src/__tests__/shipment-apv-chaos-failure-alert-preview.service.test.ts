...
import { evaluateShipmentApvChaosFailurePolicy } from
  "../services/shipment-apv-chaos-failure-metric.service.js";
...
describe("shipment APV failure alert preview", () => {
  it("requires explicit review for an active warning without delivering", () => {
    const preview = evaluateShipmentApvChaosFailureAlertPreview(health({ counts: [1, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z" }));
    expect(preview).toMatchObject({ mode: "preview_only", action: "review_warning",
  ...
  it("separates critical escalation from warning review", () => {
    expect(evaluateShipmentApvChaosFailureAlertPreview(health({ counts: [3, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z",
      criticalAt: "2026-07-13T10:02:00.000Z" }))).toMatchObject({
      action: "escalate_critical", severity: "critical",
      reasons: ["rollback_verification_critical"],
    });
  });

  it("previews recovery only when a threshold was previously crossed", () => {
    const recovered = evaluateShipmentApvChaosFailureAlertPreview(health({ phase: "recovered",
      warningAt: "2026-07-12T10:00:00.000Z",
      criticalAt: "2026-07-12T10:02:00.000Z" }));
    expect(recovered).toMatchObject({ action: "review_recovery", severity: "critical",
      reasons: ["recovered_from_critical"], approval: { required: true } });
    expect(evaluateShipmentApvChaosFailureAlertPreview(health({ phase: "recovered" })))
  ...
  it("returns a non-actionable preview for a clear or below-threshold active state", () => {
    expect(evaluateShipmentApvChaosFailureAlertPreview(health({ phase: "clear" })))
      .toMatchObject({ action: "none", severity: "healthy", delivery: { attempted: false } });
    expect(evaluateShipmentApvChaosFailureAlertPreview(health({ counts: [0, 0, 1] })))
      .toMatchObject({ action: "none", severity: "healthy" });
  });

  it("fails closed for an unsupported policy version or reason", () => {
    const unsupportedVersion = health({ counts: [1, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z" });
    const unsupportedReason = health({ counts: [1, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z" });
    expect(() => evaluateShipmentApvChaosFailureAlertPreview({
      ...unsupportedVersion, policy: { ...unsupportedVersion.policy, version: "future-policy" },
    } as never)).toThrow("SHIPMENT_APV_FAILURE_ALERT_PREVIEW_POLICY_UNAVAILABLE");
    expect(() => evaluateShipmentApvChaosFailureAlertPreview({
      ...unsupportedReason, policy: { ...unsupportedReason.policy, reasons: ["unknown_reason"] },
    } as never)).toThrow("SHIPMENT_APV_FAILURE_ALERT_PREVIEW_POLICY_UNAVAILABLE");
  });

  it("uses a deterministic identifier-free fingerprint that changes with public state", () => {
    const warning = evaluateShipmentApvChaosFailureAlertPreview(health({ counts: [1, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z" }));
    const repeated = evaluateShipmentApvChaosFailureAlertPreview(health({ counts: [1, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z" }));
    const critical = evaluateShipmentApvChaosFailureAlertPreview(health({ counts: [3, 0, 0],
      warningAt: "2026-07-13T10:00:00.000Z", criticalAt: "2026-07-13T10:02:00.000Z" }));
  ...
  it("loads the preview with one read-only aggregate statement", async () => {
    ...
    const preview = await getShipmentApvChaosFailureAlertPreview(
      { execute } as unknown as Pick<Database, "execute">, new Date(now));
