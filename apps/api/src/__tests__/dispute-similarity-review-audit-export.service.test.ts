...
describe("dispute similarity review signed audit export", () => {
  it("produces a deterministic event hash and detects mutation", () => {
    ...
    expect(hashDisputeSimilarityExpiryEvent({ ...event })).toBe(
      hashDisputeSimilarityExpiryEvent(event),
    );
    expect(hashDisputeSimilarityExpiryEvent({ ...event, reason: "UNKNOWN" })).not.toBe(
      hashDisputeSimilarityExpiryEvent(event),
    );
  });
  ...
  it("signs and verifies a valid sealed event", () => {
    ...
    const value = createSignedDisputeSimilarityReviewAuditExport({
      event,
      storedEventHash: hashDisputeSimilarityExpiryEvent(event),
