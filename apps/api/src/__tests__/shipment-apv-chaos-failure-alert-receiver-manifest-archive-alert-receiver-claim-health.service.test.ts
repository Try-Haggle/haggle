...
describe("shipment APV archive alert receiver claim health", () => {
  it("returns identifier-free healthy aggregates without deleting history", async () => {
    await expect(
      getShipmentApvReceiverManifestArchiveAlertReceiverClaimHealth(database(healthy)),
    ).resolves.toEqual({
      schemaVersion:
  ...
  it("marks every persisted invariant violation critical", async () => {
    const result = await getShipmentApvReceiverManifestArchiveAlertReceiverClaimHealth(
      database({
        ...healthy,
  ...
  it("fails closed on missing or malformed aggregate rows", async () => {
    await expect(
      getShipmentApvReceiverManifestArchiveAlertReceiverClaimHealth(database(undefined)),
    ).rejects.toThrow("SHIPMENT_APV_RECEIVER_MANIFEST_ARCHIVE_ALERT_RECEIVER_CLAIM_HEALTH_INVALID");
    await expect(
