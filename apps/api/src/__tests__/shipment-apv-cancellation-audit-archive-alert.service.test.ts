...
import {
  evaluateShipmentApvCancellationAuditArchiveAlert,
  getShipmentApvCancellationAuditArchiveAlertPolicyStatus,
  resolveShipmentApvCancellationAuditArchiveAlertConfigFromEnv,
  sendShipmentApvCancellationAuditArchiveAlert,
} from "../services/shipment-apv-payout-cancellation-audit-archive-alert.service.js";
...
const config = {
  ...
  secret: "archive-alert-secret-long",
  timeoutMs: 5000,
  cooldownMinutes: 15,
  staleThreshold: 1,
...
describe("APV cancellation audit archive alerts", () => {
  afterEach(() => {
    ...
    delete process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_SECRET;
  });

  it("classifies dead letters as critical and stale leases as warning", () => {
    expect(evaluateShipmentApvCancellationAuditArchiveAlert(health, config)).toEqual({
      wouldAlert: true,
      severity: "critical",
      reasons: [
        "audit_archive_dead_letter",
        "audit_archive_stale_processing",
    ...
    expect(
      ...
      wouldAlert: true,
      severity: "warning",
      reasons: ["audit_archive_stale_processing"],
    });
  });
  ...
  it("alerts on a retry-ready backlog at its independent threshold", () => {
    expect(
      ...
      wouldAlert: true,
      severity: "warning",
      reasons: ["audit_archive_retry_ready_backlog"],
    });
  });
  ...
  it("alerts when pending delivery exceeds the archive SLA", () => {
    expect(
      ...
      wouldAlert: true,
      severity: "warning",
      reasons: ["audit_archive_unfinished_too_old"],
    });
  });
  ...
  it("sends an HMAC-signed aggregate payload without archive identifiers", async () => {
    ...
    await expect(
      sendShipmentApvCancellationAuditArchiveAlert(
        health,
        evaluateShipmentApvCancellationAuditArchiveAlert(health, config),
        { config, fetchImpl: fetchMock, now: new Date(health.recordedAt) },
      ),
  ...
  it("rejects unsafe targets and short secrets", async () => {
    ...
    expect(() => resolveShipmentApvCancellationAuditArchiveAlertConfigFromEnv()).toThrow(
      "at least 16 characters",
    );
  });

  it("distinguishes absent, partial, invalid, and valid alert configuration", () => {
    expect(getShipmentApvCancellationAuditArchiveAlertPolicyStatus().configurationState).toBe(
      "not_configured",
    );
    ...
    process.env.HAGGLE_AUDIT_ARCHIVE_ALERT_URL = config.url;
    expect(getShipmentApvCancellationAuditArchiveAlertPolicyStatus()).toMatchObject({
      configured: true,
