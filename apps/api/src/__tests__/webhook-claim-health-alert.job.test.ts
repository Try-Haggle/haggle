...
import { findLatestDeliveredWebhookClaimIncident, sendWebhookClaimAlert } from "../services/webhook-claim-alert.service.js";
...
vi.mock("../services/webhook-claim-alert.service.js", async (importOriginal) => {
  const original = await importOriginal<typeof import("../services/webhook-claim-alert.service.js")>();
  return { ...original, findLatestDeliveredWebhookClaimIncident: vi.fn(), sendWebhookClaimAlert: vi.fn() };
});

const criticalHealth = {
  status: "critical" as const,
  totals: { processing: 1, completed: 5, failed: 0, staleProcessing: 1, retryReady: 0 },
  sources: [{ source: "stripe", processing: 1, completed: 5, failed: 0, staleProcessing: 1, retryReady: 0, maxAttemptCount: 2, oldestUnfinishedAgeSeconds: 90 }],
...
describe("webhook claim health alert job", () => {
  ...
  it("does not query the database when alert delivery is not configured", async () => {
    await expect(runWebhookClaimHealthAlert({} as Database)).resolves.toEqual({ status: "skipped", reason: "not_configured" });
  ...
  it("skips healthy claim state without reserving an alert claim", async () => {
    ...
    await expect(runWebhookClaimHealthAlert({} as Database)).resolves.toMatchObject({ status: "skipped", reason: "healthy_no_delivered_incident" });
  ...
  it("delivers one recovery for a completed incident and suppresses repeats", async () => {
    configure();
    vi.mocked(getWebhookClaimHealth).mockResolvedValue({ ...criticalHealth, status: "healthy",
      totals: { processing: 0, completed: 6, failed: 0, staleProcessing: 0, retryReady: 0 }, sources: [] });
    vi.mocked(findLatestDeliveredWebhookClaimIncident).mockResolvedValue({
      eventId: `health_${"b".repeat(64)}`, completedAt: criticalHealth.recordedAt,
    });
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "acquired", source: "haggle-webhook-claim-alert", eventId: `recovery_${"c".repeat(64)}`,
        claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 })
      .mockResolvedValueOnce({ outcome: "duplicate", source: "haggle-webhook-claim-alert", eventId: `recovery_${"c".repeat(64)}` });
    vi.mocked(sendWebhookClaimAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 200 });
    await expect(runWebhookClaimHealthAlert({} as Database)).resolves.toMatchObject({ status: "recovered",
    ...
    await expect(runWebhookClaimHealthAlert({} as Database)).resolves.toMatchObject({
      status: "skipped", reason: "recovery_already_sent_or_in_progress" });
  ...
  it("delivers one claimed alert and seals the cooldown claim", async () => {
    ...
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "acquired", source: "haggle-webhook-claim-alert", eventId: "health_1", claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 });
    vi.mocked(sendWebhookClaimAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 200 });
    await expect(runWebhookClaimHealthAlert({} as Database, { now: new Date("2026-07-12T00:00:00.000Z") }))
  ...
  it("suppresses another server when the cooldown claim already exists", async () => {
    configure();
    vi.mocked(getWebhookClaimHealth).mockResolvedValueOnce(criticalHealth);
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "duplicate", source: "haggle-webhook-claim-alert", eventId: "health_1" });
    await expect(runWebhookClaimHealthAlert({} as Database)).resolves.toMatchObject({ status: "skipped", reason: "cooldown_or_in_progress" });
  ...
  it("uses one severity bucket key even when aggregate counts change", async () => {
    ...
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "duplicate", source: "haggle-webhook-claim-alert", eventId: "health_1" })
      .mockResolvedValueOnce({ outcome: "duplicate", source: "haggle-webhook-claim-alert", eventId: "health_1" });
  ...
  it("marks the alert claim failed so a later job can retry", async () => {
    ...
    await expect(runWebhookClaimHealthAlert({} as Database)).resolves.toMatchObject({ status: "failed" });
