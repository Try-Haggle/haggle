...
function body(overrides: Record<string, unknown> = {}) {
  return JSON.stringify({ type: "conditional_settlement_preflight.health", delivery_id: deliveryId,
    state: "firing", created_at: timestamp, severity: "critical", reasons: ["rpc_timeout"],
    health: { status: "unavailable", error_code: "RPC_TIMEOUT" }, ...overrides });
}
...
describe("conditional settlement preflight alert verifier", () => {
  it("verifies the exact raw body, timestamp, and delivery id", () => {
    const raw = body();
    expect(verifyConditionalSettlementPreflightAlert({ rawBody: raw, timestamp, deliveryId,
      signature: signWebhookClaimAlertPayload(secret, timestamp, raw), secret, nowMs: Date.parse(timestamp) }))
  ...
  it("rejects stale, mismatched, and invalid state/severity combinations", () => {
    ...
    expect(verifyConditionalSettlementPreflightAlert({ rawBody: raw, timestamp, deliveryId, signature, secret,
      nowMs: Date.parse(timestamp) + 300_001 })).toEqual({ ok: false, error: "ALERT_TIMESTAMP_OUT_OF_RANGE" });
    expect(verifyConditionalSettlementPreflightAlert({ rawBody: raw, timestamp, deliveryId: `health_${"b".repeat(64)}`,
      signature, secret, nowMs: Date.parse(timestamp) })).toEqual({ ok: false, error: "ALERT_DELIVERY_ID_MISMATCH" });
    const invalid = body({ state: "recovered", severity: "critical" });
    expect(verifyConditionalSettlementPreflightAlert({ rawBody: invalid, timestamp, deliveryId,
      signature: signWebhookClaimAlertPayload(secret, timestamp, invalid), secret, nowMs: Date.parse(timestamp) }))
  ...
  it("rejects tampering and accepts a previous secret during rotation", () => {
    ...
    expect(verifyConditionalSettlementPreflightAlert({ rawBody: raw, timestamp, deliveryId,
      signature: signWebhookClaimAlertPayload(secret, timestamp, raw), secret: ["new-secret-long-enough", secret],
      nowMs: Date.parse(timestamp) }).ok).toBe(true);
  });
  ...
  it("deduplicates configured current and previous secrets without exposing them", () => {
    ...
    expect(getConditionalSettlementPreflightAlertReceiverPolicyStatus()).toEqual({
      configured: true, acceptedSecretCount: 2, timestampToleranceSeconds: 300,
    });
  });
});
