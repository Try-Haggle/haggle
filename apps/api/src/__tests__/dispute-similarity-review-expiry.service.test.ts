...
describe("dispute similarity review automatic expiry", () => {
  ...
  it("rejects invalid batch size before querying", async () => {
    const execute = vi.fn();
    await expect(expireDisputeSimilarityReviews({ execute } as unknown as Database, { batchSize: 0 }))
  ...
  it("returns the number of atomic update-plus-event transitions", async () => {
    const execute = vi.fn().mockResolvedValueOnce([
      { uploadId: "11111111-1111-4111-8111-111111111111", disputeId: "33333333-3333-4333-8333-333333333333", expiresAt: "2026-07-11T23:59:00.000Z" },
    ...
    const result = await expireDisputeSimilarityReviews({ transaction } as unknown as Database, {
      now: new Date("2026-07-12T00:00:00.000Z"), batchSize: 50,
    });
  ...
  it("returns a newest-first opaque cursor page with safe event fields", async () => {
    const execute = vi.fn().mockResolvedValue([
      {
        id: "11111111-1111-4111-8111-111111111111", upload_id: "22222222-2222-4222-8222-222222222222",
        dispute_id: "33333333-3333-4333-8333-333333333333", event_type: "AUTO_EXPIRED", actor_id: null,
        metadata: { reason: "REVIEW_WINDOW_EXPIRED", review_expires_at: "2026-07-12T00:00:00.000Z", hidden: "do-not-return" },
        created_at: "2026-07-12T00:01:00.000Z",
      },
      {
        id: "44444444-4444-4444-8444-444444444444", upload_id: "55555555-5555-4555-8555-555555555555",
        dispute_id: "66666666-6666-4666-8666-666666666666", event_type: "AUTO_EXPIRED", actor_id: null,
        metadata: {}, created_at: "2026-07-12T00:00:00.000Z",
      },
    ]);
    const result = await listDisputeSimilarityReviewExpiryEvents({ execute } as unknown as Database, { limit: 1 });
    expect(result.items).toEqual([expect.objectContaining({
      eventId: "11111111-1111-4111-8111-111111111111", actorKind: "system",
      reason: "REVIEW_WINDOW_EXPIRED", reviewExpiresAt: "2026-07-12T00:00:00.000Z",
    })]);
  ...
  it("rejects a malformed event cursor before querying", async () => {
    const execute = vi.fn();
    await expect(listDisputeSimilarityReviewExpiryEvents({ execute } as unknown as Database, { cursor: "broken" }))
