...
describe("shipment APV failure alert maker-checker decisions", () => {
  ...
  it("blocks the maker and expired requests before decision insert", async () => {
    ...
    await expect(
      decideShipmentApvFailureAlertApprovalRequest(
    ...
    const expiredExecute = vi
      .fn()
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([requestRow({ expires_at: "2026-07-13T12:29:59.999Z" })]);
    await expect(
      decideShipmentApvFailureAlertApprovalRequest(
  ...
  it("rejects reused client ids and conflicting terminal decisions", async () => {
    const replayExecute = vi.fn().mockResolvedValueOnce([
      decisionRow({
