...
describe("shipment APV failure alert test key registry", () => {
  it("registers an ephemeral public key without exposing its private key", async () => {
    ...
    expect(result).toMatchObject({ status: "REGISTERED", algorithm: "Ed25519",
      ...
      independentTrustAnchor: false, privateKeyExposed: false, replayed: false });
  ...
  it("shows the current terminal state when replaying the old registration event", async () => {
    const execute = vi.fn().mockResolvedValueOnce([registryRow({ inserted: false,
      current_event_type: "RETIRED", current_reason: "ephemeral_test_key_retired",
      current_event_created_at: "2026-07-13T18:05:00.000Z",
    })]);
    const result = await registerShipmentApvFailureAlertTestKey(
      { execute } as unknown as Pick<Database, "execute">,
      { clientEventId, registeredBy: actor, signer, now });
    expect(result).toMatchObject({ eventType: "REGISTERED", status: "RETIRED",
      lifecycleReason: "ephemeral_test_key_retired", replayed: true });
  });
  ...
  it("returns an already-active identical key without duplicating registration", async () => {
    ...
    const result = await registerShipmentApvFailureAlertTestKey(
      ...
      { clientEventId: "33333333-3333-4333-8333-333333333333",
        registeredBy: actor, signer, now });
    expect(result).toMatchObject({ status: "REGISTERED", replayed: true });
    expect(execute).toHaveBeenCalledTimes(2);
  });

  it("rejects client rebinding, key binding conflict, and terminal reuse", async () => {
    const rebound = vi.fn().mockResolvedValueOnce([registryRow({
      changed_by: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", inserted: false,
    })]);
    await expect(registerShipmentApvFailureAlertTestKey(
      { execute: rebound } as unknown as Pick<Database, "execute">,
      { clientEventId, registeredBy: actor, signer, now }))
      .rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_KEY_EVENT_REPLAY_CONFLICT");

    const conflict = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
    ...
    const terminal = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
  ...
  it("appends one RETIRED lifecycle event", async () => {
    const execute = vi.fn().mockResolvedValueOnce([])
      .mockResolvedValueOnce([registryRow({ inserted: false })])
      .mockResolvedValueOnce([registryRow({ event_type: "RETIRED",
        reason: "ephemeral_test_key_retired", current_event_type: "RETIRED",
        current_reason: "ephemeral_test_key_retired", inserted: true })]);
    ...
    expect(result).toMatchObject({ status: "RETIRED",
      lifecycleReason: "ephemeral_test_key_retired", replayed: false });
  });

  it("returns exact terminal replay and rejects opposite rebinding", async () => {
    const replay = vi.fn().mockResolvedValueOnce([registryRow({ event_type: "REVOKED",
      reason: "ephemeral_test_key_revoked", current_event_type: "REVOKED",
      current_reason: "ephemeral_test_key_revoked", inserted: false })]);
    const result = await transitionShipmentApvFailureAlertTestKey(
    ...
    const opposite = vi.fn().mockResolvedValueOnce([registryRow({ event_type: "RETIRED",
      reason: "ephemeral_test_key_retired", inserted: false })]);
    await expect(transitionShipmentApvFailureAlertTestKey(
  ...
  it("requires the registering actor and an active key for transition", async () => {
    const actorMismatch = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
      registryRow({ inserted: false }),
    ]);
    await expect(transitionShipmentApvFailureAlertTestKey(
      { execute: actorMismatch } as unknown as Pick<Database, "execute">,
      { keyId: signer.keyId, clientEventId, action: "REVOKE",
        changedBy: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", now }))
      .rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_KEY_ACTOR_MISMATCH");

    const terminal = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
      registryRow({ event_type: "RETIRED", inserted: false }),
    ]);
    await expect(transitionShipmentApvFailureAlertTestKey(
      { execute: terminal } as unknown as Pick<Database, "execute">,
      { keyId: signer.keyId, clientEventId, action: "REVOKE", changedBy: actor, now }))
  ...
  it("recovers an exact transition after losing a concurrent insert race", async () => {
    const execute = vi.fn().mockResolvedValueOnce([])
      ...
      .mockResolvedValueOnce([]).mockResolvedValueOnce([
        registryRow({ event_type: "REVOKED", reason: "ephemeral_test_key_revoked",
          current_event_type: "REVOKED", current_reason: "ephemeral_test_key_revoked",
          inserted: false }),
      ]);
    const result = await transitionShipmentApvFailureAlertTestKey(
      { execute } as unknown as Pick<Database, "execute">,
