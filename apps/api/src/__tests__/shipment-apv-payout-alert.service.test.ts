...
const healthyCursorRetention = {
  ...
  lastSucceededAt: "2026-07-12T00:00:02.000Z",
  lastFailedAt: null,
  ...
  recordedAt: "2026-07-12T00:00:03.000Z",
};
const enabledCursorRetention = {
  jobEnabled: true,
  configured: true,
  intervalSeconds: 86_400,
  health: healthyCursorRetention,
};
...
describe("shipment APV payout reservation alerts", () => {
  ...
  it("alerts when a cancellation approval waits beyond the SLA", () => {
    expect(
      evaluateShipmentApvPayoutAlert({ ...health, status: "healthy", expiredReserved: 0 }, config, {
        status: "attention",
        pendingRequests: 1,
  ...
  it("classifies missing or tampered invoices as critical and orphan files as warning", () => {
    ...
    expect(
      evaluateShipmentApvPayoutAlert(healthyPayout, config, undefined, {
      ...
      wouldAlert: true,
      severity: "critical",
      reasons: ["invoice_document_missing"],
    });
    expect(
      evaluateShipmentApvPayoutAlert(healthyPayout, config, undefined, {
        ...healthyInvoiceStorage,
      ...
      wouldAlert: true,
      severity: "warning",
      reasons: ["invoice_document_orphan"],
    });
  });
  ...
  it("classifies staging backlog as warning and missing staged bytes as critical", () => {
    const healthyPayout = { ...health, status: "healthy" as const, expiredReserved: 0 };
    expect(
      evaluateShipmentApvPayoutAlert(healthyPayout, config, undefined, healthyInvoiceStorage, {
  ...
  it("classifies expiring remediation as warning and overdue or stale apply as critical", () => {
    ...
    expect(
      evaluateShipmentApvPayoutAlert(
        ...
        healthyInvoiceStorage,
        healthyRestorationStaging,
        { ...healthyRestorationRemediation, status: "warning", expiringSoonRequests: 1 },
      ),
    ).toEqual({
      wouldAlert: true,
      severity: "warning",
      reasons: ["invoice_restoration_remediation_expiring"],
    });
    expect(
      evaluateShipmentApvPayoutAlert(
        healthyPayout,
  ...
  it("re-escalates when an acknowledged incident remains applying for 30 minutes", () => {
    ...
    expect(
      evaluateShipmentApvPayoutAlert(
        ...
        {
          ...healthyRestorationRemediation,
          acknowledgedStillApplyingOver30Minutes: 1,
          incidentLinkedStillApplyingOver30Minutes: 1,
        },
      ),
    ).toEqual({
      wouldAlert: true,
  ...
  it("escalates when incident linking misses the post-acknowledgment SLA", () => {
    ...
    expect(
      evaluateShipmentApvPayoutAlert(
        ...
        healthyInvoiceStorage,
        healthyRestorationStaging,
        { ...healthyRestorationRemediation, incidentLinkOverdueAfterAcknowledgment: 1 },
      ),
    ).toEqual({
      wouldAlert: true,
      severity: "critical",
      reasons: ["invoice_restoration_remediation_incident_link_overdue_after_ack_15m"],
    });
  });
  ...
  it("alerts only for active failed, stale, or overdue cursor retention execution", () => {
    ...
    expect(
      evaluateShipmentApvCursorRetentionAlert(
        {
          ...enabledCursorRetention,
          jobEnabled: false,
        },
        now,
      ),
    ).toEqual({ wouldAlert: false, severity: null, reasons: [] });
    expect(
      evaluateShipmentApvCursorRetentionAlert(
        {
          ...enabledCursorRetention,
          health: {
            ...healthyCursorRetention,
            firstObservedAt: "2026-07-13T01:59:00.000Z",
            lastRunStatus: "NEVER",
            lastStartedAt: null,
  ...
  it("sends a signed payload without seller, order, or offset identifiers", async () => {
    ...
    await expect(
      sendShipmentApvPayoutAlert(health, evaluateShipmentApvPayoutAlert(health, config), {
        config,
    ...
    expect(JSON.parse(body).invoice_storage_health).toMatchObject({
      status: "healthy",
      totalDocuments: 1,
    });
    expect(JSON.parse(body).invoice_restoration_staging_health).toMatchObject({
      status: "healthy",
      trackedStaging: 0,
    });
    expect(JSON.parse(body).invoice_restoration_remediation_health).toMatchObject({
      status: "healthy",
      pendingRequests: 0,
    });
    ...
    expect(body).not.toMatch(
      /staging_key|requestId|decisionRequestId|approverId|candidate|sha256|claimId|leaseExpires/i,
    );
  });
  ...
  it("rejects private targets and short signing secrets", async () => {
    ...
    expect(() => resolveShipmentApvPayoutAlertConfigFromEnv()).toThrow(
