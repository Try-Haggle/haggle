...
describe("scan retry alert snapshot retention", () => {
  it("uses bounded policy and follows the alert job enablement", () => {
    ...
    expect(getDisputeEvidenceScanRetryAlertSnapshotRetentionPolicy())
      .toEqual({ retentionDays: 30, batchSize: 100,
  ...
  it("returns aggregate-only eligible and blocked expiry health", async () => {
    ...
    await expect(getDisputeEvidenceScanRetryAlertSnapshotRetentionHealth(
      { execute } as unknown as Database,
    )).resolves.toMatchObject({
      status: "attention", eligibleExpired: 2, blockedExpired: 3,
      oldestBlockedExpiredAgeSeconds: 120, containsIdentifiers: false,
    });
  });
  ...
  it("skips when another instance owns the transaction lock", async () => {
    ...
    await expect(runDisputeEvidenceScanRetryAlertSnapshotRetention(db))
      .resolves.toEqual({ status: "skipped",
        reason: "retention_lock_held", deleted: 0 });
  ...
  it("deletes only the bounded rows returned by the guarded transaction", async () => {
    ...
    await expect(runDisputeEvidenceScanRetryAlertSnapshotRetention(
      db, { batchSize: 10 },
    )).resolves.toEqual({ status: "executed", deleted: 1,
      batchSize: 10, containsIdentifiers: false });
