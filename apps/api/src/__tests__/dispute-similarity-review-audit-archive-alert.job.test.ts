...
vi.mock("../services/dispute-similarity-review-audit-archive.service.js", () => ({
  getDisputeSimilarityReviewAuditArchiveHealth: vi.fn(),
}));
vi.mock(
...
describe("similarity review audit archive alert job", () => {
  ...
  it("skips when unconfigured or healthy without a delivered incident", async () => {
    await expect(
      runDisputeSimilarityReviewAuditArchiveAlert({} as Database),
    ).resolves.toMatchObject({ status: "skipped", reason: "not_configured" });
    configure();
    vi.mocked(getDisputeSimilarityReviewAuditArchiveHealth).mockResolvedValueOnce({
      ...health,
      status: "healthy",
      deadLetter: 0,
    });
    vi.mocked(findLatestDeliveredDisputeSimilarityReviewAuditArchiveIncident).mockResolvedValueOnce(
      null,
    );
    await expect(
      runDisputeSimilarityReviewAuditArchiveAlert({} as Database),
    ).resolves.toMatchObject({ status: "skipped", reason: "healthy_no_delivered_incident" });
  });

  it("delivers one recovery after a delivered incident and suppresses duplicates", async () => {
    configure();
    vi.mocked(getDisputeSimilarityReviewAuditArchiveHealth).mockResolvedValue({
      ...health,
      status: "healthy",
      deadLetter: 0,
    });
    vi.mocked(findLatestDeliveredDisputeSimilarityReviewAuditArchiveIncident).mockResolvedValue({
      eventId: `health_${"b".repeat(64)}`,
      completedAt: health.recordedAt,
    });
    ...
    vi.mocked(sendDisputeSimilarityReviewAuditArchiveAlert).mockResolvedValueOnce({
      status: "delivered",
      httpStatus: 200,
    });
    await expect(
      ...
      status: "recovered",
      assessment: { severity: "recovery", reasons: ["similarity_audit_archive_recovered"] },
    });
    await expect(
      runDisputeSimilarityReviewAuditArchiveAlert({} as Database),
    ).resolves.toMatchObject({ reason: "recovery_already_sent_or_in_progress" });
  ...
  it("leaves a failed recovery claim on the shared backoff path", async () => {
    ...
    await expect(
      runDisputeSimilarityReviewAuditArchiveAlert({} as Database),
    ).resolves.toMatchObject({ status: "failed", phase: "recovery" });
