...
import { findLatestDeliveredWebhookClaimIncident, getWebhookClaimAlertDeliveryState } from "../services/webhook-claim-alert.service.js";
import { claimWebhookEvent, completeWebhookEvent, failWebhookEvent } from "../services/webhook-event-claim.service.js";
import { getShipmentApvRemediationCursorRetentionJobHealth } from
  "../jobs/shipment-apv-remediation-cursor-retention.js";

vi.mock("../services/shipment-apv-payout-offset.service.js", () => ({ getShipmentApvPayoutReservationHealth: vi.fn() }));
vi.mock("../services/shipment-apv-payout-cancellation.service.js", () => ({ getShipmentApvPayoutCancellationApprovalHealth: vi.fn() }));
vi.mock("../services/shipment-apv-invoice-document.service.js", () => ({ getShipmentApvInvoiceDocumentStorageHealth: vi.fn() }));
vi.mock("../services/shipment-apv-invoice-restoration.service.js", () => ({ getShipmentApvInvoiceRestorationStagingHealth: vi.fn() }));
...
vi.mock("../jobs/shipment-apv-remediation-cursor-retention.js", async (importOriginal) => {
  const original = await importOriginal<typeof import("../jobs/shipment-apv-remediation-cursor-retention.js")>();
  return { ...original, getShipmentApvRemediationCursorRetentionJobHealth: vi.fn() };
});
vi.mock("../services/webhook-claim-alert.service.js", () => ({
  findLatestDeliveredWebhookClaimIncident: vi.fn(), getWebhookClaimAlertDeliveryState: vi.fn(),
}));
vi.mock("../services/shipment-apv-payout-alert.service.js", async (importOriginal) => {
  const original = await importOriginal<typeof import("../services/shipment-apv-payout-alert.service.js")>();
...
const attention = {
  status: "attention" as const, expiredReserved: 1, signedExpired: 1, unsignedExpired: 0,
  affectedSellers: 1, appliedOffsetMinor: 40, oldestExpiredAgeSeconds: 60,
...
describe("shipment APV payout alert job", () => {
  ...
  const prepareHealth = () => {
    ...
    vi.mocked(getShipmentApvInvoiceRestorationStagingHealth).mockResolvedValue(healthyRestorationStaging);
    vi.mocked(getShipmentApvInvoiceRestorationRemediationHealth).mockResolvedValue(healthyRestorationRemediation);
  ...
  const healthyApprovals = {
    status: "healthy" as const, pendingRequests: 0, expiringSoonRequests: 0,
    oldestPendingAgeSeconds: null, recordedAt: "2026-07-12T00:00:00.000Z",
  };

  it("skips when unconfigured or healthy", async () => {
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toEqual({ status: "skipped", reason: "not_configured" });
    configure();
    prepareHealth();
    vi.mocked(getShipmentApvPayoutReservationHealth).mockResolvedValueOnce({ ...attention, status: "healthy", expiredReserved: 0 });
    vi.mocked(getShipmentApvPayoutCancellationApprovalHealth).mockResolvedValueOnce(healthyApprovals);
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({ status: "skipped", reason: "healthy" });
  });
  ...
  it("rejects fixture overrides in production and malformed fixture sources", async () => {
    ...
    const fixture = {
      ...
      config: { url: "https://ops.example/alerts", secret: "fixture-secret", timeoutMs: 5000,
        cooldownMinutes: 15, expiredThreshold: 1, approvalPendingThreshold: 1,
        approvalMaxAgeMinutes: 15, allowInsecureHttp: false, allowPrivateNetwork: false },
      cursorRetentionStatus: { configured: true, jobEnabled: true, retentionDays: 30,
        limit: 1000, intervalSeconds: 86_400 },
    };
    try {
      ...
      await expect(runShipmentApvPayoutAlert({} as Database, {
  ...
  it("delivers one claimed alert and suppresses a duplicate server", async () => {
    ...
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "acquired", source: "haggle-shipment-apv-payout-alert", eventId: "health", claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 })
      .mockResolvedValueOnce({ outcome: "duplicate", source: "haggle-shipment-apv-payout-alert", eventId: "health" });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 200 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({ status: "delivered" });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({ status: "skipped", reason: "cooldown_or_in_progress" });
  ...
  it("marks failed delivery for retry", async () => {
    ...
    vi.mocked(getShipmentApvPayoutCancellationApprovalHealth).mockResolvedValueOnce(healthyApprovals);
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "acquired", source: "haggle-shipment-apv-payout-alert", eventId: "health", claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "failed", httpStatus: 503 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({ status: "failed" });
  ...
  it("delivers an invoice storage warning through the shared APV signed alert", async () => {
    configure();
    prepareHealth();
    vi.mocked(getShipmentApvPayoutReservationHealth).mockResolvedValue({ ...attention, status: "healthy", expiredReserved: 0 });
    vi.mocked(getShipmentApvPayoutCancellationApprovalHealth).mockResolvedValue(healthyApprovals);
    ...
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "acquired",
      source: "haggle-shipment-apv-payout-alert", eventId: "health", claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 202 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({
      status: "delivered", assessment: { severity: "warning", reasons: ["invoice_document_orphan"] },
    });
  });
  ...
  it("delivers restoration staging backlog through the shared APV signed alert", async () => {
    ...
    vi.mocked(getShipmentApvInvoiceRestorationStagingHealth).mockResolvedValueOnce({
      ...healthyRestorationStaging, status: "warning", trackedStaging: 1, pendingDisposition: 1,
    });
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "acquired",
      source: "haggle-shipment-apv-payout-alert", eventId: "health", claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 202 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({
      status: "delivered", assessment: { severity: "warning", reasons: ["invoice_restoration_staging_pending"] },
    });
    expect(sendShipmentApvPayoutAlert).toHaveBeenCalledWith(expect.anything(), expect.anything(),
      expect.objectContaining({ restorationStagingHealth: expect.objectContaining({ pendingDisposition: 1 }) }));
  });
  ...
  it("escalates a 60-minute stale remediation without identifiers", async () => {
    ...
    vi.mocked(getShipmentApvPayoutReservationHealth).mockResolvedValue({
      ...attention, status: "healthy", expiredReserved: 0,
    });
    vi.mocked(getShipmentApvPayoutCancellationApprovalHealth).mockResolvedValue(healthyApprovals);
    vi.mocked(getShipmentApvInvoiceRestorationRemediationHealth).mockResolvedValueOnce({
      ...healthyRestorationRemediation, status: "critical", applyingRequests: 1,
      ...
      oldestApplyingAgeSeconds: 3660, staleApplyingAgeBucket: "60m",
    });
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "acquired",
      source: "haggle-shipment-apv-payout-alert", eventId: "health",
      claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 202 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({
      status: "delivered", assessment: { severity: "critical", reasons: [
    ...
    expect(sendShipmentApvPayoutAlert).toHaveBeenCalledWith(expect.anything(), expect.anything(),
      expect.objectContaining({ restorationRemediationHealth: expect.objectContaining({
        staleApplyingAgeBucket: "60m", oldestApplyingAgeSeconds: 3660,
      }) }));
  });
  ...
  it("delivers active cursor retention failure through the shared signed alert", async () => {
    ...
    vi.mocked(getShipmentApvRemediationCursorRetentionJobHealth).mockResolvedValueOnce({
      lastRunStatus: "FAILED", leaseStale: false,
      firstObservedAt: "2026-07-12T00:00:00.000Z",
      lastStartedAt: "2026-07-13T00:00:00.000Z", lastSucceededAt: null,
      lastFailedAt: "2026-07-13T00:00:01.000Z", lastDeletedBuckets: 0,
      lastExpiredBuckets: 0, lastInvalidBuckets: 0, lastTruncated: false,
      lastFailureCode: "RETENTION_EXECUTION_FAILED", recordedAt: "2026-07-13T00:00:02.000Z",
    });
    vi.mocked(claimWebhookEvent).mockResolvedValueOnce({ outcome: "acquired",
      source: "haggle-shipment-apv-payout-alert", eventId: "health",
      claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 202 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({
      status: "delivered", assessment: { severity: "critical",
        reasons: ["invoice_restoration_cursor_retention_failed"] },
      cursorRetentionJob: { health: { lastRunStatus: "FAILED" } },
    });
    expect(sendShipmentApvPayoutAlert).toHaveBeenCalledWith(expect.anything(), expect.anything(),
      expect.objectContaining({ cursorRetentionJob: expect.objectContaining({
  ...
  it("delivers one recovery only when a completed APV incident remains open", async () => {
    ...
    vi.mocked(getShipmentApvPayoutCancellationApprovalHealth).mockResolvedValue(healthyApprovals);
    vi.mocked(getWebhookClaimAlertDeliveryState).mockResolvedValue({
      incidentOpen: true, lastIncidentAlertAt: "2026-07-12T00:00:00.000Z", lastRecoveryAlertAt: null,
    });
    vi.mocked(findLatestDeliveredWebhookClaimIncident).mockResolvedValue({
      eventId: `health_${"a".repeat(64)}`, completedAt: "2026-07-12T00:00:00.000Z",
    });
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "acquired", source: "haggle-shipment-apv-payout-alert",
        eventId: "recovery", claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 })
      .mockResolvedValueOnce({ outcome: "duplicate", source: "haggle-shipment-apv-payout-alert", eventId: "recovery" });
    vi.mocked(sendShipmentApvPayoutAlert).mockResolvedValueOnce({ status: "delivered", httpStatus: 200 });
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({
      status: "delivered", assessment: { severity: "recovery", reasons: [] },
    });
    expect(claimWebhookEvent).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
      eventId: expect.stringMatching(/^recovery_[0-9a-f]{64}$/),
    }));
    await expect(runShipmentApvPayoutAlert({} as Database)).resolves.toMatchObject({
      status: "skipped", reason: "cooldown_or_in_progress", assessment: { severity: "recovery" },
    });
    expect(sendShipmentApvPayoutAlert).toHaveBeenCalledOnce();
  });
});
