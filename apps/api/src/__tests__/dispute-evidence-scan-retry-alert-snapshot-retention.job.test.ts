...
describe("scan retry alert snapshot retention job health", () => {
  it("reports bounded schedule policy without identifiers", () => {
    ...
    expect(getDisputeEvidenceScanRetryAlertSnapshotRetentionJobStatus())
      .toEqual({ jobEnabled: true, cronEnabled: true,
  ...
  it("claims, executes and records one successful run", async () => {
    ...
    runRetention.mockResolvedValueOnce({
      status: "executed", deleted: 3, batchSize: 100,
      containsIdentifiers: false,
    });
    await expect(runDisputeEvidenceScanRetryAlertSnapshotRetentionJob(db, {
  ...
  it("skips before maintenance while another worker owns the lease", async () => {
    ...
    await expect(runDisputeEvidenceScanRetryAlertSnapshotRetentionJob(db))
      .resolves.toEqual({ status: "skipped",
        reason: "retention_run_in_progress", deleted: 0 });
  ...
  it("records a fixed failure code and rethrows the internal error", async () => {
    ...
    runRetention.mockRejectedValueOnce(
      new Error("database private detail must remain in logs"),
    );
    await expect(runDisputeEvidenceScanRetryAlertSnapshotRetentionJob(db))
      .rejects.toThrow("database private detail must remain in logs");
    expect(execute).toHaveBeenCalledTimes(2);
  });
  ...
  it("classifies an expired lease as critical without returning its claim", async () => {
    ...
    const { db } = databaseWithResults([{
      ...
      last_deleted_snapshots: 2, last_failure_code: null,
      claim_id: "secret-claim",
    }]);
    const health =
      await getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth(
        db, new Date("2026-07-14T00:15:00.000Z"));
    expect(health).toMatchObject({ status: "critical",
      lastRunStatus: "STALE_RUNNING", leaseStale: true,
      lastDeletedSnapshots: 2, containsIdentifiers: false });
  ...
  it("marks an enabled job attention after the 26 hour start deadline", async () => {
    ...
    process.env.ENABLE_DISPUTE_EVIDENCE_SCAN_RETRY_ALERT_JOB = "true";
    const { db } = databaseWithResults([{
      status: "SUCCEEDED", lease_expires_at: null,
      ...
      last_failed_at: null, last_deleted_snapshots: 0,
      last_failure_code: null,
    }]);
    await expect(getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth(
      db, new Date("2026-07-13T02:00:00.000Z")))
      .resolves.toMatchObject({ status: "attention", overdue: true,
  ...
  it("keeps old persisted state inactive while the job is disabled", async () => {
    ...
    await expect(getDisputeEvidenceScanRetryAlertSnapshotRetentionJobHealth(
      db, new Date("2026-07-14T00:00:00.000Z")))
      .resolves.toMatchObject({ status: "inactive", overdue: true,
