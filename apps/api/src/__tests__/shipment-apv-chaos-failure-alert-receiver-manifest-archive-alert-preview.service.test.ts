...
} from "../services/shipment-apv-chaos-failure-alert-receiver-manifest-archive-alert-preview.service.js";
...
function evaluate(value: ReturnType<typeof health>) {
  return evaluateShipmentApvFailureAlertReceiverManifestArchiveAlertPreview(value as never);
}
...
describe("shipment APV receiver manifest archive alert preview", () => {
    ...
    const preview = evaluate(
      health({
        ...
        totals: {
          ...
          latestIntentRevision: null,
          currentSourceEntries: 1001,
        },
        violations: { binding: 1, blockers: 1, unsafeSideEffect: 1, timestamp: 1, sourceLimit: 1 },
        criticalCount: 5,
        coverage: { currentReceiptIntentCovered: false, missingCurrentArchiveIntent: true },
        freshness: { slaSeconds: 86400, latestIntentAgeSeconds: null, stale: false },
      }),
  ...
  it("fingerprints public state but ignores clock-only age changes before a transition", () => {
    ...
    const later = evaluate(
      health({
        observedAt: "2026-07-13T23:31:00.000Z",
