...
vi.mock("../services/dispute-ai-audit-archive.service.js", () => ({
  getDisputeAiAuditArchiveHealth: vi.fn(),
  getDisputeAiAuditDiscoveryFailureHealth: vi.fn(),
}));
vi.mock("../services/dispute-ai-audit-archive-alert.service.js", async (importOriginal) => ({
  ...
  >()),
  findLatestDeliveredDisputeAiAuditArchiveIncident: vi.fn(),
  sendDisputeAiAuditArchiveAlert: vi.fn(),
}));
...
describe("dispute AI audit archive alert job", () => {
  ...
  const configure = () => {
    ...
    vi.mocked(getDisputeAiAuditDiscoveryFailureHealth).mockResolvedValue({
      status: "healthy",
      open: 0,
      retryRequested: 0,
      unresolved: 0,
  ...
  it("skips unconfigured and healthy-without-incident runs", async () => {
    await expect(runDisputeAiAuditArchiveAlert({} as Database)).resolves.toMatchObject({
      reason: "not_configured",
    });
    configure();
    vi.mocked(getDisputeAiAuditArchiveHealth).mockResolvedValueOnce({
      ...health,
      status: "healthy",
      deadLetter: 0,
    });
  ...
  it("delivers once and suppresses a cooldown duplicate", async () => {
    configure();
    vi.mocked(getDisputeAiAuditArchiveHealth).mockResolvedValue(health);
    ...
    vi.mocked(sendDisputeAiAuditArchiveAlert).mockResolvedValueOnce({
      status: "delivered",
      httpStatus: 200,
    });
    await expect(runDisputeAiAuditArchiveAlert({} as Database)).resolves.toMatchObject({
  ...
  it("delivers a warning when archive delivery is healthy but discovery is unresolved", async () => {
    configure();
    vi.mocked(getDisputeAiAuditArchiveHealth).mockResolvedValue({
      ...health,
      status: "healthy",
      deadLetter: 0,
    });
  ...
  it("delivers one recovery and suppresses duplicates", async () => {
    ...
    vi.mocked(findLatestDeliveredDisputeAiAuditArchiveIncident).mockResolvedValue({
