...
describe("shipment APV failure alert dry-run delivery grants", () => {
  it("atomically claims cooldown and appends a non-delivering grant", async () => {
    ...
    expect(result).toMatchObject({ schemaVersion: "shipment-apv-failure-alert-delivery-grant-v1",
      status: "GRANTED_DRY_RUN", replayed: false, dryRun: true,
      payloadPrepared: false, signatureCreated: false,
  ...
  it("requires an approved decision and the original checker actor", async () => {
    const rejectedExecute = vi.fn().mockResolvedValueOnce([])
      .mockResolvedValueOnce([bindingRow({ decision: "REJECTED" })]);
    await expect(createShipmentApvFailureAlertDeliveryGrant(
      { execute: rejectedExecute } as unknown as Pick<Database, "execute">, input))
  ...
  it("rejects expired approval windows and changed current state", async () => {
    const expiredExecute = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
      bindingRow({ request_expires_at: "2026-07-13T12:29:59.999Z" }),
    ]);
    await expect(createShipmentApvFailureAlertDeliveryGrant(
    ...
    const changedExecute = vi.fn().mockResolvedValueOnce([])
      .mockResolvedValueOnce([bindingRow()]).mockResolvedValueOnce([]);
    await expect(createShipmentApvFailureAlertDeliveryGrant(
      { execute: changedExecute } as unknown as Pick<Database, "execute">, input))
