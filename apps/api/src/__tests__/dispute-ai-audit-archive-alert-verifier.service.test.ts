...
function verify(overrides: Record<string, unknown> = {}) {
  return verifyDisputeAiAuditArchiveAlert({
    rawBody: body,
    timestamp,
    deliveryId,
...
describe("dispute AI audit archive alert receiver verification", () => {
  ...
  it("rejects stale timestamps", () => {
    expect(verify({ nowMs: Date.parse(timestamp) + 300_001 })).toEqual({
      ok: false,
  ...
  it("claims once then separates replay and payload conflict", async () => {
    ...
    if (!verified.ok) throw new Error("fixture failed");
    ...
    await expect(
      claimVerifiedDisputeAiArchiveAlert({} as Database, verified),
    ).resolves.toMatchObject({ outcome: "accepted" });
    await expect(claimVerifiedDisputeAiArchiveAlert({} as Database, verified)).resolves.toEqual({
