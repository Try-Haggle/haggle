...
const archive: DisputeSimilarityReviewAuditArchiveRecord = {
  ...
  archiveKey: `dsre_${"a".repeat(64)}`,
  eventId: "22222222-2222-4222-8222-222222222222",
  payload: { manifest: { event_id: "event-1" } },
...
describe("dispute similarity review audit WORM archive", () => {
  it("accepts a matching write-once receipt and sends idempotency headers", async () => {
    ...
    await expect(
      deliverDisputeSimilarityReviewAuditArchive(archive, config, {
  ...
  it("rejects private archive targets without explicit local-test overrides", () => {
    ...
    try {
      expect(() => resolveDisputeSimilarityReviewAuditArchiveConfigFromEnv()).toThrow();
    } finally {
  ...
  it("returns payload-free cursor failures and rejects malformed cursors before DB", async () => {
    const row = {
      ...
      last_error: "ARCHIVE_RECEIPT_HASH_MISMATCH",
      http_status: 201,
      receipt_id: null,
      receipt_sha256: null,
      delivered_at: null,
    ...
    const execute = vi
      .fn()
      .mockResolvedValue([row, { ...row, id: "44444444-4444-4444-8444-444444444444" }]);
    const result = await listDisputeSimilarityReviewAuditArchiveFailures(
      { execute } as unknown as Database,
      {
        limit: 1,
    ...
    const noQuery = vi.fn();
    await expect(
      listDisputeSimilarityReviewAuditArchiveFailures({ execute: noQuery } as unknown as Database, {
        cursor: "broken",
      }),
