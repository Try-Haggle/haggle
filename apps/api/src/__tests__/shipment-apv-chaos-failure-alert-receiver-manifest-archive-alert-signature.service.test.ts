...
} from "../services/shipment-apv-chaos-failure-alert-receiver-manifest-archive-alert-signature.service.js";
...
function bindingRow(overrides: Record<string, unknown> = {}) {
  return {
    ...
    signature_payload_outbox_id: null,
    signature_payload_sha256: null,
    signing_domain: null,
    algorithm: null,
    key_id: null,
...
const activeRegistry = {
  key_id: signer.keyId,
  public_key_spki_base64: signer.publicKeySpkiBase64,
  registered_at: "2026-07-14T00:01:00.000Z",
...
const input = { payloadOutboxId: outboxId, clientSignatureId, signedBy: checker, signer, now };
...
describe("receiver manifest archive alert payload signature", () => {
  it("appends a publicly verifiable domain-separated dry-run signature", async () => {
    ...
    const result = await createShipmentApvReceiverManifestArchiveAlertPayloadSignature(
      fixture.db,
      input,
    );
    ...
    expect(verifyShipmentApvReceiverManifestArchiveAlertPayloadSignature(result)).toBe(true);
    ...
    expect(insert.sql).toContain("shipment_apv_manifest_archive_alert_payload_signatures");
  });

  it("returns exact historical replay without current cooldown or key reads", async () => {
    const fixture = database([[], [signatureRow()]]);
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(fixture.db, input),
    ).resolves.toMatchObject({ replayed: true, signatureId });
    expect(fixture.execute).toHaveBeenCalledTimes(2);
    const replay = new PgDialect().sqlToQuery(fixture.execute.mock.calls[1]![0]).sql;
    expect(replay).not.toContain("join shipment_apv_manifest_archive_alert_cooldown_claims");
  ...
  it("fails closed on replay rebinding and corrupt persisted signatures", async () => {
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(rebound.db, input),
    ).rejects.toThrow("SIGNATURE_REPLAY_CONFLICT");
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(corrupt.db, input),
    ).rejects.toThrow("SIGNATURE_REPLAY_CONFLICT");
  });
  ...
  it("requires the outbox checker and active original cooldown", async () => {
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(actor.db, {
        ...input,
        signedBy: maker,
      }),
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(expired.db, {
        ...input,
  ...
  it("requires an active matching registry key and current archive state", async () => {
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(retired.db, input),
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(changed.db, input),
  ...
  it("blocks a second client signature for the same payload", async () => {
    ...
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(fixture.db, input),
    ).rejects.toThrow("PAYLOAD_ALREADY_SIGNED");
  });
  ...
  it("classifies a conflicting insert winner without leaking it", async () => {
    const fixture = database([
      ...
      [healthRow()],
      [],
      [signatureRow({ client_signature_id: "99999999-9999-4999-8999-999999999999" })],
    ]);
    await expect(
      createShipmentApvReceiverManifestArchiveAlertPayloadSignature(fixture.db, input),
    ).rejects.toThrow("PAYLOAD_ALREADY_SIGNED");
