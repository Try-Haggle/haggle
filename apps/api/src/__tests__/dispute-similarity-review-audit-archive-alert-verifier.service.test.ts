...
describe("similarity audit archive alert receiver verification", () => {
  ...
  it("rejects stale timestamps before accepting the delivery", () => {
    expect(verify({ nowMs: Date.parse(timestamp) + 5 * 60_000 + 1 })).toEqual({ ok: false, error: "ALERT_TIMESTAMP_OUT_OF_RANGE" });
  });
  ...
  it("claims a verified delivery once and treats the second claim as replay", async () => {
    ...
    vi.mocked(claimWebhookEvent)
      .mockResolvedValueOnce({ outcome: "acquired", source: "receiver", eventId: deliveryId, claimId: "11111111-1111-4111-8111-111111111111", attemptCount: 1 })
    ...
    await expect(claimVerifiedDisputeSimilarityArchiveAlert({} as Database, verified)).resolves.toMatchObject({ outcome: "accepted" });
    await expect(claimVerifiedDisputeSimilarityArchiveAlert({} as Database, verified)).resolves.toEqual({ outcome: "replay_or_in_progress" });
    await expect(claimVerifiedDisputeSimilarityArchiveAlert({} as Database, verified)).resolves.toEqual({ outcome: "payload_conflict" });
  });
});
