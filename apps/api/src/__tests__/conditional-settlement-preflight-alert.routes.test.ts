...
describe("conditional settlement preflight alert receiver routes", () => {
  ...
  it("accepts and completes the first valid delivery", async () => {
    ...
    const response = await app.inject({ method: "POST", url: "/internal/ops/alerts/conditional-settlement-preflight",
      headers: headers(raw, timestamp), payload: raw });
    expect(response.statusCode).toBe(202);
    expect(response.json()).toMatchObject({ accepted: true, replayed: false, delivery_id: deliveryId,
  ...
  it("returns idempotent replay and isolates a payload conflict", async () => {
    ...
    const first = await app.inject({ method: "POST", url: "/internal/ops/alerts/conditional-settlement-preflight",
      headers: headers(raw, timestamp), payload: raw });
    expect(first.statusCode).toBe(200); expect(first.json()).toMatchObject({ accepted: false, replayed: true });
    const conflict = await app.inject({ method: "POST", url: "/internal/ops/alerts/conditional-settlement-preflight",
      headers: headers(raw, timestamp), payload: raw });
  ...
  it("fails closed when unconfigured and rejects stale signatures", async () => {
    ...
    expect((await app.inject({ method: "POST", url: "/internal/ops/alerts/conditional-settlement-preflight",
    ...
    const response = await app.inject({ method: "POST", url: "/internal/ops/alerts/conditional-settlement-preflight",
      headers: headers(staleRaw, stale), payload: staleRaw });
  ...
  it("returns aggregate receiver health only to admins", async () => {
    ...
    const response = await admin.inject({ method: "GET", url: "/admin/ops/alerts/conditional-settlement-preflight/health" });
    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ receiver_kind: "conditional_settlement_preflight",
    ...
    expect((await user.inject({ method: "GET", url: "/admin/ops/alerts/conditional-settlement-preflight/health" })).statusCode).toBe(403);
    await user.close();
  });
});
