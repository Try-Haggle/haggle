...
const blockingReasons = ["independent_trust_anchor_missing",
...
const warningHealthRows = [{
  ...
  critical_observed_at: null, last_failure_at: "2026-07-13T10:05:00.000Z",
...
function bindingRow(overrides: Record<string, unknown> = {}) {
  return { signature_id: signatureId, payload_outbox_id: outboxId,
    payload_sha256: payloadSha256, key_id: keyId, signed_by: checker,
    state_fingerprint: fingerprint, cooldown_expires_at: "2026-07-13T12:45:00.000Z",
    key_event_type: "REGISTERED", intent_id: null, client_delivery_intent_id: null,
    intent_status: null, blocking_reasons: null, http_request_created: null,
    delivery_attempted: null, requested_by: null, intent_created_at: null, ...overrides };
}

function intentRow(overrides: Record<string, unknown> = {}) {
  return { id: intentId, client_delivery_intent_id: clientIntentId,
    payload_signature_id: signatureId, payload_outbox_id: outboxId,
    payload_sha256: payloadSha256, key_id: keyId,
    status: "BLOCKED_CONFIGURATION_DRY_RUN", blocking_reasons: blockingReasons,
    http_request_created: false, delivery_attempted: false, requested_by: checker,
...
describe("shipment APV failure alert blocked delivery intents", () => {
  it("appends a persistent non-executable intent with fixed blockers", async () => {
    const execute = vi.fn().mockResolvedValueOnce([])
      .mockResolvedValueOnce([bindingRow()]).mockResolvedValueOnce(warningHealthRows)
    ...
    expect(result).toMatchObject({ status: "BLOCKED_CONFIGURATION_DRY_RUN",
      blockingReasons, persistent: true, executable: false, replayed: false,
  ...
  it("requires the signature actor, active cooldown, and active registry key", async () => {
    ...
    await expect(createShipmentApvFailureAlertDeliveryIntent(
    ...
    const retiredExecute = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
      bindingRow({ key_event_type: "RETIRED" }),
    ]);
    await expect(createShipmentApvFailureAlertDeliveryIntent(
      { execute: retiredExecute } as unknown as Pick<Database, "execute">, input))
  ...
  it("rejects a second client id for an already planned signature", async () => {
    const execute = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([
      bindingRow({ intent_id: intentId,
        client_delivery_intent_id: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
        intent_status: "BLOCKED_CONFIGURATION_DRY_RUN", blocking_reasons: blockingReasons,
        http_request_created: false, delivery_attempted: false, requested_by: checker,
  ...
  it("fails closed when a client intent id is rebound", async () => {
    const execute = vi.fn().mockResolvedValueOnce([intentRow({
      requested_by: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", inserted: false,
    })]);
    await expect(createShipmentApvFailureAlertDeliveryIntent(
      { execute } as unknown as Pick<Database, "execute">, input))
