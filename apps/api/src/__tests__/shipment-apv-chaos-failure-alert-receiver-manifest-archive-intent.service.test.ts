...
function latest(overrides: Record<string, unknown> = {}) {
  return {
    ...
    intent_id: null,
    client_archive_intent_id: null,
    status: null,
    blocking_reasons: null,
    http_request_created: null,
    delivery_attempted: null,
    external_receipt_verified: null,
    production_accepted: null,
    requested_by: null,
    created_at: null,
    ...overrides,
  };
...
function intent(overrides: Record<string, unknown> = {}) {
  return {
    id: intentId,
    client_archive_intent_id: clientId,
    manifest_receipt_id: receiptId,
    manifest_revision: 1,
    manifest_digest: manifestDigest,
...
describe("shipment APV receiver manifest blocked archive intents", () => {
  ...
  it("returns an exact client replay before live health checks", async () => {
    ...
    await expect(
      createShipmentApvFailureAlertReceiverManifestArchiveIntent(
        { execute } as unknown as Pick<Database, "execute">,
        input,
      ),
  ...
  it("blocks an uncovered or stale manifest chain", async () => {
    ...
    ).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_MANIFEST_ARCHIVE_INTENT_HEALTH_BLOCKED");
  });
  ...
  it("fails closed when a client id is rebound to another actor", async () => {
    const execute = vi
      .fn()
      .mockResolvedValueOnce([
        intent({ inserted: false, requested_by: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" }),
      ]);
    await expect(
      createShipmentApvFailureAlertReceiverManifestArchiveIntent(
        { execute } as unknown as Pick<Database, "execute">,
