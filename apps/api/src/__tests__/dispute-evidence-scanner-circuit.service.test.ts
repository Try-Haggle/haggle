...
describe("dispute evidence scanner circuit", () => {
  it("validates bounded circuit and bulkhead policy", () => {
    ...
    expect(() => resolveDisputeEvidenceScannerCircuitConfigFromEnv()).toThrow(/threshold/);
    process.env.DISPUTE_EVIDENCE_SCANNER_CIRCUIT_FAILURE_THRESHOLD = "3";
    process.env.DISPUTE_EVIDENCE_SCANNER_MAX_CONCURRENT = "101";
    expect(() => resolveDisputeEvidenceScannerCircuitConfigFromEnv()).toThrow(/concurrency/);
  });
      ...
      .mockResolvedValueOnce([
        {
          ...
          nextProbeAt: "2026-07-14T00:01:00.000Z",
          probeExpiresAt: null,
        },
      ])
      .mockResolvedValueOnce([]);
      ...
      execute: vi.fn().mockResolvedValue([
        {
          ...
          activePermits: 1,
          nextProbeAt: null,
          probeExpiresAt: "2026-07-14T00:00:30.000Z",
          lastSuccessAt: null,
