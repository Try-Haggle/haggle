...
const manifest = { manifestDigest: digest, entryCount: 1,
  receiptDigests: [receiptDigest], generatedAt };
type ManifestResult = Awaited<ReturnType<
...
function row(overrides: Record<string, unknown> = {}) {
  return { revision: 1, manifest_digest: digest, previous_manifest_digest: null,
    entry_count: 1, receipt_digests: [receiptDigest],
    status: "PERSISTED_LOCAL_MANIFEST_RECEIPT_DRY_RUN", health_status: "healthy",
    contains_raw_identifiers: false, external_archive: false,
    network_delivered: false, production_accepted: false,
    generated_at: generatedAt, recorded_at: "2026-07-13T21:00:01.000Z",
...
describe("shipment APV receiver claim manifest receipts", () => {
  it("stores the first append-only local manifest receipt", async () => {
    vi.mocked(exportShipmentApvFailureAlertReceiverClaimManifest)
      .mockResolvedValueOnce(manifestResult);
    await expect(recordShipmentApvFailureAlertReceiverClaimManifestReceipt(
      database([], [], [], [row()]))).resolves.toMatchObject({
        status: "PERSISTED_LOCAL_MANIFEST_RECEIPT_DRY_RUN", revision: 1,
        manifestDigest: digest, previousManifestDigest: null, replayed: false,
  ...
  it("links a changed manifest to the latest revision", async () => {
    vi.mocked(exportShipmentApvFailureAlertReceiverClaimManifest)
      .mockResolvedValueOnce(manifestResult);
    const result = await recordShipmentApvFailureAlertReceiverClaimManifestReceipt(database(
  ...
  it("rejects a conflicting persisted snapshot", async () => {
    vi.mocked(exportShipmentApvFailureAlertReceiverClaimManifest)
      .mockResolvedValueOnce(manifestResult);
    await expect(recordShipmentApvFailureAlertReceiverClaimManifestReceipt(database(
      [], [row({ receipt_digests: ["d".repeat(64)], inserted: false })])))
  ...
  it("fails closed on an invalid latest revision or missing insert result", async () => {
    ...
    await expect(recordShipmentApvFailureAlertReceiverClaimManifestReceipt(database(
      [], [], [{ revision: "bad", manifest_digest: previous }])))
      .rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_MANIFEST_RECEIPT_INVALID");
    await expect(recordShipmentApvFailureAlertReceiverClaimManifestReceipt(database(
      [], [], [], []))).rejects.toThrow(
      "SHIPMENT_APV_FAILURE_ALERT_RECEIVER_MANIFEST_RECEIPT_UNAVAILABLE");
  });
