...
vi.mock(
  ...
  () => ({
    verifyShipmentApvReceiverManifestArchiveAlertReceiverContract: vi
      .fn()
      .mockResolvedValue({ status: "VERIFIED_LOCAL_FIXTURE_DRY_RUN" }),
  }),
);
...
const expectedDeliveryId = createHash("sha256")
  .update(`${deliveryDomain}:${intentId}:${payloadSha256}`, "utf8")
  .digest("hex");
...
describe("shipment APV archive alert local receiver claims", () => {
  ...
  it("returns an exact historical claim before live key verification", async () => {
    vi.mocked(verifyShipmentApvReceiverManifestArchiveAlertReceiverContract).mockClear();
    const result = await createShipmentApvReceiverManifestArchiveAlertReceiverClaim(
      database([claim({ inserted: false })]),
      { deliveryIntentId: intentId, now },
    );
    expect(result.replayed).toBe(true);
    expect(verifyShipmentApvReceiverManifestArchiveAlertReceiverContract).not.toHaveBeenCalled();
  });
  ...
  it("rejects a malformed historical claim instead of replaying it", async () => {
    await expect(
      createShipmentApvReceiverManifestArchiveAlertReceiverClaim(
        database([claim({ expected_payload_sha256: "d".repeat(64), inserted: false })]),
        { deliveryIntentId: intentId, now },
      ),
    ).rejects.toThrow("SHIPMENT_APV_RECEIVER_MANIFEST_ARCHIVE_ALERT_RECEIVER_CLAIM_CONFLICT");
  });
  ...
  it("rejects a concurrent claim collision", async () => {
    await expect(
      createShipmentApvReceiverManifestArchiveAlertReceiverClaim(
        database([], [binding], [claim({ payload_sha256: "d".repeat(64), inserted: false })]),
        { deliveryIntentId: intentId, now },
      ),
    ).rejects.toThrow("SHIPMENT_APV_RECEIVER_MANIFEST_ARCHIVE_ALERT_RECEIVER_CLAIM_CONFLICT");
  });
  ...
  it("reloads an exact concurrent winner outside the insert snapshot", async () => {
    ...
    ).resolves.toMatchObject({ replayed: true, deliveryId: expectedDeliveryId });
  });

  it("fails closed when a concurrent winner cannot be reloaded", async () => {
    await expect(
      createShipmentApvReceiverManifestArchiveAlertReceiverClaim(database([], [binding], [], []), {
        deliveryIntentId: intentId,
        now,
      }),
    ).rejects.toThrow("SHIPMENT_APV_RECEIVER_MANIFEST_ARCHIVE_ALERT_RECEIVER_CLAIM_UNAVAILABLE");
  });
});
