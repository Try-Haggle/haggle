...
describe("shipment APV retention alert fixture readiness", () => {
  ...
  it("reports a ready non-production fixture without mutating the database", async () => {
    ...
    expect(result).toMatchObject({ eligible: true, status: "ready", reasons: [],
      singleton: { status: "SUCCEEDED" }, executionLease: { available: true },
      schemaVersion: "shipment-apv-fixture-readiness-v1", validForSeconds: 5 });
    ...
    const repeatExecute = vi.fn()
      .mockResolvedValueOnce([{ status: "SUCCEEDED" }])
      .mockResolvedValueOnce([{ active: false }]);
    const repeated = await getShipmentApvRetentionAlertFixtureReadiness(
      { execute: repeatExecute } as unknown as Database);
  ...
  it("returns all allowlisted blockers without identifiers", async () => {
    ...
    const result = await getShipmentApvRetentionAlertFixtureReadiness(
      { execute } as unknown as Database);
    expect(result).toMatchObject({ eligible: false, status: "blocked", reasons: [
      "production_runtime", "retention_job_active", "retention_state_running", "fixture_lease_active",
    ], singleton: { status: "RUNNING" }, executionLease: { available: false } });
    expect(result.stateFingerprint).toMatch(/^[0-9a-f]{64}$/);
  ...
  it("distinguishes a missing singleton from a running singleton", async () => {
    process.env.NODE_ENV = "test";
    const execute = vi.fn().mockResolvedValueOnce([]).mockResolvedValueOnce([{ active: false }]);
    await expect(getShipmentApvRetentionAlertFixtureReadiness(
      { execute } as unknown as Database)).resolves.toMatchObject({
      eligible: false, reasons: ["retention_state_missing"], singleton: { status: "MISSING" },
    });
  });

  it("changes the diagnostic fingerprint when a public readiness input changes", async () => {
    process.env.NODE_ENV = "test";
    const ready = await getShipmentApvRetentionAlertFixtureReadiness({ execute: vi.fn()
      .mockResolvedValueOnce([{ status: "SUCCEEDED" }])
      .mockResolvedValueOnce([{ active: false }]) } as unknown as Database);
    const busy = await getShipmentApvRetentionAlertFixtureReadiness({ execute: vi.fn()
      .mockResolvedValueOnce([{ status: "RUNNING" }])
      .mockResolvedValueOnce([{ active: false }]) } as unknown as Database);
