...
describe("shipment APV local receiver claim health", () => {
  it("returns identifier-free healthy aggregates without deleting old claims", async () => {
    await expect(getShipmentApvFailureAlertReceiverClaimHealth(database(healthy)))
      .resolves.toEqual({
  ...
  it("marks any persisted invariant violation critical", async () => {
    const result = await getShipmentApvFailureAlertReceiverClaimHealth(database({
      ...healthy, binding_failure_count: "1", delivery_id_mismatch_count: "2",
      freshness_violation_count: "3", unsafe_side_effect_count: "4",
    }));
  ...
  it("fails closed on missing or malformed aggregate rows", async () => {
    await expect(getShipmentApvFailureAlertReceiverClaimHealth(database(undefined)))
      .rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CLAIM_HEALTH_INVALID");
    await expect(getShipmentApvFailureAlertReceiverClaimHealth(database({
      ...healthy, total_claims: "not-a-count",
    }))).rejects.toThrow("SHIPMENT_APV_FAILURE_ALERT_RECEIVER_CLAIM_HEALTH_INVALID");
  });
});
