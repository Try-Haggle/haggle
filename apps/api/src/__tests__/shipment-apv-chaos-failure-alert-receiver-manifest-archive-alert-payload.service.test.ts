...
function bindingRow(overrides: Record<string, unknown> = {}) {
  return { outbox_id: null, client_outbox_id: null,
    outbox_delivery_grant_id: null, outbox_state_fingerprint: null,
    payload: null, canonical_payload: null, payload_sha256: null,
    outbox_status: null, created_by: null, outbox_created_at: null,
    grant_id: grantId, client_grant_id: "77777777-7777-4777-8777-777777777777",
    approval_decision_id: decisionId, grant_state_fingerprint: fingerprint,
    grant_status: "GRANTED_DRY_RUN", granted_by: checker,
    ...
    prior_outbox_id: null, prior_client_outbox_id: null,
    prior_created_by: null, ...overrides };
}

function outboxRow(overrides: Record<string, unknown> = {}) {
  return { ...bindingRow(), outbox_id: outboxId,
    client_outbox_id: clientOutboxId, outbox_delivery_grant_id: grantId,
    outbox_state_fingerprint: fingerprint, payload, canonical_payload: canonical,
    payload_sha256: sha256, outbox_status: "UNSIGNED_DRY_RUN",
    created_by: checker, outbox_created_at: now.toISOString(), ...overrides };
}
...
const input = { deliveryGrantId: grantId, clientOutboxId,
  createdBy: checker, now };
...
describe("receiver manifest archive alert unsigned payload outbox", () => {
  it("records a canonical identifier-free payload without signing or delivery", async () => {
    const { db, execute, transaction } = database([
      [], [], [bindingRow()], [healthRow()], [{ id: outboxId }], [outboxRow()],
    ]);
    const result =
      await createShipmentApvReceiverManifestArchiveAlertPayloadOutbox(db, input);
    expect(result).toMatchObject({
      schemaVersion:
        "shipment-apv-failure-alert-receiver-manifest-archive-alert-payload-outbox-v1",
      payloadOutboxId: outboxId, clientOutboxId, deliveryGrantId: grantId,
      stateFingerprint: fingerprint, payload, payloadSha256: sha256,
      status: "UNSIGNED_DRY_RUN", replayed: false, persistent: true,
  ...
  it("returns an exact immutable replay before grant and current-state reads", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertPayloadOutbox(
      db, input)).resolves.toMatchObject({ replayed: true,
      payloadOutboxId: outboxId, payloadSha256: sha256 });
    expect(execute).toHaveBeenCalledTimes(2);
    const replayQuery = new PgDialect().sqlToQuery(execute.mock.calls[1]![0]).sql;
    expect(replayQuery).not.toContain(
  ...
  it("fails closed on client replay rebinding or malformed stored payload", async () => {
    ...
    await expect(createShipmentApvReceiverManifestArchiveAlertPayloadOutbox(
      rebound.db, input)).rejects.toThrow("PAYLOAD_REPLAY_CONFLICT");

    const malformed = database([[], [outboxRow({
      payload: { ...payload, requested_by: maker } })]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertPayloadOutbox(
      malformed.db, input)).rejects.toThrow("PAYLOAD_REPLAY_CONFLICT");
  });

  it("requires the original checker and an active grant cooldown", async () => {
    const actor = database([[], [], [bindingRow()]]);
    await expect(createShipmentApvReceiverManifestArchiveAlertPayloadOutbox(
      actor.db, { ...input, createdBy: maker }))
  ...
  it("rejects a changed current archive state before insert", async () => {
    const changed = database([[], [], [bindingRow()], [healthRow({
