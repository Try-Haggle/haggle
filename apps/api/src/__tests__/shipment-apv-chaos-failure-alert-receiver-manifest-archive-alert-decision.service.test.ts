...
function requestRow(overrides: Record<string, unknown> = {}) {
  return {
    ...
    requested_by: maker,
    expires_at: "2026-07-13T23:45:00.000Z",
    decision_id: null,
...
describe("receiver manifest archive alert maker-checker decision", () => {
  ...
  it("revalidates current state for rejection as well as approval", async () => {
    const { db, execute } = database([
      ...
      [requestRow()],
      [healthRow()],
      [decisionRow({ decision: "REJECTED", decision_reason: "checker_rejected_snapshot" })],
    ]);
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(db, {
        approvalRequestId: requestId,
        clientDecisionId,
        decidedBy: checker,
        decision: "REJECTED",
      ...
      decision: "REJECTED",
      reason: "checker_rejected_snapshot",
      replayed: false,
    });
    expect(execute).toHaveBeenCalledTimes(5);
  });
  ...
  it("rejects a replay key rebound to another checker or decision", async () => {
    const { db } = database([[], [decisionRow({ inserted: false, decided_by: maker })]]);
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(db, {
        approvalRequestId: requestId,
        clientDecisionId,
  ...
  it("fails closed on malformed replay bindings before request or health reads", async () => {
    const { db, execute } = database([
      [],
      [decisionRow({ inserted: false, decision_reason: "checker_rejected_snapshot" })],
    ]);
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(db, {
        approvalRequestId: requestId,
  ...
  it("blocks the maker and expired requests", async () => {
    ...
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(self.db, {
        approvalRequestId: requestId,
        clientDecisionId,
        decidedBy: maker,
        decision: "APPROVED",
    ...
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(expired.db, {
        approvalRequestId: requestId,
        clientDecisionId,
        decidedBy: checker,
        decision: "REJECTED",
    ...
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(invalid.db, {
        approvalRequestId: requestId,
        clientDecisionId,
        decidedBy: checker,
        decision: "REJECTED",
  ...
  it("fails closed when current state or stored request binding changed", async () => {
    ...
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(healthy.db, {
        approvalRequestId: requestId,
        clientDecisionId,
        decidedBy: checker,
        decision: "APPROVED",
    ...
    await expect(
      decideShipmentApvReceiverManifestArchiveAlertApprovalRequest(rebound.db, {
        approvalRequestId: requestId,
        clientDecisionId,
        decidedBy: checker,
        decision: "REJECTED",
