...
import { ViemConditionalSettlementContract, ViemSettlementRouterContract } from "../viem-contracts.js";
import type { ConditionalSettlementCreateRequest, SettlementRouterExecutionRequest } from "../x402-contracts.js";
...
describe("ViemConditionalSettlementContract", () => {
  ...
  beforeEach(() => {
    ...
    publicClient = {
      ...
      waitForTransactionReceipt: vi.fn().mockResolvedValue({ status: "success", transactionHash: "0xTxHash" as Hex }),
    };
  ...
  it("release sends signed release tuple with fee split", async () => {
    await contract.release({
      ...
      seller_wallet: { actor_id: "seller-1", wallet_address: "0xSeller000000000000000000000000000000001" as Address, network: "eip155:8453", custody: "external" },
      haggle_fee_wallet: { actor_id: "haggle", wallet_address: "0xFeeWallet0000000000000000000000000000001" as Address, network: "eip155:8453", custody: "merchant_managed" },
      seller_amount: { currency: "USDC", amount_minor: 98_500_000 },
      haggle_fee_amount: { currency: "USDC", amount_minor: 1_500_000 },
