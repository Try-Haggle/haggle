...
export const disputeEvidenceScanRetryAlertSnapshots = pgTable(
  ...
  (table) => ({
    sourceDeliveryUnique: unique("dispute_scan_retry_alert_snapshot_source_delivery_unique").on(
      table.source,
      table.deliveryId,
    ),
    expiryIdx: index("dispute_scan_retry_alert_snapshot_expiry_idx").on(table.expiresAt),
  }),
);
