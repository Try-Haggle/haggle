...
export const shipmentApvInvoiceDocuments = pgTable(
  "shipment_apv_invoice_documents",
  {
    ...
    integrityStatus: text("integrity_status", { enum: ["ACTIVE", "MISSING", "QUARANTINED"] })
      .notNull()
      .default("ACTIVE"),
  ...
  (table) => ({
    revisionUnique: uniqueIndex("shipment_apv_invoice_documents_revision_unique").on(
      table.revisionId,
    ),
    storageUnique: uniqueIndex("shipment_apv_invoice_documents_storage_unique").on(
      table.storageKey,
    ),
    createdIdx: index("shipment_apv_invoice_documents_created_idx").on(table.createdAt),
    integrityIdx: index("shipment_apv_invoice_documents_integrity_idx").on(
      table.integrityStatus,
      table.createdAt,
    ),
  }),
);
