...
export const shipmentApvInvoiceReconciliationRequests = pgTable(
  "shipment_apv_invoice_reconciliation_requests",
  {
    ...
    anomalyType: text("anomaly_type", { enum: ["MISSING_FILE", "SIZE_MISMATCH", "HASH_MISMATCH", "ORPHAN_FILE"] }).notNull(),
  ...
  (table) => ({
    clientUnique: uniqueIndex("shipment_apv_invoice_reconcile_client_unique").on(table.clientRequestId),
    decisionUnique: uniqueIndex("shipment_apv_invoice_reconcile_decision_unique").on(table.decisionRequestId),
    pendingTargetUnique: uniqueIndex("shipment_apv_invoice_reconcile_pending_target_unique")
      .on(table.targetFingerprint).where(sql`${table.status} IN ('PENDING', 'APPLYING')`),
    pendingIdx: index("shipment_apv_invoice_reconcile_pending_idx").on(table.status, table.expiresAt, table.createdAt),
  }),
);
...
export const shipmentApvInvoiceReconciliationEvents = pgTable(
  ...
  (table) => ({
    transitionUnique: uniqueIndex("shipment_apv_invoice_reconcile_event_transition_unique")
      .on(table.requestId, table.eventType, table.requestVersion),
    timelineIdx: index("shipment_apv_invoice_reconcile_event_timeline_idx")
