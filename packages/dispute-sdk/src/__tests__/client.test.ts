...
describe("HaggleDisputeClient", () => {
  ...
  it("sends signed escalation preview requests", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      ...
      seller_deposit_requirement: { amount_cents: 50_000, deadline_hours: 48, status: "PENDING" },
    })));
    const client = new HaggleDisputeClient({
      baseUrl: "https://api.haggle.test/",
      platformId: "platform_1",
    ...
    const result = await client.previewEscalation("dsp_1", {
      external_order_id: "order_123",
      requester_actor_id: "buyer_1",
  ...
  it("sends signed escalation create and status requests", async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({
        ...
        status: "UNDER_REVIEW",
        tier: 2,
        current_tier_cost: { tier: 2, cost_cents: 1200, reviewer_count: 5, escalation_period_hours: 24 },
        current_seller_deposit_requirement: { amount_cents: 50_000, deadline_hours: 48, status: "PENDING" },
    ...
    const escalation = await client.createEscalation("dsp_1", {
      external_order_id: "order_123",
      requester_actor_id: "buyer_1",
    }, { idempotencyKey: "esc:order_123:t2" });
    const status = await client.getCaseStatus("dsp_1", {
      external_order_id: "order_123",
  ...
  it("requires HTTPS by default and allows explicit insecure local development", () => {
    expect(() => new HaggleDisputeClient({
  ...
  it("wraps timed out requests without exposing signed headers", async () => {
    ...
    await expect(client.createCase(input, { idempotencyKey: "open:order_123" }))
      .rejects.toMatchObject({
  ...
  it("validates module dispute inputs before signing requests", async () => {
    ...
    }, { idempotencyKey: "open:order_123" })).rejects.toMatchObject({
  ...
  it("reports multiple unsafe input issues without sending requests", () => {
    ...
    expect(validateModuleDisputeCaseInput(invalid, { platformId: "platform_1" }))
      .toEqual(expect.arrayContaining([
  ...
  it("validates escalation inputs before signing", async () => {
    ...
    await expect(client.createEscalation("dsp_1", {
      external_order_id: "",
      requester_actor_id: "",
      to_tier: 4 as never,
    }, { idempotencyKey: "esc:order_123:t2" })).rejects.toMatchObject({
      name: "HaggleDisputeValidationError",
      issues: expect.arrayContaining([
    ...
    expect(validateModuleDisputeEscalationInput({
      external_order_id: "order_123",
      requester_actor_id: "buyer_1",
      to_tier: 2,
    })).toEqual([]);
  ...
  it("validates status inputs before signing", async () => {
    ...
    await expect(client.getCaseStatus("dsp_1", undefined as never, {
      idempotencyKey: "status:order_123",
    })).rejects.toMatchObject({
      name: "HaggleDisputeValidationError",
  ...
  it("rejects non-serializable module inputs before signing", async () => {
    ...
    await expect(client.previewCase(circular, { idempotencyKey: "preview:order_123" }))
      .rejects.toMatchObject({
        name: "HaggleDisputeValidationError",
        issues: expect.arrayContaining([
  ...
  it("wraps API error responses with status, code, and body", async () => {
    ...
    await expect(client.createCase(input, { idempotencyKey: "idem_sdk_123" }))
      .rejects.toMatchObject({
        name: "HaggleDisputeApiError",
        status: 409,
  ...
  it("rejects create responses with mismatched idempotency keys", async () => {
    ...
    await expect(client.createCase(input, { idempotencyKey: "open:order_123" }))
      .rejects.toMatchObject({
        name: "HaggleDisputeResponseValidationError",
        issues: expect.arrayContaining([
          expect.objectContaining({ path: "idempotency_key" }),
        ]),
      });
  });
  ...
  it("rejects weak secrets and invalid idempotency keys before sending", async () => {
    ...
    await expect(client.createCase(input, { idempotencyKey: "bad" }))
      .rejects.toThrow("idempotencyKey must be 8-128 URL-safe characters");
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it("validates settlement instruction consistency with the webhook envelope", () => {
    ...
    expect(validateSettlementInstructionWebhookData(settlement, {
      disputeId: "dsp_1",
    })).toEqual([]);

    expect(validateSettlementInstructionWebhookData({
      ...settlement,
