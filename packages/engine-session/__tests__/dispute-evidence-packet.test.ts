...
describe("HNP dispute evidence packet", () => {
  it("creates a hash-bound dispute evidence packet", () => {
    ...
    expect(validateHnpDisputeEvidencePacket(packet, { verifyHash: true })).toEqual({
      ok: true,
      warnings: [],
    });
  });
  ...
  it("rejects packets without evidence or agreement binding", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.issues).toContainEqual(
        expect.objectContaining({ code: "INVALID_AGREEMENT_HASH" }),
      );
  ...
  it("detects duplicate evidence ids and unknown finding sources", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.issues).toContainEqual(
        expect.objectContaining({ code: "UNKNOWN_FINDING_SOURCE" }),
      );
    }
  });
  ...
  it("rejects invalid adjustments, timestamps, and empty finding sources", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.issues).toContainEqual(
        expect.objectContaining({ code: "INVALID_SUBMITTED_AT" }),
      );
      ...
      expect(result.issues).toContainEqual(
        expect.objectContaining({ code: "EMPTY_FINDING_SOURCES" }),
      );
    }
  });
...
describe("HNP digital dispute evidence kinds (Phase 4 scaffold)", () => {
  it("accepts Phase 4 digital evidence kinds in a hash-bound packet", () => {
    expect(HNP_DIGITAL_DISPUTE_EVIDENCE_KINDS).toContain("digital_file_hash");
    expect(isHnpDigitalDisputeEvidenceKind("platform_transfer")).toBe(true);
    expect(isHnpDisputeEvidenceKind("onchain_transfer")).toBe(true);
    expect(isHnpDisputeEvidenceKind("card_pan")).toBe(false);

    const packet = createHnpDisputeEvidencePacket({
      agreement_id: "agr-digital-1",
      agreement_hash: agreementHash,
      reason: "item_not_as_described",
      requested_resolution: "full_refund",
      evidence: [
        {
          evidence_id: "access-1",
          kind: "digital_access",
          sha256: evidenceHash,
          submitted_at_ms: 1_777_000_100_000,
        },
  ...
  it("still rejects unknown evidence kinds", () => {
    const packet = createHnpDisputeEvidencePacket({
      agreement_id: "agr-1",
      agreement_hash: agreementHash,
      reason: "other",
      ...
      evidence: [
        {
          evidence_id: "bad-1",
          kind: "card_pan" as "message_transcript",
          submitted_at_ms: 1_777_000_100_000,
        },
      ],
      created_at_ms: 1_777_000_100_000,
    ...
    if (!result.ok) {
      expect(result.issues).toContainEqual(
        expect.objectContaining({ code: "UNSUPPORTED_EVIDENCE_KIND" }),
      );
    }
  });
});
