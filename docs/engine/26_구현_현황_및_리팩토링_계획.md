...
**config로 `'template' | 'llm'` 모드 전환 지원.** 현재 template → 향후 LLM.
...
### 12.2 라운드당 비용 (6-Stage, Demo 실측 — 2026-04-12)
