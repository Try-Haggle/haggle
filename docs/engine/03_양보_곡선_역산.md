...
Faratin 양보 곡선 `(t/T)^(1/β)`에서 β=0이면 0으로 나누기 발생. v2.0 구현:

```typescript
if (T <= 0) {
  concessionRatio = 1;  // 마감 초과 → 최대 양보
} else if (beta <= 0) {
  concessionRatio = t >= T ? 1 : 0;  // 스텝 함수: 마감 전=0, 마감 시=1
} else {
...
### 7.2 수락 임계값 R(t) NaN 가드

```typescript
const safeTau = clamp(tau, 0, 1);  // 음수 tau 방지
if (beta <= 0) {
  aspiration = safeTau >= 1 ? u_min : u_0;  // 스텝 함수
} else {
...
## 8. J(ω) Offer Search (v2.0 구현)

### 8.1 목적

역제안을 최적화하는 목적함수:

$$J(\omega) = \alpha \cdot U_{total}(\omega) + (1-\alpha) \cdot \hat{P}_{accept}(\omega) - \eta \cdot C_{move}(\omega, \omega_{prev})$$

- α: 자기 효용 가중치 (기본 0.7)
- P̂_accept: 상대 수락 확률 추정 (가중 이슈별 근접도)
- η: 이동 비용 패널티 (기본 0.1)
- C_move: 이전 제안 대비 변경폭 (computeMoveCost)

### 8.2 탐색 방법

Base offer (Faratin 양보 결과) 주변 ±5% 섭동으로 그리드 탐색:

```typescript
searchOffer(input: OfferSearchInput): OfferSearchResult {
  // 1. Base offer에서 J(ω) 계산
  // 2. 각 negotiable 이슈에 ±5% 섭동 생성
  // 3. 모든 후보의 J(ω) 계산
  // 4. 최고 J(ω)를 가진 offer 반환
}
```

### 8.3 수락 확률 추정

$$\hat{P}_{accept}(\omega) = \sum_i w_i \cdot \text{closeness}_i(\omega_i, \text{opponent\_last}_i)$$

이슈별 가중 근접도 휴리스틱. 상대의 마지막 제안에 가까울수록 높은 수락 확률.

### 8.4 인터페이스

```typescript
interface OfferSearchInput {
