...
**문서:** Haggle Engine Architecture v2.0.0 — 아키텍처 개요
...
7. **하위 호환성(Backward Compatibility)** — 단일 이슈(가격만) 모드가 기본값. 다중 이슈는 `issue_space` 파라미터 제공 시 활성화. (v1.0.2)
...
### 4.1 vNext 다중이슈 라운드 파이프라인 (v2.0 구현)

`executeMultiIssueRound(strategy, roundData, incomingOffer, definitions, previousOffer?, opponentModel?)` — 6단계 파이프라인:

```
1. 효용 계산: computeMultiIssueUtility()
   - U_contract = Σ w_i · v_i(x_i)  (negotiable 이슈만)
   - C_risk = Σ λ_k · p_k  (사기/품질/지연/분쟁 리스크)
   - B_rel = Σ ρ_j · q_j  (평판/반복거래/응답성 보너스)
   - U_total = clip(U_contract - C_risk + B_rel, 0, 1)

2. 의사결정: makeMultiIssueDecision()
   - R(t) = max{U_BATNA, U_min + (U_0 - U_min)(1 - τ^β)}
   - NEAR_DEAL 밴드: [R-δ, R) → 거래 근접 신호
   - ESCALATE: 교착(stall) 또는 마감임박(deadline_critical)

3. COUNTER 또는 NEAR_DEAL → 역제안 생성:
   - computeMultiIssueCounterOffer() (Faratin 양보)
   - searchOffer() J(ω) 최적화

4. 상대 모델 업데이트: updateMultiIssueOpponentModel()
   - Direction-aware 양보 분류
   - Enum/Boolean 이슈 추적

5. 에스컬레이션 사유 판별 (STALL / DEADLINE)

6. MultiIssueRoundResult 반환
```

**NegotiationEngine 인터페이스 (v2.0 신규):**
엔진 사용을 위한 공식 계약:
- `evaluate(context)` → EngineEvaluation (utility + decision)
- `counter(context)` → EngineCounterOffer (counter-offer + score)
- `observeOpponent(context)` → MultiIssueOpponentModel

---

## 5. Intent-First API 연동 (v1.0.2)

Intent-First API는 다중 이슈 협상을 자연스럽게 지원합니다:
...
GoalParser v1.0.2는 기존 가중치에 더해 `IssueSpace`도 생성합니다:
