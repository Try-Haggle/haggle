...
Living Memo + HIL Memory + RefereeBriefing + SkillStack Hook 결과를 조립.

```
Living Memo (Shared + Private)     ← DB에서 로드
+ HIL Memory                       ← user_memory_cards 중 item-relevant subset
+ RefereeBriefing                  ← 심판 사실 브리핑 (§4.2)
+ SkillStack.dispatchHook('decide')← 지식 + advisory 병합 (§5.4)
= Full NegotiationContext
```

HIL Memory는 유저가 이전 대화에서 명시한 예산, must-have, avoid, risk/tactic preference를 담는다. Stage 0 전략 생성과 Stage 0b Term 분석에도 같은 subset을 주입해, 충분한 정보가 있으면 반복 질문 없이 바로 추천/협상으로 넘어간다.
...
검증된 `ProtocolDecision`을 locale/currency-safe 템플릿으로 렌더링한다. 가격, 통화, 단위, non-price terms formatting은 코드가 소유한다.

> **2026-04-25 정렬:** Stage 5는 현재 Demo에서 LLM이 아니다. 자유 텍스트 생성이 가격 단위 혼선(`$280` vs `$28,000`)을 만들 수 있으므로, LLM은 Stage 3에서 action/price/terms를 결정하고 Stage 5는 구조화 렌더러가 담당한다. 향후 voice-only rewrite를 추가하더라도 price span은 코드가 lock해야 한다.
...
> **⚠️ 수정 (2026-04-25):** 이전 버전의 가격 단위 오류($0.05/1K → 실제 $0.20/1M)와 Stage 5 LLM 가정을 수정. 현재 Stage 5는 structured renderer다.
...
| RESPOND | 0 | 0 | structured renderer |
| **합계** | **~1,150** | **~250** | **~1,400 tokens/라운드 이하** |
...
라운드당 (실측):
  Input:  1,150 × $0.0000002 = $0.000230
  Output: 250 × $0.0000005  = $0.000125
  합계: ~$0.00026-0.00036/라운드

10라운드 세션:
  Init (Stage 0a + 0b):        ~$0.0004-0.001
  10라운드 × ~$0.00026-0.00036
  합계:                         ~$0.005-0.006/세션

Live E2E 검증 (2026-04-25):
  advisor 1턴 + init + round 1회 = 5 LLM 호출 → $0.0013809
```
...
| TemplateMessageRenderer | **유지/통합** — structured RESPOND가 가격/통화 lock |
| 신규: understand.ts | Stage 1 메시지 파싱 |
| 신규: responder.ts | Stage 5 구조화 메시지 렌더링 |
...
## 18. 실시간 시간 효용 + Urgency 기반 양보곡선 (2026-04-14 추가)

### 18.1 라운드 기반 → 벽시계 기반 전환

기존: `t_elapsed = round / max_rounds` (라운드 수 비율)
신규: `t_elapsed = (현재시각 - 세션시작시각) / 카테고리_최대시간`

#### 카테고리별 기본 최대 시간

| 카테고리 | 기본 시간 | 근거 |
|----------|----------|------|
| 전자제품/게임/오디오 | 7일 | 중고 전자제품 일반 거래 기간 |
| 노트북 | 14일 | 고가 → 신중한 결정 |
| 스니커즈 | 3일 | 하이프 아이템, 빠른 시장 변동 |

### 18.2 시간 압박 신호

판매자/구매자가 설정하는 급함 정도는 `urgency` enum으로 저장하지 않는다. 가격을 바꾸는 표준 경로는 **남은 시간 + 양보 곡선**이다.

```
deadline_at(UTC) + user/browser timezone metadata
→ seconds_left
→ V_t(t_elapsed, t_deadline, alpha, v_t_floor)
→ U_target(t)
→ Offer Inverter
→ price/terms
```

전략 계층은 `alpha`, `beta`, `v_t_floor`, `t_deadline`을 조정할 수 있지만, "urgent라서 가격을 X% 내린다" 같은 직접 규칙은 금지한다. 같은 리스팅도 오늘 마감과 내일 마감이면 `seconds_left`가 달라지고, 그 결과 counter price가 자연스럽게 달라진다.
...
### 18.3 Floor 보호 (가격 제한 안전장치)

**원칙: 아무리 급해도 floor 가격은 절대 넘지 않는다.**

- `FLOOR_PROTECTION_RATIO = 0.90` — floor까지 범위의 90%만 양보 허용
- Faratin의 `p_limit`을 raw floor이 아닌 `p_protected`로 설정
- 예: target=$800, floor=$500 → 범위 $300
  - protected = floor + $300 × 10% = $530 (판매자)
  - Faratin이 아무리 빨리 양보해도 $530 이하로 안 내려감
  - 나머지 $30 (floor까지) = 사용자가 수동으로만 결정 가능

**철학 매핑**: 안전 > 편리, 사용자 보호 우선 — 자동화가 사용자 이익을 침해하면 안 됨.

---

## 19. 프롬프트 인젝션 방어 (2026-04-14 추가)
...
## 20. Skill 검증 배지 시스템 (2026-04-14 추가)
...
## 21. 결제 수수료 정책 + 지갑 인프라 (2026-04-14 추가)

### 21.1 수수료 구조

| 결제 방식 | Haggle 수수료 | 외부 수수료 | 총 구매자 비용 | 판매자 수령 |
|-----------|-------------|------------|--------------|-----------|
| USDC 직접 | 1.5% | 0% | 1.5% | 합의가 - 1.5% |
| 카드 (Stripe Onramp) | 1.5% | Stripe 1.5% | 3.0% | 합의가 - 1.5% (동일) |

**핵심 원칙**: 판매자 수령액은 결제 방식과 무관하게 동일. Stripe 추가 수수료는 구매자 편의비.

### 21.2 가스비 대납 (Gas Relay)

- Haggle Relayer 지갑이 Base L2 가스비 대납 (~$0.001/tx)
- 사용자는 USDC만 있으면 됨 (ETH 불필요)
- `HAGGLE_ROUTER_RELAYER_PRIVATE_KEY`로 트랜잭션 서명
- 비용 대비 UX 개선 효과가 압도적 ($0.001 vs 사용자 이탈 방지)

### 21.3 지갑 인프라

**판매자**:
- 지갑 필요 시점: 정산 시 (리스팅/협상 시에는 불필요)
- Coinbase Smart Wallet: 이메일만으로 3초 만에 생성, 셀프 커스터디 (passkey + email OTP)
- 기존 지갑: MetaMask, Rainbow, WalletConnect 등 RainbowKit 지원 전체
- Coinbase Offramp: USDC → USD 은행 입금 (수수료 0%)

**구매자**:
- 지갑 있음 → USDC 직접 결제 (1.5%)
- 지갑 없음 → 카드 결제 Stripe Onramp (3%) — 지갑 불필요

**Non-Custodial 원칙**: Haggle은 유저 키를 절대 보유하지 않음 (CLAUDE.md §원칙).
