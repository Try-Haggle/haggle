...
- ✅ Decide 모델은 카탈로그에서 id를 고른다(`decide-model.ts`). Flash/Pro는 지금 칸이 둘인 것뿐이다. 표는 [`decide-model-routing.md`](./decide-model-routing.md). 타임아웃 기본 120초(`DEEPSEEK_TIMEOUT_MS`, 10–180초). temperature 기본 0.5 — 샘플러일 뿐 추론 스위치가 아니다. `DEEPSEEK_TEMPERATURE` 또는 `StageConfig.temperature`로 바꾼다(`0`–`2`). DB `reasoning_used`는 호환용으로 false.
