...
> **2026-04-25 정책 정렬:** `urgency`를 별도 enum으로 분기하지 않는다. 급한 판매자/구매자의 효과는 `listing_deadline`, 남은 시간, active sessions, thread length, `β`, `V_t_floor`가 함께 만드는 곡선으로 표현한다. 같은 상품이라도 오늘 마감과 내일 마감의 counter price가 달라져야 한다.
...
interface DynamicDeadlineParams {
  listing_deadline?: string;          // UTC ISO timestamp. UI 입력은 브라우저 타임존 local date에서 정규화.
  listing_timezone?: string;          // IANA timezone. 재표시/감사용, 계산은 UTC timestamp 사용.
  listing_local_date?: string;        // YYYY-MM-DD. 사용자가 본 local deadline date.
...
시간 효용은 직접 가격을 덮어쓰지 않는다. 가격은 아래 경로로만 바뀐다.

```
deadline/timezone → seconds_left
seconds_left → t_elapsed / t_deadline
V_t = f(t_elapsed, t_deadline, alpha, v_t_floor)
U_target(t) = Faratin utility-space concession curve
Offer Inverter → price/terms
```

따라서 "오늘 안에 팔고 싶다"는 정보는 `urgency=high`가 아니라 `deadline_at`과 seller strategy parameter로 저장되어야 한다. 이렇게 해야 여러 지역의 사용자가 같은 세션을 보더라도 UTC 기준으로 동일한 계산 결과가 나온다.
