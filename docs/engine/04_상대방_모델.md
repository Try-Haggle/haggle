...
## 8. vNext Direction-aware 다중이슈 상대 모델 (v2.0 구현)

### 8.1 개요

v2.0은 다중이슈 환경에서 이슈별 독립 추적기를 통해 상대방 양보 패턴을 추적합니다. 핵심 개선: **IssueDirection에 따른 양보 방향 인식**.

### 8.2 이슈별 양보 추적기

```typescript
interface IssueConcesssionTracker {
  issue_name: string;
  concession_rate: number;   // EMA 양보율
  move_count: number;
  last_value: number | string | boolean;
}

interface MultiIssueOpponentModel {
  issue_trackers: Record<string, IssueConcesssionTracker>;
  total_rounds: number;
  concession_style: 'fast' | 'slow' | 'mixed';
  estimated_priorities: Record<string, number>;  // 추정 가중치
}
```

### 8.3 Direction-aware 양보 분류

이슈의 `direction` 속성에 따라 양보 여부를 올바르게 판정합니다:

```
scalar 이슈:
  lower_better (예: price):
    구매자 양보 = delta > 0 (가격 올리기)
    판매자 양보 = delta < 0 (가격 내리기)
  higher_better (예: warranty_days):
    구매자 양보 = delta < 0 (기간 줄이기)
    판매자 양보 = delta > 0 (기간 늘리기)
```

### 8.4 Enum 이슈 추적

Enum 값은 `values` 배열의 인덱스로 변환 후 정규화:
```
index 차이 / (values.length - 1)
```

direction이 `higher_better`이면 인덱스가 커질수록 좋음, `lower_better`이면 반대.

### 8.5 Boolean 이슈 추적

값 변경 시 ±0.5 크기의 moderate 양보로 기록. 변경 없으면 silent (0).

### 8.6 우선순위 보존 (Silent Round Fix)

totalConcession === 0인 라운드에서 기존 추정 우선순위를 덮어쓰지 않고 보존:
```typescript
...
### 8.7 Concession Style 판정

전체 이슈의 평균 절대 양보율로 스타일 결정:
- `avgRate > 0.3` → 'fast'
- `avgRate < 0.1` → 'slow'
- 기타 → 'mixed'
