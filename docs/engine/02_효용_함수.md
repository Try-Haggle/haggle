...
**범위:** 총 효용 공식, V_p/V_t/V_r/V_s 전체 수식, 다중 이슈 일반화, Protobuf
...
### 6.6 MasterStrategy 연결

```typescript
interface MasterStrategy {
  // ... 기존 필드 ...
  issue_space?: IssueSpace;  // v1.0.2: null = 기존 4차원, 설정 = 다중 이슈
}
```

---

## 7. vNext 다중이슈 효용 모델 (v2.0 구현)

### 7.1 Multi-Issue 효용 공식

v2.0 구현에서 다중이슈 효용은 3요소로 분리됩니다:

$$U_{total} = \text{clip}(U_{contract} - C_{risk} + B_{rel}, \; 0, \; 1)$$

**U_contract (계약 효용):**
$$U_{contract} = \sum_i w_i \cdot v_i(x_i)$$

- `w_i`: 이슈 가중치 (IssueWeight[])
- `v_i(x_i)`: 이슈별 정규화 함수 [0, 1]
- negotiable 이슈만 대상 (category = 'negotiable')

**C_risk (리스크 비용):**
$$C_{risk} = \lambda_f \cdot p_{fraud} + \lambda_q \cdot p_{quality} + \lambda_d \cdot p_{delay} + \lambda_s \cdot p_{dispute}$$

**B_rel (관계 보너스):**
$$B_{rel} = \rho_1 \cdot q_{reputation} + \rho_2 \cdot q_{repeat} + \rho_3 \cdot q_{responsiveness}$$
...
interface IssueDefinition {
  name: string;
  type: 'scalar' | 'enum' | 'boolean' | 'deadline';
  category: 'negotiable' | 'informational';
  direction: 'lower_better' | 'higher_better';
  min?: number;      // scalar/deadline
  max?: number;      // scalar/deadline
  values?: string[]; // enum: 순서대로 [worst, ..., best]
}
```

**이슈 타입별 정규화:**
- `scalar`: 선형 정규화 `(max-x)/(max-min)` 또는 `(x-min)/(max-min)` (direction에 따라)
- `enum`: 인덱스 정규화 `index / (values.length - 1)` (direction 고려)
- `boolean`: `true=1.0, false=0.0` 또는 반대 (direction에 따라)
- `deadline`: scalar와 동일

### 7.3 런타임 검증 (v2.0 신규)

**Offer 검증 (`validateOffer`):**
- 타입 일치 확인 (scalar→number, enum→string, boolean→boolean)
- 범위 확인 (scalar: min ≤ x ≤ max)
- Enum 값 확인 (values 배열에 존재)
- 알 수 없는 이슈 경고

**가중치 검증 (`validateWeights`):**
- 합산 ≈ 1.0 (허용오차 0.01)
- 음수/NaN 가중치 감지
- 누락된 이슈 가중치 감지

**런타임 타입 가드:**
- `isValidIssueValue(value, def)`: 값-타입 일치 확인
- 유효하지 않은 값은 자동으로 건너뛰고 경고 (u=0 처리)
- 빈 definitions 배열 → `{ issue_utilities: [], u_contract: 0 }` 반환
- 비유한(non-finite) 가중치 자동 스킵

### 7.4 도메인 스키마 (구현)

| 스키마 | 이슈 수 | 예시 이슈 |
|--------|---------|----------|
| Electronics v1 (MVP) | 3 | price, warranty_days, shipping_method |
| Vehicle v1 | TBD | price, mileage, warranty, color |
| Real Estate v1 | TBD | price, move_in_date, furnishing |
| Services v1 | TBD | hourly_rate, turnaround_days, revision_count |
