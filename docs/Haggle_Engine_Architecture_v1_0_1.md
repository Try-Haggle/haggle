...
공급자: Strategy Skill
  ...
  - v1.0.1: AgentStats로부터 자동 도출 가능 (섹션 5A 참조)
```
...
| P_target | 목표 가격 (이상적 가격) | Strategy Skill (LLM이 생성). v1.0.1: AgentStats의 anchor_ratio로 도출 가능. |
...
| α | 시간 민감도 | Strategy Skill (LLM이 생성). v1.0.1: AgentStats의 patience로 도출. |
...
## 5A. 에이전트 능력치 체계 (Agent Stats System) — v1.0.1 NEW
...
### 5A.3 그룹 1: 전투 스타일 (Battle Style)

#### Anchoring (앵커링) — 첫 제안 공격성
...
**스탯 Low(10):** β ≈ 0.5. 급양보. 상대가 약간만 양보해도 빠르게 수렴.
**스탯 High(90):** β ≈ 3.0. 끝까지 버팀. 교착 가능성 높지만 더 좋은 가격.

#### Resolve (결단력) — 마지노선 엄격도
...
**스탯 Low(10):** 효용 0.31만 넘어도 수락 가능. 거의 모든 제안 수용.
**스탯 High(90):** 효용 0.75 미만은 수락 불가. 좋은 거래 아니면 포기.

### 5A.4 그룹 2: 분석력 (Intelligence)

#### Market Sense (시장 감각) — 시장 데이터 활용도
...
**스탯 Low(10):** 경쟁 조정(V_p^adjusted)이 거의 없음. 1:1 협상과 유사.
**스탯 High(90):** 경쟁 상황을 최대한 활용. 다수 세션에서 유리.

#### Risk Radar (위험 감지) — 판매자 평판 민감도
...
**스탯 Low(10):** 신규 판매자, 저평판 판매자도 기회. 리스크 수용.
**스탯 High(90):** 평판 0.54 미만은 Batch Evaluate에서 사전 필터. 안전 우선.
...
**i_completeness_minimum의 역할:** r_score_minimum과 동일한 방식. Batch Evaluate 시 i_completeness < i_completeness_minimum인 리스팅은 사전 필터. Skill Layer에서 수행.

### 5A.5 그룹 3: 시간/관계 (Context)

#### Patience (인내심) — 시간 압박 저항
...
**스탯 Low(10):** 시간이 지나면 급격히 불안. 마감 압박에 취약.
**스탯 High(90):** 시간이 사실상 의미 없음. 영원히 기다릴 수 있다.
...
**스탯 Low(10):** 초면 V_s_base = 0.35. 관계 차원 기여가 낮다. 순수 가격 협상.
**스탯 High(90):** 초면 V_s_base = 0.75. 관계를 높이 평가. n_threshold = 6이라 적은 거래로도 최대 신뢰 도달. 후반 라운드에서 상대를 자극하지 않는 부드러운 전략.
...
**전략 특성:** 시간 압박을 거의 느끼지 않음(V_t_floor ≈ 0.72). 높은 기준(U_threshold ≈ 0.64)으로 좋은 거래만 수락. 관계에 무관심—순수히 조건으로만 판단.
...
**전략 특성:** 시장 데이터를 최대한 활용(γ ≈ 0.17). 평판과 정보 완성도에 높은 기준. 저평판/불완전 리스팅은 사전 필터. 데이터가 충분하면 빠른 결정.

**적합 상황:** 전자기기, 차량 등 시장 데이터가 풍부한 카테고리. 리스크 회피 성향.

#### 5. Dolphin (돌핀) — "좋은 관계가 좋은 거래"
...
**전략 특성:** 관계를 최우선으로 평가(w_s 매우 높음). 초면 V_s_base = 0.75. 후반 라운드 공격 완화(×0.8). 보수적 앵커링으로 상대를 자극하지 않음. 빠른 양보로 합의 촉진.

**적합 상황:** 반복 거래가 예상되는 상대. 커뮤니티 기반 거래. 관계 유지가 가격보다 중요.
...
**전략 특성:** 높은 U_threshold(≈ 0.66)로 좋은 거래만 수락. 평판에 극도로 민감(w_rep ≈ 0.70, r_score_minimum ≈ 0.48). 관계 무관—순수 조건 판단. 공격적 시작이지만 안전한 상대만 선별.

**적합 상황:** 고가 제품(명품, 전자기기). 사기 위험이 높은 카테고리. 안전 최우선.
...
### 5A.10 참조 구현: Stats → Engine Parameters
...
def stats_to_parameters(stats: dict) -> dict:
    ...
    반환: MasterStrategy에 필요한 모든 파라미터.
    market_average는 외부에서 별도 제공해야 함 (p_target 계산용).
    """
    ...
    return {
        ...
        # PriceContext (p_target는 market_average 필요)
        'anchor_ratio': round(anchor_ratio, 4),
        # p_target = market_average * anchor_ratio  (외부에서 계산)
...
def get_preset_parameters(preset_name: str) -> dict:
    """프리셋 이름으로 엔진 파라미터 조회."""
    stats = PRESETS.get(preset_name.upper())
    if not stats:
        raise ValueError(f'Unknown preset: {preset_name}')
    return stats_to_parameters(stats)
```

### 5A.11 스탯-파라미터 매핑 전체 요약
...
- $\gamma$ = 경쟁 민감도 (Strategy Skill 제공, 기본 0.1). v1.0.1: AgentStats의 market_sense로 도출.
- CompetitionContext 없으면: V_p는 기본 공식 그대로. 하위 호환 100%.
- v1.0.1: market_utilization이 낮으면 CompetitionContext가 있어도 영향이 줄어든다.
...
마지노선 방향으로의 이동폭이 20% 줄어든다. 결과적으로 더 관대한 역제안을 하게 되어, 상대와의 관계를 보존한다.
...
```python
def pre_filter_listings(listings: list, params: dict) -> list:
    """
    Stats에서 도출된 최소 기준으로 리스팅 사전 필터.
    Skill Layer에서 수행. Engine Core 진입 전.
    """
    r_min = params.get('r_score_minimum', 0.0)
    i_min = params.get('i_completeness_minimum', 0.0)

    return [
        listing for listing in listings
        if listing['r_score'] >= r_min
        and listing['i_completeness'] >= i_min
    ]
```
...
message BatchEvaluateResult {
  ...
  int32 pre_filtered_count = 4;    // v1.0.1: 사전 필터로 제외된 리스팅 수
}
...
U_threshold:  수락 가능 최저 효용 (Strategy Skill이 설정. v1.0.1: AgentStats의 resolve로 도출.)
U_aspiration: 목표 효용 (Strategy Skill이 설정. v1.0.1: AgentStats의 resolve로 도출.)
...
│     빈도: 제품당 1회 (복잡하면 2-3회)                         │
│     v1.0.1: 프리셋 사용 시 LLM 불필요 (Stats→파라미터 변환)   │
│                                                              │
...
**v1.0.1 추가:** 사용자가 프리셋 페르소나(Tiger, Fox 등)를 선택하면 AgentStats → 파라미터 변환이 결정론적으로 수행되므로, **LLM 호출 없이** Master Strategy의 핵심 파라미터를 생성할 수 있다. 시장가 조회(Market Research Skill)만 필요하다.

```
v1.0.1 프리셋 사용 시:
  사용자: "Tiger로 아이패드 사고 싶어, 예산 $800"
      │
      ▼
  Market Research Skill: 시장가 조회 (API, LLM 아님)
      → market_average = $820
      │
      ▼
  Stats → Parameter Converter:
      Tiger 프리셋 → anchor_ratio=0.66, β=2.62, ...
      p_target = $820 × 0.66 = $541
      │
      ▼
  Master Strategy 생성 완료. LLM 호출: 0회.
```
...
message MasterStrategy {
  ...
  // v1.0.1 NEW: Agent Stats System
  AgentStats agent_stats = 17;             // 원본 스탯 (재현성 보장)
  StatsPreset stats_preset = 18;           // 사용된 프리셋 (CUSTOM이면 직접 배분)

  // v1.0.1 NEW: 엔진 확장 파라미터
  float anchor_ratio = 19;                 // p_target 포지셔닝 (시장가 대비)
  float market_utilization = 20;           // CompetitionContext 활용 깊이 [0, 1]
  float r_score_minimum = 21;              // 최소 평판 필터 [0, 1]
  float i_completeness_minimum = 22;       // 최소 정보 완성도 필터 [0, 1]
...
| **스탯 변경 (v1.0.1)** | Stats→파라미터 재변환 | **아니오** |
...
message WaitingIntent {
  ...
  // Master Strategy (LLM 또는 Stats로 생성, 저장됨)
...
사용자: "아이패드 프로 M4 사고 싶어, 예산 $800"
    │
    ▼
LLM (또는 Stats→파라미터) → Master Strategy 생성
    │
    ▼
리스팅 조회: 해당 카테고리 200개 발견
    │
    ▼
사전 필터 (v1.0.1): r_score_minimum, i_completeness_minimum 적용
  → 200개 → 필터 후 170개
    │
    ▼
Engine Core Batch Evaluate (34ms):
  같은 Master Strategy × 170개 리스팅 → 170개 U_total → 정렬
    │
    ▼
Top 5 세션 생성 → 병렬 협상 시작
나머지 165개 → WAITING (세션 미생성)
    │
...
**v1.0.1:** cross_pressure_sensitivity가 낮은 에이전트(Market Sense가 낮은)는 크로스 프레셔 정보를 수신해도 CompetitionContext 반영이 약하다.
...
v1.0.1: 프리셋 사용 시 LLM 비용 추가 절감.
프리셋 채택률 60% 가정 시 LLM 비용 ~40% 감소.
```
...
| 처리 시간 | 수십 초 | < 200ms (Hot Path) | < 200ms (Hot Path) |
...
### 23.2 Stats→파라미터 변환 예산 (v1.0.1 NEW)
...
Stats→파라미터 변환은 전략 생성 시 1회만 수행. 매 라운드에는 불필요.
...
| Cold Path P95 (프리셋, v1.0.1) | < 500ms (LLM 없음) |
...
**테스트 6: Fox 프리셋 스탯 검증 (v1.0.1 NEW)**
```
입력:
  stats: {anchoring:50, tenacity:50, resolve:50, market_sense:50,
          risk_radar:50, scrutiny:50, patience:50, rapport:50}
...
기대값 (Stats→파라미터 변환):
  ...
  late_round_aggression_modifier = 1.0  // rapport=50 ≤ 60
...
가중치 도출:
  ...
  total = 200.0
  w_p = 0.250, w_t = 0.250, w_r = 0.250, w_s = 0.250
```

**테스트 7: Tiger 프리셋 스탯 검증 (v1.0.1 NEW)**
```
입력:
  stats: {anchoring:85, tenacity:80, resolve:60, market_sense:40,
          risk_radar:20, scrutiny:15, patience:55, rapport:45}

기대값 (Stats→파라미터 변환):
  anchor_ratio = 1.0 - (85/100) × 0.4 = 0.660
  beta = 0.3 + (80/100) × 2.9 = 2.620
  u_threshold = 0.25 + (60/100) × 0.55 = 0.580
  u_aspiration = 0.580 + 0.15 = 0.730
  gamma = (40/100) × 0.2 = 0.080
  w_rep = 0.3 + (20/100) × 0.5 = 0.400
  w_info = 1.0 - 0.40 = 0.600
  alpha = 0.3 + (55/100) × 2.7 = 1.785
  v_t_floor = (55/100) × 0.85 = 0.468
  v_s_base = 0.3 + (45/100) × 0.5 = 0.525
  n_threshold = round(15 - (45/100) × 10) = 11
  late_round_aggression_modifier = 1.0  // rapport=45 ≤ 60

가중치 도출:
  raw_wp = 85×0.4 + 80×0.3 + 60×0.2 + 40×0.1 = 34+24+12+4 = 74.0
  raw_wt = (100-55)×0.8 + 80×0.2 = 36+16 = 52.0
  raw_wr = 20×0.5 + 15×0.5 = 10+7.5 = 17.5
  raw_ws = 45×0.9 + 55×0.1 = 40.5+5.5 = 46.0
  total = 189.5
  w_p = 74.0/189.5 = 0.3905
  w_t = 52.0/189.5 = 0.2744
  w_r = 17.5/189.5 = 0.0923
  w_s = 46.0/189.5 = 0.2428
```

**테스트 8: 스탯 예산 초과 오류 (v1.0.1 NEW)**
```
입력:
  stats: {anchoring:90, tenacity:90, resolve:90, market_sense:50,
          risk_radar:50, scrutiny:50, patience:50, rapport:50}
  // 합계 = 520 ≠ 400
...
**테스트 9: 스탯 범위 초과 오류 (v1.0.1 NEW)**
```
입력:
  stats: {anchoring:95, tenacity:50, resolve:50, market_sense:50,
          risk_radar:50, scrutiny:50, patience:50, rapport:5}

기대값:
  error: STAT_OUT_OF_RANGE
  detail: "anchoring=95, must be [10, 90]"
  (또는 rapport=5에서 먼저 검출될 수 있음)
```

**테스트 10: Dolphin 프리셋 + 실제 협상 (v1.0.1 NEW)**
```
Dolphin 프리셋으로 Stats→파라미터 변환 후, 테스트 1과 동일한 시장 상황에서 compute_utility 수행.

Step 1: Stats→파라미터
  Dolphin stats: {anchoring:25, tenacity:30, resolve:40, market_sense:45,
                  risk_radar:50, scrutiny:45, patience:75, rapport:90}

  w_rep = 0.3 + (50/100) × 0.5 = 0.550
  w_info = 0.450
  alpha = 0.3 + (75/100) × 2.7 = 2.325
  v_t_floor = (75/100) × 0.85 = 0.6375
  v_s_base = 0.3 + (90/100) × 0.5 = 0.750
  n_threshold = round(15 - (90/100) × 10) = 6
  late_round_aggression_modifier = 0.8  // rapport=90 > 60
  u_threshold = 0.25 + (40/100) × 0.55 = 0.470
  u_aspiration = 0.470 + 0.15 = 0.620

  가중치:
    raw_wp = 25×0.4 + 30×0.3 + 40×0.2 + 45×0.1 = 10+9+8+4.5 = 31.5
    raw_wt = (100-75)×0.8 + 30×0.2 = 20+6 = 26.0
    raw_wr = 50×0.5 + 45×0.5 = 25+22.5 = 47.5
    raw_ws = 90×0.9 + 75×0.1 = 81+7.5 = 88.5
    total = 193.5
    w_p = 0.1628, w_t = 0.1343, w_r = 0.2454, w_s = 0.4574

Step 2: compute_utility (테스트 1 시장 데이터 사용)
  price: {p_effective: 200, p_target: 180, p_limit: 220}
  time: {t_elapsed: 36000, t_deadline: 86400, alpha: 2.325, v_t_floor: 0.6375}
  risk: {r_score: 0.85, i_completeness: 0.90, w_rep: 0.55, w_info: 0.45}
  relationship: {n_success: 3, n_dispute_losses: 0, n_threshold: 6, v_s_base: 0.75}

기대값:
  v_p = 0.820
  v_t = max(0.6375, (1 - 36000/86400)^2.325) = max(0.6375, 0.583^2.325) = max(0.6375, 0.376) = 0.6375
  v_r = 0.55×0.85 + 0.45×0.90 = 0.4675 + 0.405 = 0.8725
  v_s = clamp(0.75 + 3/6 + 0) = clamp(1.25) = 1.000
  u_total = 0.1628×0.820 + 0.1343×0.6375 + 0.2454×0.8725 + 0.4574×1.000
         = 0.1335 + 0.0856 + 0.2141 + 0.4574 = 0.891

Decision Maker:
  u_total(0.891) ≥ u_aspiration(0.620) → ACCEPT

비교: 같은 시장 상황에서 Tiger라면?
  Tiger의 u_aspiration = 0.730
  Tiger의 가중치로 계산하면 다른 u_total → 다른 결정
```
...
| **스킬 만들기** | X | 템플릿 기반 (3개까지) | 무제한 + API 직접 |
| **노코드 빌더** | X | O | O |
| **커스텀 코드** | X | X | O (풀 SDK) |
| **외부 API 연동** | X | 기본 커넥터 | 모든 커넥터 + 커스텀 |
| **스킬 배포/판매** | X | 비공개만 (본인용) | 마켓플레이스 배포 |
| **수익 분배** | X | X | 10% (Top 3: 15%) |
| **샌드박스 테스트** | X | 100건/월 | 무제한 |
| **분석 대시보드** | X | 기본 | 고급 (성과 추적) |
...
11. **(v1.0.1) 스탯 예산 최적값.** 400포인트가 최적인가? 사용자 테스트에서 "너무 많다" / "너무 적다" 피드백에 따라 조정 가능. 300~500 범위 A/B 테스트 필요.

12. **(v1.0.1) 프리셋 밸런스.** 6개 프리셋 간 성공률 차이가 과도하면 특정 프리셋으로 쏠림 발생. 시뮬레이션 1,000건에서 프리셋별 성공률 편차가 ±10% 이내인지 검증 필요.

13. **(v1.0.1) 스탯→파라미터 곡선 형태.** 현재 모든 변환이 선형이다 (stat/100 × range). 비선형 변환(시그모이드, 구간별 선형 등)이 더 직관적인 사용자 경험을 제공하는지 검토 필요.

14. **(v1.0.1) late_round_aggression_modifier 임계값.** Rapport > 60에서 발동하는 0.8 수정자가 적절한가? 60이라는 임계값과 0.8이라는 강도 모두 실증 근거 필요.

15. **(v1.0.1) 가중치 자동 도출 공식 검증.** raw_w 계수(Anchoring×0.4, Tenacity×0.3 등)의 이론적/실증적 근거. 시뮬레이션에서 도출된 가중치가 합리적인 협상 결과를 만드는지 검증.

16. **(v1.0.1) 사전 필터와 기회 손실.** r_score_minimum, i_completeness_minimum이 높으면 좋은 딜을 놓칠 수 있다. 사전 필터 vs Engine Core 자체 V_r 반영 간의 최적 균형점.
...
| 스킬 | 반드시 제공 | 제공 금지 |
|------|-----------|----------|
| **Strategy Skill** | w_p~w_s, α, β, t_deadline, n_threshold, U_threshold, U_aspiration. v1.0.1: AgentStats→파라미터 변환도 이 스킬에서 수행. | 시장 데이터, 평판 점수 |
...
│       │                                                       │
│       ▼                                                       │
│  사전 필터 (v1.0.1): r_score_min, i_completeness_min          │
│  → 기준 미달 리스팅 제외                                       │
│       │                                                       │
│       ▼                                                       │
│  Engine Core Batch Evaluate                                   │
...
### D.2 v1.0.0 대비 v1.0.1 변경사항

| 항목 | v1.0.0 | v1.0.1 | 이유 |
|------|--------|--------|------|
| **에이전트 성격** | LLM이 파라미터 직접 생성 | 8개 스탯 → 파라미터 결정론적 변환 | 사용자 친화 + LLM 비용 절감 |
| **프리셋 페르소나** | 이름만 (Tiger, Fox 등) | 6개 프리셋 = 구체적 스탯 배분 | 즉시 사용 가능 |
| **MasterStrategy** | 기본 필드만 | AgentStats + 확장 파라미터 추가 | 스탯 원본 보존 + 새 기능 지원 |
| **Decision Maker** | 고정 임계값 | 스탯(Resolve)에서 도출되는 임계값 | 에이전트 성격 반영 |
| **Batch Evaluate** | 전체 리스팅 평가 | 사전 필터(r_score_min, i_completeness_min) 후 평가 | 불필요한 연산 제거 |
| **Faratin 곡선** | 기본 P(t) | late_round_aggression_modifier 추가 | Rapport 높은 에이전트의 관계 보존 |
| **Cold Path** | LLM 1-3회 필수 | 프리셋 사용 시 LLM 0회 | 비용 절감 + 응답 속도 향상 |
| **가중치 도출** | LLM 또는 수동 | 스탯에서 자동 도출 가능 | 일관성 + 재현성 |
| **적합성 테스트** | 5개 | 10개 (스탯 테스트 5개 추가) | 스탯 체계 검증 |
| **미결 사항** | 10개 | 16개 (스탯 관련 6개 추가) | 새 기능의 열린 질문 |
...
문서 버전 이력:
- v1.0.1 (2026-02-19): 에이전트 능력치 체계(Agent Stats System) 추가. 8개 스탯(3그룹) → 엔진 파라미터 결정론적 변환. 6개 프리셋 페르소나(Tiger, Fox, Turtle, Owl, Dolphin, Eagle). 총 예산 400포인트 제약. MasterStrategy에 AgentStats 및 확장 파라미터(anchor_ratio, market_utilization, r_score_minimum, i_completeness_minimum, late_round_aggression_modifier) 추가. Decision Maker 스탯 연동. Batch Evaluate 사전 필터. Faratin 곡선 후반 공격 완화. 적합성 테스트 5개 추가. 미결 사항 6개 추가.
