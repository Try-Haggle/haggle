...
> 브랜치 운영 원칙은 [Main_Branch_Release_Policy.md](./Main_Branch_Release_Policy.md)를 따른다.  
> 이 문서는 `main` 브랜치에 들어갈 MVP 제품 범위를 다루며, HNP/UCP 표준화의 전체 범위를 제한하지 않는다.
...
### 도메인 구조 (haggle.ai 기반)
- **MCP 서버 (툴 API)**: `https://haggle.ai/mcp`
- **Embedded UI (판매자 위젯)**: `https://haggle.ai/widgets/listing`
- **구매자 웹앱 (공유 링크)**: `https://haggle.ai/l/{public_id}`
...
│  ┌──────▼──────┐ ┌─────▼─────┐ ┌─────▼──────┐          │
│  │   Haggle    │ │    HNP    │ │  Post-MVP  │          │
│  │   Engine    │ │  Protocol │ │  Commerce  │          │
│  │  (협상 AI)  │ │  (표준)   │ │ (결제/분쟁) │          │
│  │             │ │           │ │            │          │
│  │ MVP: 룰기반 │ │ MVP: 타입 │ │ main 제외  │          │
│  │ 확장: LLM   │ │ 정의만   │ │ 후속 병합  │          │
│  └─────────────┘ └───────────┘ └────────────┘          │
│                     Service Layer                       │
└─────────────────────────┬───────────────────────────────┘
...
| 도메인 | **haggle.ai** | Vercel/Railway 모두 커스텀 도메인 HTTPS 자동 설정 |
...
- 공유 링크: `https://haggle.ai/l/{public_id}`
- UI에 Publish 버튼
