...
if (
  ...
  failures.push(
    "negotiation_rounds must keep idempotency unique per (session_id, idempotency_key)",
  );
}

if (
  schema.includes('uniqueIndex("negotiation_rounds_idempotency_key_idx").on(table.idempotencyKey)')
) {
...
if (
  !/CREATE UNIQUE INDEX IF NOT EXISTS negotiation_rounds_session_idempotency_key_idx\s+ON negotiation_rounds \(session_id, idempotency_key\);/u.test(
    migration,
  )
) {
...
if (
  !creditSchema.includes(
    'uniqueIndex("credit_ledger_entries_account_idempotency_key_idx").on(table.accountId, table.idempotencyKey)',
  )
) {
  failures.push(
    "credit_ledger_entries must keep idempotency unique per (account_id, idempotency_key)",
  );
}

if (
  /uniqueIndex\("credit_ledger_entries_idempotency_key_idx"\)\.on\(table\.idempotencyKey\)/.test(
    creditSchema,
  )
) {
...
if (
  !/CREATE UNIQUE INDEX IF NOT EXISTS "credit_ledger_entries_account_idempotency_key_idx"\s+ON "credit_ledger_entries" \("account_id", "idempotency_key"\);/u.test(
    creditMigration,
  )
) {
...
if (
  /CREATE UNIQUE INDEX[^;]*ON "credit_ledger_entries" \("idempotency_key"\)/u.test(creditMigration)
) {
